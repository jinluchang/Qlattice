...
from qlat_scripts.v1 import (
    ...
    get_r_sq_interp_idx_coef_list, get_r_list, get_all_points,
    run_f_weight_uniform, benchmark_eval_cexpr,
)
...
def get_cexpr_pipi_corr():
    ...
    def calc_cexpr():
        ...
        exprs = [
            mk_fac(1) + "1",
            mk_pipi_i22("snk_1", "snk_2", True) * mk_pipi_i22("src_1", "src_2")
            + "pipi_i22+^dag(0) * pipi_i22(-tsep)",
        ]
        cexpr = contract_simplify_compile(
            *exprs, is_isospin_symmetric_limit=True, diagram_type_dict=diagram_type_dict
        )
...
def get_cexpr_pipi_corr_psnk_psrc():
        ...
        for mode_src in [
            ...
            for mode_snk in [
                ...
                exprs += [
                    #
                    mk_fac(
                        f"pipi_wave_function(snk_1, snk_2, {mode_snk}, size, pipi_op_dis_4d_sqr_limit)"
                    )
                    * mk_fac(
                        f"pipi_wave_function(src_1, src_2, {mode_src}, size, pipi_op_dis_4d_sqr_limit)"
                    )
    ...
    base_positions_dict["pipi_op_dis_4d_sqr_limit"] = (
        0.5  # default value, to be overrided by `pd`.
    )
    return cache_compiled_cexpr(
        calc_cexpr,
        fn_base,
...
def auto_contract_pipi_corr_psnk_psrc(
    ...
    total_volume = geo.total_volume
    pipi_op_t_sep = get_param(job_tag, "measurement", "pipi_op_t_sep")
    pipi_op_dis_4d_sqr_limit = get_param(
        job_tag, "measurement", "pipi_op_dis_4d_sqr_limit"
    )
    pipi_corr_t_sep_list = get_param(job_tag, "measurement", "pipi_corr_t_sep_list")
    data_list = []
    ...
    ld = q.mk_lat_data(
        [
            ...
            [
                "pipi_op_t_sep_snk",
                pipi_op_t_sep,
                [str(i) for i in range(pipi_op_t_sep)],
            ],
            [
                "pipi_op_t_sep_src",
                pipi_op_t_sep,
...
def auto_contract_meson_jj(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob):
    ...
    q.json_results_append(
        f"{fname}: ld_sum sig", q.get_data_sig_arr(ld_sum, q.RngState(), 4)
    )
    for i, en in enumerate(expr_names):
        q.json_results_append(
        ...
        for idx in range(len(xg_fsel_arr)):
            ...
            for t_sep_1_idx, t_sep_1 in enumerate(pipi_tensor_t_sep_list):
                for t_sep_2_idx, t_sep_2 in enumerate(pipi_tensor_t_sep_list):
                    ...
                    t = x_rel_t % t_size
                    val[t_sep_1_idx, t_sep_2_idx] = eval_cexpr(
...
def run_auto_contraction(
    ...
    *,
    ...
    auto_contract_meson_corr_psnk_psrc_psel(
        job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob
    )
    auto_contract_pipi_corr_psnk_psrc(
        job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob
    )
    #
...
set_param("16IH2", "measurement", "pipi_tensor_t_sep_list")(
    [
        1,
        2,
    ]
...
set_param("48I", "measurement", "pipi_tensor_t_sep_list")(
    [
        1,
        2,
    ]
...
set_param(job_tag, "lanc_params", 0, 0, "pit_params")(
    {"eps": 0.01, "maxiter": 500, "real": True}
)
...
set_param(job_tag, "measurement", "pipi_corr_t_sep_list")(
    [
        1,
        2,
        3,
...
set_param(job_tag, "measurement", "pipi_tensor_t_sep_list")(
    [
        1,
        2,
        3,
