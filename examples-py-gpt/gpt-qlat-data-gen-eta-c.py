...
def get_cexpr_eta_c_corr():
    ...
    def calc_cexpr():
        ...
        exprs = [
            ...
            + "j5_eta_c_t(0) * j5_eta_c_t^dag(-tsep)",
        ]
...
def run_eta_c_corr(job_tag, traj, get_gf, get_gt):
    ...
    for inv_type in charm_wall_src_prop_params["charm_quark_inv_type_list"]:
        ...
        auto_contract_eta_c_corr_psnk(
            job_tag, traj, get_get_prop, inv_type, tslice_list
        )
...
def get_param_charm_wall_src_tslice_list(job_tag, traj):
    ...
    t_size = total_site[3]
    rs = q.RngState(
        f"{get_job_seed(job_tag)}-{traj}-get_param_charm_wall_src_tslice_list"
    )
...
def run_charm_wall_src_prop_params(job_tag, traj):
    assert len(get_param(job_tag, "quark_flavor_list")) == len(
        get_param(job_tag, "quark_mass_list")
    )
    inv_type_ref = 1
    ...
    obj["charm_quark_flavor_list"] = get_param(job_tag, "quark_flavor_list")[
        inv_type_ref + 1 :
    ]
    obj["charm_quark_mass_list"] = get_param(job_tag, "quark_mass_list")[
        inv_type_ref + 1 :
    ]
...
set_param(job_tag, "quark_flavor_list")(
    ...
    + [f"charm-{idx + 1}" for idx in range(17)]
)
...
set_param(job_tag, "quark_flavor_list")(
    ...
    + [f"charm-{idx + 1}" for idx in range(14)]
)
set_param(job_tag, "quark_mass_list")(
    [
        0.00107,
...
set_param(job_tag, "quark_flavor_list")(
    ...
    + [f"charm-{idx + 1}" for idx in range(16)]
)
set_param(job_tag, "quark_mass_list")(
    [
        0.0001,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Eta-c data generation script.")
    parser.add_argument(
        "--job_tag_list",
        type=str,
