...
def load_or_compute_muon_line_interpolation():
    ...
    q.load_multiple_muonline_interpolations(
        "results/muon-line-interpolation-data",
        [
            0,
            1,
        ],
...
if True:
    ...
    for idx, ws in enumerate(weights):
        ...
        q.json_results_append(
            f"{fname}: initial weights {idx}",
            q.get_data_sig(np.array(ws), q.RngState()),
            check_eps_interp,
        )
...
def test_get_muon_line_m_extra_lat(total_site, muon_mass, tag, rng_seed):
    ...
    q.json_results_append(
        f"{fname}: {total_site} {muon_mass} {tag} {rng_seed} {[cx, cy, cz]}",
