...
slv_qz_f = qz.propagator(inv.mixed_precision(slv_5d_f, g.single, g.double)).grouped(4)

slv_qm_mp = qm.propagator(
    inv.defect_correcting(
        inv.mixed_precision(slv_5d_mp, g.single, g.double), eps=1e-8, maxiter=100
    )
).grouped(4)

slv_qm_split_sloppy = qm.propagator(
    inv.defect_correcting(
        inv.mixed_precision(slv_5d_split, g.single, g.double), eps=1e-8, maxiter=1
    )
).grouped(4)

slv_qm_split = qm.propagator(
    inv.defect_correcting(
        inv.mixed_precision(slv_5d_split, g.single, g.double), eps=1e-8, maxiter=100
    )
).grouped(4)
...
slv_qm_madwf = qm.propagator(
    inv.defect_correcting(
        inv.mixed_precision(
            pc.mixed_dwf(slv_5d_f, slv_5d_pv_f, qz_f), g.single, g.double
        ),
        eps=1e-8,
        maxiter=100,
    )
...
inv_qm_split = qg.InverterGPT(
    inverter=slv_qm_split, qtimer=q.Timer("py:slv_qm_split", True)
)
inv_qm_split_sloppy = qg.InverterGPT(
    inverter=slv_qm_split_sloppy, qtimer=q.Timer("py:slv_qm_split_sloppy", True)
)
inv_qm_madwf = qg.InverterGPT(
