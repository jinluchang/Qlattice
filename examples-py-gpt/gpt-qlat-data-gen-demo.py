...
def run_job_load_data_demo(job_tag, traj):
    ...
    #
    # Load wall source psel propagator directly (light quark, inv_type=0)
    inv_type = 0
    flavor_tag = "light"
    ...
    if get_load_path(f"{path_sp}.qar", f"{path_sp}/checkpoint.txt") is not None:
        for tslice in range(total_site[3]):
            ...
            if tslice == 0:
                ...
                q.displayln_info(f"{fname}: loaded wsrc psel prop tslice={tslice}, sample norm = {elem.qnorm()}")
        q.displayln_info(f"{fname}: loaded wsrc psel propagator (light)")
    else:
        q.displayln_info(f"{fname}: wsrc psel propagator not available")
    #
    # Load wall source fsel propagator directly (light quark, inv_type=0)
    ...
    if get_load_path(f"{path_s}.qar", f"{path_s}/geon-info.txt") is not None:
        ...
        for tslice in range(total_site[3]):
            ...
            if tslice == 0:
                ...
                sp_check @= sc_prop
                q.displayln_info(f"{fname}: loaded wsrc fsel prop tslice={tslice}, sample norm = {sp_check.qnorm()}")
        sfr.close()
