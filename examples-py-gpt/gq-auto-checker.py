...
from qlat_scripts.v1 import (
    ...
    get_job_seed, check_job, run_params, run_gf, run_gt, run_wi, run_eig,
    run_eig_strange, run_prop_wsrc_full, run_f_weight_from_wsrc_prop_full,
    run_f_rand_01, run_fsel_prob, run_psel_prob, run_fsel_from_fsel_prob,
    run_psel_from_psel_prob, run_fselc, run_prop_wsrc_sparse, run_prop_psrc,
    run_get_prop, run_r_list, get_r_sq_interp_idx_coef_list, get_r_list,
    get_all_points, run_gf_ape, run_get_prop_checker, run_prop_psrc_checker,
    run_prop_wsrc_checker, benchmark_eval_cexpr,
)
from auto_contractor.operators import (
    mk_fac, mk_pi_p, mk_pi_0, mk_pi_m, mk_k_p, mk_k_m, mk_k_0, mk_k_0_bar,
    mk_j5pi_mu, mk_j5k_mu, mk_a0_p, mk_kappa_p, mk_j_mu, mk_m, mk_vec_mu,
    mk_pipi_i22, mk_sym, mk_expr, contract_simplify_compile,
    new_spin_index, new_color_index, G, Qb, Qv, show_dagger, sympy,
)
from auto_contractor.eval import eval_cexpr, get_expr_names, cache_compiled_cexpr, np
...
def get_cexpr_meson_corr():
    ...
    def calc_cexpr():
        ...
        exprs = [
            ...
            + "pi+^dag(0) * j5pi_t^dag(-tsep)",
            mk_k_p("x_2", True) * mk_j5k_mu("x_1", 3, True)
            + "K+^dag(0) * j5k_t^dag(-tsep)",
            mk_j5pi_mu("x_2", 3) * mk_pi_p("x_1") + "j5pi_t(0) * pi+(-tsep)",
            mk_j5k_mu("x_2", 3) * mk_k_p("x_1") + "j5k_t(0) * K+(-tsep)",
            mk_j5pi_mu("x_2", 3) * mk_j5pi_mu("x_1", 3, True)
            + "j5pi_t(0) * j5pi_t^dag(-tsep)",
            mk_j5k_mu("x_2", 3) * mk_j5k_mu("x_1", 3, True)
            + "j5k_t(0) * j5k_t^dag(-tsep)",
            ...
            + "j_mu(0) * j_mu(-tsep)",
            sum(
                [mk_j5pi_mu("x_2", mu) * mk_j5pi_mu("x_1", mu, True) for mu in range(4)]
            )
            + "j5pi_mu(0) * j5pi_mu^dag(-tsep)",
            sum([mk_j5k_mu("x_2", mu) * mk_j5k_mu("x_1", mu, True) for mu in range(4)])
            + "j5k_mu(0) * j5k_mu^dag(-tsep)",
        ]
...
def auto_contract_meson_corr_psnk_psrc_rand(job_tag, traj, get_get_prop):
    ...
    get_prop = get_get_prop()
    get_all_points(total_site)
    [q.Coordinate(xg) for xg in geo.xg_arr()]
    r_list = get_r_list(job_tag)
    r_sq_interp_idx_coef_list = get_r_sq_interp_idx_coef_list(job_tag)
    sample_num = get_param(
        job_tag,
        "measurement",
        "auto_contract_meson_corr_psnk_psrc_rand",
        "sample_num",
        default=128,
    )
    sample_size = get_param(
        job_tag,
        "measurement",
        "auto_contract_meson_corr_psnk_psrc_rand",
        "sample_size",
...
def mk_pi_p_wf(p1, p2, mom, is_dagger=False):
    ...
    radius = "r_pi"
    return (
        mk_meson_wf("u", "d", p1, p2, radius, mom, is_dagger)
...
def mk_kk_m_wf(p1, p2, mom, is_dagger=False):
    ...
    radius = "r_kk"
    return (
        -mk_meson_wf("s", "u", p1, p2, radius, mom, is_dagger)
        + f"K-({p1},{p2},{radius},{mom}){show_dagger(is_dagger)}"
    )
...
def mk_kk_0_bar_wf(p1, p2, mom, is_dagger=False):
    ...
    radius = "r_kk"
    return (
        -mk_meson_wf("s", "d", p1, p2, radius, mom, is_dagger)
...
def mk_pipi_i0_j0_wf(p11, p12, p21, p22, mom, is_dagger=False):
    ...
    def mk_op_mom(mom_pi):
        ...
        return (
            ...
            / sympy.sqrt(3)
            * (
                -mk_pi_0_wf(p11, p12, mom1, is_dagger)
                * mk_pi_0_wf(p21, p22, mom2, is_dagger)
                + mk_pi_m_wf(p11, p12, mom1, is_dagger)
                * mk_pi_p_wf(p21, p22, mom2, is_dagger)
                + mk_pi_p_wf(p11, p12, mom1, is_dagger)
                * mk_pi_m_wf(p21, p22, mom2, is_dagger)
            )
        )
    #
...
def mk_kkkk_i0_j0_wf(p11, p12, p21, p22, mom, is_dagger=False, *, is_sym=True):
    ...
    def mk_op_mom(mom_kk):
        ...
        return (
            ...
            / sympy.sqrt(2)
            * (
                mk_kk_0_wf(p11, p12, mom1, is_dagger)
                * mk_kk_0_bar_wf(p21, p22, mom2, is_dagger)
                + mk_kk_p_wf(p11, p12, mom1, is_dagger)
                * mk_kk_m_wf(p21, p22, mom2, is_dagger)
            )
        )
    #
...
def auto_contract_meson_corr_wf(job_tag, traj, get_get_prop):
    ...
    get_all_points(total_site)
    [q.Coordinate(xg) for xg in geo.xg_arr()]
    sample_num = get_param(job_tag, "measurement", fname, "sample_num", default=512)
    sample_size = get_param(job_tag, "measurement", fname, "sample_size", default=128)
    t_sep_range = get_param(job_tag, "measurement", fname, "t_sep_range", default=17)
    ...
    ld = q.mk_lat_data(
        [
            ...
            [
                "t_sep",
                t_sep_range,
                [str(t) for t in range(t_sep_range)],
            ],
        ]
    )
    ld.from_numpy(res_sum)
...
def get_cexpr_meson_meson_i0_j0_corr_wf():
    ...
    def calc_cexpr():
        ...
        diagram_type_dict[((("x1_1_1", "x1_2_3"), 1), (("x1_2_4", "x1_1_2"), 1))] = (
            "TypeV"
        )
        diagram_type_dict[((("x1_1_1", "x1_2_3p"), 1), (("x1_2_4p", "x1_1_2"), 1))] = (
            "TypeV"
        )
        diagram_type_dict[((("x2_1_1", "x2_2_3"), 1), (("x2_2_4", "x2_1_2"), 1))] = (
            "TypeV"
        )
        diagram_type_dict[((("x2_1_1", "x2_2_3p"), 1), (("x2_2_4p", "x2_1_2"), 1))] = (
            "TypeV"
        )
        diagram_type_dict[((("x1_1_1", "x1_1_2"), 1), (("x2_1_1", "x2_1_2"), 1))] = (
            "TypeV"
        )
        diagram_type_dict[
            (
        ...
        #
        diagram_type_dict[((("x1_1_1", "x2_1_1"), 1), (("x2_1_2", "x1_1_2"), 1))] = (
            "TypeD"
        )
        diagram_type_dict[
            (
        ...
        src_op_list = [
            mk_sigma_wf("x1_1_1", "x1_1_2", mom0) + "sigma(x1_1_1,x1_1_2)",
        ...
        snk_op_list = [
            mk_sigma_wf("x2_1_1", "x2_1_2", mom0) + "sigma(x2_1_1,x2_1_2)",
        ...
        t_sel = mk_fac("time_selector_8(x1_1_1)") + "time_selector_8(x1_1_1)"
        exprs = [
            mk_fac(1) + "1",
            t_sel,
        ]
        for src_op in src_op_list:
            exprs.append((mk_fac(1) + "src_op_only") * src_op)
        for snk_op in snk_op_list:
            exprs.append((mk_fac(1) + "snk_op_only") * snk_op)
        for src_op in src_op_list:
            for snk_op in snk_op_list:
                exprs.append(
                    (
...
def auto_contract_meson_meson_i0_j0_corr_wf(job_tag, traj, get_get_prop):
    ...
    idx_arr_of_src_op_only = np.array(
        [
            0,
        ]
        + [idx for idx, name in enumerate(expr_names) if " src_op_only " in name],
        int,
    )
    idx_arr_of_snk_op_only = np.array(
        [
            0,
        ]
        + [idx for idx, name in enumerate(expr_names) if " snk_op_only " in name],
        int,
    )
    total_site = q.Coordinate(get_param(job_tag, "total_site"))
    t_size = total_site[3]
    ...
    def feval(args):
        ...
        values_src_op = np.zeros(
            (
                t_size,
                len(expr_names[idx_arr_of_src_op_only]),
            ),
            dtype=complex,
        )
        values_snk_op = np.zeros(
            (
                t_size,
                len(expr_names[idx_arr_of_snk_op_only]),
            ),
            dtype=complex,
        )
        for idx2 in range(sample_size):
    ...
    def sum_function(val_list):
        ...
        return (
            ...
            q.glb_sum(
                values_src_op.transpose(
                    1,
                    0,
                )
            ),
            q.glb_sum(
                values_snk_op.transpose(
                    1,
                    0,
                )
            ),
    ...
    #
    q.timer_fork(0)
    res_sum, res_sum_src_op, res_sum_snk_op = q.parallel_map_sum(
        feval, load_data(), sum_function=sum_function, chunksize=1
    )
    q.displayln_info(f"{fname}: timer_display for parallel_map_sum")
    q.timer_display()
    ...
    ld_src_op = q.mk_lat_data(
        [
            [
                "expr_name",
                len(expr_names[idx_arr_of_src_op_only]),
                list(expr_names[idx_arr_of_src_op_only]),
            ],
            [
                "t_op",
                t_size,
                [str(t) for t in range(t_size)],
            ],
        ]
    )
    ...
    q.json_results_append(
        f"{fname}: ld_src_op sig", q.get_data_sig(ld_src_op, q.RngState())
    )
    for i, en in enumerate(expr_names[idx_arr_of_src_op_only]):
        q.json_results_append(
            f"{fname}: ld '{en}' sig", q.get_data_sig(ld_src_op[i], q.RngState())
        )
    ld_snk_op = q.mk_lat_data(
        [
            [
                "expr_name",
                len(expr_names[idx_arr_of_snk_op_only]),
                list(expr_names[idx_arr_of_snk_op_only]),
            ],
            [
                "t_op",
                t_size,
    ...
    q.json_results_append(
        f"{fname}: ld_snk_op sig", q.get_data_sig(ld_snk_op, q.RngState())
    )
...
def get_cexpr_pi0_gg():
    ...
    def calc_cexpr():
        ...
        diagram_type_dict[()] = "1"
        diagram_type_dict[
            ((("t_1", "x_1"), 1), (("x_1", "t_1"), 1), (("x_2", "x_2"), 1))
        ] = "TypeD"
        diagram_type_dict[
            ((("t_1", "x_1"), 1), (("x_1", "x_2"), 1), (("x_2", "t_1"), 1))
        ] = "TypeC"
        diagram_type_dict[
            ((("t_2", "x_1"), 1), (("x_1", "t_2"), 1), (("x_2", "x_2"), 1))
        ] = "TypeD"
        diagram_type_dict[
            ((("t_2", "x_1"), 1), (("x_1", "x_2"), 1), (("x_2", "t_2"), 1))
        ] = "TypeC"
        #
        jj_d_list = [
            sum(
        ...
        exprs_list_pi0_decay = [
            ...
            for pi0d in pi0d_list
            for jj_d in jj_d_list
        ]
...
def auto_contract_pi0_gg(job_tag, traj, get_get_prop):
    ...
    for i, en in enumerate(expr_names):
        q.json_results_append(
            f"{fname}: ld_sum '{en}' sig",
            q.get_data_sig(ld_sum[i], q.RngState()),
            20e-5,
        )
...
def run_job(job_tag, traj):
    ...
    fns_props = [
        (
            f"{job_tag}/prop-psrc-light/traj-{traj_gf}.qar",
            f"{job_tag}/prop-psrc-light/traj-{traj_gf}/geon-info.txt",
        ),
        (
            f"{job_tag}/prop-psrc-strange/traj-{traj_gf}.qar",
            f"{job_tag}/prop-psrc-strange/traj-{traj_gf}/geon-info.txt",
        ),
        (
            f"{job_tag}/prop-wsrc-light/traj-{traj_gf}.qar",
            f"{job_tag}/prop-wsrc-light/traj-{traj_gf}/geon-info.txt",
        ),
        (
            f"{job_tag}/prop-wsrc-strange/traj-{traj_gf}.qar",
            f"{job_tag}/prop-wsrc-strange/traj-{traj_gf}/geon-info.txt",
        ),
        (
            f"{job_tag}/psel-prop-wsrc-light/traj-{traj_gf}.qar",
            f"{job_tag}/psel-prop-wsrc-light/traj-{traj_gf}/checkpoint.txt",
        ),
        (
            f"{job_tag}/psel-prop-wsrc-strange/traj-{traj_gf}.qar",
            f"{job_tag}/psel-prop-wsrc-strange/traj-{traj_gf}/checkpoint.txt",
        ),
    ]
    #
    ...
    def run_with_eig():
        ...
        run_prop_psrc_checker(
            job_tag, traj_gf, inv_type=0, get_gf=get_gf, get_eig=get_eig, get_gt=get_gt
        )
        run_prop_wsrc_checker(
            job_tag, traj_gf, inv_type=0, get_gf=get_gf, get_eig=get_eig, get_gt=get_gt
        )
    ...
    def run_with_eig_strange():
        ...
        run_prop_psrc_checker(
            job_tag, traj_gf, inv_type=1, get_gf=get_gf, get_eig=get_eig, get_gt=get_gt
        )
        run_prop_wsrc_checker(
            job_tag, traj_gf, inv_type=1, get_gf=get_gf, get_eig=get_eig, get_gt=get_gt
        )
...
set_param("test-4nt8", "lanc_params", 0, 0, "cheby_params")(
    {
        "low": 0.5,
        "high": 5.5,
        "order": 40,
...
set_param("test-4nt8", "lanc_params", 0, 0, "irl_params")(
    {
        "Nstop": 100,
        "Nk": 150,
        "Nm": 200,
...
set_param("test-4nt8", "clanc_params", 0, 0, "block")(
    [
        4,
        4,
        2,
...
set_param("test-4nt8", "clanc_params", 0, 0, "cheby_params")(
    {
        "low": 0.5,
        "high": 5.5,
        "order": 40,
...
set_param("test-4nt8", "clanc_params", 0, 0, "save_params")(
    {
        "nsingle": 100,
        "mpi": [
            1,
...
set_param("test-4nt8", "clanc_params", 0, 0, "irl_params")(
    {
        "Nstop": 100,
        "Nk": 150,
        "Nm": 200,
...
set_param("test-4nt8", "clanc_params", 1, 0)(
    get_param("test-4nt8", "clanc_params", 0, 0).copy()
)
set_param("test-4nt8", "lanc_params", 1, 0)(
    get_param("test-4nt8", "lanc_params", 0, 0).copy()
)
set_param("test-4nt8", "lanc_params", 1, 0, "fermion_params")(
...
set_param("test-4nt16", "lanc_params", 0, 0, "cheby_params")(
    {
        "low": 0.45,
        "high": 5.5,
        "order": 20,
...
set_param("test-4nt16", "lanc_params", 0, 0, "irl_params")(
    {
        "Nstop": 1000,
        "Nk": 1100,
        "Nm": 1300,
...
set_param("test-4nt16", "clanc_params", 0, 0, "block")(
    [
        4,
        4,
        2,
...
set_param("test-4nt16", "clanc_params", 0, 0, "cheby_params")(
    {
        "low": 0.45,
        "high": 5.5,
        "order": 20,
...
set_param("test-4nt16", "clanc_params", 0, 0, "save_params")(
    {
        "nsingle": 100,
        "mpi": [
            1,
...
set_param("test-4nt16", "clanc_params", 0, 0, "irl_params")(
    {
        "Nstop": 1000,
        "Nk": 1100,
        "Nm": 1300,
...
set_param("test-4nt16", "clanc_params", 1, 0)(
    get_param("test-4nt16", "clanc_params", 0, 0).copy()
)
set_param("test-4nt16", "lanc_params", 1, 0)(
    get_param("test-4nt16", "lanc_params", 0, 0).copy()
)
set_param("test-4nt16", "lanc_params", 1, 0, "fermion_params")(
    get_param("test-4nt16", "fermion_params", 1, 0).copy()
)

set_param("test-4nt64", "traj_list")(list(range(1000, 10000)))
set_param("test-4nt64", "lanc_params", 0, 0, "cheby_params")(
    {
        "low": 0.22,
        "high": 5.5,
        "order": 30,
...
set_param("test-4nt64", "lanc_params", 0, 0, "irl_params")(
    {
        "Nstop": 1000,
        "Nk": 1100,
        "Nm": 1300,
...
set_param("test-4nt64", "clanc_params", 0, 0, "block")(
    [
        4,
        4,
        2,
...
set_param("test-4nt64", "clanc_params", 0, 0, "cheby_params")(
    {
        "low": 0.22,
        "high": 5.5,
        "order": 30,
...
set_param("test-4nt64", "clanc_params", 0, 0, "save_params")(
    {
        "nsingle": 100,
        "mpi": [
            1,
...
set_param("test-4nt64", "clanc_params", 0, 0, "irl_params")(
    {
        "Nstop": 1000,
        "Nk": 1100,
        "Nm": 1300,
...
set_param("test-4nt64", "clanc_params", 1, 0)(
    get_param("test-4nt64", "clanc_params", 0, 0).copy()
)
set_param("test-4nt64", "lanc_params", 1, 0)(
    get_param("test-4nt64", "lanc_params", 0, 0).copy()
)
set_param("test-4nt64", "lanc_params", 1, 0, "fermion_params")(
    get_param("test-4nt64", "fermion_params", 1, 0).copy()
)
...
set_param(
    "test-4nt8", "measurement", "auto_contract_meson_meson_i0_j0_corr_wf", "sample_num"
)(32)
set_param(
    "test-4nt8", "measurement", "auto_contract_meson_meson_i0_j0_corr_wf", "sample_size"
)(2)
set_param(
    "test-4nt8", "measurement", "auto_contract_meson_meson_i0_j0_corr_wf", "t_sep_range"
)(4)
...
set_param(
    "test-4nt16", "measurement", "auto_contract_meson_meson_i0_j0_corr_wf", "sample_num"
)(32)
set_param(
    "test-4nt16",
    "measurement",
    "auto_contract_meson_meson_i0_j0_corr_wf",
    "sample_size",
)(2)
set_param(
    "test-4nt16",
...
set_param("test-4nt64", "measurement", "auto_contract_meson_corr_wf", "sample_size")(
    128
)
set_param("test-4nt64", "measurement", "auto_contract_meson_corr_wf", "t_sep_range")(17)
set_param(
    "test-4nt64", "measurement", "auto_contract_meson_meson_i0_j0_corr_wf", "sample_num"
)(512)
set_param(
    "test-4nt64",
    "measurement",
...
for inv_type in [
    ...
    set_param(job_tag, "lanc_params", inv_type, 0, "fermion_params")(
        get_param(job_tag, "fermion_params", inv_type, 0).copy()
    )
...
set_param(job_tag, "clanc_params", 0, 0, "smoother_params")(
    {"eps": 1e-08, "maxiter": 20}
)
set_param(job_tag, "clanc_params", 1, 0)(
    get_param(job_tag, "clanc_params", 0, 0).copy()
)
