...
def get_cexpr_meson_jj():
    ...
    def calc_cexpr():
        ...
        jwj_list = [
            ...
            #
            mk_sw5("x_1") * mk_j_mu("x_2", 3) + "sw5(0) * j_t(x)",
            #
            sum([mk_jw_a_mu("x_1", mu) * mk_j_mu("x_2", mu) for mu in range(3)])
            + "jw_a_i(0) * j_i(x)",
            #
            sum(
                [
        ...
        jjw_list = [
            mk_jw_a_mu("x_2", 3) * mk_j_mu("x_1", 3) + "jw_a_t(0) * j_t(-x)",
            #
            mk_sw5("x_2") * mk_j_mu("x_1", 3) + "sw5(0) * j_t(-x)",
            #
            sum([mk_jw_a_mu("x_2", mu) * mk_j_mu("x_1", mu) for mu in range(3)])
            + "jw_a_i(0) * j_i(-x)",
            #
            sum(
                [
...
def get_cexpr_meson_jwjj():
    ...
    def calc_cexpr():
        ...
        jj_list = [
            ...
            (mk_j_mu("x_2", 3) * mk_j_mu("x_1", 3) + "j_t(x) * j_t(y)"),
        ]
...
def auto_contract_meson_jwjj(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob):
    ...
    wsrc_psrc_prop_norm_sqrt = np.array(
        [
            q.parallel_map(
                lambda t: load_wsrc_psrc_prop_norm_sqrt(flavor, t),
                range(t_size),
                verbose=1,
            )
            for flavor in [
    ...
    wsrc_psnk_prop_norm_sqrt = np.array(
        [
            q.parallel_map(
                lambda t: load_wsrc_psnk_prop_norm_sqrt(flavor, t),
                range(t_size),
                verbose=1,
            )
            for flavor in [
    ...
    def feval(args):
        ...
        xg2_src_t = xg2_src[3]
        x_rel = [
            q.rel_mod(xg2_src[mu] - xg1_src[mu], total_site[mu]) for mu in range(4)
        ]
        r_sq = q.get_r_sq(x_rel)
        ...
        t_1_arr = (
            np.minimum(0, np.minimum(xg1_xg_t_arr, xg2_xg_t_arr)) + xg_t_arr - tsep
        ) % t_size
        t_2_arr = (
            np.maximum(0, np.maximum(xg1_xg_t_arr, xg2_xg_t_arr)) + xg_t_arr + tsep
        ) % t_size
    ...
    def sum_function(val_list):
        ...
        for idx1, idx2, results in val_list:
            ...
            if idx_pair % (n_pairs // 1000 + 100) == 0:
                q.displayln_info(
                    1,
                    f"{fname}: {idx_pair}/{n_pairs} {xg1_src} {xg2_src} {len(results)}/{n_elems} n_total={n_total} n_selected={n_selected} ratio={n_selected / n_total}",
                )
        q.displayln_info(
            1,
    ...
    res_sum *= 2.0 / (
        total_volume / t_size
    )  # factor of 2 account for only calculating half of the pairs
    ld_sum = q.mk_lat_data(
        [
            [
                "expr_name",
...
def auto_contract_meson_jwjj2(
    ...
    u_rand_prob = q.SelectedFieldRealD(fsel, 1)
    u_rand_prob.set_rand(
        q.RngState(f"auto_contract_meson_jwjj2,{get_job_seed(job_tag)},{traj}"),
        1.0,
        0.0,
    )
    u_rand_prob_arr = np.asarray(u_rand_prob).ravel()
    ...
    def feval(args):
        ...
        t_1_arr = (
            np.minimum(0, np.minimum(xg_1_xg_t_arr, xg_2_xg_t_arr)) + xg_w_t_arr - tsep
        ) % t_size
        t_2_arr = (
            np.maximum(0, np.maximum(xg_1_xg_t_arr, xg_2_xg_t_arr)) + xg_w_t_arr + tsep
        ) % t_size
        weight_arr = weight_base * get_weight(
            idx_w, idx_1, idx_2_arr, xg_w_t_arr, t_1_arr, t_2_arr
        )
