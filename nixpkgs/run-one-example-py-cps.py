...
"""
Run a single qlat Python CPS example test using a pre-built nix environment.

This script sets up the environment from a qlat build produced by nix and then
runs the test via make with the examples-py-cps/Makefile.  The Makefile itself
...
Examples:
  ./nixpkgs/run-one-example-py-cps.py cps-qlat-io-test
"""
...
def list_available_tests(project_root):
    examples_dir = os.path.join(project_root, "examples-py-cps")
...
def validate_test_script(project_root, test_name):
    test_script = os.path.join(project_root, "examples-py-cps", f"{test_name}.py")
...
def prepare_work_dir(project_root, test_name):
    work_dir = os.path.join(project_root, "tmp", "examples-py-cps")
    ...
    if os.path.exists(p_dir):
        shutil.rmtree(p_dir)
    #
    src_dir = os.path.join(project_root, "examples-py-cps")
...
def main():
    ...
    print(f"      in {os.path.join(project_root, 'examples-py-cps')}/")
    print()
