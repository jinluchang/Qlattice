...
def list_available_tests(project_root):
    examples_dir = os.path.join(project_root, "examples-cpp-grid")
    tests = sorted(
        f
        for f in os.listdir(examples_dir)
        if os.path.isdir(os.path.join(examples_dir, f)) and not f.startswith(".")
    )
    print(
...
def validate_test_dir(project_root, test_name):
    test_dir = os.path.join(project_root, "examples-cpp-grid", test_name)
...
def prepare_work_dir(project_root, test_name):
    work_dir = os.path.join(project_root, "tmp", "examples-cpp-grid")
    ...
    #
    src_dir = os.path.join(project_root, "examples-cpp-grid")
    shutil.copytree(
...
def main():
    ...
    print(f"      in {os.path.join(project_root, 'examples-cpp-grid')}/")
    print()
