{
  # Name of the Python CPS example/test to run. Must match a .py file in examples-py-cps/.
  # Defaults to "cps-qlat-io-test" if null.
  # E.g. "cps-qlat-io-test"
  test-name ? null,
...
in let
  ...
  test-name = if test-name-ini == null then "cps-qlat-io-test" else test-name-ini;
...
in (pkgs.python3.pkgs.buildPythonPackage.override { stdenv = pkgs.qlat-stdenv; }) rec {
  ...
  pname = "qlat-example-py-cps-${test-name}";
  version = pkgs.qlat-version;

  src = pkgs.qlat-src.examples-py-cps;

  pyproject = false;
  enableParallelBuilding = true;

  build-system = [ qlat qlat_cps qlat_grid gpt-lehner ];
  ...
  dependencies = [ qlat qlat_grid qlat_cps gpt-lehner ];
  ...
  in extra + ''
    ...
    cp ${test-name}.py "$out/share/qlat/examples-py-cps/"
    cp ${test-name}.log.json "$out/share/qlat/examples-py-cps/" 2>/dev/null || true
    cp ${test-name}.log "$out/share/qlat/examples-py-cps/"
    cp -r ${test-name}.py.p "$out/share/qlat/examples-py-cps/" 2>/dev/null || true
  '';
