...
The test is run inside ./tmp/examples-py/<test-name>.py.p/.  After the run,
...
Examples:
  ...
  ./nixpkgs/run-one-example-py.py qtopo-measure --cudasupport
"""
...
def prepare_work_dir(project_root, test_name):
    work_dir = os.path.join(project_root, "tmp", "examples-py")
    ...
    if os.path.exists(p_dir):
        shutil.rmtree(p_dir)
    #
    src_dir = os.path.join(project_root, "examples-py")
...
def main():
    ...
    print(f"      in {os.path.join(project_root, 'examples-py')}/")
    print()
