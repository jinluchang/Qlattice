{
  # Name of the Python GPT example/test to run. Must match a .py file in examples-py-gpt/.
  # Defaults to "qtopo-measure-dwf" if null.
  # E.g. "qtopo-measure-dwf", "gpt-qlat-auto-simple", "grid-qlat-io-test"
  test-name ? null,
...
in let
  ...
  test-name = if test-name-ini == null then "qtopo-measure-dwf" else test-name-ini;
  ...
  qlat = pkgs.qlat;
  qlat_grid = pkgs.qlat_grid;
  gpt-lehner = pkgs.gpt-lehner;
...
in (pkgs.python3.pkgs.buildPythonPackage.override { stdenv = pkgs.qlat-stdenv; }) rec {
  ...
  pname = "qlat-example-py-gpt-${test-name}";
  version = pkgs.qlat-version;

  src = pkgs.qlat-src.examples-py-gpt;

  pyproject = false;
  enableParallelBuilding = true;

  build-system = [ qlat qlat_grid gpt-lehner ];
  ...
  dependencies = [ qlat qlat_grid gpt-lehner ];
  ...
  in extra + ''
    ...
    cp ${test-name}.py "$out/share/qlat/examples-py-gpt/"
    cp ${test-name}.log.json "$out/share/qlat/examples-py-gpt/" 2>/dev/null || true
    cp ${test-name}.log "$out/share/qlat/examples-py-gpt/"
    cp -r ${test-name}.py.p "$out/share/qlat/examples-py-gpt/" 2>/dev/null || true
  '';
