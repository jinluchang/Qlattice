{
  # Name of the C++ Grid example/test to run. Must match a subdirectory in examples-cpp-grid/.
  # Defaults to "grid-with-qlat" if null.
  # E.g. "grid-with-qlat"
  test-name ? null,
...
in let
  ...
  test-name = if test-name-ini == null then "grid-with-qlat" else test-name-ini;
...
in (pkgs.python3.pkgs.buildPythonPackage.override { stdenv = pkgs.qlat-stdenv; }) rec {
  ...
  pname = "qlat-example-cpp-grid-${test-name}";
  version = pkgs.qlat-version;

  src = pkgs.qlat-src.examples-cpp-grid;
  ...
  in extra + ''
    ...
    cp -r ${test-name} "$out/share/qlat/examples-cpp-grid/" 2>/dev/null || true
    cp ${test-name}/log "$out/share/qlat/examples-cpp-grid/${test-name}.log" 2>/dev/null || true
    cp ${test-name}/log.full "$out/share/qlat/examples-cpp-grid/${test-name}.log.full" 2>/dev/null || true
  '';
