...
def check_environment() -> None:
    ...
    """
    print_step(
        1, "Environment sanity checks: VERSION existence, git root, and clean worktree"
    )
    ...
    if status:
        print(
            "ERROR: Working tree is not clean. Commit or stash changes before proceeding."
        )
        sys.exit(1)
    #
...
def build_pull_loop() -> None:
    ...
    while True:
        ...
        if mtime_after == mtime_before:
            print(
                f"ERROR: {result_link} was not updated by build script (stale symlink)."
            )
            sys.exit(1)
        #
...
def tag_version() -> None:
    ...
    existing = subprocess.run(
        ["git", "tag", "--list", version], stdout=subprocess.PIPE, text=True
    ).stdout.strip()
    if existing:
...
def create_github_release() -> None:
    ...
    try:
        for attempt in range(3):
            try:
                ...
                if (
                    "tag already exists" in error_output.lower()
                    or "already exists" in error_output.lower()
                ):
                    print(
                        f"INFO: GitHub Release '{version}' already exists, skipping creation."
                    )
                ...
                if attempt < 2:
                    ...
                    print("WARNING: GitHub Release creation failed after 3 attempts.")
    ...
    #
        ...
        print("WARNING: Unexpected error creating GitHub Release:")
...
def commit_bump() -> None:
    ...
    subprocess.run(
        ["git", "commit", "-m", f"Upgrade version to {new_version}"], check=True
    )
