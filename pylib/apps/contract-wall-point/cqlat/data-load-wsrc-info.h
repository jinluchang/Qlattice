...
{
  if (type == 0) {
    return ssprintf("%s/wall-src-info-light/%s/traj=%d.txt",
                    get_data_path().c_str(), job_tag.c_str(), traj);
  } else if (type == 1) {
    return ssprintf("%s/wall-src-info-strange/%s/traj=%d.txt",
                    get_data_path().c_str(), job_tag.c_str(), traj);
  } else {
    qassert(false);
    return "";
                                  ...
                                  const int type)
{
  ...
  if (check_prop_wsrc(job_tag, traj, type)) {
    ...
    std::string path;
    if (type == 0) {
      path = ssprintf("%s/wall-src-info-light", get_data_path().c_str());
    } else if (type == 1) {
      path = ssprintf("%s/wall-src-info-strange", get_data_path().c_str());
    }
    qmkdir_info(path);
