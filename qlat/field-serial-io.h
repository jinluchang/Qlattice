  ...
  if (get_id_node() == 0) {
    ...
    for (int new_id_node = 0; new_id_node < new_num_node; ++new_id_node) {
      ...
      if (0 == id_node) {
        ...
        MPI_Recv(v.data(), v.data_size(), MPI_BYTE, id_node, mpi_tag, get_comm(), MPI_STATUS_IGNORE);
      }
      qwrite_data(v, fp);
    }
    qclose(fp);
  ...
  // eg. new_size_node = Coordinate(1,1,1,2)
{
  ...
  if (get_id_node() == 0) {
    ...
    for (size_t i = 0; i < fs.size(); ++i) {
      ...
      MPI_Recv(v.data(), v.data_size(), MPI_BYTE, 0, mpi_tag, get_comm(), MPI_STATUS_IGNORE);
    }
  }
  ...
  // eg. new_size_node = Coordinate(1,1,1,2)
{
  ...
  if (fs.size() > 0) {
    FILE* fp = qopen(path, "r");
    qassert(fp != NULL);
    fseek(fp, offset + fs[0].geo.geon.id_node * get_data(fs[0]).data_size(), whence);
    for (size_t i = 0; i < fs.size(); ++i) {
      Vector<M> v = get_data(fs[i]);
      qread_data(v, fp);
    }
