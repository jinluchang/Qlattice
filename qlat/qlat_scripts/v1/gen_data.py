...
def run_prop_wsrc_full(job_tag, traj, *, inv_type, get_gf, get_eig, get_gt, get_wi):
    ...
    if get_load_path(path_s) is not None:
        q.displayln_info(
            f"{fname}: {job_tag} {traj} {inv_type_name} already have sparse wsrc prop. Skip calculating the full prop."
        )
        return
    if q.obtain_lock(f"locks/{job_tag}-{traj}-{fname}-{inv_type_name}"):
        ...
        eig = get_eig()
        wi = get_wi()
        compute_prop_wsrc_full_all(
            job_tag, traj, inv_type=inv_type, gf=gf, gt=gt, wi=wi, eig=eig
        )
        q.release_lock()
        return [
...
def avg_weight_from_prop_full(geo, prop_nf_dict):
    ...
    for inv_type in [
        ...
        avg_nf_glb_sum_tslice[inv_type] = avg_nf_glb_sum_tslice[inv_type] / (
            geo.total_volume / total_site[3] * n_samples[inv_type]
        )
        q.displayln_info(
            -1,
            fname,
            "avg_nf_glb_sum_tslice",
            inv_type_name,
            avg_nf_glb_sum_tslice[inv_type],
        )
    ...
    for k, v in prop_nf_dict.items():
        ...
        weight = (
            prop_nf[:].reshape(local_field_shape) / avg_gst_local[:, None, None, None]
        )
...
def make_fsel_from_weight(f_weight, f_rand_01, rate):
    ...
    q.displayln_info(
        -1,
        f"{fname}: rate = {rate} ; expect_num = {geo.total_volume * rate} ; actual_num = {q.glb_sum(fsel.n_elems)}",
    )
    return fsel
    ...
    *,
    ...
    for inv_type in inv_type_list:
        ...
        for tslice in range(total_site[3]):
            ...
            ] = prop_nf, prop_nf.glb_sum_tslice()
    assert idx >= 1
    ...
    for inv_type in [
        ...
        inv_type_name = inv_type_name_list[inv_type]
        f_weight_avg[inv_type].save_double(
            get_save_path(fn_f_weight_type_list[inv_type])
        )
        q.displayln_info(
            -1,
            fname,
            "field-selection-weight",
            inv_type_name,
            f_weight_avg[inv_type].glb_sum_tslice()[:].ravel(),
        )
...
def run_f_weight_from_wsrc_prop_full(job_tag, traj):
    ...
    for inv_type in inv_type_list:
        if get_load_path(path_f_list[inv_type]) is None:
            ...
            q.displayln_info(
                -1,
                f"WARNING: {fname}: {job_tag} {traj} {inv_type_name} full prop is not available yet.",
            )
            return None
...
def run_f_rand_01(job_tag, traj):
    ...
    rs = (
        q.RngState(f"{seed}-{traj}")
        .split("run_sel_from_wsrc_prop_full")
        .split("f_rand_01")
    )
...
def run_fsel_prob(job_tag, traj, *, get_f_rand_01, get_f_weight):
    ...
    if get_load_path(fn_fsel) is not None:
        if get_load_path(fn_fsel_prob) is not None:
            ...
            q.displayln_info(
                f"{fname}: WARNING: field-selection exist but prob is not available. Assuming loading load old data format."
            )
...
def run_psel_prob(job_tag, traj, *, get_f_rand_01, get_f_weight, tag=None):
    ...
    if get_load_path(fn_psel) is not None:
        if get_load_path(fn_psel_prob) is not None:
            ...
            q.displayln_info(
                f"{fname}: WARNING: points-selection exist but prob is not available. Assuming loading load old data format."
            )
...
def run_fsel_prob_sub_sampling(
    ...
    *,
    ...
    def get_fsel_prob_sub():
        ...
        q.displayln_info(
            -1,
            f"{fname}: sub_sampling_rate = {sub_sampling_rate} ; expect_num = {expect_num} ; actual_num = {actual_num}",
        )
...
def run_psel_prob_sub_sampling(
    ...
    *,
    ...
    def get_psel_prob_sub():
        ...
        use_simple_psel_sub_sampling = get_param(
            job_tag, "use_simple_psel_sub_sampling", default=False
        )
...
def run_fsel_split(
    ...
    *,
    ...
    fname = q.get_fname()
    path_psel_list = (
        f"{job_tag}/field-selection-split/traj-{traj}/num-piece-{num_piece}"
    )
    #
    @q.lazy_call
    @q.timer(is_timer_fork=True)
...
def save_prop_wsrc_sparse(
    ...
    if tag in sfw:
        ...
        q.displayln_info(
            0, f"{fname}: tag='{tag}' of '{job_tag}/{traj}' already saved. Skipping."
        )
        return
    prop = load_prop()
    if prop is None:
...
def run_prop_wsrc_sparse(
    ...
    *,
    ...
    is_performing_inversion = get_param(
        job_tag, "is_performing_inversion_if_no_full_prop_available", default=False
    )
...
def compute_prop_2(
    ...
    *,
    ...
    q.displayln_info(
        0,
        f"compute_prop_psrc: tag='{tag}' ; num_fsel={num_fsel} ; num_fsel_ps={num_fsel_ps} ; num_fsel_combine={num_fsel_combine}",
    )
    s_sol_ps_sel_prob = q.SelectedFieldRealD(fsel_ps)
    s_sol_ps_sel_prob @= sol_ps_sel_prob
    s_sol_ps_sel_prob.save_double(
        sfw, f"{tag} ; fsel-prob-psrc-prop", skip_if_exist=True
    )
...
def compute_prop_psrc(
    ...
    *,
    ...
    total_site = q.Coordinate(get_param(job_tag, "total_site"))
    fsel_psrc_prop_norm_threshold = get_param(
        job_tag, "field_selection_fsel_psrc_prop_norm_threshold"
    )
    geo = q.Geometry(total_site)
...
def compute_prop_psrc_all(
    job_tag, traj, *, inv_type, gf, gt, psel, fsel, f_rand_01, eig
):
    inv_type_name_list = [
        "light",
        "strange",
    ...
    is_saving_hvp_ts = get_param(
        job_tag, "run_prop_psrc", "is_saving_hvp_ts", default=True
    )
    if is_saving_hvp:
        sfw_hvp = q.open_fields(
            get_save_path(path_s_hvp + ".acc"),
            "a",
            q.Coordinate(
                [
                    2,
    ...
    seed = get_job_seed(job_tag)
    rs = q.RngState(f"seed {seed} {traj}").split("compute_prop_psrc_all(ama)")
...
def compute_hvp_average(job_tag, traj, *, inv_type, psel_prob, data_path, geo):
    ...
    for p_idx in range(len(psel)):
        ...
        ama_val = q.mk_ama_val(
            val_list[0], xg_src.to_tuple(), val_list, rel_acc_list, prob_list
        )
...
def run_hvp_average(job_tag, traj, *, inv_type, get_psel_prob):
    ...
    if not q.obtain_lock(f"locks/{job_tag}-{traj}-{fname}-{inv_type_name}"):
        return
    psel_prob = get_psel_prob()
    hvp_average = compute_hvp_average(
        job_tag,
        traj,
        inv_type=inv_type,
        psel_prob=psel_prob,
    ...
    q.displayln_info(
        f"{fname}: {job_tag} {traj} {inv_type} num_fields={len(psel_prob.psel)}"
    )
    q.release_lock()
    return load
...
def run_field_rand_u1_dict(
    ...
    geo = q.Geometry(total_site)
    seed = get_job_seed(job_tag)
    rs_rand_u1 = q.RngState(f"seed {seed} {traj}").split(
        "compute_prop_rand_sparse_u1_src(rand_u1)"
    )
    for psel_list_type in ["fsel", "psel"]:
        ...
        if is_test():
            q.json_results_append(
                f"{fname}: {psel_list_type} fu1",
                q.get_data_sig_arr(fu1, q.RngState(), 4),
                1e-15,
            )
            q.json_results_append(
                f"{fname}: {psel_list_type} fu1_dag",
                q.get_data_sig_arr(fu1_dag, q.RngState(), 4),
                1e-15,
            )
...
def run_prop_sparse_rand_u1_src(
    ...
    *,
    ...
    path_s = (
        f"{job_tag}/prop-rand-u1-{psel_list_type}-sparse-{quark_flavor}/traj-{traj}"
    )
    ...
    if not q.obtain_lock(
        f"locks/{job_tag}-{traj}-{fname}-{psel_list_type}-{quark_flavor}"
    ):
        return
    gf = get_gf()
    geo = gf.geo
    ...
    seed = get_job_seed(job_tag)
    rs_ama = q.RngState(f"seed {seed} {traj}").split("compute_prop_rand_u1(ama)")
    #
    @q.timer
    def compute_and_save(idx_psel, is_dagger, inv_acc):
        ...
        if q.get_id_node() == 0:
            if f"{tag}.lat" in qar_sp:
                q.displayln_info(
                    -1,
                    f"WARNING: {fname}: '{tag}.lat' already exist in {qar_sp.path()}",
                )
        if f"{tag} ; fu1" in sfw:
            q.displayln_info(
                -1, f"WARNING: {fname}: '{tag} ; fu1' already exist in {sfw.path()}"
            )
        q.check_stop()
        q.check_time_limit()
        ...
        qar_sp.write(
            f"{tag} ; psel_rand_src.lati",
            "",
            psel_rand_src.save_str(),
            skip_if_exist=True,
        )
        qar_sp.write(
            f"{tag} ; psel_rand_src ; fu1.lat",
            "",
            sp_fu1.save_str(),
            skip_if_exist=True,
        )
        qar_sp.write(
            f"{tag} ; psel_rand_src ; prop.lat",
            "",
            ps_prop_prs.save_str(),
            skip_if_exist=True,
        )
        qar_sp.write(f"{tag}.lat", "", ps_prop.save_str(), skip_if_exist=True)
        qar_sp.flush()
    ...
    if is_test():
        q.json_results_append(
            f"{fname} {job_tag} {traj} inv_type={inv_type} psel_list_type={psel_list_type}"
        )
        ...
        for tag in sfr_list:
            ...
            q.json_results_append(
                f"{fname} {tag}", q.get_data_sig_arr(s_prop, q.RngState(), 2), 1e-4
            )
        sfr.close()
        ...
        for tag in qar_sp_list:
            if tag.endswith(".lati"):
                ...
                assert False
            q.json_results_append(
                f"{fname} {tag}", q.get_data_sig_arr(ld, q.RngState(), 2), 1e-4
            )
        qar_sp.close()
    q.release_lock()
    return [
...
def compute_prop_rand_u1_type_acc(
    *, sfw, job_tag, traj, gf, eig, fsel, idx_rand_u1, inv_type, inv_acc
):
    """
    ...
    seed = get_job_seed(job_tag)
    rs = (
        q.RngState(f"seed {seed} {traj}")
        .split("compute_prop_rand_u1(rand_u1)")
        .split(str(idx_rand_u1))
    )
...
def compute_prop_rand_u1(*, job_tag, traj, inv_type, gf, path_s, fsel, eig=None):
    ...
    #
    seed = get_job_seed(job_tag)
    rs = q.RngState(f"seed {seed} {traj}").split("compute_prop_rand_u1(ama)")
...
def compute_prop_3(
    ...
    *,
    ...
    qar_sp.write(
        f"{tag} ; smear-snk.lat", "", sol_smear_psel.save_str(), skip_if_exist=True
    )
...
def compute_prop_smear_all(
    ...
    *,
    ...
    seed = get_job_seed(job_tag)
    rs = q.RngState(f"seed {seed} {traj}").split("compute_prop_smear_all(ama)")
...
def run_prop_smear(
    ...
    *,
    ...
    inv_type_name = inv_type_name_list[inv_type]
    if (
        get_load_path(f"{job_tag}/prop-smear-{inv_type_name}/traj-{traj}/geon-info.txt")
        is not None
    ):
        assert (
            get_load_path(
                f"{job_tag}/psel-prop-smear-{inv_type_name}/traj-{traj}/checkpoint.txt"
            )
            is not None
        )
        assert (
            get_load_path(
                f"{job_tag}/psel_smear_median-prop-smear-{inv_type_name}/traj-{traj}/geon-info.txt"
            )
            is not None
        )
        return
