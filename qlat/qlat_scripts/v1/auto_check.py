...
def run_get_inverter_checker(
    job_tag, traj, *, inv_type, get_gf, get_gt=None, get_eig=None
):
    if None in [
        get_gf,
    ]:
...
def compute_prop_wsrc_checker(
    job_tag, tslice, inv_type, inv_acc, *, idx, gf, gt, sfw, path_sp, eig, finished_tags
):
    from . import rbc_ukqcd as ru
    #
    ...
    if tag in finished_tags:
        return None
    q.displayln_info(
        f"compute_prop_wsrc: idx={idx} tslice={tslice}", job_tag, inv_type, inv_acc
    )
...
def compute_prop_wsrc_all_checker(job_tag, traj, *, inv_type, gf, gt, eig):
    ...
    q.qar_create_info(
        get_save_path(path_sp + ".qar"),
        get_save_path(path_sp),
        is_remove_folder_after=True,
    )
    q.qar_create_info(
        get_save_path(path_s + ".qar"),
        get_save_path(path_s),
        is_remove_folder_after=True,
    )
...
def run_prop_wsrc_checker(job_tag, traj, *, inv_type, get_gf, get_eig, get_gt):
    ...
    inv_type_name = inv_type_names[inv_type]
    if (
        get_load_path(f"{job_tag}/prop-wsrc-{inv_type_name}/traj-{traj}/geon-info.txt")
        is not None
    ):
        return
    if q.obtain_lock(f"locks/{job_tag}-{traj}-wsrc-{inv_type_name}"):
        ...
        gt = get_gt()
        eig = get_eig()
        compute_prop_wsrc_all_checker(
            job_tag, traj, inv_type=inv_type, gf=gf, gt=gt, eig=eig
        )
        q.release_lock()
...
def compute_prop_psrc_checker(
    job_tag, xg_src, inv_type, inv_acc, *, idx, gf, gt, sfw, eig, finished_tags
):
    from . import rbc_ukqcd as ru
    #
    xg = xg_src.to_list()
...
def load_prop_psrc(job_tag, traj, inv_type):
    ...
    cache = q.mk_cache("prop_cache", f"{job_tag}", f"{traj}", inv_tag)
        ...
        with q.TimerFork():
            ...
            def get_prop(flavor, p_snk, p_src):
                ...
                if p_snk_tag == "point" and p_src_tag == "point":
                    ...
                    prop_list = cache["psnk-wsrc"]
                    p_src_idx = p_src_xg.to_index(total_site)
                    return q.wilson_matrix_g5_herm(
                        prop_list[p_snk_xg].get_elem_wm(p_src_idx)
                    )
