...
def mk_sample_gauge_field_v3(job_tag, fn):
    ...
    hmc_beta = get_param(job_tag, "mk_sample_gauge_field", "hmc_beta", default=6.0)
    hmc_n_step = get_param(job_tag, "mk_sample_gauge_field", "hmc_n_step", default=6)
    hmc_is_always_accept = get_param(
        job_tag, "mk_sample_gauge_field", "hmc_is_always_accept", default=True
    )
    geo = q.Geometry(total_site)
    gf = q.GaugeField(geo)
...
def load_config(job_tag: str, fn: str):
    ...
    if params is not None:
        ...
        for mu in range(4):
            ...
            if lmom != 0.0:
                q.displayln_info(
                    f"load_config fn='{fn}' twist_boundary_at_boundary lmom={lmom} mu={mu}"
                )
