...
def mk_pc_ne(job_tag, inv_type, inv_acc, *, eig=None, parity=None):
    ...
    if "omega" in params and eig is None:
        q.displayln_info(
            f"WARNING: {fname}: pc.eo2_kappa_ne() does not support split_cg. Try to avoid (use MDWF) if possible."
        )
...
def mk_ceig(job_tag, gf, inv_type, inv_acc=0, *, parity=None, pc_ne=None):
    ...
    fermion_params = params["fermion_params"]
    q.displayln_info(
        f"mk_ceig: job_tag={job_tag} inv_type={inv_type} inv_acc={inv_acc}"
    )
    ...
    #
    grid_coarse = g.block.grid(
        qm.F_grid_eo, [get_ls_from_fermion_params(fermion_params)] + cparams["block"]
    )
    ...
    eps2 = g.norm2(
        cop * cstart - b.project * ccheby(w.Mpc) * b.promote * cstart
    ) / g.norm2(cstart)
    ...
    for i, cv in enumerate(cevec):
        ...
        evals = g.algorithms.eigen.evals(w.Mpc, [tmpf], calculate_eps2=False, real=True)
...
def mk_gpt_inverter(
    ...
    *,
    ...
    if n_grouped is None:
        n_grouped = 12
    q.displayln_info(
        f"mk_gpt_inverter: job_tag={job_tag} inv_type={inv_type} inv_acc={inv_acc} mpi_split={mpi_split} n_grouped={n_grouped}"
    )
    ...
    if eig is not None:
        ...
        is_madwf = get_ls_from_fermion_params(params) != get_ls_from_fermion_params(
            params0
        )
    ...
    inv = g.algorithms.inverter
    cg_mp = inv.cg(
        {"eps": eps, "maxiter": get_param_cg_mp_maxiter(job_tag, inv_type, inv_acc)}
    )
    cg_pv_f = inv.cg(
        {
            "eps": eps,
            "maxiter": get_param(
                job_tag, f"cg_params-{inv_type}-{inv_acc}", "pv_maxiter", default=150
            ),
        }
    )
    if mpi_split is None or not mpi_split:
        cg_split = cg_mp
        mpi_split = None
    else:
    ...
    if inv_acc == 0:
        maxcycle = get_param(
            job_tag, f"cg_params-{inv_type}-{inv_acc}", "maxcycle", default=1
        )
    elif inv_acc == 1:
        maxcycle = get_param(
            job_tag, f"cg_params-{inv_type}-{inv_acc}", "maxcycle", default=2
        )
    elif inv_acc == 2:
        maxcycle = get_param(
            job_tag, f"cg_params-{inv_type}-{inv_acc}", "maxcycle", default=200
        )
    elif inv_acc == 3:
        maxcycle = get_param(
            job_tag, f"cg_params-{inv_type}-{inv_acc}", "maxcycle", default=200
    ...
    slv_qm = qm.propagator(
        inv.defect_correcting(
            inv.mixed_precision(slv_5d, g.single, g.double), eps=eps, maxiter=maxcycle
        )
    ).grouped(n_grouped)
    q.displayln_info("mk_gpt_inverter: make inv_qm")
    if qtimer is True:
