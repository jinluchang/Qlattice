...
inline WilsonLinePathSegment make_wilson_line_path_segment(const Coordinate& target)
{
  ...
  // wlf will be initialized
{
  ...
  if (pacc.stops[Coordinate()].paths.size() == 1 && pacc.stops.size() == 2) {
    set_multiply_simple_wilson_line_field_partial_comm(wlf, wlf1, gf1, pacc.stops[Coordinate()].paths[0]);
    return;
  }
  const Geometry geo = geo_reform(gf1.geo);
  std::vector<Coordinate> cs;
...
inline WilsonLinePath make_wilson_loop_path(const Coordinate& target_l, const int t)
{
