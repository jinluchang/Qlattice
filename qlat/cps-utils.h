...
bool isInitialized(const Geometry& geo) {
...
void print(const Complex& v)
{
  printf("%.16E:+%.16E", v.real(), v.imag());
}
...
inline bool notnan(const double& x)
{
  return !isnan(x);
}
...
inline bool notnan(const SpinorMatrix& x)
{
  return notnanDoubles(x);
}
...
inline bool notnan(const GridComm<M>& gc)
{
  ...
  for (long index = 0; index < geo.localVolume(); ++index) {
    int xl[4]; geo.coordinateFromIndex(xl, index);
...
class Coordinate {
  public:
    ...
    Coordinate(int x_, int y_, int z_, int t_) {
      init(x_, y_, z_, t_);
    }
    Coordinate(const int x_[4]) {
      init(x_);
    }
    ...
    void init(const int x_[4]) {
      memcpy(x, x_, 4 * sizeof(int));
    }
    //
    int& operator[](int mu) {
      assert(0 <= mu && mu < 4);
...
inline void relativeCoordinateG(int xgrel[4], const Geometry& geo, const int xg[4], const int xgref[4]) {
  for (int mu = 0; mu < 4; mu++) {
    xgrel[mu] = signMod(xg[mu] - xgref[mu], geo.totalSite(mu));
  }
}

inline void absolueCoordinateG(int xg[4], const Geometry& geo, const int xgrel[4], const int xgref[4]) {
  for (int mu = 0; mu < 4; mu++) {
    xg[mu] = mod(xgrel[mu] + xgref[mu], geo.totalSite(mu));
  }
}

inline void middleCoordinateG(int xmg[4], const Geometry& geo, const int x1g[4], const int x2g[4]) {
  for (int mu = 0; mu < 4; mu++) {
...
inline Coordinate mkCoordinateFromRefRel(const Geometry& geo, const Coordinate& c1, const Coordinate& c2) {
  Coordinate c;
...
inline long distance2TwoCoordinateG(const int xg1[4], const int xg2[4], const Geometry& geo) {
  int xgref[4]; relativeCoordinateG(xgref, geo, xg1, xg2);
  return distance2RelativeCoordinateG(xgref);
}

inline long distance2TwoCoordinateG(const Coordinate& xg1, const Coordinate& xg2, const Geometry& geo) {
  Coordinate xgref;
...
#ifdef USE_QMP
  MPI_Allgather((void*)sendbuf, sendsize, MPI_BYTE, recvbuf, sendsize, MPI_BYTE, QMP_COMM_WORLD);
#else
...
inline int getDataDirMu(void* recv, void* send, const long size, const int dir, const int mu) {
  ...
  const int ret = MPI_Recv(recv, size, MPI_BYTE, idf, 0, QMP_COMM_WORLD, MPI_STATUS_IGNORE);
...
inline mode_t& defaultDirMode()
{
  static mode_t mode = 0775;
  return mode;
}

inline int checkdir(const std::string& path, const mode_t mode = defaultDirMode()) {
  TIMER("checkdir");
  int ret = 0;
...
inline int makedir(const std::string& path, const mode_t mode = defaultDirMode()) {
  TIMER("makedir");
...
inline void saveStringAllNode(const std::string& filename, const std::string& content, bool isAppend = false) {
...
inline void saveString(const std::string& filename, const std::string& content, bool isAppend = false) {
  if (0 == getIdNode()) {
...
inline std::string showWilsonMatrix(const WilsonMatrix& mat, bool human_readable = false) {
  Float *p = (Float *)&mat;
...
inline std::string showSpinMatrix(const WilsonMatrix& mat, const int color1 = 0, const int color2 = 0, bool human_readable = false) {
  Float *p = (Float *)&mat;
  const int sizewm = sizeof(WilsonMatrix) / sizeof(WilsonVector);
  const int sizewv = sizeof(WilsonVector) / sizeof(Float);
  std::ostringstream out;
  ...
  for (int i = color1; i < sizewm; i += 3) {
    for (int j = color2 * 2; j < sizewv; j += 6) {
      out << std::setw(8) << p[i * sizewv + j] << " " << std::setw(8) << p[i * sizewv + j + 1] << " ";
    }
    out << std::endl;
  }
  return out.str();
...
inline std::string showSpinMatrix(const SpinMatrix& mat, bool human_readable = false) {
  Float *p = (Float *)&mat;
  std::ostringstream out;
  if (human_readable) {
    out.precision(1);
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      out << std::setw(8) << mat(i, j).real() << " " << std::setw(8) << mat(i, j).imag() << " ";
    }
    out << std::endl;
  }
  return out.str();
...
inline void setZero(std::complex<double>& x) {
  x = 0.0;
}

inline void setZero(Matrix& m) {
  memset(&m, 0, sizeof(m));
}

inline void setZero(WilsonMatrix& m) {
  memset(&m, 0, sizeof(m));
}

inline void setZero(SpinMatrix& m) {
  memset(&m, 0, sizeof(m));
}

inline void setUnit(Complex& x) {
  x = 1;
}
...
inline void multiplyPlus(SpinMatrix& m, const SpinMatrix& m1, const SpinMatrix& m2) {
  const int dim = 4;
  for (int i = 0; i < dim; i++) {
    for (int j = 0; j < dim; j++) {
      for (int k = 0; k < dim; k++) {
...
inline const SpinMatrix& gamma(const int dir) {
...
inline void gammaLeft(WilsonMatrix& m, const int dir) {
  m.gl(dir);
}
