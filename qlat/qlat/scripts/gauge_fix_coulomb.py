...
if len(sys.argv) == 1:
    q.displayln_info(
        "Usage: gauge-fix-coulomb [ --source source_config ] [ --output gt.field ] [ --guess gt-guess.field ]"
    )
    q.displayln_info(
        "If the output file already exist, it will only check output file."
    )
...
# Parameters
p_mpi_split = g.default.get_ivec(
    "--mpi_split",
    [
        1,
        1,
...
def load_gt_guess():
    if p_guess is None:
        ...
        q.displayln_info(
            f"WARNING: Guess file '{p_guess}' does not exist. Gauge fix from scratch."
        )
        return None
    q.displayln_info(f"Find '{p_guess}'.")
    gt = q.GaugeTransform()
    if p_guess.endswith(".gfix"):
        q.displayln_info("Assuming CPS gauge fix format.")
        gt.load_cps(p_guess)
    else:
