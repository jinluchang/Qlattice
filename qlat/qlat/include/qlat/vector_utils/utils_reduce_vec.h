...
__global__ void reduce6(const Ty* g_idata, Ty* g_odata, unsigned long n,
                        unsigned int divide)
{
  ...
  unsigned long i_end = (iv + 1) * n;
  unsigned long off_divide = gridDim.x * blockSize;
  while (i < i_end) {
    sdata[tid] += g_idata[i];
    i += off_divide;
  }
...
inline void reduce_T_global6(const Ty* src, Ty* res, const Long n, const Int nv,
                             Long nt, Long blockS)
{
  ...
  switch (threads) {
    case 1024:
      reduce6<1024, Ty, clear><<<dimGrid, dimBlock>>>(src, res, n, divide);
      break;
    case 512:
      reduce6<512, Ty, clear><<<dimGrid, dimBlock>>>(src, res, n, divide);
      break;
    case 256:
      reduce6<256, Ty, clear><<<dimGrid, dimBlock>>>(src, res, n, divide);
      break;
    case 128:
      reduce6<128, Ty, clear><<<dimGrid, dimBlock>>>(src, res, n, divide);
      break;
    case 64:
      reduce6<64, Ty, clear><<<dimGrid, dimBlock>>>(src, res, n, divide);
      break;
    case 32:
      reduce6<32, Ty, clear><<<dimGrid, dimBlock>>>(src, res, n, divide);
      break;
    case 16:
      reduce6<16, Ty, clear><<<dimGrid, dimBlock>>>(src, res, n, divide);
      break;
    case 8:
      reduce6<8, Ty, clear><<<dimGrid, dimBlock>>>(src, res, n, divide);
      break;
    case 4:
      reduce6<4, Ty, clear><<<dimGrid, dimBlock>>>(src, res, n, divide);
      break;
    case 2:
      reduce6<2, Ty, clear><<<dimGrid, dimBlock>>>(src, res, n, divide);
      break;
    case 1:
      reduce6<1, Ty, clear><<<dimGrid, dimBlock>>>(src, res, n, divide);
      break;
  }
  qacc_barrier(dummy);
}
...
#pragma omp parallel for
    for (Int ompi = 0; ompi < Nv; ompi++) {
      ...
      if (temi * bsize + bsize > (LInt)n) {
        bsize_end = n - temi * bsize;
      }
      for (size_t isp = 0; isp < bsize_end; isp++) {
        pres += psrc[isp];
      }
    }
    //
...
template <typename Ty>
inline unsigned long reduce_T(const Ty* src, Ty* res, const unsigned long n,
                              const Int nv, const unsigned long Lx)
{
                       ...
                       const Int fac = 16)
{
  ...
  for (Int si = 0; si < 1000; si++) {
    if (Nres <= cutN) {
      for (Int i = 0; i < nv; i++) {
        reduce_cpu(&psrc[i * Nres], res[i], Nres);
      }
      return;
    }
...
template <typename Ty>
void reduce_vecs(const Ty* src, Ty* res, const Long n, const Int nv,
                 Int GPU = 1)
{
  TIMERA("reduce_vec");
  if (GPU == 0) {
  ...
  if (GPU != 0) {
    ...
    // \n",cores,maxthreads,maxblock); if(blockS_use > maxblock)blockS_use =
    // maxblock; #endif
...
{
  Int GPU = 1;
  if (src.mem_type == MemType::Cpu) {
    GPU = 0;
  }
  vector<Ty> res;
  res.resize(1);  // unified memeory
