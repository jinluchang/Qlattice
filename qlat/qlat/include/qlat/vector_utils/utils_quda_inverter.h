...
struct quda_inverter {
  ...
  qlat::vector<qlat::ComplexD> quda_gf_default_twist;
  QudaTboundary t_boundary_default;
  QBOOL qinv_twist_link;
  bool qinv_use_recon;
  bool initialized;
  ...
  quda::Dirac* split_dirac_pc;
  bool split_setup;
  bool split_comm_buff;
  bool split_prec_double;
  ...
  {
    if (split_comm_buff == false) {
      return;
    } else {
      rotate_quda_split(true);
    }
  }
  inline void copy_to_buf_split(std::vector<quda::ColorSpinorField>& src,
                                const Int i);
  inline void copy_from_buf_split(std::vector<quda::ColorSpinorField>& res,
                                  const Int i);
  inline void set_quda_split(const std::vector<Int>& grid, const Int num_src,
                             const bool split_prec_double_ = true);
  ...
  */
  ...
  // another set for twist eigen
  std::vector<void*> eigenL_default;
  std::vector<Int> eigen_precL_default;
  bool eigen_with_nvec_default;
  std::vector<Int> eigen_with_nvecL_default;
  //
  std::vector<void*> eigenL_twist;
  std::vector<Int> eigen_precL_twist;
  bool eigen_with_nvec_twist;
  std::vector<Int> eigen_with_nvecL_twist;
  ...
  inline void deflate_Ty(qlat::vector<void*>& Pres, qlat::vector<void*>& Psrc,
                         double mass, Int buf_prec, Int mode = 0, Int clear = 1,
                         const bool data_from_quda = true);
  //
  // src need to be unchanged if res != src
  // res could be the same as src
  // use gtmp0
  template <typename Ty, typename Tk>
  inline void get_low_prop(qlat::vector<Ty*>& res, qlat::vector<Ty*>& src,
                           qlat::vector<Tk*>& buf, Int qlat_format = 1);
  //
  template <typename Ty>
  inline void get_inversion_bufs(qlat::vector<Ty*>& res, const Int nvecs,
                                 const Int halfV = 0)
  {
    LInt Vd =
        geo().local_volume() * 3 * sizeof(Ty) / sizeof(qlat::ComplexT<double>);
    if (halfV == 1) {
      Vd = Vd / 2;
    }
    Qassert(Vd / (geo().local_volume() * 3) <= 64);  // at most 64 vectors in
                                                     // buf
  ...
  //
  // inline void setup_inc_eigencg(const Int n_ev, const Int n_kr, const Int
  // n_conv, const Int df_grid, const double tol, const double inc_tol, const
  // double tol_restart, const Int restart_n, const Int pipeline, const Int
  // inv_type = 1);
  //
  // template<typename Ty>
  // inline void do_inv(Ty* res, Ty* src, const double mass, const double err =
  // 1e-10, const Int niter = 10000 , const Int prec_type = 0); inline void
  // deflate(quda::ColorSpinorField &sol, const quda::ColorSpinorField& src,
  //    std::vector<quda::ColorSpinorField> &evecs, const
  //    std::vector<quda::Complex > &evals, bool accumulate = false);
  //
  // inline void deflate(std::vector<quda::ColorSpinorField* > &sol, const
  // std::vector<quda::ColorSpinorField* > &src,
  //    std::vector<quda::ColorSpinorField> &evecs, const
  //    std::vector<quda::Complex > &evals, bool accumulate = false);
  //
  inline void callMultiSrcQuda(qlat::vector<void*>& res,
                               qlat::vector<void*>& src, Int max_src = -1);
  //
  void print_plaq();
  //
  ...
  //
  // inline void save_evecs(const char* filename, const bool read = false, const
  // Int ndouble = -1, const Int split_save = 1 ); inline void save_evecsF(const
  // char* filename, const bool read = false); inline void check_residualF();
  //
  //  ghost function to set precision for inv and gauge, not loaded at all, need
  //  setup_inv_param_prec to force load gauge again or use setup_link inline
  //  void setup_inv_param_prec_type(Int prec_type = 0);

  // automatic change presion based on current gauge, qinv buffers, update
  // dslash still need to set inv.param.cpu_prec for different inputs
  inline void setup_inv_param_prec(Int prec_type = 0, bool force_reload = false,
                                   bool reload_link_default = true);
  ...
  //}
  // inline void invertQuda_COPY(quda::ColorSpinorField& x,
  // quda::ColorSpinorField& b, Int solve_mode_ = 0);
  // the COPY could use single precison as input
  // inline void invertQuda_COPY(quda::ColorSpinorField& res,
  // quda::ColorSpinorField& src, Int solve_mode_ = 0);
  inline void invertQuda_COPY_single(quda::ColorSpinorField& res,
                                     quda::ColorSpinorField& src);
  inline void invertQuda_COPY_single(std::vector<quda::ColorSpinorField>& res,
                                     std::vector<quda::ColorSpinorField>& src);
  //
  ...
  //
  template <class Ty>
  inline void qlat_cf_to_quda_cf_P(void* res, Ty* src, qlat::vector<Long>& map,
                                   quda::ColorSpinorParam& param)
  {
    param.v = (void*)res;
    quda::ColorSpinorField* Qvec = NULL;
    Qvec = new quda::ColorSpinorField(param);
    qlat_cf_to_quda_cf(*Qvec, src, geo(), map);
    delete Qvec;
  }
  //
  template <class Ty>
  inline void quda_cf_to_qlat_cf_cf_P(Ty* res, void* src,
                                      ...
                                      quda::ColorSpinorParam& param)
  {
    ...
    quda::ColorSpinorField* Qvec = NULL;
    Qvec = new quda::ColorSpinorField(param);
    quda_cf_to_qlat_cf(res, *Qvec, geo(), map);
    delete Qvec;
  }
  //
  /*
  ...
  */
  inline void quda_to_single_order(vector<void*>& src, const Int prec,
                                   const Int dir = 0)
  {
    ...
    //
    qlat::vector_gpu<int8_t>& quda_buf =
        get_vector_gpu_plan<int8_t>(0, "quda_field_copy_buffers", 1);
    ...
    for (Long iv = 0; iv < src.size(); iv++) {
      ...
      //
      if (dir == 0) {
        cpy_GPU((ComplexT<double>*)quda_buf.data(), (ComplexT<float>*)src[iv],
                Ndata);
        *Qf = *Qd;
      }
      if (dir == 1) {
        *Qd = *Qf;
        cpy_GPU((ComplexT<float>*)src[iv], (ComplexT<double>*)quda_buf.data(),
                Ndata);
      }
      delete Qf;
    }
...
inline void quda_inverter::setup_eigen(eigen_cs<T1>* e0, eigen_cs<T2>* e1)
{
  ...
  for (Int i = 0; i < 2; i++) {
    ...
    if (Ea != NULL) {
      if (prec == 0) {
        eigen_cs<qlat::ComplexT<double>>* eigen =
            (eigen_cs<qlat::ComplexT<double>>*)Ea;
        total_n += eigen->get_nvec();
      }
      if (prec == 1) {
        eigen_cs<qlat::ComplexT<float>>* eigen =
            (eigen_cs<qlat::ComplexT<float>>*)Ea;
        total_n += eigen->get_nvec();
      }
    }
  ...
  if (qinv_twist_link == QTRUE) {
    ...
    eigen_with_nvecL_default = eigen_with_nvecL;
  }
}
                                        ...
                                        const QBOOL twist_link)
{
  ...
  if (twist_link == QTRUE) {
    ...
    gauge_param.t_boundary = t_boundary_default;
    eigenL = eigenL_default;
    eigen_precL = eigen_precL_default;
    eigen_with_nvec = eigen_with_nvec_default;
    eigen_with_nvecL = eigen_with_nvecL_default;
  }
  //{
  // qlat::ComplexD res =  vec_norm2(quda_gf, quda_gf, geo().local_volume() * 4
  ...
  if (qinv_twist_link == QTRUE) {
    if (quda_gf != quda_gf_default_twist.data()) {
      if (quda_gf_default_twist.size() != V * 3 * 3 * 4) {
        ...
        quda_gf_default_twist.resize(V * 3 * 3 * 4);
      }
                                      ...
                                      const QBOOL twist_link)
{
  ...
  static Int quda_link_Acc =
      qlat::get_env_long_default(std::string("qlat_quda_link_Acc"), 1);
  if (quda_link_Acc == 1) {
    quda_gf.set_mem_type(MemType::Acc);
  } else {
    quda_gf.set_mem_type(MemType::Cpu);
  }
  setup_link_p(quda_gf.data(), apply_stag_phase, force_phase, twist_link);
...
inline void quda_inverter::print_plaq()
{
  ...
  if (quda::comm_rank_global() == 0)
    printfQuda(
        "Computed plaquette is %.8e (spatial = %.8e, temporal = %.8e), "
        "topological charge = %.8e \n",
...
inline void quda_inverter::free_csfield(const Int mode)
{
  ...
  //
  if (mode == 0 or mode == 2) {
    // if(gsrcD != NULL){delete gsrcD;gsrcD=NULL;}if(gresD!= NULL){delete
    // gresD;gresD=NULL;}
    if (gsrc != NULL) {
      delete gsrc;
      gsrc = NULL;
...
inline void quda_inverter::alloc_csfield_initial()
{
  ...
  quda::ColorSpinorParam gpuParam0(temV, inv_param, gpuGauge.X(), pc_solution,
  ...
  cs_gpu.composite_dim = 1;
  //
  // cs_gpu = quda::ColorSpinorParam(cs_cpu);cs_gpu.location =
  // QUDA_CUDA_FIELD_LOCATION;
  cs_gpu.create = QUDA_ZERO_FIELD_CREATE;
  // cs_gpu.setPrecision(inv_param.cuda_prec, inv_param.cuda_prec, true);
  ...
  //
  // cs_gpu.setPrecision(inv_param.cuda_prec, inv_param.cuda_prec_eigensolver,
  // true); cs_gpu.setPrecision(inv_param.cuda_prec, inv_param.cuda_prec, true);
  // cs_gpu.setPrecision(inv_param.cuda_prec_eigensolver,
  // inv_param.cuda_prec_eigensolver, true);
  ...
  // if(fermion_type == 1){}
  pc_solution = true;
  quda::ColorSpinorParam gpuParam1(temV, inv_param, gpuGauge.X(), pc_solution,
                                   QUDA_CUDA_FIELD_LOCATION);
  ...
  // cs_gpu_tem.setPrecision(QUDA_SINGLE_PRECISION, QUDA_SINGLE_PRECISION,
  // true); gsrcF = quda::ColorSpinorField::Create(cs_gpu_tem);
...
//   bool read = false;
//   qlat::load_qlat_noisesT(filename, prop, read, single_file, VECS_TYPE,
//   std::string("NONE"), 0, n0, false);
// }
...
//   if(read == true)
//   {
//     //for (Int i = 0; i < ZSpace.size(); i++){delete
//     ZSpace[i];}ZSpace.resize(0);
//     //for(Int
//     n=0;n<n0;n++)ZSpace.push_back(quda::ColorSpinorField::Create(cs_gpu));
//     ZSpace.resize(n0);
...
//
//   bool single_file = false;
//   qlat::load_qlat_noisesT(filename0.c_str(), eigD, read, single_file,
//   VECS_TYPE, INFO_LIST, 0, n0, io_rotate_bfac);
//
//   if(read == true )
//   for(Int n=0;n<n0;n++){
...
//     }
//     save_txt_eigenvalues(values, errors, filename1.c_str(), "Staggered
//     Fermions");
//   }
//
//   if(read == true){
//     load_txt_eigenvalues(values, errors, filename1.c_str());Qassert(n0 <=
//     values.size()); evals_ZERO.resize(n0);evals_ERR.resize(n0);
//     /////for(Int n=  0;n<n0;n++){evals_ZERO[n] = quda::Complex(values[n*2+0],
//     values[n*2+1]);} for(Int n=  0;n<n0;n++){evals_ZERO[n] =
//     quda::Complex(values[n*2+0] - 4.0*mre*mre, values[n*2+1]);} for(Int n=
//     0;n<n0;n++){evals_ERR[ n] = errors[n];}
//
...
// inline void quda_inverter::save_evecs(const char* filename, const bool read,
// const Int ndouble, const Int split_save )
//{
//   TIMERB("save_evecs");
//   if(nvec <= 0){return ;}
...
//
//   if(read == false){
//     if(n0!=0){single_file = false;qlat::load_qlat_noisesT(filename0.c_str(),
//     eigD, read, single_file, VECS_TYPE, INFO_LIST, 0, n0, false);}
//     if(n1!=0){single_file = true ;qlat::load_qlat_noisesT(filename1.c_str(),
//     eigF, read, single_file, VECS_TYPE, INFO_LIST, 0, n1, false);}
//
//   }
//   if(read == true){
//     if(n0!=0){qlat::load_qlat_noisesT(filename0.c_str(), eigD, read,
//     single_file, VECS_TYPE, INFO_LIST, 0,   n0, false);}
//
//     if(split_save == 0)
//     if(n1!=0){qlat::load_qlat_noisesT(filename0.c_str(), eigF, read,
//     single_file, VECS_TYPE, INFO_LIST,n0,n0+n1, false);} if(split_save == 1)
//     if(n1!=0){qlat::load_qlat_noisesT(filename1.c_str(), eigF, read,
//     single_file, VECS_TYPE, INFO_LIST, 0,   n1, false);}
//   }
//
//   /////===load to gpu
//   if(read == true){
//   //for (Int i = 0; i < kSpace.size(); i++) delete
//   kSpace[i];kSpace.resize(0);
//   //for (Int i = 0; i < fSpace.size(); i++) delete
//   fSpace[i];fSpace.resize(0); kSpace.resize(ns0); fSpace.resize(ns1);
//
//   eig_param.n_ev_deflate = ns0;eig_solveK =
//   quda::EigenSolver::create(&eig_param, *mat_E); eig_param.n_ev_deflate =
//   ns1;eig_solveF = quda::EigenSolver::create(&eig_param, *mat_E);
...
//       errors[n] = evals_ERR[n];
//     }
//     save_txt_eigenvalues(values, errors, fileE.c_str(), "Staggered
//     Fermions");
//   }
//
//   if(read == true){
//     load_txt_eigenvalues(values, errors, fileE.c_str());Qassert(ns0 + ns1 <=
//     values.size()/2);
//
//     evals_ZERO.resize(ns0+ns1);evals_ERR.resize(ns0+ns1);
//     for(Int n=  0;n<ns0+ns1;n++){evals_ZERO[n] = quda::Complex(values[n*2+0]
//     - 4.0*mre*mre, values[n*2+1]);}
//
...
inline void quda_inverter::free_mem()
{
  ...
  // for (Int i = 0; i < kSpace_cpu.size(); i++) delete
  // kSpace_cpu[i];kSpace_cpu.resize(0); evalsK.resize(0); evalsF.resize(0);
  // evalsZ.resize(0);
  free_csfield();
  //
  ...
  // nvec = 0;
  //
  // if(df_preconditioner !=
  // NULL){destroyDeflationQuda(df_preconditioner);df_preconditioner=NULL;}
  // clear_CG();
  clear_mat();
}
...
inline void quda_inverter::setup_stagger_inv()
{
  ...
  inv_param.residual_type = static_cast<QudaResidualType_s>(
      inv_param.residual_type | QUDA_L2_RELATIVE_RESIDUAL);
  //
  inv_param.tol_hq =
      1e-5;  // specify a tolerance for the residual for heavy quark residual
  // inv_param.tol_hq = 1e-10; // specify a tolerance for the residual for heavy
  // quark residual
  //
  ...
  // inv_param.native_blas_lapack = (native_blas_lapack ? QUDA_BOOLEAN_TRUE :
  // QUDA_BOOLEAN_FALSE);
...
{
  if (prec_type_check == prec_type and force_reload == false) {
    return;
  }
  TIMER("setup_inv_param_prec");
  if (prec_type_check == -2) {
    force_reload = true;
  }  /// force_reload if setup_inv_param_prec_type is called
  inv_param.cpu_prec = QUDA_DOUBLE_PRECISION;
  if (prec_type == 0) {
    inv_param.cuda_prec = QUDA_DOUBLE_PRECISION;
    inv_param.cuda_prec_sloppy = QUDA_SINGLE_PRECISION;
  ...
  if (reload_link_default) {
    ...
    if (gauge_param.cuda_prec != inv_param.cuda_prec or
      ...
      // Qassert(fermion_type == 1);
      // freeGaugeQuda();
      setup_link_p(&quda_gf_buf[0], 0, 0, qinv_twist_link);
      alloc_csfield_gpu();
      // CG_reset = true;
...
inline void quda_inverter::setup_mat_mass(const double mass,
                                          const bool force_do)
{
  ...
  if (std::fabs(mass_mat - mass) > 1e-15) {
    flag_do = 1;
  }
  if (force_do == true) {
    flag_do = 1;
  ...
  //   inv_param.clover_coeff = inv_param.clover_csw * inv_param.kappa;
  //   ////if(quda::comm_rank()== 0)printfQuda("Kappa = %.8f Mass = %.8f\n",
  //   inv_param.kappa, inv_param.mass); return ;
  // }
  //
  if (fermion_type == 1) {
    inv_param.mass = mass;
  ...
  //   (inv_param.solve_type == QUDA_NORMOP_PC_SOLVE) || (inv_param.solve_type
...
// inline void quda_inverter::setup_inc_eigencg(const Int n_ev, const Int n_kr,
// const Int n_conv, const Int df_grid, const double tol, const double inc_tol,
// const double tol_restart, const Int restart_n, const Int pipeline, const Int
// inv_type)
//{
//   /////// inv_param.solve_type = QUDA_NORMOP_SOLVE;
//   /////// inv_param.solve_type = QUDA_DIRECT_SOLVE;
//   ///inv_param.inv_type = QUDA_CG_INVERTER;
//   if(inv_type == 0){inv_param.inv_type = QUDA_EIGCG_INVERTER
//   ;inv_param.solve_type = QUDA_DIRECT_PC_SOLVE;}
//   //if(inv_type == 0){inv_param.inv_type = QUDA_EIGCG_INVERTER
//   ;inv_param.solve_type = QUDA_NORMOP_SOLVE;}
//   //if(inv_type == 1){inv_param.inv_type =
//   QUDA_INC_EIGCG_INVERTER;inv_param.solve_type = QUDA_NORMOP_PC_SOLVE;}
//   if(inv_type == 1){inv_param.inv_type =
//   QUDA_INC_EIGCG_INVERTER;inv_param.solve_type = QUDA_NORMOP_SOLVE;}
//   if(inv_type == 2){inv_param.inv_type =
//   QUDA_INC_EIGCG_INVERTER;inv_param.solve_type = QUDA_DIRECT_SOLVE;}
//   if(inv_type == 3){inv_param.inv_type =
//   QUDA_INC_EIGCG_INVERTER;inv_param.solve_type = QUDA_DIRECT_PC_SOLVE;}
//   if(inv_type == 4){inv_param.inv_type =
//   QUDA_INC_EIGCG_INVERTER;inv_param.solve_type = QUDA_NORMOP_PC_SOLVE;}
//   if(inv_type == 5){inv_param.inv_type =
//   QUDA_INC_EIGCG_INVERTER;inv_param.solve_type = QUDA_NORMERR_PC_SOLVE;}
//   //if(inv_type == 2){inv_param.inv_type = QUDA_GMRESDR_INVERTER
//   ;inv_param.solve_type = QUDA_NORMOP_SOLVE;}
//   //if(inv_type == 2){inv_param.inv_type = QUDA_GMRESDR_INVERTER
//   ;inv_param.solve_type = QUDA_NORMOP_SOLVE;}
//   //if(inv_type == 3){inv_param.inv_type =
//   QUDA_FGMRESDR_INVERTER;inv_param.solve_type = QUDA_NORMOP_SOLVE;}
...
//     {
//       quda::Complex tmp = quda::Complex(evals_ZERO[ni].real() + 4.0 * mass *
//       mass, 0.0); if(ni <  n0){evalsK[ni     ] = tmp;} if(ni >= n0){evalsF[ni
//       - n0] = tmp;}
//       ///evalsZ[ni] = tmp;
//     }
//   }
...
{
  for (Int i = 0; i < int(seed) % 20 + 20 * quda::comm_rank_global(); i++) {
    quda::comm_drand();
  }
  Int random_mode = 1;
  random_Ty((qlat::ComplexD*)gsrc->data(), gsrc->Volume() * spinor_site_size, 1,
            seed + 111111, random_mode);
...
//     ////quda::blas::ax(-1.0/(std::sqrt(2)*lab.real()), ZSpace[n*2 +
//     1].Odd());
...
//     quda::blas::caxpy(-1.0*Im/(std::sqrt(2)*lab), (ZSpace[n*2 + 0]).Even(),
//     (ZSpace[n*2 + 0]).Odd()); quda::blas::caxpy(+1.0*Im/(std::sqrt(2)*lab),
//     (ZSpace[n*2 + 0]).Even(), (ZSpace[n*2 + 1]).Odd());
//
...
//     quda::Complex evals  = quda::blas::cDotProduct( gs, g1 ) /
//     quda::blas::norm2(gs);
//     /////quda::Complex factor = sqrt(quda::blas::norm2( g1 )) /
//     sqrt(quda::blas::norm2( gs)); quda::blas::caxpby(evals, gs , n_unit, g1
//     ); ///tmp = ev*src + n * gtmp quda::Complex residual =
//     sqrt(quda::blas::norm2( g1 ) / srcn);
//
//     quda::blas::zero( g1 );
//     //(*mat)(*gtmp1, *gres, *gtmp0, *gtmp2);
//     (*mat)( g1, gr );
//     quda::Complex even_v  = quda::blas::cDotProduct(gs.Even(), g1.Even()) /
//     quda::blas::norm2(gs.Even()); quda::blas::caxpby(even_v, gs.Even() ,
//     n_unit, g1.Even() ); quda::Complex res_e =
//     sqrt(quda::blas::norm2(g1.Even())/ quda::blas::norm2(gs.Even()));
//
//     quda::blas::zero(g1);
//     //(*mat)(*gtmp1, *gres, *gtmp0, *gtmp2);
//     (*mat)( g1, gr );
//     quda::Complex odd_v  = quda::blas::cDotProduct(gs.Odd(), g1.Odd()) /
//     quda::blas::norm2(gs.Odd()); quda::blas::caxpby(odd_v, gs.Odd() , n_unit,
//     g1.Odd() ); quda::Complex res_o = sqrt(quda::blas::norm2(g1.Odd())/
//     quda::blas::norm2(gs.Odd()));
//
//     if(quda::comm_rank_global()== 0){
//       //printf("===vec %5d, v %+.1e %+.1e, e %+.1e %+.1e, residual %.3e, %.3e
//       %.3e, %+.1e %+.1e %+.1e %+.1e \n", n,
//       //    residual.real(), vals[n].real(), vals[n].imag(), evals.real(),
//       evals.imag(), res_e.real(), res_o.real(),
//       //    even_v.real(), even_v.imag(), odd_v.real(), odd_v.imag() );
//       printf("===vec %5d, norm %+.2e v %+.1e %+.1e, e %+.1e %+.1e, residual
//       %+.3e, %+.3e %+.3e \n", n, 1 - std::sqrt(srcn.real()),
//           vals[n].real(), vals[n].imag(), evals.real()-vals[n].real(),
//           evals.imag()-vals[n].imag(), residual.real(), res_e.real(),
//           res_o.real() );
//     }
//   }
// }
...
//
//   if(use_eigen_pc == 0 or nvec <= 0){for(Int
//   di=0;di<num_src_inv;di++){quda::blas::zero((*gres).Component(di));}return;}
...
//       dirac0->Dslash((*gtmp1).Component(di).Odd(),
//       (*gsrc).Component(di).Odd(), QUDA_EVEN_PARITY); quda::blas::ax(-1.0,
//       (*gtmp1).Component(di).Odd());
...
//         if(kf==0){n_defl = kSpace.size();eig_vecs.resize(n_defl);for(Int
//         i=0;i<n_defl;i++){eig_vecs[i]=kSpace[i].create_alias();}}
//         if(kf==1){n_defl = fSpace.size();eig_vecs.resize(n_defl);for(Int
//         i=0;i<n_defl;i++){eig_vecs[i]=fSpace[i].create_alias();}}
//
...
//
//         quda::blas::caxpby(n_unit, gtmp2->Component(di).Even() , n_unit,
//         gres->Component(di).Even());
//         ////sol[0] = &(gres->Odd());
//
//         quda::blas::zero(*buf[0]);
//         quda::blas::block::caxpy(B, eig_vecs, buf_);
//         gtmp2->Component(di).Odd()  = *buf[0];
//         quda::blas::caxpby(n_unit, gtmp2->Component(di).Odd()  , n_unit,
//         gres->Component(di).Odd());
//         ////gres->Odd()  = gres->Odd() + *buf[0];
//       }
//
//       quda::blas::zero((*gtmp1).Component(di).Odd());
//       dirac0->Dslash((*gtmp1).Component(di).Odd(),
//       (*gres).Component(di).Odd(), QUDA_ODD_PARITY);
//       //dirac->Dslash((*gtmp1).Component(di).Odd(),
//       (*gres).Component(di).Odd(), QUDA_EVEN_PARITY); quda::blas::ax(-1.0,
//       (*gtmp1).Component(di).Odd());
...
//
//     //if(quda::comm_rank_global()== 0)printf("===Even %.3e, Odd %.3e , s %.1e
//     %.1e , n %.1e %.1e %.1e %.1e \n", res_e.real(), res_o.real(),
//     //    s0_e.real(), s0_o.real(), n0_e.real(), n0_o.real(), n1_e.real(),
//     n1_o.real());
//   }
//
...
//     //quda::cvector_ref<quda::ColorSpinorField > SRC_ = {src_.begin(),
//     src_.begin() + 1};
//     //SRC_[0] = *src_[0];
...
//     //quda::cvector_ref<quda::ColorSpinorField > SOL_(sol.begin(),
//     sol.begin()+1); quda::blas::block::caxpy(s, ZSpace, sol);
...
// inline void quda_inverter::deflate(quda::ColorSpinorField &sol, const
// quda::ColorSpinorField &src, std::vector<quda::ColorSpinorField> &evecs,
// const std::vector<quda::Complex> &evals, bool accumulate)
//{
...
//     }
//   }
//   quda::ColorSpinorField *src_tmp = src.Precision() != evecs[0].Precision() ?
//   tmp1 : const_cast<quda::ColorSpinorField *>(&src); quda::ColorSpinorField
//   *sol_tmp = sol.Precision() != evecs[0].Precision() ? tmp2 :
//   const_cast<quda::ColorSpinorField *>(&sol); quda::blas::copy(*src_tmp,
//   src); // no-op if these alias
//
...
/*
  ...
  If single inputs, transform the data orderings with QUDA and then transform
  back
*/
inline void quda_inverter::deflate_Ty(qlat::vector<void*>& Pres,
                                      qlat::vector<void*>& Psrc, double mass,
                                      Int buf_prec, Int mode, Int clear,
                                      const bool data_from_quda)
{
  // Qassert(get_data_type<Ty>() == ComplexD_TYPE or get_data_type<Ty>() ==
  // ComplexF_TYPE);
  ...
  if (eigen_with_nvec == false) {
    for (unsigned int vi = 0; vi < Pres.size(); vi++) {
      if (buf_prec == 0) {
        zero_Ty((qlat::ComplexT<double>*)Pres[vi], Ndata, 1, QFALSE);
      }
      if (buf_prec == 1) {
        zero_Ty((qlat::ComplexT<float>*)Pres[vi], Ndata, 1, QFALSE);
      }
    }
    qacc_barrier(dummy);
    return;
  ...
  std::vector<Int> clearL;
  clearL.resize(eigenL.size());
  Int found_eig = 0;
  ...
  //    double d0 = 0.0;
  //    double d1 = 0.0;
  //    if(buf_prec == 0){d0 = norm_vec((qlat::ComplexT<double>*) Psrc[vi],
  //    Ndata);} if(buf_prec == 1){d0 = norm_vec((qlat::ComplexT<float >*)
  //    Psrc[vi], Ndata);} if(buf_prec == 0){d1 =
  //    norm_vec((qlat::ComplexT<double>*) Pres[vi], Ndata);} if(buf_prec ==
  //    1){d1 = norm_vec((qlat::ComplexT<float >*) Pres[vi], Ndata);}
    ...
    if (Ea != NULL) {
      if (eigen_prec == 0) {
        eigen_cs<qlat::ComplexT<double>>* eigen =
            (eigen_cs<qlat::ComplexT<double>>*)Ea;
        if (buf_prec == 0) {
          eigen->deflate(Pres, Psrc, rD, mass, mode, true, 0, -1, 1, clearL[i]);
        }
        if (buf_prec == 1) {
          eigen->deflate(Pres, Psrc, rF, mass, mode, true, 0, -1, 1, clearL[i]);
        }
      }
      //
      if (eigen_prec == 1) {
...
*/
template <typename Ty, typename Tk>
inline void quda_inverter::get_low_prop(qlat::vector<Ty*>& res,
                                        qlat::vector<Ty*>& src,
                                        qlat::vector<Tk*>& buf, Int qlat_format)
{
  // double input, single buf
  if (get_data_type_is_double<Ty>() == 1 and
      get_data_type_is_double<Tk>() == 0) {
    Qassert(false);
  }
  //
  //// odd dslash
  ...
  for (Int vi = 0; vi < nsrc; vi++) {
    if (qlat_format == 1) {
      ...
      //  quda::ColorSpinorField* Qvec = NULL;
      //  Qvec = new quda::ColorSpinorField(param);
      //  qlat_cf_to_quda_cf(*Qvec, src[vi], geo(), map_index);
  ...
  //
  std::vector<quda::ColorSpinorField*> Qvec;
  Qvec.resize(nsrc * 2);
...
// inline void quda_inverter::deflate(std::vector<quda::ColorSpinorField* >
// &sol, const std::vector<quda::ColorSpinorField* > &src,
//                             std::vector<quda::ColorSpinorField> &evecs, const
//                             std::vector<quda::Complex> &evals, bool
//                             accumulate)
//{
//   TIMER("quda deflate");
...
//   //std::vector<quda::ColorSpinorField *> src_ = const_cast<decltype(src)
//   &>(src);
...
//   //// It was probably a bad design decision to encode whether the system is
//   even/odd preconditioned (PC) in
//   //// solve_type and solution_type, rather than in separate members of
//   QudaInvertParam.  We're stuck with it
//   //// for now, though, so here we factorize everything for convenience.
//   ////bool pc_solution = (param.solution_type == QUDA_MATPC_SOLUTION) ||
//   ////  (param.solution_type == QUDA_MATPCDAG_MATPC_SOLUTION);
//   ////bool pc_solve = (param.solve_type == QUDA_DIRECT_PC_SOLVE) ||
//   ////  (param.solve_type == QUDA_NORMOP_PC_SOLVE) || (param.solve_type ==
//   QUDA_NORMERR_PC_SOLVE);
//   ////bool direct_solve = (param.solve_type == QUDA_DIRECT_SOLVE) ||
//   ////  (param.solve_type == QUDA_DIRECT_PC_SOLVE);
//
//   //solverParam = quda::SolverParam(param);
//   //// solve_cg = quda::Solver::create(solverParam, *m_cg, *mSloppy, *mPre,
//   *mEig);
//   //}
...
inline void quda_inverter::callMultiSrcQuda(qlat::vector<void*>& res,
                                            qlat::vector<void*>& src,
                                            Int max_src)
{
...
inline void quda_inverter::invertQuda_COPY_single(quda::ColorSpinorField& res,
                                                  quda::ColorSpinorField& src)
{
  ...
  if (src.Precision() != inv_param.cpu_prec) {
    ...
    if (src.Precision() == QUDA_DOUBLE_PRECISION) {
      Qassert(prec_type_check == 0 or prec_type_check == 2 or
              prec_type_check == 10);
    }
    if (src.Precision() == QUDA_SINGLE_PRECISION) {
      Qassert(prec_type_check == 1 or prec_type_check == 11);
    }
...
inline void quda_inverter::invertQuda_COPY_single(
    std::vector<quda::ColorSpinorField>& res,
    std::vector<quda::ColorSpinorField>& src)
{
  // TIMERA("Quda invertQuda_COPY_single");
  // Qassert(inv_param.solution_type == QUDA_MATPC_SOLUTION);
  ...
  if (src[0].Precision() != inv_param.cpu_prec) {
    ...
    if (src[0].Precision() == QUDA_DOUBLE_PRECISION) {
      Qassert(prec_type_check == 0 or prec_type_check == 2 or
              prec_type_check == 10);
    }
    if (src[0].Precision() == QUDA_SINGLE_PRECISION) {
      Qassert(prec_type_check == 1 or prec_type_check == 11);
    }
...
// inline void quda_inverter::invertQuda_COPY(quda::ColorSpinorField& res,
// quda::ColorSpinorField& src, Int solve_mode_)
//{
...
//
//   ////src b, result x
//   //const bool no_clear_guess = (param.use_init_guess ==
//   QUDA_USE_INIT_GUESS_YES && !param.chrono_use_resident);
//   /////->Even(), gres->Odd()
...
//     }
//
//     // rescale the source and solution vectors to help prevent the onset of
//     underflow if (param.solver_normalization == QUDA_SOURCE_NORMALIZATION) {
...
//       for(Int di=0;di<num_src_inv;di++)
//       {
//         quda::Solver* solve_cg = quda::Solver::create(solverParam, *m_cg,
//         *mSloppy, *mPre, *mEig);
//         //(*solve_cg)(r, src.Component(di));
//         (*solve_cg)(res.Component(di), src.Component(di));
//         delete solve_cg;
//         // TODO some strange issue for reusing this solve_cg, will zero
//         res[0]....
//         //res.Component(di) = r;//some strange make zeros... for the initial
//         Component
//
...
// template<typename Ty>
// inline void quda_inverter::do_inv(Ty* res, Ty* src, const double mass, const
// double err, const Int niter , const Int prec_type )
//{
//   TIMER_FLOPS("QUDA CG");
...
//   //setup_CG();
//
//   //if((void*)csrc->data() != src)qudaMemcpy((void*)csrc->data(), src,
//   csrc->Volume() * spinor_site_size * sizeof(quda::Complex),
//   qudaMemcpyHostToHost); Qassert((void*) (*gsrc).data() != (void*) src);
...
//   //      //(*cres) = (*gres);
//   //      //if((void*)cres->data() != res){qudaMemcpy(res,
//   (void*)cres->data(),
//   //      //        cres->Volume() * spinor_site_size *
//   sizeof(quda::Complex), qudaMemcpyHostToHost);}
//   //      gettimeofday(&tm1, NULL);double time0 = tm1.tv_sec -
//   tm0.tv_sec;time0 += (tm1.tv_usec - tm0.tv_usec)/1000000.0;
//   //      if(quda::comm_rank_global() == 0)printfQuda("prepare low Done: %.6f
//   secs \n", time0);
//   //      return ;
//   //    }
//
...
//
//   //      dirac_pc->prepare(srcP, sol0, (*gtmp0).Component(di),
//   (*gsrc).Component(di), QUDA_MAT_SOLUTION);
//   //      dirac_pc->prepare(srcP, sol1, (*gres ).Component(di),
//   (*gsrc).Component(di), QUDA_MAT_SOLUTION);
//
//   //      quda::blas::zero(sol0);quda::blas::zero(sol1);
//   //      if(kSpace.size()!=0){deflate((sol0), (srcP), kSpace, evalsK,
//   false);}
//   //      if(fSpace.size()!=0){deflate((sol1), (srcP), fSpace, evalsF,
//   false);}
//
//   //      quda::Complex lambda  = quda::Complex(1.0, 0.0);
//   //      if(kSpace.size()!=0){lambda  = lambda -
//   quda::blas::cDotProduct(sol0, sol1) / quda::blas::norm2(sol0);}
...
//   }
//
//   //if(fermion_type == 0){invertQuda((void*)(cres->Even()).data(),
//   (void*)(csrc->Even()).data(), &inv_param);} if(fermion_type == 1)
//   {
//     inv_param.solution_type = QUDA_MATPC_SOLUTION;
...
//     //if(add_high >= 1)
//     {
//       //invertQuda((void*)((*ctmp1).Component(0).Even()).data(),
//       (void*)((*ctmp1).Component(0).Odd()).data(), &inv_param);
//       //for(Int di=0;di<num_src_inv;di++)
//       {
...
//
//         //invertQuda((void*)(*gres).Component(di).Even().data(),
//         (void*)(*gres).Component(di).Odd().data(), &inv_param);
//         //invertQuda((void*)(*gresH).Component(di).data(),
//         (void*)(*gsrcH).Component(di).data(), &inv_param);
...
//       //  quda_cf_to_qlat_cf(res_tmp.data(), (*gres).Component(0), geo,
//       map_index);
...
//
//       //const size_t Nd = (*gsrcH).Component(0).Volume() * spinor_site_size *
//       sizeof(quda::Complex);
//       //qudaMemcpy((void*)cres.data(), (*gresH).Component(0).data(), Nd,
//       qudaMemcpyDeviceToHost);
//       //qudaMemcpy((void*)csrc.data(), (*gsrcH).Component(0).data(), Nd,
//       qudaMemcpyDeviceToHost);
//       ////csrc = (*gsrcH).Component(0);
//       ////cres = (*gresH).Component(0);
//       //invertQuda((void*)(cres).data(), (void*)(csrc).data(), &inv_param);
//       //qudaMemcpy((*gresH).Component(0).data(), (void*)cres.data(), Nd,
//       qudaMemcpyHostToDevice);
//       //qudaMemcpy((*gsrcH).Component(0).data(), (void*)csrc.data(), Nd,
//       qudaMemcpyHostToDevice);
//
...
//
//       quda::Complex evals  = quda::blas::cDotProduct(src, buf) /
//       quda::blas::norm2(src); quda::Complex factor =
//       sqrt(quda::blas::norm2(buf)) / sqrt(quda::blas::norm2(src));
//       quda::blas::caxpby(evals, src , n_unit, buf);
//       //if(fermion_type == 1){
//       //}
//       quda::Complex residual = sqrt(quda::blas::norm2(buf)/
//       quda::blas::norm2(src)); quda::Complex nor_e1 = sqrt(
//       quda::blas::norm2((res).Even())); quda::Complex nor_o1 = sqrt(
//       quda::blas::norm2((res).Odd() ));
//
//       quda::Complex nor_e = sqrt( quda::blas::norm2((src).Even()));
//       quda::Complex nor_o = sqrt( quda::blas::norm2((src).Odd() ));
//       quda::Complex res_e = sqrt(quda::blas::norm2((buf).Even())/
//       quda::blas::norm2((src))); quda::Complex res_o =
//       sqrt(quda::blas::norm2((buf).Odd())/ quda::blas::norm2((src)));
//
//       if(quda::comm_rank_global()== 0)printf("===solution residual %.3e,
//       factor %.3e, sol norm %.8e, e %.8e , o %.8e \n",
//             residual.real(), factor.real(), temp.real(), res_e.real(),
//             res_o.real());
//
//       //if(quda::comm_rank_global()== 0)printf("===solution residual %.3e,
//       factor %.3e, sol norm %.8e, e %.8e %.3e %.3e, o %.8e %.3e %.3e \n",
//       //      residual.real(), factor.real(), temp.real(), res_e.real(),
//       nor_e1.real(), nor_e.real(), res_o.real(), nor_o1.real(),
//       nor_o.real()); inv_residue = residual.real();
//     }
//
//     //qacc_barrier(dummy);
//     //fflush_MPI();
...
//   cres->Volume() * spinor_site_size * sizeof(quda::Complex),
//   qudaMemcpyHostToHost);} Qassert((void*) (*gres).data() != (void*) res);
//   //*ctmp0 = *gres;
//   //const size_t Nd = (*gsrc).Component(0).Volume() * spinor_site_size *
//   sizeof(quda::Complex);
//   //qlat::vector<qlat::ComplexD > buf;buf.resize(Nd/sizeof(quda::Complex));
//   //for(Int di=0;di<num_src_inv;di++){
//   //  quda_cf_to_qlat_cf((qlat::ComplexD*) buf.data(), (qlat::ComplexD*)
//   ctmp0->Component(di).data(), geo, 3);
//   //  qudaMemcpy(&res[di*Nd/sizeof(quda::Complex)], (void*) buf.data(), Nd,
//   qudaMemcpyDeviceToDevice);
//   //}
//   quda_cf_to_qlat_cf(res, (*gres), geo, map_index);
//
//   ////{errorQuda("QUDA may have bugs right now for the code on CUDA FIELD
//   below\n");}
//
//   gettimeofday(&tm1, NULL);double time0 = tm1.tv_sec - tm0.tv_sec;time0 +=
//   (tm1.tv_usec - tm0.tv_usec)/1000000.0;
//
//   //int verbos = 0;
...
//   {
//     qmessage("Done: %8d iter / %.6f secs = %.3f Gflops %.3f Gflops/GPU, Cost
//     %.3f Gflops \n",
//           inv_param.iter, inv_param.secs, inv_param.gflops / inv_param.secs,
//           inv_param.gflops / (inv_param.secs * qlat::get_num_node()),
//           inv_param.gflops);
//   }
//   inv_time   = time0;
//   inv_iter   = inv_param.iter;
//   inv_gflops = inv_param.gflops / (inv_param.secs * qlat::get_num_node());
//   timer.flops +=  inv_param.gflops * 1024 * 1024 * 1024 /
//   qlat::get_num_node();
//
//   ////quda::saveTuneCache();
// }

//===split grid dslash related
inline void quda_inverter::set_dirac_mrh(const Int mrh)
{
  if (dirac_pc != NULL) {
    delete dirac_pc;
    dirac_pc = NULL;
...
inline void quda_inverter::free_quda_split()
{
  split_fields.resize(0);
  dev_buf.resize(0);
  split_fields_buffer.resize(0);
  split_setup = false;
  if (split_dirac_pc != NULL) {
    // swap current gauges with the split ones if split ones are not null
    swapGaugeSplit(true);
    // free the split gauges and swap the split gauges with the buffered gauges,
    // if split gauges are not      null
    swapGaugeSplit(false);
    delete split_dirac_pc;
    split_dirac_pc = NULL;
  }
}

inline void quda_inverter::rotate_quda_split(const bool swap_gauge)
{
...
inline void quda_inverter::set_quda_split(const std::vector<Int>& grid,
                                          const Int num_src,
                                          const bool split_prec_double_)
{
  ...
  for (int i = 0; i < 4; i++) {
    Qassert(mv[i] >= grid[i] and mv[i] % grid[i] == 0);
  }
  //
  ...
  //
  UpdateSplitGauge(
      &split_param, false, false,
      split_param.split_grid);  // split the gauges into split_key form
  quda::comm_barrier();
  //
  Int off_factor = 1;
  split_prec_double = split_prec_double_;
  if (split_prec_double) {
    cs_param_split = cs_gpuD;
    off_factor = 1;
  }
  if (!split_prec_double) {
    cs_param_split = cs_gpuF;
    off_factor = 2;
  }
  cs_param_split.setPrecision(split_param.cuda_prec, split_param.cuda_prec,
                              true);                   // Native format
  ...
  split_fields_buffer.resize(6);
  split_fields.resize(split_fields_buffer.size());
  size_t V_tot = geo().local_volume() * 3 / 2;  // srh even-even size
  for (int d = 0; d < quda::CommKey::n_dim; d++) {
    cs_param_split.x[d] *= split_key[d];
    V_tot *= split_key[d];
  }
  for (unsigned int j = 0; j < split_fields.size(); j++) {
    split_fields_buffer[j].resize(split_param.num_src_per_sub_partition *
                                  V_tot);
    split_fields[j].resize(split_param.num_src_per_sub_partition);
    for (Int i = 0; i < split_param.num_src_per_sub_partition; i++) {
      /*
        off_factor for ComplexF offset
        assume no offset usage outsize qinv
      */
      cs_param_split.v = &split_fields_buffer[j][i * V_tot / off_factor];
  ...
  if (split_prec_double) {
    devbuf_param = cs_gpuD;
  }
  if (!split_prec_double) {
    devbuf_param = cs_gpuF;
  }
  // devbuf_param = quda::ColorSpinorParam(*gquda0);
  devbuf_param.location =
      cs_param_split.location;  // same location as collected (for copyOffset)
  devbuf_param.setPrecision(split_param.cuda_prec, split_param.cuda_prec,
                            true);  // Native format
  // std::vector<quda::ColorSpinorField> dev_buf(num_sub_partition,
  // devbuf_param);
  ...
  // the input spinors to be on the host, and will use param_copy.cpu_prec to
  // set the precision. We want to avoid the situation, where the internal prec
  // and the cpu_prec are somehow different.
  ...
  split_param_copy.matpc_type = QUDA_MATPC_EVEN_EVEN;
  quda::DiracParam diracParam;
  setDiracParam(diracParam, &split_param_copy, pc_solve);
  split_dirac_pc = quda::Dirac::create(diracParam);
  //
  // rotate back to original communicator
  rotate_quda_split(true);
}

inline void quda_inverter::copy_to_buf_split(
    std::vector<quda::ColorSpinorField>& src, const Int i)
{
  TIMER("copy_to_buf_split");
  rotate_from_quda_split();  // swap to default comm to split fields
  Qassert(src.size() ==
          split_param.num_src_per_sub_partition * num_sub_partition);
  ...
  for (int n = 0; n < split_param.num_src_per_sub_partition; n++) {
    ...
    for (int j = 0; j < num_sub_partition; j++)
      dev_buf[j].copy(src[n * num_sub_partition + j]);
    quda::split_field_device(split_fields[i][n],
                             {dev_buf.begin(), dev_buf.end()}, split_key,
                             QUDA_4D_PC);
  }
  rotate_to_quda_split();  // swap to splitted comms
}

inline void quda_inverter::copy_from_buf_split(
    std::vector<quda::ColorSpinorField>& res, const Int i)
{
  ...
  Qassert(i < split_fields.size());
  for (int n = 0; n < split_param.num_src_per_sub_partition; n++) {
    quda::join_field_device({dev_buf.begin(), dev_buf.end()},
                            split_fields[i][n], split_key, QUDA_4D_PC);
    //
    // export to desired location and layout
    for (int j = 0; j < num_sub_partition; j++)
      res[n * num_sub_partition + j].copy(dev_buf[j]);
  }
}

quda_inverter::~quda_inverter() { free_mem(); }
...
/*
  ...
  inv_even_even : flag to do even-even / odd-odd inversions, odd-odd have no
  eigen support may not need this, odd source will be only a mass rescale
*/
template <typename Ty>
void get_staggered_prop_group(quda_inverter& qinv, qlat::vector<Ty*>& src,
                              qlat::vector<Ty*>& prop, const double mass,
                              const double err, const Int niter,
                              Int low_only = 0, const Int prec_type = 0,
                              const bool inv_even_even = true)
{
  ...
  Qassert(low_only == 0 or low_only == 1 or
          low_only ==
              -1);  /// 0 for full, 1 for low only, -1 other ways to solve
  ...
  if (!inv_even_even) {
    Qassert(low_only == 0 and qinv.eigen_with_nvec == false and
            restart_cg == 0);
  ...
  if (low_only == 0 or low_only == -1) {
    ...
    param.setPrecision(qinv.inv_param.cuda_prec, qinv.inv_param.cuda_prec,
                       true);
    // param.setPrecision(QUDA_DOUBLE_PRECISION, qinv.inv_param.cuda_prec,
    // true);
    param.create = QUDA_REFERENCE_FIELD_CREATE;
    //
    ...
    Int buf_prec = 0;
    if (qinv.inv_param.cuda_prec == QUDA_DOUBLE_PRECISION) {
      buf_prec = 0;
    }
    if (qinv.inv_param.cuda_prec == QUDA_SINGLE_PRECISION) {
      buf_prec = 1;
    }
    if (buf_prec == -1) {
      abort_r("QUDA buffer prec not supported yet!");
    }
    //
    ...
    // if(get_data_type_is_double<Ty >()){
    //  param_src.setPrecision(QUDA_DOUBLE_PRECISION, QUDA_DOUBLE_PRECISION,
    //  true);
    //}else{
    //  param_src.setPrecision(QUDA_SINGLE_PRECISION, QUDA_SINGLE_PRECISION,
    //  true);
    //}
    ...
    for (Int vi = 0; vi < nsrc; vi++) {
      ...
      // if(buf_prec==0)cpy_GPU((qlat::ComplexT<double >*) Qvec[vi]->data(),
      // buf[vi], Nd); if(buf_prec==1)cpy_GPU((qlat::ComplexT<float  >*)
      // Qvec[vi]->data(), buf[vi], Nd);
      //
      param.v = (void*)buf[vi];
      Qvec[nsrc + vi] = new quda::ColorSpinorField(param);
    ...
    if (low_only == 0) {
      ...
      for (Int vi = 0; vi < nsrc; vi++) {
        quda::ColorSpinorField srcP;
        quda::ColorSpinorField sol1;
        dirac_pc->prepare(srcP, sol1, (*Qvec[nsrc + vi]), (*Qvec[vi]),
                          QUDA_MAT_SOLUTION);
      ...
      if (restart_cg == 0) {
        ...
        //
        qlat::vector<void*> srcI;
        srcI.resize(nsrc);
        qlat::vector<void*> resI;
        resI.resize(nsrc);
        for (Int vi = 0; vi < nsrc; vi++) {
          if (inv_even_even) {
        ...
        //
        for (Int vi = 0; vi < nsrc; vi++) {
          dirac_pc->reconstruct((*Qvec[nsrc + vi]), (*Qvec[vi]),
                                QUDA_MAT_SOLUTION);
          // if(inv_even_even){
          //   qinv.dirac_pc->reconstruct(    (*Qvec[nsrc + vi]), (*Qvec[vi]),
          //   QUDA_MAT_SOLUTION);
          // }else{
          //   qinv.dirac_pc_odd->reconstruct((*Qvec[nsrc + vi]), (*Qvec[vi]),
          //   QUDA_MAT_SOLUTION);
          // }
        }
        ...
        //
        std::vector<quda::ColorSpinorField> gsumV;
        gsumV.resize(nsrc);
        std::vector<quda::ColorSpinorField> gresV;
        gresV.resize(nsrc);
        std::vector<quda::ColorSpinorField> gsrcV;
        gsrcV.resize(nsrc);
        std::vector<quda::ColorSpinorField> gbufV;
        gbufV.resize(nsrc);
        quda::ColorSpinorField tmp0(cs_tmp);
        quda::ColorSpinorField tmp1(cs_tmp);
        std::vector<double> residual_vecs;
        residual_vecs.resize(nsrc);
        std::vector<double> norm_vecs;
        norm_vecs.resize(nsrc);
        ...
        for (Int it = 0; it < maxN; it++) {
          ...
          if (count_diverge / nsrc >= 2) {
            qmessage("Too many inversion diverge %d, nsrc %d!", count_diverge,
                     nsrc);
            break;
          }
          for (Int vi = 0; vi < nsrc; vi++) {
          ...
          qinv.inv_param.iter = 0;
          {
            ...
            // invertQuda(Qvec[nsrc + vi]->Even().data(), Qvec[nsrc +
            // vi]->Odd().data(), &qinv.inv_param);
          }
          //
          // for(Int vi=0;vi<nsrc;vi++){
          //   ////TIMER("invertQuda group");
          ...
          for (Int vi = 0; vi < nsrc; vi++) {
            ...
            // quda::blas::caxpy( 1.0*One * norm_vecs[vi], gresV[vi],
            // gsumV[vi]);
            //
            // invertQuda((void*)Qvec[nsrc + vi]->Even().data(),
            // (void*)Qvec[nsrc + vi]->Odd().data(), &qinv.inv_param);
            //
            gsrcV[vi] = Qvec[nsrc + vi]->Odd();
    ...
    if (low_only == -1) {
      ...
      //  if(eigenD != NULL){if(buf_prec == 0){eigenD->deflateA(Psrc, Psrc, rD,
      //  mass,  2);}} if(eigenD != NULL){if(buf_prec ==
      //  1){eigenD->deflateA(Psrc, Psrc, rF, mass,  2);}}
      //}
      //
      qinv.inv_param.solution_type = QUDA_MATPC_SOLUTION;
      for (Int vi = 0; vi < nsrc; vi++) {
        // the COPY could use single precison as input
        qinv.invertQuda_COPY_single(Qvec[nsrc + vi]->Even(),
                                    Qvec[nsrc + vi]->Odd());
      }
      ...
      //  if(eigenD != NULL){if(buf_prec == 0){eigenD->deflateA(Pres, Pbuf, rD,
      //  mass,  0);}} if(eigenD != NULL){if(buf_prec ==
      //  1){eigenD->deflateA(Pres, Pbuf, rF, mass,  0);}}
      //}
      //
      for (Int vi = 0; vi < nsrc; vi++) {
        dirac_pc->reconstruct((*Qvec[nsrc + vi]), (*Qvec[vi]),
    ...
    for (Int vi = 0; vi < nsrc; vi++) {
      ...
      // if(buf_prec==0){
      //   quda_cf_to_qlat_cf(prop[vi], (qlat::ComplexT<double >*) (*Qvec[nsrc +
      //   vi]).data(), DIM, geo, map);
      // }
      // if(buf_prec==1){
      //   quda_cf_to_qlat_cf(prop[vi], (qlat::ComplexT<float  >*) (*Qvec[nsrc +
      //   vi]).data(), DIM, geo, map);
      // }
    }
    //
    ...
    if ((qinv.quda_verbos >= 0 or qinv.quda_verbos == -2) and
        qinv.inv_param.iter > 0) {
      qmessage(
          "Done: %8d iter / %.6f secs = %.3f Gflops %.3f Gflops/GPU, Cost %.3f "
          "Gflops \n",
          qinv.inv_param.iter, qinv.inv_param.secs,
          qinv.inv_param.gflops / qinv.inv_param.secs,
          qinv.inv_param.gflops / (qinv.inv_param.secs * qlat::get_num_node()),
          qinv.inv_param.gflops);
    }
  }
  //
  if (low_only == 1) {
...
template <typename Ty>
void get_staggered_prop_group(quda_inverter& qinv, std::vector<colorFT>& src,
                              std::vector<colorFT>& prop, const double mass,
                              const double err, const Int niter,
                              Int low_only = 0, const Int prec_type = 0)
{
  qlat::vector<Ty*> s0;
...
void get_staggered_prop(quda_inverter& qinv, Ty* src, Ty* prop,
                        const double mass, const double err, const Int niter,
                        Int low_only = 0, const Int prec_type = 0)
{
  qinv.setup_mat_mass(mass);
  qlat::vector<Ty*> sP;
  ...
  // if(low_only == 0){
  //   //qinv.do_inv(qinv.cres->data(), qinv.csrc->data(), mass, err, niter,
  //   prec_type); qinv.do_inv(prop, src, mass, err, niter, prec_type);
  // }
  //
  // if(low_only == 1){
...
template <typename Ty>
void get_staggered_prop(quda_inverter& qinv, qlat::FieldM<Ty, 3>& src,
                        qlat::FieldM<Ty, 3>& prop, const double mass,
                        const double err, const Int niter, Int low_only = 0,
                        const Int prec_type = 0)
{
  ...
  if (!prop.initialized) {
    prop.init(geo);
  }  ////allocate mem for prop
  Ty* propP = (Ty*)qlat::get_data(prop).data();
  //
  // get_staggered_prop(qinv, srcP, propP, mass, err, niter, low_only,
  // prec_type);
  //
  qlat::vector<Ty*> sP;
  qlat::vector<Ty*> rP;
...
template <typename Ty>
void get_staggered_prop(quda_inverter& qinv,
                        std::vector<qlat::FieldM<Ty, 3>>& srcL,
                        std::vector<qlat::FieldM<Ty, 3>>& propL,
                        const double mass, const double err, const Int niter,
                        Int low_only = 0, const Int prec_type = 0)
{
  ...
  // for(Int iv=0;iv<Ninv;iv++)
  //{
  //   get_staggered_prop(qinv,srcL[iv], propL[iv], mass, err, niter, low_only,
  //   prec_type);
  // }
}

template <typename Ty>
void get_staggered_prop(quda_inverter& qinv, qlat::vector_gpu<Ty>& src,
                        qlat::vector_gpu<Ty>& prop, const double mass,
                        const double err, const Int niter, Int low_only = 0,
                        const Int prec_type = 0)
{
  ...
  //
  get_staggered_prop_group(qinv, sP, rP, mass, err, niter, low_only, prec_type);
  //   get_staggered_prop(qinv, resP, resP,  mass, err, niter, low_only,
  //   prec_type);
  mv_civ.move_civ_in(resP, resP, 1, 3, Vl, 1, true);
}

// even-even dslash flops
                              ...
                              const double err, const Int niter)
{
  ...
  for (unsigned int i = 0; i < src.size(); i++) {
    Qassert(src[i].initialized);
  }
  for (unsigned int i = 0; i < res.size(); i++) {
    Qassert(res[i].initialized);
  }
  const Geometry& geo_src = src[0].geo();
  Qassert(geo_src.node_site == geo.node_site and
          geo_src.geon.size_node == geo.geon.size_node);
  //
  ...
  //
  std::vector<quda::ColorSpinorField>& out_multishift_gpu =
      qinv.out_multishift_gpu;
  std::vector<quda::ColorSpinorField>& src_multishift_gpu =
      qinv.src_multishift_gpu;
  //
  bool init = false;
  if (out_multishift_gpu.size() != multishift) {
    init = true;
  }
  if (src_multishift_gpu.size() != 1) {
    init = true;
  }
  if (init == false) {
    for (unsigned int i = 0; i < out_multishift_gpu.size(); i++) {
      if (out_multishift_gpu[i].TotalBytes() != Lsize) {
        init = true;
      }
    }
    for (unsigned int i = 0; i < src_multishift_gpu.size(); i++) {
      if (src_multishift_gpu[i].TotalBytes() != Lsize) {
        init = true;
      }
    }
  }
  if (init == true) {
    ...
    cs_param.is_composite = false;
    cs_param.is_component = false;
  ...
  if (multishift == 1) {
    ...
    for (Int i = 0; i < multishift; i++) {
      ...
      // inv_param.offset[i] = 4 * masses[i] * masses[i] - 4 * masses[0] *
      // masses[0];
  ...
  for (Int srci = 0; srci < Nsrc; srci++) {
    qlat_cf_to_quda_cf(src_multishift_gpu[0],
                       (Ty*)qlat::get_data(src[srci]).data(), geo, map);
    //{
    // double na = quda::blas::norm2(src_multishift_gpu[0].Even());
    // qmessage("src %5d, %+.8e, offset %.8e, err %.3e, iter %8d \n", srci, na,
    // inv_param.offset[0], inv_param.tol, inv_param.maxiter);
    //}
    //
    if (multishift == 1) {
      invertQuda(_hp_multi_x[0], src_multishift_gpu[0].Even().data(),
                 &inv_param);
    } else {
      invertMultiShiftQuda(_hp_multi_x.data(),
                           src_multishift_gpu[0].Even().data(), &inv_param);
    }
    //
    for (Int mi = 0; mi < multishift; mi++) {
      quda_cf_to_qlat_cf((Ty*)qlat::get_data(res[mi * Nsrc + srci]).data(),
                         out_multishift_gpu[mi], geo, map);
    }
  }
  //
  ...
  if (qinv.quda_verbos >= 0 or qinv.quda_verbos == -2) {
    qmessage(
        "Done multishift: %8d iter / %.6f secs = %.3f Gflops %.3f Gflops/GPU, "
        "Cost %.3f Gflops \n",
        inv_param.iter, inv_param.secs, inv_param.gflops / inv_param.secs,
        inv_param.gflops / (inv_param.secs * qlat::get_num_node()),
        inv_param.gflops);
...
template <typename Ty>
void get_staggered_multishift_even(quda_inverter& qinv, std::vector<Ty*> res,
                                   std::vector<Ty*> src,
                                   const std::vector<double>& masses2,
                                   const double err, const Int niter)
{
  ...
  // const Geometry& geo_src = src[0].geo();
  // Qassert(geo_src.node_site == geo.node_site and geo_src.geon.size_node ==
  // geo.geon.size_node);
  //
  /// qinv.setup_mat_mass(0.0);
  // qinv.setup_CG();
  ...
  // if(qinv.inv_param.cuda_prec == QUDA_SINGLE_PRECISION){buf_prec = 1;}
  // if(buf_prec == -1){abort_r("QUDA buffer prec not supported yet!");}
  // Qassert(qinv.inv_param.cuda_prec == QUDA_DOUBLE_PRECISION or
  // qinv.inv_param.cuda_prec == QUDA_SINGLE_PRECISION);
  //
  if (get_data_type_is_double<Ty>()) {
    Qassert(qinv.inv_param.cuda_prec == QUDA_DOUBLE_PRECISION);
  } else {
  ...
  if (multishift == 1) {
    ...
    if (val != "") {
      qlat_multi_refine = stringtonum(val);
    }
    //
    ...
    // inv_param.cuda_prec_sloppy              = QUDA_DOUBLE_PRECISION;
    if (inv_param.cuda_prec_sloppy == QUDA_SINGLE_PRECISION and
        inv_param.cuda_prec == QUDA_DOUBLE_PRECISION) {
      max_tol = 1e-8;
    }
    if (inv_param.cuda_prec_sloppy == QUDA_SINGLE_PRECISION and
    ...
    for (Int i = 0; i < multishift; i++) {
      ...
      //
      inv_param.tol_hq_offset[i] =
          5e-8;  /// donot check heavy quark, tol_hq, not used currently
    }
  }
  //
  inv_param.solution_type = QUDA_MATPC_SOLUTION;
  ...
  //
  if (multishift == 1) {
    qlat::vector<void*> srcP;
    srcP.resize(Nsrc);
    qlat::vector<void*> resP;
    resP.resize(Nsrc);
    for (Int srci = 0; srci < Nsrc; srci++) {
      srcP[srci] = src[srci];
      resP[srci] = res[srci];
...
inline void quda_inverter::gauge_fix(qlat::ComplexD* quda_gf, Int gf_gauge_dir,
                                     Int gf_maxiter, double gf_tolerance,
                                     Int fix_type)
{
  ...
  //
  if (fix_type == 0) {
    quda::gaugeFixingOVR(*U, gf_gauge_dir, gf_maxiter, gf_verbosity_interval,
                         gf_ovr_relaxation_boost, gf_tolerance,
                         gf_reunit_interval, gf_theta_condition);
  }
  //
  if (fix_type == 1) {
    quda::gaugeFixingFFT(*U, gf_gauge_dir, gf_maxiter, gf_verbosity_interval,
                         gf_fft_alpha, gf_fft_autotune, gf_tolerance,
                         gf_theta_condition);
  }
  //
  {
