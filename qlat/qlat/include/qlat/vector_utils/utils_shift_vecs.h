...
struct shift_vec {
  ...
  shift_vec(const Coordinate& site, bool GPU_set = true)
  {
    init(site, GPU_set);
  }
  ...
  template <typename Ty>
  void shift_Evec(std::vector<qlat::vector<Ty>>& srcE,
                  std::vector<qlat::vector<Ty>>& srcEf, std::vector<Int>& iDir,
                  Int civ_or);

  template <typename Ty>
  void shift_vecs(std::vector<Ty*>& src, std::vector<Ty*>& res,
                  std::vector<Int>& iDir, Int civ_or);
  ...
  template <typename Ty>
  void shift_vecs_dir(qlat::Field<Ty>& src, qlat::Field<Ty>& res, Int mu,
                      Int sign);

  template <typename Ty, Int civ_>
  void shift_vecs_dir(std::vector<qlat::FieldM<Ty, civ_>>& src,
                      std::vector<qlat::FieldM<Ty, civ_>>& res, Int mu,
                      Int sign);

  template <typename Ty>
  void shift_vecs_dirG(std::vector<qlat::FieldG<Ty>>& src,
                       std::vector<qlat::FieldG<Ty>>& res, Int mu, Int sign);

  template <typename Ta>
  void set_gauge(Ta* gauge_, Int gbfac_, Int gd0_, bool Conj_ = false,
                 bool src_gauge_ = false)
  {
    ...
    if (cur == 0 or cur == 1) {
      ...
      Qassert(gauge_is_double != -1);
    }
    gauge = gauge_;
    gbfac = gbfac_;
    gd0 = gd0_;
    Conj = Conj_;
    src_gauge = src_gauge_;
  }

  void set_bfacs(Int gbfac_, Int gd0_, bool Conj_ = false,
                 bool src_gauge_ = false)
  {
    Qassert(gauge != NULL);
    gbfac = gbfac_;
...
inline void shift_vec::init(fft_desc_basic& fds, bool GPU_set)
{
  ...
  nv.set_mem_type(MemType::Uvm);
  //
  N0 = fds.Nv[fds.orderN[0]];
  N1 = fds.Nv[fds.orderN[1]];
  N2 = fds.Nv[fds.orderN[2]];
  Nt = fds.Nt;
  //
...
inline void shift_vec::shift_set()
{
  ...
  for (Int diru = 0; diru < 8; diru++) {
    ...
    //
    std::vector<LInt> sendoffVa;
    sendoffVa.resize(0);
    std::vector<LInt> sendoffVb;
    sendoffVb.resize(0);
    std::vector<LInt> sendoffVx;
    sendoffVx.resize(0);
    std::vector<LInt> buffoffVa;
    buffoffVa.resize(0);
    std::vector<LInt> buffoffVb;
    buffoffVb.resize(0);
    //
    for (LInt off0 = 0; off0 < Length; off0++) {
      ...
      //
      LInt off1 = ((p[3] * N0 + p[fd.orderN[0]]) * N1 + p[fd.orderN[1]]) * N2 +
                  p[fd.orderN[2]];
      //
      if (p[dir] >= 0 and p[dir] < Nv[dir]) {
        ...
        off1 = ((p[3] * N0 + p[fd.orderN[0]]) * N1 + p[fd.orderN[1]]) * N2 +
               p[fd.orderN[2]];
...
void shift_vec::set_MPI_size(Int biva_or, Int civ_or, Int dir_or)
{
  ...
  if (biva_or == biva and civ_or == civ and bsize == sizeof(Ty)) {
    if (sendoffa[dir_cur]->size() == 0) {
      ...
      if (sendbufP[dir_cur]->size() / sizeof(Ty) ==
          (LInt)biva_or * civ_or * (sendoffa[dir_cur]->size())) {
        return;
      }
    }
  }
  ...
  if (sizeof(Ty) != bsize) {
    ...
    Qassert(MPI_off % M_size == 0);
    MPI_off = MPI_off / M_size;
    Qassert(MPI_off != 0);
  }
  //
  ////===assign biva and civ
...
{
  if (civ == -1 or biva == -1 or dir_or == -1) {
    abort_r("Need to set up dir_cur , civ and biva first. \n");
  }
  if (dir_or < 0 or dir_or > 8) {
    qmessage("dir_cur size wrong %8d. \n", dir_or);
    abort_r();
  }
  if (bsize != sizeof(Ty)) {
    abort_r("Ty type not match with previous usage.!\n");
  }
  //
...
void shift_vec::write_send_recv(Ty* src, Ty* res)
{
  ...
  if (flag == 0 and sendoffa[dir_cur]->size() != 0) {
    ...
    // }
    cpy_data_from_index(&s_tem[0], &src[0], s1, s0, sendoffa[dir_cur]->size(),
                        civ, GPU, QTRUE, biva, writeN * civ, Length * civ);
  }
  //
  ////Write Result
  if (flag == 1 and sendoffb[dir_cur]->size() != 0) {
    ...
    // }
    cpy_data_from_index(&res[0], &r_tem[0], s1, s0, sendoffb[dir_cur]->size(),
                        civ, GPU, QTRUE, biva, Length * civ, writeN * civ);
  }
  //
  if (flag == 2 and buffoffa[dir_cur]->size() != 0) {
    ...
    // }
    cpy_data_from_index(&res[0], &src[0], s1, s0, buffoffa[dir_cur]->size(),
                        civ, GPU, QTRUE, biva, Length * civ, Length * civ);
  }
  //
  s_tem = NULL;
  r_tem = NULL;
...
__global__ void multiply_gauge_global(Ty* a, Ty* b, const Int dir_gauge,
                                      const Int biva, const Int gbfac)
{
  __shared__ Ty ls[9];
  ...
  while (off < 9) {
    // if(!Conj)ls[(off%3)*3 + off/3] = a[(index*dir_limit*2 + dir_gauge)*9 +
    // off]; if( Conj)ls[(off%3)*3 + off/3] = qlat::qconj(a[(index*dir_limit*2 +
    // dir_gauge)*9 + off]);
    if (!Conj) ls[off] = a[(index * dir_limit * 2 + dir_gauge) * 9 + off];
    if (Conj)
      ls[off] = qlat::qconj(a[(index * dir_limit * 2 + dir_gauge) * 9 + off]);
    off += nt;
  }
  __syncthreads();
  for (Int bi = 0; bi < biva; bi++)
    for (Int g1 = 0; g1 < gbfac; g1++) {
      ...
      for (Int ic = 0; ic < 3; ic++) {
        offB[tid] += ls[threadIdx.y * 3 + ic] * ds[ic * gs + threadIdx.x];
      }
      __syncthreads();
    }
}
...
void multiply_gauge(void* src, void* gauge, const Int dir_gauge, const Int biva,
                    const Long Length, const Int gbfac, const Int gd0,
                    const bool GPU)
{
  (void)GPU;
  const Int dir_limit = 4;
...
////gpu fast mode
  ...
  if (fast_eigen == 2) {
    ...
    if (!Conj)
      multiply_gauge_global<Cy, gs, false>
          <<<dimGrid, dimBlock>>>((Cy*)gauge, (Cy*)src, dir_gauge, biva, gbfac);
    if (Conj)
      multiply_gauge_global<Cy, gs, true>
          <<<dimGrid, dimBlock>>>((Cy*)gauge, (Cy*)src, dir_gauge, biva, gbfac);
    qacc_barrier(dummy);
  }
#endif
  ...
  if (fast_eigen == 1) {
    qthread_for(index, Long(Length), {
      ...
      // if(!Conj)for(Int ci=0;ci<9;ci++){buf[(ci%3)*3 + ci/3] = ((Cy*)
      // gauge)[(index*dir_limit*2 + dir_gauge)*9 +  ci];} if( Conj)for(Int
      // ci=0;ci<9;ci++){buf[(ci%3)*3 + ci/3] = qlat::qconj(((Cy*)
      // gauge)[(index*dir_limit*2 + dir_gauge)*9 +  ci]);}
      if (!Conj)
        for (Int ci = 0; ci < 9; ci++) {
          buf[ci] = ((Cy*)gauge)[(index * dir_limit * 2 + dir_gauge) * 9 + ci];
        }
      if (Conj)
        for (Int ci = 0; ci < 9; ci++) {
          buf[ci] = qlat::qconj(
              ((Cy*)gauge)[(index * dir_limit * 2 + dir_gauge) * 9 + ci]);
        }
      Eigen::Matrix<Cy, 3, 3, Eigen::ColMajor>& lE =
          *((Eigen::Matrix<Cy, 3, 3, Eigen::ColMajor>*)buf);
      /////lE * dE0 ???
      for (Int bi = 0; bi < biva; bi++)
        for (Int g1 = 0; g1 < gbfac; g1++) {
          Cy* d0 = &((Cy*)src)[((bi * Length + index) * gbfac + g1) * 3 * gs];
          if (gs != 1) {
            Eigen::Matrix<Cy, gs, 3, Eigen::ColMajor>& dEC =
                *((Eigen::Matrix<Cy, gs, 3, Eigen::ColMajor>*)d0);
            dEC *= lE;
          }
          if (gs == 1) {
            Eigen::Matrix<Cy, gs, 3, Eigen::RowMajor>& dER =
                *((Eigen::Matrix<Cy, gs, 3, Eigen::RowMajor>*)d0);
            dER *= lE;
          }
        ...
        for (Int g1 = 0; g1 < gbfac; g1++) {
          ...
          for (Int c0 = 0; c0 < 3; c0++) {
            for (Int c1 = 0; c1 < 3; c1++) {
              /////Cy tem = buf[c1*3 + c0];
              Cy tem = buf[c0 * 3 + c1];
              for (Int g0 = 0; g0 < gd0; g0++) {
                res[c0 * gd0 + g0] += d0[c1 * gd0 + g0] * tem;
              }
            }
          }
          for (Int g0 = 0; g0 < 3 * gd0; g0++) {
...
void shift_vec::mult_gauge(void* pt, Int dir_gauge)
{
  ...
  if (!id) {
    Qassert(Long(civ * sizeof(Ty) / 8) == gbfac * 3 * gd0);
    Qassert(gauge_is_double == 0);
  }
  bool cfind = false;
#define shift_macros(bf)                                                       \
  if (bf == gd0) {                                                             \
    cfind = true;                                                              \
    if (id)                                                                    \
      multiply_gauge<qlat::ComplexD, bf, 1, Conj_>(pt, gauge, dir_gauge, biva, \
                                                   Length, gbfac, gd0, GPU);   \
    if (!id)                                                                   \
      multiply_gauge<qlat::ComplexF, bf, 1, Conj_>(pt, gauge, dir_gauge, biva, \
                                                   Length, gbfac, gd0, GPU);   \
  }
  shift_macros(1);
  shift_macros(2);
  ...
  if (!cfind) {
    cfind = true;
    if (id)
      multiply_gauge<qlat::ComplexD, 1, -1, Conj_>(pt, gauge, dir_gauge, biva,
                                                   Length, gbfac, gd0, GPU);
    if (!id)
      multiply_gauge<qlat::ComplexF, 1, -1, Conj_>(pt, gauge, dir_gauge, biva,
                                                   Length, gbfac, gd0, GPU);
  }
#undef shift_macros
  Qassert(cfind);
...
void shift_vec::call_MPI(Ty* src, Ty* res, Int dir_or)
{
  ...
  const Int tags =
      10240 + 777;  // AMD machine MPI have tag issues with Quda ...
  ...
  if (MPI_size[dir_cur] != 0) {
    ...
    //
    ////MPI_Recv((Ftype*) &recvbuf[0]
    ///,MPI_size,CMPI,rank_sr[dir_cur][1],tagr,comm, &status);
    MPI_Isend(s_tem, MPI_size[dir_cur] * MPI_off, MPI_curr, rank_sr[dir_cur][0],
              tags, get_comm(), &send_req);
...
template <typename Ty>
void shift_vec::shift_vecs(std::vector<Ty*>& src, std::vector<Ty*>& res,
                           std::vector<Int>& iDir, Int civ_or)
{
  ...
  {
    Int count = 1;
    for (LInt di = 0; di < iDir.size(); di++) {
      count += int(std::abs(iDir[di]));
    }
  ...
  if (iDir.size() != 4) {
    qmessage("shift directions wrong .");
    flag_abort = 1;
  }
  if (biva_or <= 0 or civ_or <= 0) {
    qmessage("Cannot do it with biva_or <=0 or civ_or==0");
    flag_abort = 1;
  }
  if (flag_abort == 1) {
    abort_r();
  ...
  if (dir_curl.size() == 0) {
    ...
    VectorGPUKey gkey(size_t(Nsize) * sizeof(Ty), ssprintf("shift_vec_buf"),
                      GPU);
  ...
  //
  // qmessage("Flag %d, civ %d %d , biva %d \n",int(flag_shift_set),civ_or,civ,
  // biva); if(flag_shift_set == false){if(civ_or==1)set_MPI_size<Ty
  // >(1,12);if(civ_or != 1)set_MPI_size<Ty >(1, civ_or);}
  if (civ == -1 or biva == -1) {
    if (civ_or == 1) {
      set_MPI_size<Ty>(1, 12);
    }
    if (civ_or != 1) {
      set_MPI_size<Ty>(1, civ_or);
    }
  }
  if (civ_or != 1) {
    if (civ_or != civ) {
      qmessage("civor %3d, civ %3d \n", civ_or, civ);
      abort_r("Configuration not equal \n");
    }
  }
  //
  // Qassert(biva_or == biva); // code can be grouped with internal biva,
  // biva_or is the outter loops
  ...
  //
  for (Int fftn = 0; fftn < (size_vec + biva * civ - 1) / (biva * civ);
       fftn++) {
    if (flagend == 1) {
      break;
    }
    ...
    for (Int li = 0; li < biva; li++) {
      if (civ_or == 1) {
        {
          ...
          for (Int ci = 0; ci < civ; ci++) {
            ...
            cpy_data_thread(&vec_s[(li * civ + ci) * Ng + 0], &ptem0[ci][0], Ng,
                            GPU, QFALSE);
          }
        }
      }
      ...
      if (civ_or != 1) {
        if (count < size_vec) {
          ...
          cpy_data_thread(&vec_s[li * Ng * civ_or + 0], &src[count / civ_or][0],
                          Ng * civ_or, GPU, QFALSE);
          count = count + civ_or;
        }
    ...
    if (civ_or == 1 and civ != 1)
      mv_civ.move_civ_in(&vec_s[0], &vec_s[0], biva, civ, Ng, 1, GPU);
    //
    ...
    if (civ_or == 1 and civ != 1)
      mv_civ.move_civ_out(&vec_s[0], &vec_s[0], biva, Ng, civ, 1, GPU);
    //
    ...
    for (Int li = 0; li < biva; li++) {
      ...
      if (civ_or == 1) {
        ...
        for (Int ci = 0; ci < civ; ci++) {
          ...
          cpy_data_thread(&ptem0[ci][0], &vec_s[(li * civ + ci) * Ng + 0], Ng,
                          GPU, QFALSE);
        }
      }
      ...
      if (civ_or != 1) {
        if (start < size_vec) {
          ...
          cpy_data_thread(&res[start / civ_or][0], &vec_s[li * Ng * civ_or + 0],
                          Ng * civ_or, GPU, QFALSE);
          start = start + civ_or;
        }
        qacc_barrier(dummy);
...
{
  std::vector<Ty*> srcL(1);
  srcL[0] = src;
  std::vector<Ty*> resL(1);
  resL[0] = res;
  shift_vecs(srcL, resL, iDir, civ_or);
}

template <typename Ty>
void shift_vec::shift_Evec(std::vector<qlat::vector<Ty>>& srcE,
                           std::vector<qlat::vector<Ty>>& srcEf,
                           std::vector<Int>& iDir, Int civ_or)
{
  Int flag_abort = 0;
  if (srcE.size() == 0) {
    qmessage("Cannot do it with srcE.size()==0");
    flag_abort = 1;
  }
  if (civ_or <= 0) {
    qmessage("Cannot do it with civ_or==0");
    flag_abort = 1;
  }
  if (iDir.size() != 4) {
    qmessage("shift directions wrong .");
  ...
  if (srcE[0].size() != Nt * N0 * N1 * N2 * civ_or) {
    qmessage("omp num %3d \n", omp_get_thread_num());
    qmessage(
        "Cannot do it with srcE[0].size()!=Nt*N0*N1*N2*civ_or,srcE[0].size() "
        "%6d,Nt*N0 %6d,N1 %6d,N2 %6d \n",
        int(srcE[0].size()), Nt * N0, N1, N2 * civ_or);
    flag_abort = 1;
  }
  if (srcEf.size() == 0) {
    qmessage("Cannot do it with srcEf.size()==0");
    flag_abort = 1;
  }
  if (srcEf[0].size() != Nt * N0 * N1 * N2 * civ_or) {
    qmessage("omp num %3d \n", omp_get_thread_num());
    qmessage(
        "Cannot do it with srcEf[0].size()!=N0*N1*N2*civ_or,srcEf[0].size() "
        "%6d,Nt*N0 %6d,N1 %6d,N2 %6d \n",
        int(srcEf[0].size()), Nt * N0, N1, N2 * civ_or);
    flag_abort = 1;
  }
  //
  if (flag_abort == 1) {
...
template <typename Ty>
void shift_vec::shift_vecs_dir(qlat::Field<Ty>& src, qlat::Field<Ty>& res,
                               const Int mu, const Int sign)
{
  Qassert(IsTypeComplex<Ty>());
  Qassert(get_mem_order(src) == QLAT_DEFAULT and
          get_mem_order(res) == QLAT_DEFAULT);
...
template <typename Ty, Int civ_>
void shift_vec::shift_vecs_dir(std::vector<qlat::FieldM<Ty, civ_>>& src,
                               std::vector<qlat::FieldM<Ty, civ_>>& res, Int mu,
                               Int sign)
{
  Qassert(IsTypeComplex<Ty>());
  ...
  for (Int si = 0; si < Nsrc; si++) {
    ...
    Qassert(get_mem_order(src[si]) == QLAT_DEFAULT and
            get_mem_order(res[si]) == QLAT_DEFAULT);
...
template <typename Ty>
void shift_vec::shift_vecs_dirG(std::vector<qlat::FieldG<Ty>>& src,
                                std::vector<qlat::FieldG<Ty>>& res, Int mu,
                                Int sign)
{
  Qassert(IsTypeComplex<Ty>());
  Qassert(src.size() == res.size());
...
void shift_fieldM(shift_vec& svec, std::vector<qlat::FieldM<Ty, civ>>& src,
...
template <class Ty>
void shift_fields(shift_vec& svec, Ty* src, Ty* res, std::vector<Int>& iDir,
                  const Int biva, const Int civ)
{
...
template <class Td>
void shift_fieldM(shift_vec& svec, std::vector<Propagator4dT<Td>*>& src,
                  std::vector<Propagator4dT<Td>*>& res, std::vector<Int>& iDir)
{
  if (src.size() < 1) return;
  Int biva_or = src.size();
  //
  std::vector<ComplexT<Td>*> srcP;
  std::vector<ComplexT<Td>*> resP;
  srcP.resize(biva_or);
  resP.resize(biva_or);
  for (Int bi = 0; bi < biva_or; bi++) {
...
template <class Td>
void shift_fieldM(shift_vec& svec, Propagator4dT<Td>& src,
                  Propagator4dT<Td>& res, std::vector<Int>& iDir)
{
  std::vector<Propagator4dT<Td>*> srcP(1);
  srcP[0] = &src;
  std::vector<Propagator4dT<Td>*> resP(1);
  resP[0] = &res;
...
void shift_fieldM(shift_vec& svec, std::vector<Propagator4dT<Td>>& src,
                  std::vector<Propagator4dT<Td>>& res, std::vector<Int>& iDir)
{
  std::vector<Propagator4dT<Td>*> srcP(0);
  srcP.resize(src.size());
  std::vector<Propagator4dT<Td>*> resP(0);
  resP.resize(res.size());
...
template <class Td>
void symmetric_shift(shift_vec& svec, std::vector<Propagator4dT<Td>>& src,
                     std::vector<Propagator4dT<Td>>& res,
                     std::vector<Propagator4dT<Td>>& buf, Int idir)
{
  if (src.size() == 0) {
    res.resize(0);
    return;
  ...
  qlat::vector<qlat::ComplexT<Td>*> srcP;
  srcP.resize(src.size(), MemType::Uvm);
  qlat::vector<qlat::ComplexT<Td>*> resP;
  resP.resize(src.size(), MemType::Uvm);
  qlat::vector<qlat::ComplexT<Td>*> bufP;
  bufP.resize(src.size(), MemType::Uvm);
  for (unsigned int i = 0; i < res.size(); i++) {
    if (!res[i].initialized) {
      res[i].init(src[0].geo());
    }
    if (!buf[i].initialized) {
      buf[i].init(src[0].geo());
    }
...
template <typename Ty>
void shift_vecs_dir_qpropT(std::vector<qlat::FieldM<Ty, 12 * 12>>& src,
                           std::vector<qlat::FieldM<Ty, 12 * 12>>& res, Int mu,
                           Int sign, shift_vec& svec)
{
  if (src.size() == 0) {
    res.resize(0);
    return;
...
*/
template <typename Ty>
void shift_vecs_cov_fieldG(std::vector<std::vector<FieldG<Ty>>>& res,
...
void shift_fields_qlat(Ty* src, Ta* res, const std::vector<Int>& iDir,
                       const Int Nvec, const Geometry& geo,
                       const Int move_in = 1)
{
  ...
  if (move_in == 1) {
    mv_civ.move_civ_in(p0, p0, 1, Nvec, V, 1, true);
  }
  //
  field_shift_direct(f1, f0, shift);
  //
  if (move_in == 1) {
    mv_civ.move_civ_out(p1, p1, 1, V, Nvec, 1, true);
  }
                         ...
                         const Int mode = 1)
{
  ...
  */
  for (Int s = 0; s < 3; s++) {
    for (Int i = 0; i < 4; i++) {
      if (s == 0) {
        total_siteL[s][i] = total_site[i] / local_site[i];
      }
      if (s == 1) {
        total_siteL[s][i] = total_site[i];
        if (local_site[i] % 2 == 0) {
          total_siteL[s][i] = total_site[i] / 2;
        }
        if (local_site[i] % 3 == 0) {
          total_siteL[s][i] = total_site[i] / 3;
        }
        if (local_site[i] % 4 == 0) {
          total_siteL[s][i] = total_site[i] / 4;
        }
      }
      if (s == 2) {
        total_siteL[s][i] = total_site[i];
                        ...
                        const Int max_group = -1)
{
  ...
  for (LInt jobi = 0; jobi < jobA.size() / 2; jobi++) {
    ...
    shift_fields_gridPT(&src[bini], &res[bini], iDir, bcut, civ, geo, mode);
  }
}
