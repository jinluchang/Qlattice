...
template <class Td>
void smear_propagator_gwu_convension_shift(Propagator4dT<Td>& prop,
                                           ...
                                           const double width, const Int step)
{
  ...
  Propagator4dT<Td> buf0;
  buf0.init(geo);
  Propagator4dT<Td> buf1;
  buf1.init(geo);
  Propagator4dT<Td> bufc;
  bufc.init(geo);
  Propagator4dT<Td> bufa;
  bufa.init(geo);
  qlat::ComplexT<Td> bw = qlat::ComplexT<Td>(
      width * width / (4.0 * step - 6.0 * width * width), 0.0);
  qlat::ComplexT<Td> norm =
      qlat::ComplexT<Td>((1 - 3.0 * width * width / (2 * step)), 0.0);
  ...
  for (Int i = 0; i < step; ++i) {
    ...
    for (Int mu = 0; mu < 3; mu++) {
      qlat::ComplexT<Td>* src =
          (qlat::ComplexT<Td>*)qlat::get_data(bufc).data();
...
template <class Td>
void smear_propagator_gwu_convension_1shift(Propagator4dT<Td>& prop,
                                            const GaugeFieldT<Td>& gf,
                                            const double width, const Int step)
{
  const Geometry& geo = gf.geo();
...
template <class Td>
void smear_propagator_gwu_convension_2shift_ori(Propagator4dT<Td>& prop,
                                                const GaugeFieldT<Td>& gf,
                                                const double width,
                                                const Int step)
{
...
void prepare_gauge_buffer(std::vector<GaugeFieldT<Ta>>& gfL,
                          const GaugeFieldT<Tb>& gf)
{
    ...
    for (Int y = 0; y < 2; y++)
      for (Int x = 0; x < 2; x++) {
        ...
        qacc_for(index, gfh.geo().local_volume(), {
          ...
          const Coordinate xl1 = Coordinate(xlh[0] * 2 + x, xlh[1] * 2 + y,
                                            xlh[2] * 2 + z, xlh[3]);
          //
          for (Int dir = 0; dir < dir_limit; ++dir) {
            ////auto&
            const Coordinate xld = coordinate_shifts(xl1, dir);
            qlat::ComplexT<Ta>* res =
                (qlat::ComplexT<Ta>*)&gfh.get_elem(xlh, dir)(0, 0);
            qlat::ComplexT<Tb>* s0 =
                (qlat::ComplexT<Tb>*)&gf1.get_elem(xl1, dir)(0, 0);
            qlat::ComplexT<Tb>* s1 =
                (qlat::ComplexT<Tb>*)&gf1.get_elem(xld, dir)(0, 0);
            for (Int a = 0; a < 3; a++)
              for (Int b = 0; b < 3; b++) {
                res[a * 3 + b] = 0.0;
                for (Int c = 0; c < 3; c++) {
...
void prop_full_copy_to_prop_8(std::vector<qlat::vector_gpu<Ty>>& propL,
                              Ty* prop, const Int Ndata, const Geometry& geo,
                              const Geometry& geoh, const Int even,
                              const Int idx = 0, Int NdataP = -1)
{
  TIMERA("prop_full_copy_to_prop_8");
  if (propL.size() != 8 and dir == 1) {
    propL.resize(8);
  }
  if (dir == 0) {
  ...
  for (Int z = 0; z < 2; z++)
    for (Int y = 0; y < 2; y++)
      for (Int x = 0; x < 2; x++) {
        ...
        if (dir == 1) {
          if (propL[writei].size() < Nfull) {
            propL[writei].resizeL(Nfull);
          }
        }
        if (dir == 0) {
          Qassert(propL[writei].size() >= Nfull);
        }
        //
        ...
        qacc_for(index, geoh.local_volume(), {
          ...
          if (dir == 1)
            for (Int di = 0; di < NdataP; di++) {
              proph[sh * Ndata + idx * NdataP + di] = prop[sp * NdataP + di];
            }
          if (dir == 0)
            for (Int di = 0; di < NdataP; di++) {
              prop[sp * NdataP + di] = proph[sh * Ndata + idx * NdataP + di];
            }
        });
      }
}

// wuppertal convension smearing of color fields
// dup_off try to create new smf buffers for different moms
template <class Ty, class Td>
void smear_propagator_wuppertal_convension(
    qlat::FieldM<Ty, 3>& prop, GaugeFieldT<Td>& gf, const double width,
    ...
    const Int dup_off = 0)
{
  ...
  Ty* propP = (Ty*)get_data(prop).data();
  smear_propagator_gwu_convension_inner<Ty, 1, 1, Td>(
      propP, gf, factor_sigma, step, mom, false, 1, dup_off);
}

// wuppertal convension
// dup_off try to create new smf buffers for different moms
template <class Ty, class Td, Int c0, Int d0>
void smear_propagator_gwu_convension_2shift_modi(
    std::vector<qlat::vector_gpu<Ty>>& propL, const Geometry& geo,
    std::vector<GaugeFieldT<Td>>& gfL, const double width, const Int step,
    const Int even = -1, const Int force_update = -1,
    const Int wuppertal_conv = 1, const CoordinateD& mom = CoordinateD(),
    const Int dup_off = 0)
{
  (void)geo;
  if (width == 0 or step == 0) {
  ...
  for (Int i = 0; i < 8; i++) {
    ...
    Ty* proph = (Ty*)propL[writei].data();
    smear_propagator_gwu_convension_inner<Ty, c0, d0, Td>(
        proph, gfL[i], factor_sigma, step, mom, false, 1, i + dup_off,
        force_update);
  }
}
...
template <class Ty, class Td, Int c0, Int d0>
void smear_propagator_gwu_convension_2shift_modi(
    Ty* prop, const Geometry& geo, std::vector<GaugeFieldT<Td>>& gfL,
    const double width, const Int step,
    std::vector<qlat::vector_gpu<Ty>>& propL, const Int even = -1,
    const Int wuppertal_conv = 1, const CoordinateD& mom = CoordinateD(),
    const Int dup_off = 0)
{
  Qassert(gfL.size() == 8);
  ...
  smear_propagator_gwu_convension_2shift_modi<Ty, Td, c0, d0>(
      propL, geo, gfL, width, step, even, -1, wuppertal_conv, mom, dup_off);
  prop_full_copy_to_prop_8<Ty, 0>(propL, prop, Ndata, geo, geoh, even);
}

// wuppertal convension
template <class Td>
void smear_propagator_wuppertal_convension_2shift(
    Propagator4dT<Td>& prop, std::vector<GaugeFieldT<Td>>& gfL,
    const double width, const Int step,
    std::vector<qlat::vector_gpu<qlat::ComplexT<Td>>>& propL,
    const Int even = -1, const Int wuppertal_conv = 1,
    const CoordinateD& mom = CoordinateD(), const Int dup_off = 0)
{
  Qassert(prop.initialized);
  if (width == 0 or step == 0) {
    return;
  ...
  smear_propagator_gwu_convension_2shift_modi<qlat::ComplexT<Td>, Td, 1,
                                              12 * 4>(
      src, prop.geo(), gfL, width, step, propL, even, wuppertal_conv, mom,
      dup_off);
  rotate_prop(prop, 1);
}
...
// gwu convension
template <class Td>
void smear_propagator_gwu_convension_2shift(
    Propagator4dT<Td>& prop, std::vector<GaugeFieldT<Td>>& gfL,
    const double width, const Int step,
    std::vector<qlat::vector_gpu<qlat::ComplexT<Td>>>& propL,
    const Int even = -1, const CoordinateD& mom = CoordinateD(),
    const Int dup_off = 0)
{
  const Int wuppertal_conv = 0;
  smear_propagator_wuppertal_convension_2shift(
      prop, gfL, width, step, propL, even, wuppertal_conv, mom, dup_off);
}

template <class Ty, class Td>
void smear_propagator_gwu_convension_2shift(
    FieldG<Ty>& prop, std::vector<GaugeFieldT<Td>>& gfL, const double width,
    const Int step, std::vector<qlat::vector_gpu<qlat::ComplexT<Td>>>& propL,
    const Int even = -1, const CoordinateD& mom = CoordinateD(),
    const Int dup_off = 0)
{
  if (0 == step) {
  ...
  smear_propagator_gwu_convension_2shift_modi<Ty, Td, 1, 12 * 4>(
      src, prop.geo(), gfL, width, step, propL, even, 0, mom, dup_off);
  //
  qmem_for(isp, Nvol, gmem, {
    Ty buf[12 * 12];
    for (Int i = 0; i < 12 * 12; i++) {
...
template <class Ty, class Td>
void smear_propagator_gwu_convension_2shift(
    qpropT& prop, std::vector<GaugeFieldT<Td>>& gfL, const double width,
    const Int step, std::vector<qlat::vector_gpu<qlat::ComplexT<Td>>>& propL,
    const Int even = -1, const CoordinateD& mom = CoordinateD(),
    const Int dup_off = 0)
{
  ...
  smear_propagator_gwu_convension_2shift(tmp_prop, gfL, width, step, propL,
                                         even, mom, dup_off);
}

template <typename Ty, typename Td>
void smear_propagator_gwu_convension_2shift_modi(
    colorFT& prop, std::vector<GaugeFieldT<Td>>& gfL, const double width,
    const Int step, std::vector<qlat::vector_gpu<Ty>>& propL,
    const Int even = -1, const Int wuppertal_conv = 1,
    const CoordinateD& mom = CoordinateD(), const Int dup_off = 0)
{
  Qassert(prop.initialized);
  Ty* src = (Ty*)qlat::get_data(prop).data();
  smear_propagator_gwu_convension_2shift_modi<Ty, Td, 1, 1>(
      src, prop.geo(), gfL, width, step, propL, even, wuppertal_conv, mom,
      dup_off);
}
...
template <typename Ty, typename Td>
void smear_propagator_gwu_convension_2shift_modi(
    std::vector<colorFT>& prop, std::vector<GaugeFieldT<Td>>& gfL,
    const double width, const Int step,
    std::vector<qlat::vector_gpu<Ty>>& propL, const Int even = -1,
    const Int wuppertal_conv = 1, const CoordinateD& mom = CoordinateD(),
    const Int dup_off = 0)
{
  for (unsigned int i = 0; i < prop.size(); i++) {
    smear_propagator_gwu_convension_2shift_modi(
        prop[i], gfL, width, step, propL, even, wuppertal_conv, mom, dup_off);
  }
}
