...
__global__ void cpy_data_from_index_global(
    T* Pres, T* Psrc, const TInt* map_res, const TInt* map_src, const TI1 bfac,
    const Int Ngap = 1, const LInt Lgap0 = 0, const LInt Lgap1 = 0)
{
  // blockDim, gridDim
...
template <typename T, typename TInt, typename TI0, typename TI1>
void cpy_data_from_index(T* Pres, T* Psrc, const TInt* map_res,
                         const TInt* map_src, const TI0 Nvol, const TI1 bfac,
                         Int GPU = 1, QBOOL dummy = QTRUE, const Int Ngap = 1,
                         const LInt Lgap0 = 0, const LInt Lgap1 = 0)
{
  TIMERB("copy data form index");
  (void)dummy;
  (void)GPU;
  Qassert(long(Nvol) > 0 and long(bfac) > 0);
  // qmessage("===vol %d, b %d, N %d %d %d \n", int(Nvol), int(bfac), int(Ngap),
  // int(Lgap0), int(Lgap1));
//
#ifdef QLAT_USE_ACC
  if (GPU == 1) {
    ...
    cpy_data_from_index_global<T, TInt, TI0, TI1><<<dimGrid, dimBlock>>>(
        Pres, Psrc, &map_res[0], &map_src[0], bfac, Ngap, Lgap0, Lgap1);
...
template <typename T>
void cpy_data_from_index(qlat::vector<T>& res, qlat::vector<T>& src,
                         const qlat::vector<Long>& map_res,
                         const qlat::vector<Long>& map_src, const Long bfac,
                         Int GPU = 1, QBOOL dummy = QTRUE)
{
...
#ifdef QLAT_USE_ACC
template <typename T, typename TInt, typename TI0, typename TI1>
__global__ void cpy_data_from_group_global(T* Pres, T* Psrc,
                                           const TInt* map_res,
                                           const TInt* map_src,
                                           const TInt* map_off, const TI1 bfac)
{
  TI0 index = blockIdx.y * gridDim.x + blockIdx.x;
...
//////Faster when T* is GPU memory without unified
template <typename T, typename TInt, typename TI0, typename TI1>
void cpy_data_from_group(T* Pres, T* Psrc, const TInt* map_res,
                         const TInt* map_src, const TInt* map_off,
                         const TI0 Nvol, const TI1 bfac, Int GPU = 1,
                         QBOOL dummy = QTRUE)
{
...
//
  ...
  if (GPU == 1) {
    ...
    cpy_data_from_group_global<T, TInt, TI0, TI1><<<dimGrid, dimBlock>>>(
        Pres, Psrc, &map_res[0], &map_src[0], &map_off[0], bfac);
    if (dummy == QTRUE) {
      qacc_barrier(dummy);
    }
    /////qacc_for(iv, Nvol, {for(Int bi=0;bi<bfac;bi++)Pres[map_res[iv]*bfac+bi]
    ///= Psrc[map_src[iv]*bfac+bi];});
    return;
  }
#endif
  //
...
template <typename T>
void cpy_data_from_group(qlat::vector<T>& res, qlat::vector<T>& src,
                         const qlat::vector<Long>& map_res,
                         const qlat::vector<Long>& map_src,
                         const qlat::vector<Long>& map_off, const Long bfac,
                         Int GPU = 1, QBOOL dummy = QTRUE)
{
  // Qassert(map_res.size() ==     src.size());
  // Qassert(map_res.size() ==     res.size());
...
template <typename T0, typename T1, typename TInt, Int bfac, Int ADD_FAC,
          typename Tadd>
__global__ void cpy_data_thread_global(T0* Pres, const T1* Psrc,
                                       const TInt Nvol, const Tadd ADD)
{
...
void CPY_data_thread_basic(T0* Pres, const T1* Psrc, const TInt Nvol, Int GPU,
                           QBOOL dummy, const Tadd ADD)
{
  (void)dummy;
  if (GPU != 0 and GPU != 1) {
    Qassert(false);
  ...
  bool do_copy = true;
  if (double(qlat::qnorm(ADD)) < QLAT_COPY_LIMIT) {
    do_copy = true;
  }
  if (double(qlat::qnorm(ADD)) >= QLAT_COPY_LIMIT) {
    do_copy = false;
...
//
  ...
  if (GPU == 1) {
    ...
    const Int Biva = (4 * 16 + sizeof(T0) - 1) / sizeof(T0);
    Long Nb = (Nvol + Threads * Biva - 1) / (Threads * Biva);
    dim3 dimBlock(Threads, 1, 1);
    dim3 dimGrid(Nb, 1, 1);
    //
    if (do_copy)
      cpy_data_thread_global<T0, T1, TInt, Biva, 0, T1>
          <<<dimGrid, dimBlock>>>(Pres, Psrc, Nvol, tmp);
    if (!do_copy)
      cpy_data_thread_global<T0, T1, TInt, Biva, 1, T1>
          <<<dimGrid, dimBlock>>>(Pres, Psrc, Nvol, tmp);
    //
    if (dummy == QTRUE) {
      qacc_barrier(dummy);
...
template <typename T0, typename T1, typename TInt, typename Tadd>
void cpy_data_threadT(T0* Pres, const T1* Psrc, const TInt Nvol, Int GPU,
                      QBOOL dummy, const Tadd ADD, void* stream = NULL)
{
  (void)stream;
  if (Nvol <= 0) {
    return;
  ...
  if (GPU == 0 or GPU == 1) {
#ifdef QLAT_USE_ACC
    if (sizeof(T0) == sizeof(T1) and
        double(qlat::qnorm(ADD)) < QLAT_COPY_LIMIT) {
      if (stream == NULL) {
        if (GPU == 0) {
          qacc_Errchk(qacc_MemcpyAsync((void*)Pres, (void*)Psrc,
                                       Nvol * sizeof(T0),
                                       qacc_MemcpyHostToHost));
        }
        if (GPU == 1) {
          qacc_Errchk(qacc_MemcpyAsync((void*)Pres, (void*)Psrc,
                                       Nvol * sizeof(T0),
                                       qacc_MemcpyDeviceToDevice));
        }
      } else {
        qacc_Stream_t* pstream = (qacc_Stream_t*)stream;
        if (GPU == 0) {
          qacc_Errchk(qacc_MemcpyAsync((void*)Pres, (void*)Psrc,
                                       Nvol * sizeof(T0), qacc_MemcpyHostToHost,
                                       *pstream));
        }
        if (GPU == 1) {
          qacc_Errchk(qacc_MemcpyAsync((void*)Pres, (void*)Psrc,
                                       Nvol * sizeof(T0),
                                       qacc_MemcpyDeviceToDevice, *pstream));
        }
      }
...
#endif
    CPY_data_thread_basic(Pres, Psrc, Nvol, GPU, dummy, ADD);
    return;
  }
...
//
  ...
  if (GPU == 2) {
    ...
    if (sizeof(T0) == sizeof(T1) and
        double(qlat::qnorm(ADD)) < QLAT_COPY_LIMIT) {
      if (stream == NULL) {
        qacc_Errchk(qacc_MemcpyAsync(Pres, Psrc, Nvol * sizeof(T0),
                                     qacc_MemcpyHostToDevice));
      } else {
        qacc_Stream_t* pstream = (qacc_Stream_t*)stream;
        qacc_Errchk(qacc_MemcpyAsync(Pres, Psrc, Nvol * sizeof(T0),
                                     qacc_MemcpyHostToDevice, *pstream));
      }
      if (dummy == QTRUE) {
        qacc_barrier(dummy);
      }
      ...
      qlat::vector<T0> buf;
      buf.resize(Nvol, MemType::Uvm);
      T0* s0 = (T0*)qlat::get_data(buf).data();
      /////host to host
...
void cpy_data_thread(T0* Pres, const T1* Psrc, const TInt Nvol, Int GPU = 1,
                     QBOOL dummy = QTRUE, const double ADD = 0,
                     void* stream = NULL)
{
...
template <typename T0, typename T1, typename TInt>
void cpy_data_threadC(T0* Pres, const T1* Psrc, const TInt Nvol, Int GPU = 1,
                      QBOOL dummy = QTRUE, const T1 ADD = 0,
                      void* stream = NULL)
{
...
// 2--> ===from host to device, 3 ===from device to host
template <typename T0, typename T1, typename TInt>
void cpy_GPU(T0* Pres, const T1* Psrc, const TInt Nvol, const Int Gres = 1,
             const Int Gsrc = 1, QBOOL dummy = QTRUE, void* stream = NULL)
{
  Int GPU = 1;  ////default from device (Gsrc == -1)
  if (Gsrc == 0 and Gres == 0) {
    GPU = 0;
  }
  if (Gsrc != 0 and Gres != 0) {
    GPU = 1;
  ...
  cpy_data_threadT<T0, T1, TInt, double>(Pres, Psrc, Nvol, GPU, dummy, 0.0,
                                         stream);
}

template <typename T0, typename T1, typename TInt>
void cpy_mem(T0* Pres, const T1* Psrc, const TInt Nvol, QBOOL dummy = QTRUE,
             void* stream = NULL)
{
...
template <typename T0, typename T1, typename TInt>
Int cpy_GPU2D_G(T0* Pres, const T1* Psrc, const TInt Nvol, const TInt NOff,
                const TInt rOff, const TInt sOff, const QMEM Gres = QMGPU,
                const QMEM Gsrc = QMGPU, void* stream = NULL)
{
  (void)stream;
  Int diff_cuda = 0;
  Int did = 0;
  ...
  if (sizeof(T0) == sizeof(T1)) {
    ...
    if (diff_cuda == 1) {
      qacc_Stream_t* pstream = (qacc_Stream_t*)stream;
      qacc_ErrCheck(qacc_Memcpy2DAsync(Pres, rOff * sizeof(T0), Psrc,
                                       sOff * sizeof(T0), Nvol * sizeof(T0),
                                       NOff, tmp, *pstream));
      did = 1;
    }
    //
...
// 2--> ===from host to device, 3 ===from device to host
template <typename T0, typename T1, typename TInt>
void cpy_GPU2D(T0* Pres, const T1* Psrc, const TInt Nvol, const TInt NOff,
               const TInt rOff, const TInt sOff, const QMEM Gres = QMGPU,
               const QMEM Gsrc = QMGPU, const QBOOL dummy = QTRUE,
               void* stream = NULL)
{
  ////TIMERA("cpy_GPU2D");
  Int diff_cuda = 0;
  ...
  if (sizeof(T0) != sizeof(T1) and diff_cuda == 1) {
    ...
    if (sizeof(T0) < sizeof(T1)) {
      qlat::vector<T0> tmp;
      tmp.resize(Ntotal, MemType::Uvm);
      did += cpy_GPU2D_G((T0*)tmp.data(), Psrc, Nvol, NOff, Nvol, sOff, QMSYNC,
                         Gsrc, stream);
      qacc_barrier(dummy);
      did += cpy_GPU2D_G(Pres, (T0*)tmp.data(), Nvol, NOff, rOff, Nvol, Gres,
                         QMSYNC, stream);
      qacc_barrier(dummy);
    }
    //
    if (sizeof(T0) > sizeof(T1)) {
      qlat::vector<T1> tmp;
      tmp.resize(Ntotal, MemType::Uvm);
      did += cpy_GPU2D_G((T1*)tmp.data(), Psrc, Nvol, NOff, Nvol, sOff, QMSYNC,
                         Gsrc, stream);
      qacc_barrier(dummy);
      did += cpy_GPU2D_G(Pres, (T1*)tmp.data(), Nvol, NOff, rOff, Nvol, Gres,
                         QMSYNC, stream);
      qacc_barrier(dummy);
    }
    Qassert(did == 2);
...
// void touch_GPU(Ty* Mres, long long Msize,long long offM = 0,long long size =
// -1, Int mode = 1)
//{
//   (void)Mres;
//   (void)mode;
...
//   qacc_MemAdvise(&Mres[offM], Total*sizeof(Ty), qacc_MemAdviseSetReadMostly,
//   gpu_id); if(mode == 1){ qacc_MemPrefetchAsync(&Mres[offM],
//   Total*sizeof(Ty), gpu_id, cudaStreamLegacy);} #endif
// }
...
//   Int gpu_id = -1;
//   qacc_ErrCheck(qacc_GetDevice(&gpu_id));
//   qacc_MemAdvise(&Mres[0], Total*sizeof(Ty), qacc_MemAdviseUnsetReadMostly,
//   gpu_id); #endif
// }
                       ...
                       size_t Vol, Int GPU = 1)
{
  ...
  Qassert(Ncopy <= N0);
  Qassert(Ncopy <= N1);
  if (N0 == N1 and Ncopy == N0) {
    cpy_data_thread(res, src, Vol * N0, GPU_set, QFALSE);
  } else {
#pragma omp parallel for
    for (size_t isp = 0; isp < Vol; isp++) {
      if (GPU_set == 0) {
        memcpy(&res[isp * N0 + 0], &src[isp * N1 + 0], Ncopy * sizeof(Ty));
      }
#ifdef QLAT_USE_ACC
      if (GPU_set == 1) {
        qacc_MemcpyAsync(&res[isp * N0 + 0], &src[isp * N1 + 0],
                         Ncopy * sizeof(Ty), qacc_MemcpyDeviceToDevice);
      }
      if (GPU_set == 2) {
        qacc_MemcpyAsync(&res[isp * N0 + 0], &src[isp * N1 + 0],
                         Ncopy * sizeof(Ty), qacc_MemcpyHostToDevice);
      }
      if (GPU_set == 3) {
        qacc_MemcpyAsync(&res[isp * N0 + 0], &src[isp * N1 + 0],
                         Ncopy * sizeof(Ty), qacc_MemcpyDeviceToHost);
      }
#endif
    }
  }
