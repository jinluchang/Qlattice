...
struct ga_M {
  ...
  inline void check_sum(unsigned long& resp, unsigned long& resm)
  {
    ...
    resp = 0;
    for (Int i = 0; i < 4; i++) {
      resp += ((int(g[i].real()) + 5) * 100 + (int(g[i].imag()) + 5) * 10 +
               ind[i]) *
              Long(std::pow(1000, i));
    }
    ...
    resm = 0;
    for (Int i = 0; i < 4; i++) {
      resm += ((-1 * int(g[i].real()) + 5) * 100 +
               (-1 * int(g[i].imag()) + 5) * 10 + ind[i]) *
              Long(std::pow(1000, i));
    }
  }
...
void ga_M::print()
{
  for (Int i = 0; i < 4; i++) {
    for (Int j = 0; j < 4; j++)
      if (ind[i] == j)
        printf("(%6.2e,%6.2e)", g[i].real(), g[i].imag());
      else
        printf("(%6.2e,%6.2e)", 0.0, 0.0);
    printf("\n");
  }
}
...
Int ga_M::ga_sign()
{
  ...
  for (Int i = 0; i < 4; i++) res += abs(tmp.g[i]);
  return (res < 1e-5) ? 1 : -1;
}
...
void set_GAM(GAM& GA)
{
  ...
  //
  ga_M* ga0[6] = {&GA.unit,    &GA.ga_i[0], &GA.ga_i[1],
                  &GA.ga_i[2], &GA.ga_i[3], &GA.ga_i[4]};
  for (Int i = 0; i < 6; i++)
    for (Int j = 0; j < 6; j++)
      for (Int k = 0; k < 4; k++) {
 ...
 public:
  ...
  ga_matrices_PS()
  {
    ...
    for (Int i = 0; i < 4; i++) {
      unit.g[i] = 1.0;
      unit.ind[i] = i;
      ga_i[0].g[i] = Imag * Ftype(2 * (i / 2) - 1.0);
      ga_i[0].ind[i] = 3 - i;  // gamma1
      ga_i[1].g[i] = a[i];
      ga_i[1].ind[i] = 3 - i;  // gamma2
      ga_i[2].g[i] = Imag * a[i];
      ga_i[2].ind[i] = (i + 2) % 4;  // gamma3
      ga_i[3].g[i] = 1.0 - 2 * (i / 2);
      ga_i[3].ind[i] = i;  // gamma4
      ga_i[4].g[i] = -1.0;
      ga_i[4].ind[i] = (i + 2) % 4;  // gamma5
    }
    set_GAM(*this);
  }
};
 ...
 public:
  ...
  ga_matrices_milc()
  {
    ...
    for (Int i = 0; i < 4; i++) {
      ...
      ga_i[2].ind[i] = (i + 2) % 4;  // gamma3
      ga_i[3].g[i] = -1.0;
      ga_i[3].ind[i] = (i + 2) % 4;  // gamma4
      ga_i[4].g[i] = 1.0 - 2 * (i / 2);
      ga_i[4].ind[i] = i;  // gamma5
    }
    set_GAM(*this);
  }
};
 ...
 public:
  ...
  ga_matrices_cps()
  {
    ...
    for (Int i = 0; i < 4; i++) {
      unit.g[i] = 1.0;
      unit.ind[i] = i;
      ga_i[0].g[i] = Imag * Ftype(1.0 - 2 * (i / 2));
      ga_i[0].ind[i] = 3 - i;  // gamma1
      ga_i[1].g[i] = a[i];
      ga_i[1].ind[i] = 3 - i;  // gamma2
      ga_i[2].g[i] = Imag * Ftype(-1.0 * a[i]);
      ga_i[2].ind[i] = (i + 2) % 4;  // gamma3
      ga_i[3].g[i] = 1.0;
      ga_i[3].ind[i] = (i + 2) % 4;  // gamma4
      ga_i[4].g[i] = 1.0 - 2 * (i / 2);
...
//  qacc_MemcpyToSymbol(gamma_com_cps, &ga_cps.gL[iv].g[0]  ,
//  4*sizeof(Complexq),iv*4*sizeof(Complexq), qacc_MemcpyHostToDevice);
//  qacc_MemcpyToSymbol(gamma_int_cps, &ga_cps.gL[iv].ind[0], 4*sizeof(int),
//  iv*4*sizeof(int),qacc_MemcpyHostToDevice);
//}
...
inline void get_g_pointer(std::vector<ga_M>& gL, qlat::vector<Complexq*>& gP,
                          qlat::vector<Int*>& iP)
{
