...
struct fft_desc_basic {
  ...
  fft_desc_basic(Int order_ch_or = 0)
  {
    ...
    //
    ////Need set
    nv.resize(4, MemType::Uvm);
    Nv.resize(4, MemType::Uvm);
    mv.resize(4, MemType::Uvm);
    iniv.resize(4, MemType::Uvm);
  }
  ...
  inline LInt index_l_from_g_coordinate(const Coordinate& pos,
                                        Int rank_set = -1) const;
...
inline Long fft_desc_basic::get_xyzmi_curr() { return get_mi_curr(3); }

inline Coordinate fft_desc_basic::coordinate_l_from_index(LInt isp) const
{
  if (variable_set == -1) {
    abort_r("fft_desc not set! \n");
  }
  //
  ...
  g0[3] += isp / (Nv[0] * Nv[1] * Nv[2]);
  Long tem = isp % (Nv[0] * Nv[1] * Nv[2]);
  g0[orderN[0]] += tem / (Nv[orderN[1]] * Nv[orderN[2]]);
  tem = tem % (Nv[orderN[1]] * Nv[orderN[2]]);
...
inline LInt fft_desc_basic::index_g_from_g_coordinate(
    const Coordinate& g0) const
{
  if (variable_set == -1) {
    abort_r("fft_desc not set! \n");
  }
  ...
  //
  LInt index = ((g0[3] * Nv[orderN[0]] + g0[orderN[0]]) * Nv[orderN[1]] +
                g0[orderN[1]]) *
                   Nv[orderN[2]] +
               g0[orderN[2]];
  return index;
...
inline bool fft_desc_basic::coordinate_g_is_local(const Coordinate& g0) const
{
  ...
  bool is_local = true;
  for (Int i = 0; i < 4; i++) {
    if (g0[i] < Pos0[rank][i] or g0[i] >= (Pos0[rank][i] + Nv[i])) {
      is_local = false;
    }
  }
  return is_local;
...
inline LInt fft_desc_basic::index_g_from_g_coordinate(Int t, Int z, Int y,
                                                      Int x) const
{
  if (variable_set == -1) {
    abort_r("fft_desc not set! \n");
...
inline Coordinate fft_desc_basic::coordinate_g_from_index(LInt isp,
                                                          Int rank_set) const
{
  if (variable_set == -1) {
    abort_r("fft_desc not set! \n");
...
inline LInt fft_desc_basic::index_l_from_g_coordinate(const Coordinate& pos,
                                                      Int rank_set) const
{
  if (variable_set == -1) {
    abort_r("fft_desc not set! \n");
  ...
  for (Int i = 0; i < 4; i++) {
    ...
    if (p[i] < 0 or p[i] >= Nv[i]) {
      abort_r("pos not local!\n");
    }
  }
  //
  LInt index =
      ((p[3] * Nv[orderN[0]] + p[orderN[0]]) * Nv[orderN[1]] + p[orderN[1]]) *
          Nv[orderN[2]] +
      p[orderN[2]];
  //
  return index;
...
inline void fft_desc_basic::print_info()
{
  ...
  qmessage(
      "=Need=====Size of dim,   x %10d  y %10d  z %10d  t %10d nvol %ld \n", nx,
      ny, nz, nt, vol * nt);
  qmessage(
      "=Need=====Size of dim,  Nx %10d Ny %10d Nz %10d Nt %10d Nvol %ld \n", Nx,
      Ny, Nz, Nt, Nvol);
  qmessage(
      "=Order====Size of dim,  M0 %10d M1 %10d M2 %10d, d0 %2d d1 %2d d2 %2d. "
      "\n",
      Nv[orderN[0]], Nv[orderN[1]], Nv[orderN[2]], orderN[0], orderN[1],
      orderN[2]);
  //
  fflush_MPI();
}
...
inline void fft_desc_basic::set_variable()
{
  ...
  // mv[0], mv[1], mv[2], mv[3], iniv[0], iniv[1], iniv[2], iniv[3]);
  // printf("rank %5d : nv %5d %5d %5d %5d,  Nv %5d %5d %5d %5d \n", rank,
  // nv[0], nv[1], nv[2], nv[3], Nv[0], Nv[1], Nv[2], Nv[3]);
  //
  nx = nv[0];
  ny = nv[1];
  nz = nv[2];
  ...
  std::vector<Int> Pos0_tem;
  Pos0_tem.resize(Nmpi * 4);
  for (unsigned int i = 0; i < Pos0_tem.size(); i++) {
    Pos0_tem[i] = 0;
  }
  ...
  for (unsigned int i = 0; i < mi_list_tem.size(); i++) {
    mi_list_tem[i] = 0;
  }
    ...
    for (Int zi = 0; zi < mz; zi++)
      for (Int yi = 0; yi < my; yi++)
        for (Int xi = 0; xi < mx; xi++) {
          ...
          Int flagE = 1;
          for (Int i = 0; i < 4; i++) {
            if (p_tem[i] != Pos0[rank][i]) {
              flagE = 0;
            }
          }
          if (flagE == 1) {
            mi_list_tem[tmi * mz * my * mx + off] = rank;
          }
        }
  }
...
inline void fft_desc_basic::check_mem()
{
  ...
  for (size_t isp = 0; isp < size_t(noden); isp++) {
    ...
    for (Int i = 0; i < 4; i++) {
      ranged[i].push_back(g0[i]);
    }
  }
  //
  for (Int i = 0; i < 4; i++) {
    std::sort(ranged[i].begin(), ranged[i].end());
    ranged[i].erase(unique(ranged[i].begin(), ranged[i].end()),
                    ranged[i].end());
  }
  //
  Int flagC = 0;
  for (unsigned int i = 0; i < 4; i++) {
    unsigned int tem = Nv[i];
    if (tem != ranged[i].size()) {
      flagC = 1;
    }
  }
  for (unsigned int i = 0; i < 4; i++) {
    if (Pos0[rank][i] != ranged[i][0]) {
      flagC = 1;
    }
  }
  ...
  if (flagC > 0) {
    for (Int i = 0; i < 4; i++)
      qmessage("%5d %5d, ", Nv[i], int(ranged[i].size()));
    qmessage("\n");
    for (Int i = 0; i < 4; i++)
      qmessage("%5d %5d, ", Pos0[rank][i], int(ranged[i][0]));
    qmessage("\n");
  ...
  //
  for (Int ri = 0; ri < Nmpi; ri++)
    MPI_Bcast(&map_Nitoi[ri][0], noden * sizeof(LInt), MPI_CHAR, ri,
              get_comm());
  //
  Int flag = 0;
  for (Int i = 0; i < Nmpi; i++)
    for (size_t isp = 0; isp < size_t(noden / Nx); isp++) {
      ...
      for (Int xi = 0; xi < Nx; xi++)
        if (map_Nitoi[i][isp * Nx + xi] != inix + xi) {
          flag = 1;
        }
    }
  sum_all_size(&flag, 1);
  if (flag > 0) {
    abort_r("Layout not continuous in x, B! \n");
  }
}

/// mode 0, mxyz ->  Nt ; mode 1, Nt -> mxyz
inline void desc_xyz_in_one(fft_desc_basic& fd, const Geometry& geo,
                            Int mode = 1)
{
  ...
  if (mode == 0) {
    fd.iniv[3] = tA * nv[3] + gs[3];
  }
  if (mode == 1) {
    fd.iniv[3] = gs[3] * (mv[2] * mv[1] * mv[0]) + tA * Nv[3];
  }
  fd.set_variable();
}
...
inline bool operator<(const FFTdescKey& x, const FFTdescKey& y)
{
  ...
  if (x.order_ch_or < y.order_ch_or) {
    return true;
  }
  if (y.order_ch_or < x.order_ch_or) {
    return false;
  }
  return false;
}
