...
void reduce_MPI_type(Iy num, MPI_Datatype& curr, unsigned int& size)
{
  ...
  // if(num%(sizeof(ComplexT<double>)) == 0){curr = MPI::DOUBLE_COMPLEX ;
  // size=sizeof( ComplexT<double> );return;} if(num%(sizeof(ComplexT<float >))
  // == 0){curr = MPI::COMPLEX        ; size=sizeof( ComplexT<float > );return;}
  //
  if (num % (sizeof(std::int64_t)) == 0) {
    curr = MPI_INT64_T;
    size = sizeof(std::int64_t);
    return;
  }
  if (num % (sizeof(std::int32_t)) == 0) {
    curr = MPI_INT32_T;
    size = sizeof(std::int32_t);
    return;
  }
  if (num % (sizeof(std::int16_t)) == 0) {
    curr = MPI_INT16_T;
    size = sizeof(std::int16_t);
    return;
  }
  if (num % (sizeof(std::int8_t)) == 0) {
    curr = MPI_INT8_T;
...
bool IsBasicTypeReal()
{
  ...
  if (type == std::string("RealF") or type == std::string("RealD") or
      type == std::string("RealDD")) {
    return true;
  } else {
    return false;
  }
...
bool IsTypeComplex()
{
  ...
  if (type == std::string("ComplexD") or type == std::string("ComplexF") or
      type == std::string("ComplexDD")) {
    return true;
  } else {
    return false;
  }
...
struct GetBasicDataType<GaugeFieldT<D>> {
  ...
  using ElementaryType =
      typename IsBasicDataType<ColorMatrixT<D>>::ElementaryType;
};
...
struct GetBasicDataType<Propagator4dT<D>> {
  ...
  using ElementaryType =
      typename IsBasicDataType<WilsonMatrixT<D>>::ElementaryType;
};
...
// template<class M>
// struct get_MPI_Type{ static MPI_Datatype c=MPI_LOGICAL; static constexpr Int
// size = 0;};
//
// template<>
// struct get_MPI_Type<char    >{      static MPI_Datatype c=MPI_CHAR; static
// constexpr Int size = 1;};
// //template<>
// //struct get_MPI_Type<unsigned char>{ static MPI_Datatype
// c=MPI_UNSIGNED_CHAR; static constexpr Int size = 1;}; template<> struct
// get_MPI_Type<int8_t  >{ static MPI_Datatype c=MPI_INT8_T  ; static constexpr
// Int size = 1;}; template<> struct get_MPI_Type<int16_t >{ static MPI_Datatype
// c=MPI_INT16_T ; static constexpr Int size = 1;}; template<> struct
// get_MPI_Type<int32_t >{ static MPI_Datatype c=MPI_INT32_T ; static constexpr
// Int size = 1;}; template<> struct get_MPI_Type<int64_t >{ static MPI_Datatype
// c=MPI_INT64_T ; static constexpr Int size = 1;}; template<> struct
// get_MPI_Type<uint8_t >{ static MPI_Datatype c=MPI_UINT8_T ; static constexpr
// Int size = 1;}; template<> struct get_MPI_Type<uint16_t>{ static MPI_Datatype
// c=MPI_UINT16_T; static constexpr Int size = 1;}; template<> struct
// get_MPI_Type<uint32_t>{ static MPI_Datatype c=MPI_UINT32_T; static constexpr
// Int size = 1;}; template<> struct get_MPI_Type<uint64_t>{ static MPI_Datatype
// c=MPI_UINT64_T; static constexpr Int size = 1;}; template<> struct
// get_MPI_Type<RealF   >{ static MPI_Datatype c=MPI_FLOAT   ; static constexpr
// Int size = 1;}; template<> struct get_MPI_Type<RealD   >{ static MPI_Datatype
// c=MPI_DOUBLE  ; static constexpr Int size = 1;}; template<> struct
// get_MPI_Type<RealDD >{ static MPI_Datatype c=MPI_DOUBLE  ; static constexpr
// Int size = 2;};
...
*/
template <typename Ty>
void bcast_all_size(Ty* src, const Long size, const Int root = 0,
                    MPI_Comm* commp = NULL)
{
  ...
  //
  MPI_Datatype curr = MPI_DOUBLE;
  unsigned int M_size = sizeof(RealD);
  M_size = get_mpi_type<Ty>(curr);
  //
  Qassert(sizeof(Ty) % M_size == 0);
  int M_fac = sizeof(Ty) / M_size;
...
*/
template <typename Ty>
void gather_all_size(Ty* res, Ty* src, const Long size, const Int root = 0,
                     const MPI_Comm* commp = NULL)
{
...
template <typename Ty>
void sum_all_size(Ty* src, Ty* sav, Long size, Int GPU = 0,
                  const MPI_Comm* commp = NULL)
{
  ...
  if (qlat::get_num_node() == 1) {
    ...
    if (src != sav) {
      cpy_data_thread(sav, src, size, GPU);
      return;
    }
  }
  //
  ...
  GPU_set = 0;
#endif
  VectorGPUKey gkey(size_t(size) * sizeof(Ty) / sizeof(int8_t),
                    ssprintf("sum_all_size_buf_%d", iomp),
                    GPU_set);  ////read buffers for global sum
  if (src == sav) {
    const vector_gpu<int8_t>& tmp = get_vector_gpu_plan<int8_t>(gkey);
    buf_res = (Ty*)tmp.data();
  } else {
    buf_res = sav;
  }  ////small modify for pointers
  //
  MPI_Datatype curr = MPI_DOUBLE;
  unsigned int M_size = sizeof(RealD);
  M_size = get_mpi_type<Ty>(curr);
  ...
  // M_fac, int(sizeof(Ty)), int(sizeof(int8_t)) );
  //
  // bool do_copy = false;
  const bool do_copy =
      true;  // always copy for global sum, AMD machine has some issue
  // H100 need to avoid direct gpu collective communications
  // collective communications directly between GPUs need NCCL for this if
  // necessary... bool copy_sum_gpu  = GPU_set;
...
//
  ...
  if (do_copy == true) {
    ...
    tem_src = (Ty*)tem_sHIP.data();
    tem_res = (Ty*)tem_rHIP.data();
  }
  //
  if (commp == NULL) {
    MPI_Allreduce(tem_src, tem_res, size * M_fac, curr, MPI_SUM, get_comm());
  } else {
    MPI_Allreduce(tem_src, tem_res, size * M_fac, curr, MPI_SUM, *commp);
  }
  //
  // MPI_Request request;
  // if(commp == NULL){MPI_Iallreduce(tem_src,tem_res, size * M_fac, curr,
  // MPI_SUM, get_comm(), &request);} else{MPI_Iallreduce(tem_src,tem_res, size
  // * M_fac, curr, MPI_SUM, *commp, &request);} MPI_Wait(&request,
  // MPI_STATUS_IGNORE);
  //
  if (do_copy == true) {
  ...
  // GPU_set, int(do_copy), int(copy_sum_gpu));
...
void MPI_Alltoallv_Send_Recv(char* src, Iy0* send, Iy1* spls, char* res,
                             Iy0* recv, Iy1* rpls, const MPI_Comm& comm)
{
  Int num_node;
  MPI_Comm_size(comm, &num_node);
  Int id_node;
  MPI_Comm_rank(comm, &id_node);
  ...
  //
  /////===get proper M_size
  MPI_Datatype curr = MPI_BYTE;
  unsigned int M_size = 1;
  unsigned int M_tem = 1;
  ...
  for (Int n = 0; n < num_node; n++) {
    if (send[n] != 0) {
      MPI_Isend(&src[spls[n]], int(send[n] / M_size), curr, n, mpi_tag, comm,
                &send_reqs[c1]);
      c1 += 1;
    }
  }
  ...
  for (Int n = 0; n < num_node; n++) {
    if (recv[n] != 0) {
      MPI_Irecv(&res[rpls[n]], int(recv[n] / M_size), curr, n, mpi_tag, comm,
                &recv_reqs[c2]);
      c2 += 1;
    }
  }
  if (c2 != 0) {
    MPI_Waitall(c2, recv_reqs.data(), MPI_STATUS_IGNORE);
  }
  if (c1 != 0) {
...
template <typename Ty>
void MPI_Alltoallv_mode(Ty* src0, Int* send, Int* spls, Ty* res0, Int* recv,
                        Int* rpls, const MPI_Comm& comm, Int mode = 0,
                        Int GPU = 0)
{
  (void)GPU;
  Ty* src = NULL;
  ...
  if (do_copy == true) {
    ...
    //
    tem_src.resize(max_src);
    tem_res.resize(max_res);
  ...
  //
  if (mode == 0) {
    MPI_Alltoallv(src, send, spls, MPI_CHAR, res, recv, rpls, MPI_CHAR, comm);
  }
  if (mode == 1) {
    MPI_Alltoallv_Send_Recv((char*)src, send, spls, (char*)res, recv, rpls,
                            comm);
  }
  //
  if (do_copy == true) {
    cpy_data_thread(res0, &tem_res[0], max_res, 2);
  }
}
                         ...
                         Int GPU = 0)
{
  ...
  {
    ////TIMER("MPI call CPU");
    MPI_Alltoallv(src, (int*)&send[0], (int*)&spls[0], MPI_CHAR, buf.data(),
                  (int*)&recv[0], (int*)&rpls[0], MPI_CHAR, get_comm());
  }
  //
...
void Bcast_all_Nt(Ty* src, Long size, const Geometry& geo)
{
  ...
  //
  MPI_Alltoallv(src, (int*)&send[0], (int*)&spls[0], MPI_CHAR, src,
                (int*)&recv[0], (int*)&rpls[0], MPI_CHAR, get_comm());
}
...
inline void sum_all_mpi(RealDD* src, const Long size, const MPI_Comm* commp)
{
  ...
  const MemType gmem =
      check_mem_type(src) != MemType::Cpu ? MemType::Acc : MemType::Cpu;
...
template <>
inline void sum_all_mpi(ComplexT<RealDD>* src, const Long size,
                        const MPI_Comm* commp)
{
