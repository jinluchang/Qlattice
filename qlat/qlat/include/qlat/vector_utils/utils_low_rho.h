...
__global__ void multiplyNab_global(const Complexq* Nab, Ftype* Mres,
                                   const Ftype* Mvalues, const Int nt,
                                   const Int nmass, const unsigned long bufN0,
                                   int8_t* GmapM)
{
  ...
  while (off < 32) {
    G0[off] = GmapM[0 * 32 + off];
    off += blockDim.x;
  }
  off = tid;
  while (off < 32) {
    G1[off] = GmapM[1 * 32 + off];
    off += blockDim.x;
  }
  off = tid;
  while (off < 32) {
    G2[off] = GmapM[2 * 32 + off];
    off += blockDim.x;
  }
  off = tid;
  while (off < 32) {
    G3[off] = GmapM[3 * 32 + off];
    off += blockDim.x;
  }
  //
  ...
  /// while(off<nmass*2){NMv_values[offT + off] = Mvalues[off];off +=
  /// blockDim.x;}
  __syncthreads();
  //
...
__global__ void prodab_global(const Complexq* a, const Complexq* b,
                              Complexq* fd, const Int Nvol, const Int Nsum)
{
  Complexq as[12];
  Complexq bs[12];
    ...
    //
    {
      for (Int bi = 0; bi < 4; bi++) {
        ...
        for (Int ai = 0; ai < 4; ai++) {
          ...
          //
          resab[tid * 16 + iv] = 0;
          for (Int doti = 0; doti < 3; doti++) {
            resab[tid * 16 + iv] +=
                qlat::qconj(bs[bi * 3 + doti]) * as[ai * 3 + doti];
          }
          //
          // #endif
          iv += 1;
  ...
  if (tid < 16) {
    for (Int is = 1; is < 16; is++) {
      resab[0 * 16 + tid] += resab[is * 16 + tid];
    }
  } else {
    for (Int is = 17; is < 32; is++) {
      resab[16 * 16 + tid - 16] += resab[is * 16 + tid - 16];
    }
  }
  __syncthreads();
  //
  if (tid < 16) {
    resab[0 * 16 + tid] += resab[16 * 16 + tid];
  }
  __syncthreads();
  //
  ...
  if (tid < 16) {
    f0[tid * offv] = resab[tid];
  }
}
#endif

void prodab(Complexq* a0, Complexq* b0, const qlat::Geometry& geo, Complexq* fM,
            Int mode_reduce = 1)
{
...
#pragma omp parallel for
    for (Long i = 0; i < Nt * 4 * 4; i++) {
      f0[i] = 0;
    }
    // #pragma omp parallel for
    // for(Int it=0;it<Nt;it++){
    //   matrix_prod_cpu(&b[it*m*w], &a[it*n*w], &f0[it*m*n] , m,n, w, 1, true,
    //   true);
    // }
...
inline void reducefM(qlat::vector<Complexq>& fd, Complexq* NabL, Long bufN,
                     std::vector<ga_M>& gL, const Geometry& geo, const Int nvec,
                     const Ftype facvol, unsigned long bufi,
                     Int mode_reduce = 1)
{
  unsigned long Nvol = geo.local_volume();
  Int Nt = geo.node_site[3];
  qlat::vector<Complexq> reduce_sum;
  reduce_sum.resize((nvec * Nt) * 16);
  //
  if (mode_reduce == 1) {
    Long Nsum = Nvol / Nt;
    if (Nsum % 32 != 0) {
      qmessage("Assumed Nsum divide 32 == 0, %8d \n", int(Nsum % 32));
      Qassert(false);
    }
    Nsum = Nsum / 32;
    //
    ...
    Int bSum = 256 / 4;
    Int cutN = 32;
    reduce_gpu2d_6(fd.data(), &reduce_sum[0], Nsum, nvec * Nt * 16, 1, bSum,
                   cutN);
  }
  if (mode_reduce == 0) {
#pragma omp parallel for
    for (Int i = 0; i < reduce_sum.size(); i++) {
      reduce_sum[i] = fd[i];
    }
  }
  //
  ...
  //   NabL_tem[ivec*Nt*16 + it*16 + gi] = reduce_gamma(&reduce_sum[ivec*Nt*16 +
  //   it*16+0], gL[gi])/facvol;
  // });
  //
  // qacc_for(op0, nvec*Nt*16, {
  //   Int ivec = op0/(Nt*16);int op = op0%(Nt*16);
  //   Int it = op/16; Int gi = op%16;
  //   NabL[ivec*Nt*bufN*16 + it*bufN*16 + bufi*16 + gi] += NabL_tem[ivec*Nt*16
  //   + it*16 + gi];
  // });
  //
  qlat::vector<Complexq*> gP;
  qlat::vector<Int*> iP;
  get_g_pointer(gL, gP, iP);
  /////qacc_for(i0, 1,{
  /////  NabL[0] = gP[0][0];
  /////});
  //
  qacc_for(op0, nvec * Nt * 16, {
    ...
    NabL[ivec * Nt * bufN * 16 + it * bufN * 16 + bufi * 16 + gi] +=
        reduce_gamma(&reduce_sum[ivec * Nt * 16 + it * 16 + 0], gP[gi],
                     iP[gi]) /
        facvol;
  });
  /////============================
  ...
  //   Int it = op/16; Int gi = op%16;
  //   ////Nab[ivec*Nt*16 + it*16 + gi] += reduce_gamma(&reduce_sum[ivec*Nt*16 +
  //   it*16+0],gL[gi])/facvol; NabL[ivec*nt*bufN*16 + (it+tini)*bufN*16 +
  //   bufi*16 + gi] += reduce_gamma(&reduce_sum[ivec*Nt*16 +
  //   it*16+0],gL[gi])/facvol;
  // });
  //
  // #pragma omp parallel for
...
inline void multiplyNab_Global(const Complexq* Nab, qlat::vector<Ftype>& Mres,
                               std::vector<Int> avL, std::vector<Int> bvL,
                               const qlat::vector<Complexq>& values,
                               qlat::vector<int8_t>& GmapM, const Int& nmass,
                               const Int& nt, const Int nzero,
                               const unsigned long bufN0, Int mode_reduce = 1)
{
  ...
  //
  /////Set up Mvalues
  qlat::vector<Ftype> MvaluesV;
  MvaluesV.resize(bufN * nmass * 2);
...
#pragma omp parallel for
    for (LInt bi = 0; bi < bufN; bi++) {
      ...
      std::vector<Ftype> buf;
      buf.resize(nmass * 2);
      Ftype* src = (Ftype*)&Mvalues[offAB];
      for (Int off = 0; off < nmass * 2; off++) {
        buf[off] = src[off];
      }
      //
...
inline std::vector<unsigned long> get_loop_cut(Int Nx, Int Ny, Int Nycut,
                                               Int Nxcut)
{
...
inline void get_map_gammaL(std::vector<ga_M>& g0, std::vector<ga_M>& gL,
                           qlat::vector<int8_t>& Gmap)
{
  Gmap.resize(32);
  for (Int i = 0; i < 16; i++) {
    unsigned long r0;
...
struct Nab_distribute {
  ...
  void communicate(Complexq* NabL)
  {
    TIMER("Reduce Nab");
    if (mxyz != 1) {
      sum_all_size(&NabL[0], NabN.data(), bufN * NabL_size, 1, &xyz_comm);
    } else {
      cpy_data_thread(NabN.data(), &NabL[0], bufN * NabL_size, 1);
      /// #ifdef QLAT_USE_ACC
      /// qacc_Memcpy(&NabN[0], &NabL[0], bufN*NabL_size*sizeof(Complexq),
      /// qacc_MemcpyDeviceToDevice); #else memcpy(&NabN[0], &NabL[0],
      /// bufN*NabL_size*sizeof(Complexq)); #endif
    }
    //
    // MPI_Alltoallv(NabN.data(),(int*) &send[0],(int*) &spls[0], MPI_CHAR,
    //               &NabL[0]   ,(int*) &recv[0],(int*) &rpls[0], MPI_CHAR,
    //               t_comm);
    //
    Int GPU = 1;
    MPI_Alltoallv_mode(NabN.data(), (int*)&send[0], (int*)&spls[0], &NabL[0],
                       (int*)&recv[0], (int*)&rpls[0], t_comm, 1, GPU);
  }
...
//
//   /////rank  --> Nt/nt --> bufi --> 16
//   //if(mode_nt == 1){Redistribute_all_Nt(reinterpret_cast<Ftype* >
//   (&NabL[0]), 2*sumsize,geo, 1);}
//
//   //if(mode_nt == 0)
//   //{
...
//   //  //MPI_Allreduce(&src[src_off],&src[src_off], size_sum, curr, MPI_SUM,
//   vec_comm);
//
...
inline void get_low_rho(std::vector<qlat::FieldM<Complexq, 12>>& eigen,
                        const qlat::vector<Complexq>& values, const Int& nzero,
                        qlat::vector<Ftype>& Mres, const Geometry& geo,
                        Int GPUFM = 1)
{
  ////Input must be chiral vectors, eigen_chi, n_vec --> chi --> d/2 --> t,y,z,x
  ///--> c --> complex /values --> n_vec --> massi
  ////
  ...
  //
  {
    ...
    for (Int i = 0; i < 32; i++) {
      GmapM[0 * 32 + i] = Gmap0[i];
    }
    get_map_gammaL(g05, gL, Gmap1);
    for (Int i = 0; i < 32; i++) {
      GmapM[1 * 32 + i] = Gmap1[i];
    }
    get_map_gammaL(g1, gL, Gmap2);
    for (Int i = 0; i < 32; i++) {
      GmapM[2 * 32 + i] = Gmap2[i];
    }
    get_map_gammaL(g15, gL, Gmap3);
    for (Int i = 0; i < 32; i++) {
      GmapM[3 * 32 + i] = Gmap3[i];
    }
    //
    // #ifdef QLAT_USE_ACC
    // qacc_MemcpyToSymbol(Gmap0C, &Gmap0[0],32*sizeof(int8_t),0 ,
    // qacc_MemcpyHostToDevice); qacc_MemcpyToSymbol(Gmap1C,
    // &Gmap1[0],32*sizeof(int8_t),0 , qacc_MemcpyHostToDevice);
    // qacc_MemcpyToSymbol(Gmap2C, &Gmap2[0],32*sizeof(int8_t),0 ,
    // qacc_MemcpyHostToDevice); qacc_MemcpyToSymbol(Gmap3C,
    // &Gmap3[0],32*sizeof(int8_t),0 , qacc_MemcpyHostToDevice); #endif
  }
  //
  ...
  //
  Int meas = 4;
  int Fcount = 3 + 1;  ////((complex multi 6 + plus 2)/2)
...
/////Get Ncut
  ...
  double freeD = freeM * pow(0.5, 30);
  double totalD = totalM * pow(0.5, 30);
  if (membufN * bufN > extra * totalD) {
    bufN = int(extra * totalD / (membufN));
    if (bufN == 0) bufN = 1;
  }
  ...
  ////==propagate setups
  {
    ...
    sum_all_size(&Ncutbuf, 1);
    sum_all_size(&Ncut, 1);
  }
  qmessage(
      "==rank %d, n_vec %8d, Ncut %5d/%5d , Fac %.3e , free %.3e GB, total "
      "%.3e GB \n",
      qlat::get_id_node(), n_vec, Ncut, Nfull, n_vec * 1.0 / Ncut, freeD,
      totalD);
  ////==propagate setups
//
#ifdef __HIP_PLATFORM_HCC__
  if (n_vec != Ncut) {
    abort_r("HIPCC has propblem with memory move!\n");
  }
#endif
//
#endif
  ...
  qmessage("===rank %d, bufN %lu, mem MresL %.3e, NabL %.3e \n",
           qlat::get_id_node(), bufN,
           bufN * MresL_size * sizeof(Complexq) * pow(0.5, 30),
           bufN * NabL_size * sizeof(Complexq) * pow(0.5, 30));
  qmessage("===rank %d, bufE %d, mem %.3e \n", qlat::get_id_node(), Ncutbuf,
           Ncutbuf * npoints * sizeof(Complexq) * pow(0.5, 30));
  //
  ...
  if (modeCopy == 1) {
    ...
    bufE.resize(Ncutbuf);
    for (unsigned long iv = 0; iv < bufE.size(); iv++) {
      gpuMalloc(bufE[iv], npoints, Complexq, 1);
    }
    // for(unsigned long
    // iv=0;iv<bufE.size();iv++){bufE[iv].init(eigen[0].geo());} for(unsigned
    // long iv=0;iv<bufE.size();iv++){qacc_Malloc(&bufE[iv],
    // npoints*sizeof(Complexq));}
  }
  //
  ...
  //
  qlat::vector_gpu<Complexq> NabV;
  NabV.resize(bufN * NabL_size);
  NabV.set_zero();
  //
...
// Buffer for bv
    ...
    if (modeCopy == 1) {
      ...
      //
      if (bv >= bufb0 and bv < bufb1) {
        b0p = bufE[bv % Ncut];
      } else {
        if (bv % Ncut == 0) {
          for (Int iv = 0; iv < Ncut; iv++) {
            if (bv + iv < n_vec)
              qacc_MemcpyAsync(bufE[iv], qlat::get_data(eigen[bv + iv]).data(),
                               npoints * sizeof(Complexq),
      ...
      //
      if (av >= bufb0 and av < bufb1) {
        a0p = bufE[av % Ncut];
      } else {
        if (av >= bufa0 and av < bufa1) {
          ...
          if (av % Ncut == 0) {
            for (Int iv = 0; iv < Ncut; iv++) {
              if (av + iv < n_vec)
                qacc_MemcpyAsync(
                    bufE[Ncut + iv], qlat::get_data(eigen[av + iv]).data(),
    ...
    if (mode_reduce == 1)
      off_FM = ((countrun % Nmpi) * geo.local_volume() * 16) / 32;
    {
      TIMER("Prod core a b");
    ...
    if ((countrun % Nmpi == 0) or countrun == totrun) {
      Int nvec = Nmpi;
      if (countrun % Nmpi != 0) nvec = countrun % Nmpi;
      {
        TIMER("Reduce prodFM");
        reducefM(prodFMV, NabL, bufN, gL, geo, nvec, facvol, bufi, mode_reduce);
      }
      ////bufi += nvec;
      bufi += 1;
      //
      Int rank = qlat::get_id_node();
      if (rank < nvec) {
        avL_local.push_back(avL[rank]);
        bvL_local.push_back(bvL[rank]);
      }
      //
      if (avL_local.size() == bufN or countrun == totrun) {
        ...
        //
        {
          ...
          multiplyNab_Global(NabL, MresL, avL_local, bvL_local, values, GmapM,
                             nmass, nt, nzero, bufN, mode_reduce);
        }
        //
        avL_local.resize(0);
        bvL_local.resize(0);
    ...
    if (jobi % (n_vec) == 0) {
      ...
      double flops_pers_round = vGb * eachrun / (1.0 * time1);
      eachrun = 0;
      gettimeofday(&tm2, NULL);
      qmessage(
          "==jobi %10d, ai %5d , bi %5d , per %.3f, use %.3e sec, %.3f Gflops, "
          "%.3f Gflops/r . \n",
...
//
    ...
    qacc_for(isp, Nsize, {
      ...
      if (mode_reduce == 1)
        for (unsigned long bi = 0; bi < bufN; bi++) {
          Ftype* src =
              &MresL[(((ipr * nmass + mi) * bufN + bi) * nt + t0) * nt + 0];
          /// #pragma omp parallel for
          for (Int ti = 0; ti < nt; ti++) {
            res[ti] += src[ti];
          }
      ...
      if (mode_reduce == 0)
        for (unsigned long bi = 0; bi < bufN; bi++) {
          Ftype* src =
              &MresL[((bi * nt + t0) * nt + 0) * 16 * nmass + ipr * nmass + mi];
          /// #pragma omp parallel for
          for (Int ti = 0; ti < nt; ti++) {
            res[ti] += src[ti * 16 * nmass];
          }
        }
    });
    //
  ...
  //
  {
    TIMER("Final sum Mres");
    sum_all_size(Mres.data(), Mres.size());
  }
  //
  {
    TIMER("Free memory");
