...
qacc void su3_multi_kerB(Ty* res, Ty* Q1, Ty* Q2, Ty* BUF)
{
  for (Int a = 0; a < 3; a++)
    for (Int b = 0; b < 3; b++) {
      ...
      buf = 0.0;
      for (Int d = 0; d < 3; d++) {
        if (dag1 == false and dag2 == false) {
          buf += Q1[b * 3 + d] * Q2[d * 3 + a];
        }
        if (dag1 == false and dag2 == true) {
          buf += Q1[b * 3 + d] * qlat::qconj(Q2[a * 3 + d]);
        }
        if (dag1 == true and dag2 == false) {
          buf += qlat::qconj(Q1[d * 3 + b]) * Q2[d * 3 + a];
        }
        if (dag1 == true and dag2 == true) {
          buf += qlat::qconj(Q1[d * 3 + b]) * qlat::qconj(Q2[a * 3 + d]);
        }
      }
    }
  if (clear) {
...
qacc void normalize_array_c(Ty* src)
{
  Ty norm = Ty(0.0, 0.0);
  for (Long i = 0; i < N; i++) {
    norm += (qlat::qconj(src[i]) * src[i]).real();
  }
...
qacc void orthogonalize_array_c(Ty* p2, Ty* p1)
{
  ...
  // p1[0].real(), p1[0].imag(), p2[0].real(), p2[0].imag()); if (not(c == 0.0))
  {
    for (Long i = 0; i < N; i++) {
      p2[i] -= c * p1[i];
      // p2[i] = p2[i] - c * p1[i];
...
{
  r[2 * 3 + 0] =
      qlat::qconj(r[0 * 3 + 1] * r[1 * 3 + 2] - r[0 * 3 + 2] * r[1 * 3 + 1]);
  r[2 * 3 + 1] =
      qlat::qconj(r[0 * 3 + 2] * r[1 * 3 + 0] - r[0 * 3 + 0] * r[1 * 3 + 2]);
  r[2 * 3 + 2] =
      qlat::qconj(r[0 * 3 + 0] * r[1 * 3 + 1] - r[0 * 3 + 1] * r[1 * 3 + 0]);
}
...
{
  r[0 * 3 + 2] =
      qlat::qconj(r[1 * 3 + 0] * r[2 * 3 + 1] - r[2 * 3 + 0] * r[1 * 3 + 1]);
  r[1 * 3 + 2] =
      qlat::qconj(r[2 * 3 + 0] * r[0 * 3 + 1] - r[0 * 3 + 0] * r[2 * 3 + 1]);
  r[2 * 3 + 2] =
      qlat::qconj(r[0 * 3 + 0] * r[1 * 3 + 1] - r[1 * 3 + 0] * r[0 * 3 + 1]);
}
...
void set_rand_link(GaugeFieldT<Td>& gf, const Int seed = -1)
{
  if (seed == -1) {
    ...
    // set_unit(gf.get_elem_offset(isp), 1.0);});
...
void copy_gf(GaugeFieldT<Ta>& g1, GaugeFieldT<Td>& g0)
{
  ...
  // qlat::ComplexT<Td >* src = (qlat::ComplexT<Td >*)
  // qlat::get_data(g0).data(); qlat::ComplexT<Td >* res = (qlat::ComplexT<Td
  // >*) qlat::get_data(g1).data(); qlat::cpy_GPU(res, src, Ndata,   1, 1);
  //
...
void Gauge_antihermition(GaugeFieldT<Td>& gf)
{
  ...
  qacc_for(isp, V, {
    ...
    for (Int mu = 0; mu < 4; mu++) {
      qlat::ComplexT<double>* s1 =
          (qlat::ComplexT<double>*)gf.get_elem(xl, mu).p;
