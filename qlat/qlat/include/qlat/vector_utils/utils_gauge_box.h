...
struct API FieldBoxT {
  ...
  void init(FieldM<M, civ>& fr)
  {
    ...
    //
    if (type == Su3::u9) {
      Qassert(Multiplicity % 9 == 0);
    }
    if (type == Su3::u6) {
      Qassert(Multiplicity % 6 == 0);
    }
    if (type == Su3::uA) {
      Qassert(Multiplicity % 6 == 0);
    }
  ...
  void show()
  {
    ...
    qmessage("FieldM %2d, nc %2d, V %10ld, Vex %10ld, ", Multiplicity, nc(), V,
             V_ex);
...
void copy_ux(FieldBoxT<Ty, ta>& fr, FieldBoxT<Tf, tb>& fs)
{
  ...
  Qassert(fs.initialized() and fr.initialized()) const Geometry& geo = fs.geo();
  const Long V = geo.local_volume();
  const Int Nr = fr.nc();
  const Int Ns = fs.nc();
...
{
  if (fr.data() != fs.data()) {
    copy_fields<Ty, Tf>(fr.data(), fs.data(), fs.Multiplicity, fr.geo(),
                        fs.geo());
  }
}

template <class Ty, Su3 type>
void refresh_expanded_GPU(FieldBoxT<Ty, type>& g, const std::string& tag = "",
                          const Int GPU = 1, const QBOOL dummy = QTRUE)
{
  refresh_expanded_GPU(g.data(), g.geo(), g.Multiplicity, tag, GPU, dummy);
}

template <class Ty, Su3 type>
void refresh_expanded_GPU(FieldBoxT<Ty, type>& g, const Int dir,
                          const Int GPU = 1, const QBOOL dummy = QTRUE)
{
