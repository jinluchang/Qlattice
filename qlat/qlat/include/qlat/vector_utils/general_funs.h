                        ...
                        size_t sizeF, Int flag, Int size_inner)
{
  ...
  if (biva == 0 or civ == 0 or sizeF == 0 or size_inner == 0) {
    return;
  }
  //
  std::vector<int8_t*> psrc;
  psrc.resize(civ);
  std::vector<int8_t> tmp;
  tmp.resize(biva * sizeF * civ * size_inner);
  if (size_inner <= 1) {
    abort_r("size_innter too small ! \n");
    return;
  }
  //
  if (flag == 1) {
    memcpy((int8_t*)&tmp[0], (int8_t*)&src[0],
           sizeof(int8_t) * biva * sizeF * civ * size_inner);
  }
  //
  for (size_t bi = 0; bi < size_t(biva); bi++) {
    for (Int ci = 0; ci < civ; ci++) {
      if (flag == 0)
        psrc[ci] = &src[(bi * sizeF * civ + ci * sizeF + 0) * size_inner];
      if (flag == 1)
        psrc[ci] = &res[(bi * sizeF * civ + ci * sizeF + 0) * size_inner];
    }
//
#pragma omp parallel for
    for (LInt si = 0; si < sizeF; si++)
      for (Int ci = 0; ci < civ; ci++) {
        if (flag == 0) {
          memcpy(&tmp[(bi * sizeF * civ + si * civ + ci) * size_inner],
                 &psrc[ci][si * size_inner], sizeof(int8_t) * size_inner);
        }
        if (flag == 1) {
          memcpy(&psrc[ci][si * size_inner],
                 &tmp[(bi * sizeF * civ + si * civ + ci) * size_inner],
                 sizeof(int8_t) * size_inner);
        }
      }
  }
  //
  if (flag == 0) {
    memcpy((int8_t*)&res[0], (int8_t*)&tmp[0], biva * sizeF * civ * size_inner);
  }
}
...
__global__ void move_index_global(Ty* src, Ty* res, Long sizeF, Int civ,
                                  Int inner)
{
  ...
  for (Int ni = 0; ni < nC; ni++) {
    ...
    if (flag) {
      ...
      for (Int xi = 0; xi < Biva; xi++) {
        if (off < Total) {
          buf[xi * Threads + tid] = src[s0 * civ * inner + off];
          off += Threads;
        }
      }
      __syncthreads();
    ...
    for (Int xi = 0; xi < nB; xi++) {
      ...
      if (ci < civ and off1 >= 0)
        if ((off1 < Threads * Biva) and (off1 < (Total - z0))) {
          if (flag) {
            res[(ci * sizeF + s0 + si) * inner + i0] = buf[off1];
          }
          if (!flag) {
            buf[off1] = src[(ci * sizeF + s0 + si) * inner + i0];
          }
        }
      off += Threads;
    }
    ...
    if (!flag) {
      ...
      for (Int xi = 0; xi < Biva; xi++) {
        if (off < Total) {
          res[s0 * civ * inner + off] = buf[xi * Threads + tid];
          off += Threads;
        }
      }
      __syncthreads();
...
struct move_index {
  ...
  template <typename Ty>
  void move_civ_out(Ty* src, Ty* res, Int biva, Long sizeF, Int civ,
  ...
  /////order follow src memory order
  template <typename Ty>
  void move_civ_in(Ty* src, Ty* res, Int biva, Int civ, Long sizeF,
                   Int size_inner, bool GPU = false)
  {
             ...
             Int size_inner, bool GPU = false)
  {
    ...
    if (size_inner < 1) {
      qlat::displayln_info(
          qlat::ssprintf("size_inner too small %d !\n", size_inner));
    ...
    //
    if (src == res) {
      VectorGPUKey gkey(Off * sizeof(Ty) / sizeof(int8_t),
                        std::string("move_index_buf"), GPU);
    ...
    // if(GPU)
    if (src == res)
      if ((Off * sizeof(Ty)) % sizeof(qlat::ComplexF) != 0) {
        qlat::displayln_info(
            qlat::ssprintf("size not divided by 16, too small. \n"));
        Qassert(false);
      }
    /// #endif
    //
      ...
      if (GPU) {
        {
          ...
          const Int Biva = (16 * 16 + sizeof(Ty) - 1) / sizeof(Ty);
          ...
          // int(sizeof(int8_t)));
          if (flag == 0)
            move_index_global<Ty, false, Threads, Biva>
                <<<dimGrid, dimBlock>>>(s0, s1, sizeF, civ, size_inner);
          if (flag == 1)
            move_index_global<Ty, true, Threads, Biva>
                <<<dimGrid, dimBlock>>>(s0, s1, sizeF, civ, size_inner);
          qacc_barrier(dummy);
        }
        //
        ...
        if (src == res) {
          ...
          // cpy_data_thread((qlat::ComplexF*) &res[bi*Off], (qlat::ComplexF*)
          // s1, Nvol, 1);
          cpy_GPU((qlat::ComplexF*)&res[bi * Off], (qlat::ComplexF*)s1, Nvol, 1,
                  1);
        }
        //
        continue;
...
#pragma omp parallel for
      ...
      if (src == res) {
        ...
        cpy_data_thread((qlat::ComplexF*)&res[bi * Off], (qlat::ComplexF*)s1,
                        Nvol, 0);
      }
    }
  }
...
template <typename Ty>
inline void quick_move_civ_in(Ty* res, const Int biva, const Int civ,
                              const Long sizeF, const Long size_inner,
                              const Int dir = 1, bool GPU = true)
{
  ...
  for (Int bi = 0; bi < biva; bi++) {
    ...
    qGPU_for(isp, V, GPU, {
      ...
      if (dir == 1)
        for (Int ci = 0; ci < civ; ci++) {
          src[(bi * civ + ci) * size_inner + bj] =
              buf[ci * V + bi * size_inner + bj];
        }
      //
      if (dir == 0)
        for (Int ci = 0; ci < civ; ci++) {
  ...
  //
  {
    ...
    if (print_type == 0)
      printf("node info lR %d, lS %d, mR %d, mS %d, Ni %3d / %3d , host %s \n",
             localRank, localSize, masterRank, masterSize, id_node, num_node,
             host_name);
    if (print_type == 1) {
      qmessage("Host names : ");
...
inline void set_GPU(Int set_gpu_id = 1)
{
  ...
  // get environment variable to set gpu device
  {
    ...
    if (val != "") {
      qlat_set_device = stringtonum(val);
    }
  }
  //
  ...
  if (gpu_verbos) {
    ...
    Qassert(gethostname(host_name, 500) == 0);
    printf(
        "node info lR %d, lS %d, mR %d, mS %d, gi %d, Ng %d, Ni %3d / %3d , "
        "host %s \n",
        localRank, localSize, masterRank, masterSize, gpu_id, num_gpus, id_node,
        num_node, host_name);
  }
  //
  {
...
#pragma omp parallel
    {
      unsigned int cpu_thread_id = omp_get_thread_num();
      unsigned int num_cpu_threads = omp_get_num_threads();
      if (cpu_thread_id % num_gpus != 0) {
        qmessage("Wrong mapping of omp !\n");
        Qassert(false);
      }
...
#ifdef QLAT_USE_ACC
  qmessage(
      "===CPU free %.8e GB, total %.3e GB; GPU free %.8e GB, total %.3e GB. \n",
      s_info.freeram * pow(0.5, 30), s_info.totalram * pow(0.5, 30), freeD,
      totalD);
                      ...
                      const Int mode = 0)
{
  ...
  qGPU_for(isp, Nuse, GPU, {
    for (Long i = 0; i < bfac; i++) {
      ...
      if (off < size_t(N0)) {
        if (mode == 0) {
          a[off] = Ty(std::cos((ini + isp + i) * 0.5),
                      (5.0 / (isp + 1 + i)) * ini * 0.1);
        }
        if (mode == 1) {
          a[off] = Ty(std::cos((ini + isp + i) * 0.5),
                      std::sin(5.0 * (isp + 1 + i)) * ini * 0.1);
        }
      }
    }
  });
...
template <typename Ty>
inline double norm_vec(Ty* buf, const size_t Nd,
                       const MemType mem_type = MemType::Acc)
{
  const Int off = 16;
...
template <typename Ty>
inline void random_EigenM(std::vector<qlat::vector<Ty>>& a, Int GPU = 0,
                          Int seed = 0)
{
  Int N0 = a.size();
  if (N0 == 0) return;
  for (size_t i = 0; i < size_t(N0); i++) {
    random_EigenM(a[i], GPU, seed + i);
  }
}
...
template <typename Ty>
inline void zeroE(std::vector<qlat::vector<Ty>>& a, Int GPU = 0,
                  QBOOL dummy = QTRUE)
{
  for (LInt iv = 0; iv < a.size(); iv++) {
    zeroE(a[iv], GPU, false);
  }
  if (dummy == QTRUE) {
    qacc_barrier(dummy);
  }
...
void random_prop(Propagator4dT<Td>& prop, Int seed = -1)
{
  ...
  qacc_for(isp, geo.local_volume(), {
    ...
    for (Int ci = 0; ci < 12 * 12; ci++) {
      v0.p[ci] =
          (ci / (12 * 12.0)) *
          qlat::ComplexT<Td>(std::cos((ini + isp + ci * 2) * 0.5 + ci),
                             (ci + (5.0 + ci) / (isp + 1)) * ini * 0.1 + 0.2);
    }
  });
}
...
void random_link(GaugeFieldT<Td>& gf, const Int seed = -1)
{
  if (seed == -1) {
    ...
    random_Ty(res,
              geo.local_volume() * gf.multiplicity * sizeof(ColorMatrixT<Td>) /
                  (sizeof(Td) * 2),
              1, seed);
    //
    // qacc_for(isp, gf.field.size(), {
    // set_unit(gf.get_elem_offset(isp), 1.0);}); ColorMatrixT<Td>& unit =
    // gf.get_elem_offset(0);set_unit(unit, 1.0);
    ColorMatrixT<Td> unit;
    set_unit(unit, 1.0);
...
inline std::vector<unsigned int> get_num_power(
    const size_t x, const std::vector<unsigned int>& a)
{
                              ...
                              const std::vector<unsigned int>& a)
{
  ...
  for (LInt i = 0; i < Mpow.size(); i++) {
    ...
    if (Mpow[i] != 0) {
      ...
      // std::vector<Int>::iterator result = std::max_element(nL.begin(),
      // nL.end()); int pos_max = std::distance(nL.begin(), result);
...
inline Coordinate guess_nodeL(Int n, const Coordinate& Lat, const Int mode = 0)
{
  ...
  Coordinate re;
  if (mode == 0) {
    re = spread_powT(n, Lat, a);
  }
  if (mode == 1) {
    re = spread_even(n, Lat, a);
  }
  return re;
}
...
//
//      /* Recovery of the local rank of the process via the environment
//      variable
//         set by Slurm, as  MPI_Comm_rank cannot be used here because this
//         routine is used BEFORE the initialisation of MPI*/
...
//         }
//     } else {
//         printf("Error : impossible to determine the local rank of the
//         process\n"); exit(1);
//     }
// }
                      ...
                      const Int with_GPU = 1, const Int read_input_Lat = 0)
{
  ...
  if (read_input_Lat >= 0) {
    ...
    if (in.layout != std::string("NONE")) {
      ...
      if (in.mode_dis % 10 >= 0 and in.mode_dis % 10 < 2) {
        spreadT = guess_nodeL(n_node, Lat, 0);
      }
      if (in.mode_dis % 10 >= 2 and in.mode_dis % 10 < 4) {
...
inline const Geometry& get_geo_in(inputpara& in, Int& nx, Int& ny, Int& nz,
                                  Int& nt)
{
  nx = in.nx;
  ny = in.ny;
...
inline Int end_Lat(const Int with_mpi = 0, const Int with_timer = 1)
{
  ...
  fflush_MPI();
  qlat::end();
  if (with_mpi == 1 and qlat::is_MPI_initialized()) {
    MPI_Finalize();
  }
  return 0;
}
...
template <typename Ty>
inline void allocate_buf(std::vector<qlat::vector_gpu<Ty>>& buf, size_t n0,
                         size_t n1)
{
...
template <typename Ty>
inline void allocate_buf(std::vector<qlat::vector<Ty>>& buf, size_t n0,
                         size_t n1)
{
  TIMERA("CUDA Buf mem allocation");
  buf.resize(0);
...
inline Ty inv_self(const Ty& lam, double m, double rho, Int one_minus_halfD = 1)
{
  ...
  ComplexT<double> v0 =
      (one_minus_halfD > 0)
          ? (1.0 - tem / 2.0) / (rho * tem + m * (1.0 - tem / 2.0))
          : 1.0 / (rho * tem + m * (1.0 - tem / 2.0));
  Ty res(v0.real(), v0.imag());
  return res;
}

template <typename Ty>
vector<Ty*> EigenM_to_pointers(std::vector<qlat::vector_gpu<Ty>>& src,
                               Long Nvol = -1)
{
  vector<Ty*> res;
...
vector<Ty*> FieldG_to_pointers(std::vector<FieldG<Ty>>& src, Long Nvol = -1)
{
  ...
  res.resize(Nvec * Nt);
  for (size_t iv = 0; iv < Nvec; iv++) {
    Qassert(src[iv].initialized and
            src[iv].multiplicity == src[0].multiplicity);
...
template <typename Ty>
qlat::vector<Ty*> FieldM_to_pointers(
...
template <typename Ty>
qlat::vector<Ty*> FieldM_to_Tpointers(
    std::vector<qlat::FieldM<Ty, 12 * 12>>& src, Long Nvol = -1)
{
  qlat::vector<Ty*> res;
  const size_t Nvec = src.size();
  if (Nvec == 0) {
...
Ty sum_local_to_global_vector(Ty src, MPI_Comm* commp = NULL)
{
  ...
  if (commp != NULL) {
    MPI_Comm_size(*commp, &Nmpi);
    MPI_Comm_rank(*commp, &rank);
  }
  //
  qlat::vector<Long> size_global;
  size_global.resize(Nmpi);
  ...
  Ty res;
  res.resize(total);
  for (unsigned long pos = current; pos < src.size(); pos++) {
    res[pos] = src[pos - current];
  }
  //
...
inline std::vector<Int> num_to_site(const Long num,
                                    const std::vector<Int> key_T)
{
...
/*
  ...
  may need to update sum_all_size to general memory types and avoid using
  unified memeory for communication
*/
template <typename Td, typename Ty>
inline void Reduce_mrh(Td* res, Ty* buf, const Long Ndata, const Int mrh,
                       const Int GPU = 1, const bool single_node_vecs = false)
{
  TIMERB("Reduce");
  vector<double> tmp;
...
template <typename Ty>
inline Ty Reduce(Ty* buf, const Long Ndata, const Int GPU = 1,
                 const bool single_node_vecs = false)
{
  TIMERB("Reduce");
  vector<Ty> rsum;
  rsum.resize(1);
...
/*
  return the index to sort the list in descending_order as src[res[0]] to be
  smallest
*/
template <typename Ty>
std::vector<Long> get_sort_index(Ty* src, Long size)
{
  ...
  std::sort(vec.begin(), vec.end(),
            [](std::pair<Ty, Long>& a, std::pair<Ty, Long>& b) {
...
template <typename Ty>
void sort_vectors_by_axis(std::vector<std::vector<Ty>>& src,
                          std::vector<std::vector<Ty>>& res, Int axis = 0)
{
  Int Naxis = src.size();
  Qassert(Naxis > axis);
  res.resize(src.size());
...
Long std_find(const std::vector<Ty>& src, const Ty& elem)
{
  ...
  typename std::vector<Ty>::const_iterator it =
      std::find(src.begin(), src.end(), elem);
  ;
