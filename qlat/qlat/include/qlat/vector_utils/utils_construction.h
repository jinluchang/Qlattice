...
void prop_to_vec(qlat::vector<Ty*>& propP, qlat::vector_gpu<Ty>& resTa,
                 fft_desc_basic& fd)
{
  TIMERB("Get corr vec");
  ...
  resTa.resize(32 * nmass * NTt *
               Nxyz);  // qlat::set_zero(resTa);////resTa.set_zero();
  //
  ...
  //
  // cpy_data_thread( (resTa.data()              ), resT0.data(), resT0.size(),
  // 1, true); vec_corrE(resTa, Eres, fd, clear);
}

template <typename Ty>
void prop_to_vec(std::vector<qlat::vector_gpu<Ty>>& Eprop,
                 qlat::vector_gpu<Ty>& resTa, fft_desc_basic& fd)
{
  check_prop_size(Eprop, fd);
  //
  const Long Nxyz = fd.Nv[0] * fd.Nv[1] * fd.Nv[2];
...
template <typename Ty>
void prop_to_vec(std::vector<qpropT>& Eprop, qlat::vector_gpu<Ty>& resTa,
                 fft_desc_basic& fd)
{
  const Long Nxyz = fd.Nv[0] * fd.Nv[1] * fd.Nv[2];
  ////Eprop, nmass --> 12 * 12 * Nvol
...
void prop_to_vec(std::vector<qlat::Propagator4dT<Td>*>& prop4d,
...
template <typename Ty>
void prop_to_corr_mom0(std::vector<qlat::vector_gpu<Ty>>& Eprop,
                       qlat::vector<Ty>& Eres, fft_desc_basic& fd,
                       qlat::vector_gpu<Ty>& resTa, Int clear = 1)
{
  prop_to_vec(Eprop, resTa, fd);
...
template <typename Td>
void prop_corrE(Propagator4dT<Td>& p1, qlat::vector<qlat::ComplexT<Td>>& Eres,
                const Coordinate& mom = Coordinate(), const Int tini = 0)
{
  const Geometry& geo = p1.geo();
  std::vector<Propagator4dT<Td>*> pL;
  pL.resize(1);
  pL[0] = &p1;
  //
  qlat::vector_gpu<qlat::ComplexT<Td>> resTa;
...
template <typename Td>
void prop_corrE(Propagator4dT<Td>& p1, const std::string& filename,
                const Coordinate& mom = Coordinate(), const Int tini = 0,
