...
struct API FieldG : Field<M> {
  ...
  Int get_memtype() { return field_gpu.GPU; }

  Long get_nsites() { return Field<M>::geo().local_volume_expanded(); }
  ...
  void init(const Geometry& geo_, const Int multiplicity_, QMEM GPU = QMGPU,
            QMEM_ORDER mem_order_ = QLAT_DEFAULT)
  {
    clear_copy();
  ...
  void init_zero(const Geometry& geo_, const Int multiplicity_,
                 QMEM GPU = QMGPU, QMEM_ORDER mem_order_ = QLAT_DEFAULT)
  {
    bool need_init = false;
    if (Field<M>::initialized) {
      Qassert(field_gpu.GPU == GPU);
      if (Field<M>::geo() != geo_ or Field<M>::multiplicity != multiplicity_ or
          field_gpu.GPU != GPU or mem_order_ != mem_order) {
        need_init = true;
      } else {
        // check parameters
  ...
  {
    if (!Field<M>::initialized or Field<M>::geo() != f.geo() or
        Field<M>::multiplicity != f.multiplicity or
        field_gpu.GPU != f.field_gpu.GPU or mem_order != f.mem_order) {
  ...
  void set_pointer(M* srcp, const Long Nd, const Geometry& geo_,
                   const QMEM GPU = QMGPU,
  ...
  void set_pointer(Field<M>& src, QMEM_ORDER mem_order_, const Long size = 0,
                   const Long offset = 0)
  {
    TIMER("FieldG set_pointer");
    const Geometry& geo = src.geo();
  ...
  void set_zero() { field_gpu.set_zero(); }
...
struct API SelectedFieldG : SelectedField<M> {
  ...
  Long get_nsites() { return SelectedField<M>::n_elems; }
  ...
  // initialize only not initilized, could change parameters compare to initial
  // ones
  void init_zero(const Geometry& geo_, const Long n_elems_,
                 const Int multiplicity_, QMEM GPU = QMGPU,
                 QMEM_ORDER mem_order_ = QLAT_DEFAULT)
  {
    bool need_init = false;
    if (SelectedField<M>::initialized) {
      Qassert(field_gpu.GPU == GPU);
      if (SelectedField<M>::geo() != geo_ or
          SelectedField<M>::n_elems != n_elems_ or
          SelectedField<M>::multiplicity != multiplicity_ or
          field_gpu.GPU != GPU or mem_order_ != mem_order) {
        need_init = true;
      } else {
        // check parameters
  ...
  // construct from vector_gpu
  void set_pointer(M* srcp, const Long n_elems_, const Int multiplicity_,
                   const Geometry& geo_, const QMEM GPU = QMGPU,
                   const QMEM_ORDER order_ = QLAT_DEFAULT)
  {
  ...
  // construct from vector_gpu
  void set_pointer(const vector_gpu<M>& src, const Long n_elems_,
                   const Geometry& geo_, const QMEM_ORDER order_ = QLAT_DEFAULT)
  {
