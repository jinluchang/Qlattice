...
void prop4d_to_Fermion(std::vector<qlat::FermionField4dT<Tc>>& buf,
                       Propagator4dT<Td>& prop, Int dir = 1)
// prop4d (sink 12, src 12) --> vec(src 12), Fermion(sink 12)
{
  if (dir == 1) {
    buf.resize(0);
    buf.resize(12);
    for (Int iv = 0; iv < 12; iv++) {
      if (!buf[iv].initialized) {
        buf[iv].init(prop.geo());
      }
    }
  }
  if (dir == 0) {
...
#pragma omp parallel for
  for (Long index = 0; index < prop.geo().local_volume(); ++index) {
    ...
    for (Int d0 = 0; d0 < 12; d0++) {
      ...
      qlat::ComplexT<Tc>* res =
          (qlat::ComplexT<Tc>*)&(buf[d0].get_elem_offset(index));
...
template <typename Td, typename Tc>
void Fermion_to_prop4d(Propagator4dT<Td>& prop,
                       std::vector<qlat::FermionField4dT<Tc>>& buf)
{
  Qassert(buf.size() == 12);
...
void prop4d_to_fieldG(FieldG<Ty>& res, Propagator4dT<Td>& src, Int dir = 1)
{
  ...
  if (dir == 0) {
    Qassert(res.mem_order == QLAT_OUTTER);
  }
  //
...
template <typename Td, typename Ty>
void prop_gpu_to_qprop(qlat::Field<Td>& res, qlat::vector_gpu<Ty>& src,
                       Int dir = 1)
{
  const Long Ndc = 12 * 12;
  Qassert(res.initialized and res.multiplicity == Ndc);
  if (dir == 1) {
    Qassert(Long(src.size()) == res.geo().local_volume() * Ndc);
  }
  if (dir == 0) {
    src.resize(res.geo().local_volume() * Ndc);
  }
...
void copy_bsize_prop_to_FieldP(std::vector<Fieldy>& res, Ty* src, const LInt nV,
                               LInt b_size, qlat::fft_desc_basic& fd,
                               Int GPU = 1, bool rotate = false, Int dir = 0)
{
  TIMERA("copy_bisze_src_to_FieldM");
  const Int civ = 12 * 12;
  if (civ % 12 != 0) {
    abort_r("FieldM type not supported!\n");
  }
  // unsigned int nV = 0;
  Int cfac = civ / 12;
  move_index mv_civ;
  ...
  if (total % b_size != 0) {
    abort_r("bsize vec configurations wrong! \n");
  }
  //
  if (dir == 0) {
    // Long dsize = src.size();
  ...
  if (dir == 1) {
    ...
    Qassert(nV == res.size() * cfac) Qassert(res[0].multiplicity == civ);
  }
  //
  ...
  //
  const Long Nfac = get_threads(32, each);
  Qassert(each % Nfac == 0);
  ...
  qGPU_for(
      isp, dsum, GPU,
      {
        ...
        Ty* s0 = &psrc[(chi * bfac + bi) * nV * b_size + d0 * b_size + bj +
                       offi * Nfac];
        ComplexT<Dy>* s1 =
            &resP[d0a][((d0b * 12 + d1) * NTt + ti) * Nxyz + vi + offi * Nfac];
        if (dir == 0) {
...
void copy_bsize_prop_to_FieldM(std::vector<Fieldy>& res,
                               qlat::vector_gpu<Ty>& src, LInt b_size,
                               qlat::fft_desc_basic& fd, Int GPU = 1,
                               bool rotate = false, Int dir = 0)
{
  ...
  Ty* srcP = src.data();
  copy_bsize_prop_to_FieldP<Ty, Fieldy, 0>(res, srcP, nV, b_size, fd, GPU,
                                           rotate, dir);
}

template <typename Ty, class Fieldy>
void copy_FieldM_to_bsize_prop(qlat::vector_gpu<Ty>& res,
                               std::vector<Fieldy>& src, LInt b_size,
                               qlat::fft_desc_basic& fd, Int GPU = 1,
                               bool rotate = false)
{
...
// template <typename Ty, class Fieldy >
// void copy_bsize_prop_to_FieldG(std::vector<Fieldy >& res, qlat::vector_gpu<Ty
// >& src, LInt b_size, const Geometry& geo, Int dir = 0)
//{
...
//
// template <typename Ty, class Fieldy >
// void copy_FieldG_to_bsize_prop(qlat::vector_gpu<Ty >& res, std::vector<Fieldy
// >& src, LInt b_size, const Geometry& geo)
//{
//   fft_desc_basic& fd = get_fft_desc_basic_plan(geo);
//   for(LInt iv=0;iv<res.size();iv++){
...
void ini_propG(std::vector<qlat::vector_gpu<Ty>>& prop, const Long nmass,
               size_t Nsize, bool clear = true)
{
  if (Long(prop.size()) != nmass) {
    prop.resize(nmass);
  }
...
void copy_eigen_prop_to_EigenG(std::vector<qlat::vector_gpu<Ty>>& resG, T* src,
                               LInt b_size, Int nmass, qlat::fft_desc_basic& fd,
                               Int GPU = 1, Int dir = 1)
{
  ...
  qGPU_for(isp, dsum, GPU,
           {
             ...
             T* s0 = &src[(chi * bfac + bi) * Ns * b_size + d0 * b_size + bj +
                          offi * Nfac];
             Ty* s1 = (Ty*)&resP[massi][((d0i * 12 + d1) * NTt + ti) * Nxyz +
                                        vi + offi * Nfac];
             //
             if (dir == 0) {
               for (Long i = 0; i < Nfac; i++) {
...
template <typename Ty>
void FieldM_src_to_FieldM_prop(qlat::FieldM<Ty, 1>& src,
                               qlat::FieldM<Ty, 12 * 12>& res, Int GPU = true,
                               bool dummy = true)
{
  ...
  //
  // bool do_ini = true;
  // if(res.size() == src.size())if(res[src.size()-1].initialized){do_ini =
  // false;} if(do_ini){res.resize(nV);for(Int
  // iv=0;iv<nV;iv++){res[iv].init(geo);}}
...
template <typename Ty>
void FieldM_src_to_FieldM_prop(std::vector<qlat::FieldM<Ty, 1>>& src,
                               std::vector<qlat::FieldM<Ty, 12 * 12>>& res,
                               Int GPU = true)
{
  if (src.size() == 0) {
    return;
  ...
  for (Int iv = 0; iv < nV; iv++)
    FieldM_src_to_FieldM_prop(src[iv], res[iv], GPU, false);
  qacc_barrier(dummy);
}
...
void FieldG_src_to_FieldG_prop(qlat::FieldG<Ty>& src, qlat::FieldG<Ty>& res)
{
  ...
  Qassert(res.geo() == geo);
  clear_fields(res);  // set all zero... Bug introduced..., below only diagnal
                      // will be set
  //
  Long Ncopy = geo.local_volume();
  //
