...
void apply_eo_signT(Ty* sP, Ty* rP, qlat::FieldM<int8_t, 1>& eo,
                    const int8_t dir = 1)
{
  TIMER("apply_eo_sign");
  ...
  qacc_for(isp, geo.local_volume(), {
    ...
    for (Int ic = 0; ic < Nd; ic++) {
      rP[index * Nd + ic] = sign * sP[index * Nd + ic];
    }
  });
}

template <class Ty, Int civ>
void apply_eo_signT(qlat::vector<Ty*>& sP, qlat::vector<Ty*>& rP,
                   ...
                   qlat::FieldM<int8_t, 1>& eo, const int8_t dir = 1, const int8_t eo_dir = -1)
{
  ...
  if (!eo.initialized) {
    qlat_map_eo_site(eo, geo, eo_dir);
  }
                   ...
                   qlat::FieldM<int8_t, 1>& eo, const int8_t dir = 1, const int eo_dir = -1)
{
  ...
  for (unsigned int si = 0; si < src.size(); si++) {
    apply_eo_sign<Ty, civ>(src[si], res[si], eo, dir, eo_dir);
  }
}

///////src and res can be the same pointer
template <class Ty, Int civ>
void apply_eo_zeros(Ty* sP, Ty* rP, qlat::FieldM<int8_t, 1>& eo,
                    const int8_t dir = 1)
{
  ...
  qacc_for(isp, geo.local_volume(), {
    ...
    if (eP[isp] == 0 and dir == 0) {
      sign = 0;
    }  ////even to be zero
    if (eP[isp] == 1 and dir == 1) {
      sign = 0;
    }  ////odd  to be zero
    // qlat::ComplexD sign = qlat::ComplexD(-1.0 * dir *(eP[isp]*2 - 1), 0);
    for (Int ic = 0; ic < civ; ic++) {
      rP[isp * civ + ic] = sign * sP[isp * civ + ic];
    }
  });
}
...
template <class Ty, Int civ>
void apply_eo_zeros(std::vector<qlat::FieldM<Ty, civ>>& src,
                    std::vector<qlat::FieldM<Ty, civ>>& res,
                    qlat::FieldM<int8_t, 1>& eo, const int8_t dir = 1)
{
  Qassert(src.size() == res.size());
...
void reduce_color_Re(qlat::vector_gpu<Ty>& res, qlat::FieldM<Ty, civ>& p0,
                     qlat::FieldM<Ty, civ>& p1, const Int sign)
{
  TIMER("reduce_color");
  ...
  qacc_for(isp, V, {
    if (Real == 1)
      for (Int ic = 0; ic < civ; ic++) {
        resP[isp] +=
            (sign *
             (qlat::qconj(pa[isp * civ + ic]) * pb[isp * civ + ic]).real());
      }
    if (Real == 0)
      for (Int ic = 0; ic < civ; ic++) {
        resP[isp] +=
            (sign * (qlat::qconj(pa[isp * civ + ic]) * pb[isp * civ + ic]));
      }
    if (Real == -1)
      for (Int ic = 0; ic < civ; ic++) {
        resP[isp] +=
            (sign *
             (qlat::qconj(pa[isp * civ + ic]) * pb[isp * civ + ic]).imag());
      }
  });
}

template <class Ty, Int civ>
void reduce_color(qlat::vector_gpu<Ty>& res, qlat::FieldM<Ty, civ>& p0,
                  qlat::FieldM<Ty, civ>& p1, const Int sign, const int8_t Real)
{
  if (Real == 1) {
    reduce_color_Re<Ty, civ, 1>(res, p0, p1, sign);
  }
  if (Real == 0) {
    reduce_color_Re<Ty, civ, 0>(res, p0, p1, sign);
  }
  if (Real == -1) {
    reduce_color_Re<Ty, civ, -1>(res, p0, p1, sign);
  }
...
template <class Ty, Int civ>
void reduce_color(std::vector<qlat::vector_gpu<Ty>>& res,
                  std::vector<qlat::FieldM<Ty, civ>>& p0,
                  std::vector<qlat::FieldM<Ty, civ>>& p1, const Int sign,
                  const int8_t Real)
{
...
template <class Ty>
void reduce_color(qlat::vector<Ty>& resC, qlat::vector_gpu<Ty>& p0,
                  qlat::vector_gpu<Ty>& p1, const Int sign, const Int t0,
                  const Int clear, const Geometry& geo)
{
  TIMER("reduce_color");
  const Long V = geo.local_volume();
...
inline void get_index_mappings_reverse(qlat::vector<Long>& map,
                                       const Geometry& geo)
{
...
void qlat_cf_to_quda_cfT(T1* quda_cf, Ty* src, const Int Dim,
                         const Geometry& geo, qlat::vector<Long>& map_)
{
  ...
  qacc_for(qlat_idx_4d, V, {
    ...
    for (Int dc = 0; dc < Dim; dc++) {
      ...
      if (dir == 1) {
        quda_cf[(eo * Dim + dc) * Vh + qi] = src[qlat_idx_4d * Dim + dc];
      }
      if (dir == 0) {
        src[qlat_idx_4d * Dim + dc] = quda_cf[(eo * Dim + dc) * Vh + qi];
      }
    }
  });
}
...
void qlat_cf_to_quda_cf_pointer(T1* quda_cf, Ty* src, const Int Dim,
...
template <class T1, class Ty>
void quda_cf_to_qlat_cf_pointer(Ty* res, T1* quda_cf, const Int Dim,
                                const Geometry& geo, qlat::vector<Long>& map)
{
...
void copy_eo_cs_to_fieldMT(qlat::vector_gpu<Ty>& res, const Int civ,
                           const Geometry& geo, vector_cs<Ty>& even,
                           vector_cs<Ty>& odd, Int e0, Int e1, Int o0, Int o1,
                           qlat::vector<Long>& map)
{
  ...
  const Long Vh = V / 2;
  if (dir == 1) {
    if (Long(res.size()) != civ * V) {
      res.resize(civ * V, QMSYNC);
    }
  }
  if (dir == 0) {
    Qassert(Long(res.size()) == V * civ);
  }
  ...
  Qassert(civ % DIM == 0);
  if (map.size() == 0) {
    get_index_mappings_reverse(map, geo);
  }
  Int nvec = civ / DIM;
  ...
  for (Int ei = 0; ei < ne; ei++) {
    eP[ei] = even.get_pointers(ei + e0);
  }
  for (Int oi = 0; oi < no; oi++) {
    eP[ne + oi] = odd.get_pointers(oi + o0);
  }
      ...
      for (Int ni = 0; ni < ne; ni++)
        for (Int eo = 0; eo < 2; eo++) {
          ...
          for (Int c = 0; c < DIM; c++) {
            ...
            if (mode == 1) {
              bv = c * Vh + qi;
            }  ////quda vectors this order
            const Long bi = bv / b_size;
            const Long bj = bv % b_size;
            {
              {
                if (dir == 1) {
                  rr[c * nvec + ni] = eP[eo * ne + ni][bi][bj];
                }
                if (dir == 0) {
                  eP[eo * ne + ni][bi][bj] = rr[c * nvec + ni];
                }
              }
            }
          }
...
void copy_eo_cs_to_fieldM(qlat::vector_gpu<Ty>& res, const Int civ,
                          const Geometry& geo, vector_cs<Ty>& even,
                          vector_cs<Ty>& odd, Int e0, Int e1, Int o0, Int o1,
                          qlat::vector<Long>& map, Int mode = 0, Int dir = 1)
{
  if (dir == 0 and mode == 0) {
    copy_eo_cs_to_fieldMT<Ty, 0, 0>(res, civ, geo, even, odd, e0, e1, o0, o1,
                                    map);
    return;
  }
  if (dir == 0 and mode == 1) {
    copy_eo_cs_to_fieldMT<Ty, 0, 1>(res, civ, geo, even, odd, e0, e1, o0, o1,
                                    map);
    return;
  }
  if (dir == 1 and mode == 0) {
    copy_eo_cs_to_fieldMT<Ty, 1, 0>(res, civ, geo, even, odd, e0, e1, o0, o1,
                                    map);
    return;
  }
  if (dir == 1 and mode == 1) {
    copy_eo_cs_to_fieldMT<Ty, 1, 1>(res, civ, geo, even, odd, e0, e1, o0, o1,
                                    map);
    return;
  }
...
template <typename Ty>
void copy_fieldM_to_eo_cs(vector_cs<Ty>& even, vector_cs<Ty>& odd,
                          qlat::vector_gpu<Ty>& res, const Int civ,
                          const Geometry& geo, Int e0, Int e1, Int o0, Int o1,
                          qlat::vector<Long>& map, Int mode = 0)
{
