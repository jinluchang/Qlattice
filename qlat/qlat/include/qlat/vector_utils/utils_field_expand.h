...
struct expand_index_buf {
  ...
  //
  expand_index_buf(const Geometry& geo_, const Int multiplicity,
                   const std::string& tag)
  {
    // const Int multiplicity = 1;//always 1 for the buffers reuse
    Multiplicity = multiplicity;
    if (tag == std::string("")) {
      setup_expand(geo_, multiplicity, pack_send, pack_recv,
                   set_marks_field_all, tag);
    } else {
      setup_expand(geo_, multiplicity, pack_send, pack_recv,
                   set_marks_field_dir, tag);
    }
  }
  //
  ~expand_index_buf()
...
struct expand_index_Key {
  ...
  Int multiplicity;
  std::string tag;
  expand_index_Key(const Geometry& geo_, const Int multiplicity_,
...
{
  static Cache<expand_index_Key, expand_index_buf> cache("expand_index_Key",
                                                         MAX_EXPAND_BUF + 1);
  return cache;
}
...
inline expand_index_buf& get_expand_index_buf_plan(const expand_index_Key& ekey)
{
  if (!get_expand_index_buf_cache().has(ekey)) {
    get_expand_index_buf_cache()[ekey] =
        expand_index_buf(ekey.geo, ekey.multiplicity, ekey.tag);
  }
...
inline void clear_expand_plan_cache() { get_expand_index_buf_cache().clear(); }
...
struct expand_field_buffer {
  ...
  //
  template <typename T>
  void init(const Geometry& geo_, const std::string& tag_, T* res,
            const Int MULTI, const Int GPU)
  {
  ...
  //
  inline Long buf_size() { return (bs.size() + br.size()) * sizeof(char); }
  //
  ...
  void excute_copyT(Int type, Int MULTI)
  {
    ...
    // Int GPU = 1;
    if (bs.mem_type == MemType::Cpu or bs.mem_type == MemType::Comm) {
  ...
  inline void excute_copy(Int type)
  {
    ...
    bool find = false;
    if (!find and dsize % sizeof(int64_t) == 0) {
      excute_copyT<int64_t>(type, dsize / sizeof(int64_t));
      find = true;
    }
    if (!find and dsize % sizeof(int32_t) == 0) {
      excute_copyT<int32_t>(type, dsize / sizeof(int32_t));
      find = true;
    }
    if (!find and dsize % sizeof(int16_t) == 0) {
      excute_copyT<int16_t>(type, dsize / sizeof(int16_t));
      find = true;
    }
    if (!find and dsize % sizeof(int8_t) == 0) {
      excute_copyT<int8_t>(type, dsize / sizeof(int8_t));
      find = true;
    }
    if (!find and dsize % sizeof(float) == 0) {
      excute_copyT<float>(type, dsize / sizeof(float));
      find = true;
    }
    if (!find and dsize % sizeof(char) == 0) {
      excute_copyT<char>(type, dsize / sizeof(char));
      find = true;
    }
    Qassert(find == true);
  }
...
inline std::vector<expand_field_buffer>& get_expand_buffer_full_list()
{
  ...
  if (bufs.size() != MAX_EXPAND_BUF) {
    bufs.resize(MAX_EXPAND_BUF);
  };
  return bufs;
}
...
{
  //
  static const Long max_bytes = Long(
      get_env_double_default("q_expand_buffer_size", 0.5) * 1024 * 1024 * 1024);
...
inline void expand_buffer_wait_mpi()
{
  ...
  static Int do_comm_step = qlat::get_env_long_default(
      std::string("qlat_field_expand_buffer_AMD"), 0);
...
inline expand_field_buffer& expand_buffer(const Geometry& geo,
                                          const std::string& tag, T* res,
                                          const Int MULTI, const Int GPU,
                                          Int& idx)
{
  std::vector<expand_field_buffer>& bufs = get_expand_buffer_full_list();
  // check if res is already in buffer or not
  static Int do_comm_default = qlat::get_env_long_default(
      std::string("qlat_field_expand_buffer_asyn"), 1);
...
template <class M>
void refresh_expanded_GPUT_v0(
    M* res, const Geometry& geo, const Int MULTI,
    const SetMarksField& set_marks_field = set_marks_field_all,
    const std::string& tag = std::string(""), Int GPU = 1,
    const QBOOL dummy = QTRUE)
{
  ...
  qlat::vector_gpu<int8_t>& sbuf =
      qlat::get_vector_gpu_plan<int8_t>(0, std::string("general_buf0"), GPU);
  qlat::vector_gpu<int8_t>& rbuf =
      qlat::get_vector_gpu_plan<int8_t>(0, std::string("general_buf1"), GPU);
  expand_index_buf& ebuf = get_expand_index_buf_plan(geo, multiplicity, tag);
  Qassert(ebuf.pack_send.size() == 2 * plan.total_send_size and
          ebuf.pack_recv.size() == 2 * plan.total_recv_size);
  //
...
template <class M>
void refresh_expanded_GPUT(
    M* res, const Geometry& geo, const Int MULTI,
    const SetMarksField& set_marks_field = set_marks_field_all,
    const std::string& tag = std::string(""), Int GPU = 1,
    const QBOOL dummy = QTRUE)
{
  static Int do_test_expand =
      qlat::get_env_long_default(std::string("qlat_do_test_expand"), 0);
  if (do_test_expand == 1) {
    refresh_expanded_GPUT_v0(res, geo, MULTI, set_marks_field, tag, GPU, dummy);
    return;
  }
  Qassert(sizeof(M) % sizeof(RealD) == 0);
  // const Int MULTI = geo.multiplicity;
...
void refresh_expanded_GPU(M* res, const Geometry& geo, const Int MULTI,
                          const std::string& tag, const Int GPU = 1,
                          const QBOOL dummy = QTRUE)
{
