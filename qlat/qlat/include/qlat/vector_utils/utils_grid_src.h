...
*/
template <typename Ty>
void check_noise_pos(qlat::FieldM<Ty, 1>& noise, Coordinate& pos,
                     Coordinate& off_L, Int printS = 0, Int mod = 0)
{
  qlat::Geometry& geo = noise.geo();
  qlat::vector<Int> nv, Nv, mv;
  geo_to_nv(geo, nv, Nv, mv);
  ...
  for (Int iL = 0; iL < 4; iL++) {
    ...
    for (LInt giL = 0; giL < grid[iL].size(); giL++) {
      grid[iL][giL] = 0.0;
    }
  }
  ...
  int t_ini = 0;
  if (mod == 1) {
    get_num_time(noise, number_t, t_ini);
  }
  ...
  for (Int iL = 0; iL < 4; iL++)
    for (Int k = 0; k < NL[iL]; k++)
      if (grid[iL][k] > 0.0) off_L[iL] += 1;
  // for(Int x=0;x<nx;x++){if(grid[0][x]>0.0)off_L[0] += 1;}
  if (int(grid_count) != off_L[0] * off_L[1] * off_L[2] * off_L[3]) {
    qmessage(
        "Source Check Failed grid_count %10d, offx %5d, offy %5d, offz %5d, "
        "offt %5d!\n",
  ...
  //
  qmessage(
      "Check T %5d %5d %5d %5d, offx %5d, offy %5d, offz %5d, offt %5d. \n",
      pos[0], pos[1], pos[2], pos[3], off_L[0], off_L[1], off_L[2], off_L[3]);
  //
  if (printS == 1) {
    for (unsigned int isp = 0; isp < Nsite; isp++) {
      Coordinate xl0 = geo.coordinate_from_index(isp);
      ...
      ////position p = noise.desc->get_position(isp,get_node_rank());
      {
        ...
        if (qnorm(tem_source) > 0.01) {
          printf("Check K %5d %5d %5d %5d node %5d %13.5f %13.5f !\n", p[0],
                 p[1], p[2], p[3], qlat::get_id_node(), tem_source.real(),
                 tem_source.imag());
        }
      }
    }
  ...
  if (printS == 2) {
    /////int x = pos/10000000;int y = (pos%10000000)/100000;int z =
    ///(pos%100000)/1000;int t = pos%1000;
    for (unsigned int isp = 0; isp < Nsite; isp++) {
      Coordinate xl0 = geo.coordinate_from_index(isp);
      Coordinate p = geo.coordinate_g_from_l(xl0);
      //
      /// position p = noise.desc->get_position(isp,get_node_rank());
      {
        ...
        Int printv = 1;
        for (Int i = 0; i < 4; i++) {
          if (p[i] != pos[i] and p[i] != pos[i] + off_L[i]) {
            printv = 0;
          }
          /////p[i] = pos[i]
        }
...
void get_noise_pos(Fieldy& noise, std::vector<Coordinate>& grids,
                   std::vector<RealD>& Zlist, const Int Zn = 3,
                   const Int printS = 0, const bool need_copy = false)
{
  ...
  std::vector<RealD> zk_mpi;
  zk_mpi.resize(total_pos);
  std::vector<Int> pos_mpi;
  pos_mpi.resize(total_pos * 4);
  for (Long i = 0; i < total_pos * 4; i++) {
    pos_mpi[i] = 0;
  }
...
template <class Fieldy>
void get_noise_posG(Fieldy& noise, std::vector<Coordinate>& grids,
                    std::vector<RealD>& Zlist, const Int Zn,
                    const Int printS = 0, const bool need_copy = false)
{
  (void)need_copy;
...
template <class Fieldy>
void check_noise_high(Fieldy& noise, std::vector<Int>& sinkt, Long& npoints,
...
inline void grid_list_pos(const Coordinate& off_L, qlat::vector<Long>& Ngrid)
{
  ...
  if (off_L.size() != 4) {
    abort_r("dimention of positions wrong!\n ");
  }
  Int Ntot = off_L[0] * off_L[1] * off_L[2];
  double max_d = 1.0 * off_L[0] * off_L[0] + 1.0 * off_L[1] * off_L[1] +
                 1.0 * off_L[2] * off_L[2] + 1.0;
  //
  Ngrid.resize(Ntot);
...
inline Coordinate get_grid_off(Long j0, const Coordinate& off_L,
                               const Coordinate& pos_ini, const Coordinate& Lat)
{
  if (pos_ini.size() != 4 or off_L.size() != 4) {
    abort_r("dimension of positions wrong!\n ");
  }
  ...
  for (Int i = 0; i < 4; i++) {
    pos[i] += ((Lat[i] / (off_L[i])) * off_pos[i]) % (Lat[i]);
  }
  //
...
inline void grid_list_posT(std::vector<std::vector<Coordinate>>& LMS_points,
                           const Coordinate& off_L, const Coordinate& pos,
                           const Int combineT, const Coordinate& Lat)
{
  ...
  LMS_points.resize(0);
  if (combineT == int(0)) {
    LMS_points.resize(Nfull.size() * off_L[3]);
  }
  if (combineT == int(1)) {
    LMS_points.resize(Nfull.size());
  }
  //
  Coordinate cur_pos;
  Coordinate cur_off;
    ...
    if (combineT == 0) {
      for (Int it = 0; it < off_L[3]; it++) {
        ...
        cur_off[3] += ((Lat[3] / (off_L[3])) * it) %
                      Lat[3];  ////need be careful not to exceed boundaries
        std::vector<Coordinate> lms_res;
        lms_res.resize(1);
        lms_res[0] = cur_off;
        LMS_points[ci] = lms_res;
        ci += 1;
      }
    }
    if (combineT == 1) {
      std::vector<Coordinate> lms_res;
      lms_res.resize(off_L[3]);
      for (Int it = 0; it < off_L[3]; it++) {
        cur_off = cur_pos;
  ...
  // if(combineT == int(0)){Qassert(Long(LMS_points.size()) ==
  // Long(Nfull.size()*off_L[3]));} if(combineT ==
  // int(1)){Qassert(Long(LMS_points.size()) == Long(Nfull.size()         ));}
}

template <typename Ty>
Ty write_grid_point_to_src(FieldG<Ty>& res, const FieldG<Ty>& src,
                           const std::vector<Coordinate>& pos, Int b_size,
                           qlat::fft_desc_basic& fd)
{
  ...
  const Int Ndc = 12 * 12;
  if (!res.initialized) {
    res.init(geo, Ndc, QMGPU, QLAT_OUTTER);
  }
...
//////asuume res on GPU
template <typename Ty>
Ty write_grid_point_to_src(Ty* res, const Ty* srcP,
                           const std::vector<Coordinate>& posL, Int b_size,
                           qlat::fft_desc_basic& fd)
{
  ...
  const Int Ns = 12;
  if (total % b_size != 0) {
    abort_r("eigen system configurations wrong! \n");
  }
  //
  Ty phase = 0.0;
...
template <typename Ty>
Ty write_grid_point_to_src(Ty* res, const qnoiT& src,
                           const std::vector<Coordinate>& posL, Int b_size,
                           qlat::fft_desc_basic& fd)
{
  const Ty* srcP = (Ty*)get_data(src).data();
...
template <typename Ty>
Ty write_grid_point_to_src(Ty* res, const qnoiT& src, const Coordinate& pos,
                           Int b_size, qlat::fft_desc_basic& fd)
{
...
inline void get_grid_psel(std::vector<Coordinate>& psel, const Coordinate& nv,
                          const Coordinate& grid, qlat::RngState& rs,
                          Int t0 = -1, const Int even = -1,
                          const Coordinate& ini_ = Coordinate(-1, -1, -1, -1))
{
  ...
  //
  Coordinate ini;
  if (ini_ != Coordinate(-1, -1, -1, -1)) {
    ini = ini_;
  } else {
    for (Int i = 0; i < 4; i++) {
      ini[i] = int(qlat::u_rand_gen(rs) * (nv[i] / grid[i]));
    }
  }
  if (t0 != -1) {
    ini[3] = t0;
    ...
    for (Int yi = 0; yi < grid[1]; yi++)
      for (Int zi = 0; zi < grid[2]; zi++)
        for (Int ti = 0; ti < grid[3]; ti++) {
          ...
          //
          for (Int i = 0; i < 4; i++) {
            xg[i] = (ini[i] + ci[i] * (nv[i] / grid[i])) % (nv[i]);
          }
          //
          psel.push_back(xg);
        }
...
template <typename Ty>
void get_noises_Coordinate(const qlat::FieldM<Ty, 1>& noise,
                           std::vector<Coordinate>& psel, Int printv = 0)
{
  const qlat::Geometry& geo = noise.geo();
  qlat::vector<Int> nv, Nv, mv;
  geo_to_nv(geo, nv, Nv, mv);
  ...
  for (LInt isp = 0; isp < Nsite; isp++) {
    if (qnorm(noise.get_elem_offset(isp)) > 0.01) {
      ...
      Coordinate xg0 = geo.coordinate_g_from_l(xl0);
      for (Int i = 0; i < DIM; i++) {
        grid_pos.push_back(xg0[i]);
      }
      if (printv == 2) {
        Ty tem = noise.get_elem_offset(isp);
        printf("rank %d, x %d, y %d, z %d, t %d, value %.3e %.3e \n",
               qlat::get_id_node(), xg0[0], xg0[1], xg0[2], xg0[3], tem.real(),
               tem.imag());
      }
    }
  }
  ...
  for (Long p = 0; p < total; p++) {
    for (Int i = 0; i < DIM; i++) {
      tem[i] = grid_pos_global[p * 4 + i];
    }
    psel[p] = tem;
  }
  //
  if (printv >= 1) {
    printf("rank %d, number of non-zeros %ld \n", qlat::get_id_node(), total);
  }
}
...
void get_mix_color_src(qlat::FieldM<Ty, civ>& src, const Coordinate& sp,
                       ...
                       const FieldSelection& fsel, const Int type_src = 0,
                       Int seed = 0, const Int offT = -1,
                       const Coordinate& offG = Coordinate(1, 1, 1, 1))
{
  ...
  {
    std::vector<qlat::RngState> rsL;
    rsL.resize(omp_get_max_threads());
    for (Int is = 0; is < omp_get_max_threads(); is++) {
      rsL[is] = qlat::RngState(
          seed + qlat::get_id_node() * omp_get_max_threads() + is);
    }
    //
  ...
  //
  if (type_src == 12 or type_src == 13 or
      type_src == 14)  ////Wall src, even or odd
  {
    std::vector<qlat::RngState> rsL;
    rsL.resize(omp_get_max_threads());
    for (Int is = 0; is < omp_get_max_threads(); is++) {
  ...
  //
  if (type_src ==
      20)  ////T grid src, all spatial the same for momenta projections
  {
    Qassert(offT > 0);
...
template <class Ty>
void vec_apply_cut(qlat::vector_gpu<Ty>& res, const Coordinate& sp,
                   const double rmax, const Geometry& geo)
{
  TIMER("vec_apply_cut");
  if (rmax < 0) {
    return;
...
void get_point_color_src(std::vector<qlat::FieldM<Tr, civ>>& srcL,
                         const std::vector<Coordinate>& grids,
                         const std::vector<Ty>& phases)
{
...
void get_wall_color_src(std::vector<qlat::FieldM<Tr, civ>>& srcL,
                        const Coordinate& ps, const Int src_eo = -1)
{
...
template <class T>
void make_point_prop(Propagator4dT<T>& prop,
                     const Coordinate& sp = Coordinate(0, 0, 0, 0))
{
...
template <class Td>
void make_grid_src(Propagator4dT<Td>& src, const Coordinate& sp,
                   const Coordinate& offG = Coordinate(1, 1, 1, 1),
                   Int seed = 0)
{
  ...
  for (Int is = 0; is < omp_get_max_threads(); is++) {
    rsL[is] =
        qlat::RngState(seed + qlat::get_id_node() * omp_get_max_threads() + is);
  }
  //
  qlat::vector<Int> nv, Nv, mv;
  geo_to_nv(geo, nv, Nv, mv);
  ...
  qthread_for(isp, geo.local_volume(), {
    ...
    if (found == 4) {
      ...
      // for(Int dc=0;dc<12;dc++)
      for (Int dc = 0; dc < 12; dc++) {
        srcP[(isp * 12 + dc) * 12 + dc] =
            qlat::ComplexT<Td>(std::cos(r), std::sin(r));
      }
    }
  });
}
...
template <class Td>
void make_volume_src(Propagator4dT<Td>& src, Int seed = 0, Int mix_color = 0,
                     Int mix_spin = 0, Int tini = -1)
{
        ...
        if (mix_color == 1 and mix_spin == 0) {
          for (Int c0 = 0; c0 < 3; c0++)
            for (Int c1 = 0; c1 < 3; c1++) {
              ...
              double r = 2 * PI * qlat::u_rand_gen(rs);
              for (Int d = 0; d < 4; d++) {
                srcP[(isp * 12 + d * 3 + c0) * 12 + d * 3 + c1] =
                    qlat::ComplexT<Td>(std::cos(r), std::sin(r)) /
                    qlat::ComplexT<Td>(3.0, 0.0);
              }
            }
        }
        //
          ...
          for (Int d0 = 0; d0 < 2; d0++)
            for (Int d1 = 0; d1 < 2; d1++)
              for (Int c0 = 0; c0 < 3; c0++)
                for (Int c1 = 0; c1 < 3; c1++) {
                  double r = 2 * PI * qlat::u_rand_gen(rs);
                  srcP[(isp * 12 + d0 * 3 + c0) * 12 + d1 * 3 + c1] =
                      qlat::ComplexT<Td>(std::cos(r), std::sin(r)) /
                      qlat::ComplexT<Td>(6.0, 0.0);
                }
          //
...
inline void split_points_simple(std::vector<std::vector<Coordinate>>& res,
...
*/
inline void sort_grid_points(std::vector<Coordinate>& res,
                             std::vector<Coordinate>& grids,
                             const Coordinate& Lat, const Int num_points = 0)
{
  const Long Ng = grids.size();
...
*/
inline void split_grid_points(std::vector<std::vector<Coordinate>>& res,
                              std::vector<Coordinate>& grids,
                              const Coordinate& Lat, const Int combineT = 0,
                              const Int num_points = 0)
{
  ...
  if (num_points == 0 and Ntsrc == 1) {
    do_simple = true;
  }
  if (max_g == Ng and Ntsrc == 1) {
    do_simple = true;
  }
  if (do_simple) {
    split_points_simple(res, grids);
    return;
  }
  //
  if (combineT != 0) {
  ...
  //
  std::vector<std::vector<Coordinate>> sort_grids;
  sort_grids.resize(Nt);
  for (Long ti = 0; ti < Nt; ti++) {
    sort_grid_points(sort_grids[ti], spx_list[ti], Lat, num_points);
  }
  //
  const std::vector<Long> map_gr_dis =
      simple_dis_pick(Nx_grid);  // simple try of distance in 1D
  res.resize(Nr);
  if (Ntsrc > 1) {
...
void check_prop_noise_positions(qlat::Propagator4dT<Td>& prop, Coordinate& pos,
                                Coordinate& off_L, Int printS = 0, Int mod = 0)
{
  qlat::Geometry& geo = prop.geo();
  using Ty = ComplexT<Td>;
  qlat::FieldM<Ty, 1> noise;
  noise.init(geo);
  const Long Nvol = geo.local_volume();
  Ty* pD = (Ty*)get_data(prop).data();
  Ty* nD = (Ty*)get_data(noise).data();
  qacc_for(isp, Nvol, { nD[isp] = pD[isp * 12 * 12 + 0]; });
