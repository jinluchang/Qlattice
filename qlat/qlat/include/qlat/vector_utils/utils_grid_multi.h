...
*/
template <class Ty>
void grid_memory_reshape(qlat::vector<Ty*>& res, qlat::vector<Ty*>& src,
                         const Int civ, const Geometry& geo1,
                         const Geometry& geo0, const Geometry& geoA)
{
  ...
  for (LInt jobi = 0; jobi < jobA.size() / 2; jobi++) {
    ...
    qacc_for(index, Vol, {
      ...
      //
      const LInt c0 =
          ((y0[3] * nfac0[2] + y0[2]) * nfac0[1] + y0[1]) * nfac0[0] + y0[0];
      const LInt c1 =
          ((y1[3] * nfac1[2] + y1[2]) * nfac1[1] + y1[1]) * nfac1[0] + y1[0];
...
template <class Ty>
void grid_memory_reshape(qlat::vector<Ty*>& res, qlat::vector<Ty*>& src,
                         const Int civ, const Coordinate& n1,
                         const Coordinate& n0, const Coordinate& nA)
{
