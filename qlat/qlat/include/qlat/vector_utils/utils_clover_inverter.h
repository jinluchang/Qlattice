...
struct quda_clover_inverter {
  ...
  quda_clover_inverter(const Geometry& geo_, QudaTboundary t_boundary,
                       Int num_src_ = 1);
  ...
  inline void do_inv(Ty* res, Ty* src, const double kappa,
                     const double err = 1e-10, const Int niter = 10000);
  ...
  */
  ...
  // inline void setup_inv_param_prec(Int prec_type = 0, bool force_reload =
  // false);
...
inline void quda_clover_inverter::setup_gauge_param(QudaTboundary t_boundary)
{
  ...
  // QUDA_RECONSTRUCT_NO; gauge_param.reconstruct_precondition      =
  // QUDA_RECONSTRUCT_NO; gauge_param.reconstruct_eigensolver       =
  // QUDA_RECONSTRUCT_NO; gauge_param.reconstruct_refinement_sloppy =
  // QUDA_RECONSTRUCT_NO;  /////link_recon
  //
  gauge_param.reconstruct =
      QUDA_RECONSTRUCT_12;  ////gauge_param.reconstruct = link_recon;
...
quda_clover_inverter::quda_clover_inverter(const Geometry& geo_,
...
inline void quda_clover_inverter::setup_link_p(qlat::ComplexD* quda_gf)
{
  ...
  //   QUDA_STAGGERED_DSLASH); loadGaugeQuda((void *) quda_gf, &gauge_param);
...
inline void quda_clover_inverter::setup_link(
    qlat::vector<qlat::ComplexD>& quda_gf)
{
...
inline void quda_clover_inverter::alloc_csfield_gpu()
{
  ...
  cs_gpu_quda.gammaBasis =
      QUDA_UKQCD_GAMMA_BASIS;  // default quda dslash basis...
...
inline void quda_clover_inverter::alloc_csfield_cpu()
{
  ...
  quda::ColorSpinorParam cpuParam_tem(temV, inv_param, cpuGauge.X(),
                                      pc_solution, inv_param.input_location);
  ...
  //
  cs_gpu = quda::ColorSpinorParam(cs_cpu);
  ...
  // inv_param.cuda_prec_eigensolver, true); if(cs_gpu.nSpin != 1)
  {
  ...
  quda::ColorSpinorParam gpuParam_tem(temV, inv_param, cpuGauge.X(),
                                      pc_solution, QUDA_CUDA_FIELD_LOCATION);
  //
...
inline void quda_clover_inverter::save_prop(const void* srcP,
                                            const char* filename)
{
  TIMER("quda save_prop");
  const Int n0 = 1;
  std::vector<qlat::FieldM<qlat::ComplexD, 3>> prop;
  prop.resize(n0);
...
void quda_clover_inverter::gamma_ukqcd_to_qlat(vector_cs<Ty>& eigen)
{
  ...
  for (int vi = 0; vi < eigen.nvec; vi++) {
    ...
    *gsrc = *gquda0;
    quda_cf_to_qlat_cf_pointer((ComplexT<double>*)(*gres).data(),
                               (ComplexT<double>*)(*gsrc).data(), 12, geo(),
                               mapI);
...
template <typename Ty>
void quda_clover_inverter::copy_field_to_qlat(Ty* res, const int field_num,
                                              const int dir)
{
  TIMER("copy_field_to_qlat");
  if (field_num != 0 and field_num != 1) {
    Qassert(false);
  }
  if (dir != 0 and dir != 1) {
    Qassert(false);
  ...
  if (dir == 1) {
    ...
    quda_cf_to_qlat_cf_pointer(res, (ComplexT<double>*)(*gsrc).data(), 12,
                               geo(), mapI);
  }
  //
  if (dir == 0) {
    qlat_cf_to_quda_cf_pointer((ComplexT<double>*)(*gsrc).data(), res, 12,
                               geo(), mapI);
    if (field_num == 0) {
      *gquda0 = *gsrc;
    }
...
inline void quda_clover_inverter::setup_clover(const double kappa,
                                               const double clover_csw)
{
  if (clover_setup == false or kappa != inv_param.kappa or
    ...
    // inv_param.tol_hq = 1e-10; // specify a tolerance for the residual for
    ...
    //
    {
      ...
      //
      // constructHostCloverField((void*) quda_clover.data(), (void*)
      // quda_clover_inv.data(), inv_param);
      /////===host
      //
      bool compute_clover = true;
      if (inv_param.clover_csw == 0) {
        compute_clover = false;
      }
      // double norm = 0.00; // clover components are random numbers in the
      ...
      // loadCloverQuda((void*) quda_clover.data(), (void*)
      // quda_clover_inv.data(), &inv_param); loadCloverQuda((void*)
      // quda_clover.data(), (void*)  quda_clover_inv.data(), &inv_param);
      ...
      if (compute_clover) {
        ...
        //
        loadCloverQuda((void*)QUDA_clover.data(), (void*)QUDA_clover_inv.data(),
                       &inv_param);
      } else {
        inv_param.compute_clover_inverse = 0;
        inv_param.return_clover_inverse = 0;
...
template <typename Ty>
inline void quda_clover_inverter::do_inv(Ty* res, Ty* src, const double kappa,
                                         const double err, const Int niter)
{
  ...
  // ctmp0 = quda::ColorSpinorField::Create(cs_cpu);
  // ctmp1 = quda::ColorSpinorField::Create(cs_cpu);
  // qudaMemcpy((void*)ctmp0->data(), (void*)(*gsrc).data(), ctmp0->Volume() *
  // spinor_site_size * sizeof(quda::Complex), qudaMemcpyDeviceToHost);
  //
  ...
  //*ctmp1 = *gresH;
  //
  // qudaMemcpy((void*)(*gres).data(), (void*)ctmp1->data(), ctmp0->Volume() *
  // spinor_site_size * sizeof(quda::Complex), qudaMemcpyHostToDevice); for(Int
  // si=0;si<(*gsrc).CompositeDim();si++)
  //{
  //   invertQuda((void*) (*gres).Component(si).data(), (void*)
  //   (*gsrc).Component(si).data(), &inv_param);
  // }
  ...
  if (quda_verbos >= -1)
    if (quda::comm_rank_global() == 0) {
      qmessage(
          "Done: %8d iter / %.6f secs = %.3f Gflops, Cost %.3f Gflops, %.6f "
          "secs \n",
          inv_param.iter, inv_param.secs, inv_param.gflops / inv_param.secs,
          inv_param.gflops, time0);
    }
  inv_time = time0;
...
inline void quda_clover_inverter::set_quda_split(const std::vector<Int>& grid,
                                                 const Int num_src)
{
  ...
  //   flops0)/(geo.local_volume()));
  //   //timer.flops += (flops1 - flops0) / qlat::get_num_node();
  //   timer.flops += geo.local_volume() * 1368 * dslash_mrh *
  //   num_sub_partition;
  // }
  // }
  UpdateSplitGauge(
  ...
  //  for (int j = 0; j < num_sub_partition; j++) dev_buf[j].copy(QL[0][n *
  //  num_sub_partition + j]); quda::split_field(_collect_b[n],
  //  {dev_buf.begin(), dev_buf.end()}, split_key, QUDA_4D_PC);
  //}
  //
  ...
  //  //for (auto i = 0u; i < x_raw.size(); i++) x_raw[i] =
  //  _collect_x[i].data();
  //  //for (auto i = 0u; i < b_raw.size(); i++) b_raw[i] =
  //  _collect_b[i].data();
  ...
  //    //const double flops1 = quda::Tunable::flops_global();
  //    //qmessage("quda flops %.3f per site \n", (flops1 -
  //    flops0)/(geo.local_volume())); qacc_barrier(dummy);
  //    //timer.flops += (flops1 - flops0) / qlat::get_num_node();
  //    timer.flops += geo.local_volume() * 1368 * dslash_mrh *
  //    num_sub_partition;
  //  }
  ...
  //
  //  Join spinors: _h_x are aliases to host pointers in 'external order: QDP++,
  //  QDP-JIT, etc'
  // for (int n = 0; n < split_param.num_src_per_sub_partition; n++) {
  //   // join fields
  //   quda::join_field({dev_buf.begin(), dev_buf.end()}, _collect_x[n],
  //   split_key, QUDA_4D_PC);
  //   //
  //   // export to desired location and layout
  //   for (int j = 0; j < num_sub_partition; j++) QL[2][n * num_sub_partition +
  //   j].copy(dev_buf[j]);
  // }
  //
  /*
...
inline void quda_clover_inverter::copy_to_buf_split(
...
inline void quda_clover_inverter::copy_from_buf_split(
...
quda_clover_inverter::~quda_clover_inverter() { free_mem(); }
...
void constructCloverField(Float* res, double norm, double diag, Long V)
{
...
void constructQudaCloverField(void* clover, double norm, double diag,
                              QudaPrecision precision, Long V)
{
...
void get_clover_prop(quda_clover_inverter& qinv, qlat::FermionField4dT<Td>& src,
                     qlat::FermionField4dT<Td>& prop, const double kappa,
                     const double err, const Int niter)
{
...
template <typename Td>
void get_clover_prop(quda_clover_inverter& qinv, qlat::Propagator4dT<Td>& src,
                     qlat::Propagator4dT<Td>& prop,
...
template <typename Td>
void get_clover_prop(quda_clover_inverter& qinv, qlat::Propagator4dT<Td>& src,
                     qlat::Propagator4dT<Td>& prop, const double kappa,
                     const double err, const Int niter)
{
  std::vector<qlat::FermionField4dT<Td>> b0;
  std::vector<qlat::FermionField4dT<Td>> b1;
