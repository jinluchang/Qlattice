...
void proton_vectorE_gwu(std::vector<qpropT>& prop1, std::vector<qpropT>& prop2,
                        std::vector<qpropT>& prop3, qlat::vector<Ty>& res,
                        const ga_M& ga2, const Int ind2, const ga_M& ga1,
                        const Int ind1, Int clear = 1)
{
  ...
  */
  ...
  if (clear == 0) {
    Qassert(res.size() == Long(nmass * NTt * Nxyz));
  }
  //
  //  Prop format, src d-4, c-3, sink d-4, c-3, Nt, EigenVTa<Nxyz>
  if (res.size() % NTt != 0 or res.size() == 0) {
...
//
#if USEQACC == 1
                qacc_forNB(i, Long(Nxyz), {
                  tr0[i] -=
                      ((tp1[i] * tp2[i] * tp3[i] + tn1[i] * tn2[i] * tn3[i]) *
                       giE);
                });
#else
                EAy vp1(tp1, Nxyz);
...
void proton_vectorE_gwu(std::vector<qpropT>& prop1, std::vector<qpropT>& prop2,
                        std::vector<qpropT>& prop3, qlat::vector<Ty>& res,
                        const ga_M& ga1, Int t0, Int dT, Int clear = 1,
                        Int oppo = 0)
{
  TIMER("Proton_vectorE");
  if (prop1.size() == 0) {
  ...
  qlat::vector<Ty> resE0;
  resE0.resize(res.size());
  qlat::vector<Ty> resE1;
  resE1.resize(res.size());
...
////container
template <typename Ty>
void proton_corrE(std::vector<qpropT>& prop1, std::vector<qpropT>& prop2,
                  std::vector<qpropT>& prop3, const ga_M& ga2, const Int ind2,
                  const ga_M& ga1, const Int ind1, qlat::vector<Ty>& res,
                  Int clear = 1, const Coordinate& mom = Coordinate())
{
  qlat::vector<Ty> resE;
...
void proton_corrE(std::vector<qpropT>& prop1, std::vector<qpropT>& prop2,
                  std::vector<qpropT>& prop3, qlat::vector<Ty>& res,
                  const ga_M& ga1, const Int t0, const Int dT, Int clear = 1,
...
*/
template <typename Ty>
void baryon_vectorE(std::vector<qpropT>& prop1, std::vector<qpropT>& prop2,
                    std::vector<qpropT>& prop3, qlat::vector<Ty>& res, ga_M& A,
                    ga_M& B, qlat::vector<Ty>& G, qlat::vector<Int>& mL,
                    Int clear = 1)
{
  TIMER("Proton_vectorE");
  if (prop1.size() == 0) {
              ...
              for (Int m1 = 0; m1 < 4; m1++)
                for (Int n2 = 0; n2 < 4; n2++)
                  for (Int n1 = 0; n1 < 4; n1++) {
                    Ty Gv = G[m1 * 4 + n1];
                    double norm = std::sqrt(Gv.real() * Gv.real() +
                                            Gv.imag() * Gv.imag());
                    if (norm < 1e-20) continue;
                    //
                    Int m3 = A.ind[m2];
                    Int n3 = B.ind[n2];
                    Ty giE = epsl[a1][a2] * epsl[b1][b2] * A.g[m2] * B.g[n2] *
                             G[m1 * 4 + n1];
                    nmL[0] = n1;
                    nmL[1] = n2;
                    nmL[2] = n3;
...
template <typename Ty>
void baryon_vectorE(std::vector<qpropT>& prop1, std::vector<qpropT>& prop2,
                    std::vector<qpropT>& prop3, std::vector<qpropT>& resP,
                    ga_M& A, ga_M& B, qlat::vector<Ty>& G,
                    qlat::vector<Int>& mL, Int insertion, Int clear = 1)
{
  ...
  //
  if (clear == 1) {
    if (int(resP.size()) != nmass) {
      resP.resize(nmass);
    }
    for (Int mi = 0; mi < nmass; mi++) {
      if (!resP[mi].initialized) {
        resP[mi].init(geo);
      } else {
        qlat::set_zero(resP[mi]);
      }
    }
  }
  Qassert(int(resP.size()) == nmass);
  for (Int mi = 0; mi < nmass; mi++) {
    Qassert(resP[mi].initialized);
  }
  //
...
#pragma omp parallel for
                    for (Int ji = 0; ji < nmass * NTt; ji++) {
                      ...
                      Int r0 = 0;
                      if (insertion == 0) {
                        r0 = (m1 * 3 + a1) * 12 + (nm1 * 3 + bm1);
                      }
                      if (insertion == 1) {
                        r0 = (m2 * 3 + a2) * 12 + (nm2 * 3 + bm2);
                      }
                      if (insertion == 2) {
                        r0 = (m3 * 3 + a3) * 12 + (nm3 * 3 + bm3);
                      }
                      //
                      Ty* tp1 = &p1[massi][(o1 * NTt + ti) * Nxyz];
                      Ty* tp2 = &p2[massi][(o2 * NTt + ti) * Nxyz];
...
//
                      ...
                      if (insertion == 0)
                        qacc_forNB(i, Long(Nxyz),
                                   { tr0[i] += (tp2[i] * tp3[i] * giE); });
                      if (insertion == 1)
                        qacc_forNB(i, Long(Nxyz),
                                   { tr0[i] += (tp1[i] * tp3[i] * giE); });
                      if (insertion == 2)
                        qacc_forNB(i, Long(Nxyz),
                                   { tr0[i] += (tp1[i] * tp2[i] * giE); });
#else
                      EAy vp1(tp1, Nxyz);
                      EAy vp2(tp2, Nxyz);
                      EAy vp3(tp3, Nxyz);
...
template <typename Ty>
void baryon_vectorE_ud_insert(std::vector<qpropT>& prop1,
                              std::vector<qpropT>& prop2,
                              std::vector<qpropT>& prop3,
                              std::vector<std::vector<qpropT>>& resP,
                              const Int ud, const std::vector<Int>& map_sec,
                              std::vector<std::vector<qpropT>>& buf)
{
  ...
  for (Int i = 1; i < 4; i++) {
    factor[i] = Ty(1.0 / 2.0, 0.0);
  }  // complex sign convention here?
  //
  ...
  //     clear_qv(G);G[ind2*4 + ind1] = +1.0/2.0; //// (1+r4)/2, could be
  //     non-zero for all elements... mL[0] = 0;mL[1] = 1;mL[2] = 2;
  //     baryon_vectorE(prop1, prop2, prop3, resP[0], A, B, G, mL, insertion,
  //     0);
  ...
  for (Int ind = 0; ind < 16; ind++)
    for (Int insertion = 0; insertion < 3; insertion++) {
      if (udL[insertion] == ud) {
        ...
        clear_qv(G);
        G[ind2 * 4 + ind1] =
            +1.0 / 1.0;  //// (1+r4)/2, could be non-zero for all elements...
        mL[0] = 0;
        mL[1] = 1;
        mL[2] = 2;
        baryon_vectorE(prop1, prop2, prop3, buf[ind], A, B, G, mL, insertion,
                       0);
        ////baryon_vectorE(prop1,prop2,prop3, resE, ga2,ga1, G, mL, fd, 1);
        clear_qv(G);
        G[ind2 * 4 + ind1] = -1.0 / 1.0;
        mL[0] = 1;
        mL[1] = 0;
        mL[2] = 2;
        baryon_vectorE(prop1, prop2, prop3, buf[ind], A, B, G, mL, insertion,
    ...
    for (Int ti = 0; ti < Nt; ti++) {
      ...
      for (Int si = 0; si < 2; si++)  //// 1 + r4 sectors
        for (Int gd = 0; gd < 4; gd++) {
          ...
          Int bind =
              gL[prj * 2 + si].ind[gd] * 4 + gd;  //// need reverse or not
          //
          if (seci == 1 and si == 1) {
            sign = (-1.0) * sign;
          }  //// reverse sign if seci == 1
...
__global__ void baryon_vectorEV_global(Ty** p1, Ty** p2, Ty** p3, Ty* resP,
                                       signed char** gPP, unsigned char** oPP,
                                       const Int* ivP, const Int nmass,
                                       const Int NTt, const Long Nxyz,
                                       const Int Ngv)
{
  ...
  for (Int iv = 0; iv < Ngv; iv++) {
    ...
    if (Nth >= 256) {
      if (ai < 128) {
        buf[ai * bfac + bi] += buf[(ai + 128) * bfac + bi];
      }
      __syncthreads();
    }
    if (Nth >= 128) {
      if (ai < 64) {
        buf[ai * bfac + bi] += buf[(ai + 64) * bfac + bi];
      }
      __syncthreads();
    }
    if (Nth >= 64) {
      if (ai < 32) {
        buf[ai * bfac + bi] += buf[(ai + 32) * bfac + bi];
      }
      __syncthreads();
    }
    if (Nth >= 32) {
      if (ai < 16) {
        buf[ai * bfac + bi] += buf[(ai + 16) * bfac + bi];
      }
      __syncthreads();
    }
    if (Nth >= 16) {
      if (ai < 8) {
        buf[ai * bfac + bi] += buf[(ai + 8) * bfac + bi];
      }
      __syncthreads();
    }
    if (Nth >= 8) {
      if (ai < 4) {
        buf[ai * bfac + bi] += buf[(ai + 4) * bfac + bi];
      }
      __syncthreads();
    }
    if (Nth >= 4) {
      if (ai < 2) {
        buf[ai * bfac + bi] += buf[(ai + 2) * bfac + bi];
      }
      __syncthreads();
    }
    //
    if (ai == 0) {
      // if(clear == 0){resP[iv*Ntotal + bi*Nbfac + gi] += (buf[bi] +
      // buf[bfac+bi]);} if(clear == 1){resP[iv*Ntotal + bi*Nbfac + gi]  =
      // (buf[bi] + buf[bfac+bi]);}
...
void baryon_vectorEV_kernel(Ty** p1, Ty** p2, Ty** p3, Ty* resP,
                            signed char** gPP, unsigned char** oPP,
                            const Int* ivP, const Int nmass, const Int NTt,
                            const Long Nxyz, const Int Ngv)
{
  Long Ntotal = nmass * NTt * Nxyz;
  if (Ntotal % bfac != 0) {
    qmessage(
        "Please correct your bfac! nmass %5d, NTt %5d, vol %ld, bfac %d. \n",
...
//
  ...
  baryon_vectorEV_global<Ty, bfac, Blocks><<<dimGrid, dimBlock>>>(
      p1, p2, p3, resP, gPP, oPP, ivP, nmass, NTt, Nxyz, Ngv);
  qacc_barrier(dummy);
#else
  if ((nmass * NTt) % bfac != 0) {
    qmessage(
                     ...
                     fft_desc_basic& fd, Int clear = 1)
{
  ...
  //
  if (clear == 1) {
    zero_Ty(resP, Ngv * nmass * NTt * Nxyz, 1);
  }
  //
  qlat::vector<Ty> epslV;
  epslV.resize(9);
  for (Int i = 0; i < 3; i++) {
    epslV[i * 3 + i] = 0;
    epslV[i * 3 + (i + 1) % 3] = 1;
...
/////contraction Kernel
  ...
  std::vector<std::vector<signed char>> giEL;
  giEL.resize(Ngv);  // giEL.resize(  Ngv*Loff);
  std::vector<std::vector<unsigned char>> oiL;
  oiL.resize(Ngv);  // oiL.resize(3*Ngv*Loff);
  Int bmL[3];
  Int nmL[3];
  Int count_flops = 0;
  for (Int iv = 0; iv < Ngv; iv++) {
              ...
              for (Int m1 = 0; m1 < 4; m1++)
                for (Int n2 = 0; n2 < 4; n2++)
                  for (Int n1 = 0; n1 < 4; n1++) {
                    ...
                    const Int m3 = gIA[m2];
                    const Int n3 = gIB[n2];
                    const Ty giE = epsl[a1 * 3 + a2] * epsl[b1 * 3 + b2] *
                                   gCA[m2] * gCB[n2] * G[m1 * 4 + n1];
                    nmL[0] = n1;
                    nmL[1] = n2;
                    nmL[2] = n3;
                    bmL[0] = b1;
  ...
  ////int mode = 0;mode = clear;
  {
    ...
    const Int BFACG_DEFAULT =
        sizeof(Ty) == 16 ? BFACG_SHARED : BFACG_SHARED * 2;
//
#ifdef QLAT_USE_ACC
    baryon_vectorEV_kernel<Ty, BFACG_DEFAULT>(p1, p2, p3, resP, gPP, oPP, ivP,
                                              nmass, NTt, Nxyz, Ngv);
#else
    bool get = false;
    const Long Ntot = nmass * NTt;
    const Int Bfac = BFACG_DEFAULT * 2;
#define baryon_macros(ba)                                                  \
  if (Ntot % ba == 0 and get == false) {                                   \
    get = true;                                                            \
    baryon_vectorEV_kernel<Ty, ba>(p1, p2, p3, resP, gPP, oPP, ivP, nmass, \
                                   NTt, Nxyz, Ngv);                        \
  }
    baryon_macros(Bfac);
    baryon_macros(16);
              ...
              for (Int m1 = 0; m1 < 4; m1++)
                for (Int n2 = 0; n2 < 4; n2++)
                  for (Int n1 = 0; n1 < 4; n1++) {
                    ...
                    Int m3 = gIA[m2];
                    Int n3 = gIB[n2];
                    Ty giE = epsl[a1 * 3 + a2] * epsl[b1 * 3 + b2] * gCA[m2] *
                             gCB[n2] * G[m1 * 4 + n1];
                    nmL[0] = n1;
                    nmL[1] = n2;
                    nmL[2] = n3;
...
#pragma omp parallel for
                    for (Int ji = 0; ji < nmass * NTt; ji++) {
                      ...
                      Int ti = ji % NTt;
                      //
                      Int o1 = massi * 12 * 12 + (m1 * 3 + a1) * 12 +
                               (nm1 * 3 + bm1);
                      Int o2 = massi * 12 * 12 + (m2 * 3 + a2) * 12 +
                               (nm2 * 3 + bm2);
                      Int o3 = massi * 12 * 12 + (m3 * 3 + a3) * 12 +
                               (nm3 * 3 + bm3);
                      //
                      Ty* tp1 = p1[o1 * NTt + ti];
                      Ty* tp2 = p2[o2 * NTt + ti];
...
void baryon_vectorEV(EigenTy& prop1, EigenTy& prop2, EigenTy& prop3,
                     qlat::vector_gpu<Ty>& res, ga_M& A, ga_M& B,
                     qlat::vector<Ty>& GV, qlat::vector<Int>& mLV,
                     fft_desc_basic& fd, Int clear = 1)
{
  if (prop1.size() == 0) {
    res.resize(0);
  ...
  check_prop_size(prop1, fd);
  check_prop_size(prop2, fd);
  check_prop_size(prop3, fd);
  //
  Int NTt = fd.Nv[3];
  Long Nxyz = fd.Nv[0] * fd.Nv[1] * fd.Nv[2];
  ////check_prop_size(prop1);check_prop_size(prop2);check_prop_size(prop3);
...
void baryon_corrE(EigenTy& prop1, EigenTy& prop2, EigenTy& prop3,
                  qlat::vector_gpu<Ty>& res, ga_M& ga2, Int ind2, ga_M& ga1,
                  Int ind1, fft_desc_basic& fd, Int clear = 1,
                  const Coordinate& mom = Coordinate())
{
  if (prop1.size() == 0) {
    res.resize(0);
...
void Omega_corrE(EigenTy& prop1, EigenTy& prop2, EigenTy& prop3,
                 qlat::vector_gpu<Ty>& res, ga_M& ga2, Int ind2, ga_M& ga1,
                 Int ind1, Int clear = 1, const Coordinate& mom = Coordinate())
{
  if (prop1.size() == 0) {
    res.resize(0);
    return;
