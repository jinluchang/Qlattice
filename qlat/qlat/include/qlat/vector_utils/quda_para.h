...
//   return *this;
// }
// inline qlat::ComplexT<double> operator+(const quda::Complex &a, const
// qlat::ComplexT<double> &b) {
//     return qlat::ComplexT<double>(a.real() + b.real(), a.imag() + b.imag());
// }
// inline quda::Complex operator+(const quda::Complex &a, const
// qlat::ComplexT<double> &b) {
...
void massRescale(ColorSpinorField& b, QudaInvertParam& param,
                 bool for_multishift)
{
  ...
  double kappa = (param.dslash_type == QUDA_DOMAIN_WALL_DSLASH ||
                  ...
                  param.dslash_type == QUDA_MOBIUS_DWF_EOFA_DSLASH)
                     ? kappa5
                     : param.kappa;
  //
  ...
  if (param.dslash_type == QUDA_ASQTAD_DSLASH ||
    ...
    switch (param.solution_type) {
      ...
      case QUDA_MATPC_SOLUTION:
        if (param.mass_normalization == QUDA_KAPPA_NORMALIZATION)
          blas::ax(2.0 * param.mass, b);
        break;
      case QUDA_MATDAG_MAT_SOLUTION:
      case QUDA_MATPCDAG_MATPC_SOLUTION:
        if (param.mass_normalization == QUDA_KAPPA_NORMALIZATION)
          blas::ax(4.0 * param.mass * param.mass, b);
        break;
      default:
  ...
  //
  // multiply the source to compensate for normalization of the Dirac operator,
  // if necessary you are responsible for restoring what's in param.offset
  switch (param.solution_type) {
    case QUDA_MAT_SOLUTION:
      if (param.mass_normalization == QUDA_MASS_NORMALIZATION ||
        ...
        blas::ax(2.0 * kappa, b);
        if (for_multishift)
          for (qlat::Int i = 0; i < param.num_offset; i++)
            param.offset[i] *= 2.0 * kappa;
      }
      break;
    case QUDA_MATDAG_MAT_SOLUTION:
      if (param.mass_normalization == QUDA_MASS_NORMALIZATION ||
        ...
        if (for_multishift)
          for (qlat::Int i = 0; i < param.num_offset; i++)
            param.offset[i] *= 4.0 * kappa * kappa;
      }
      break;
    case QUDA_MATPC_SOLUTION:
      if (param.mass_normalization == QUDA_MASS_NORMALIZATION) {
        ...
        if (for_multishift)
          for (qlat::Int i = 0; i < param.num_offset; i++)
            param.offset[i] *= 4.0 * kappa * kappa;
      } else if (param.mass_normalization ==
    ...
    case QUDA_MATPCDAG_MATPC_SOLUTION:
      if (param.mass_normalization == QUDA_MASS_NORMALIZATION) {
        ...
        if (for_multishift)
          for (qlat::Int i = 0; i < param.num_offset; i++)
            param.offset[i] *= 16.0 * std::pow(kappa, 4);
      } else if (param.mass_normalization ==
                 QUDA_ASYMMETRIC_MASS_NORMALIZATION) {
        blas::ax(4.0 * kappa * kappa, b);
        if (for_multishift)
...
{
  qlat::Coordinate nodeD;
  int dims[4];
  int coords[4];
  for (Int d = 0; d < 4; d++) {
    nodeD[d] = quda::comm_dim(d);
    dims[d] = quda::comm_dim(d);
...
inline void quda_begin_internal(Int mpi_layout[4], Int quda_rankx = 1)
{
  ...
  //     "
  //     "(%03d,%03d,%03d,%03d).\n",
  //     comm_rank(), comm_coord(0), comm_coord(1), comm_coord(2),
  //     comm_coord(3), get_id_node(), get_coor_node()[0], get_coor_node()[1],
  //     get_coor_node()[2], get_coor_node()[3]);
  ...
  qacc_ErrCheck(qacc_DeviceSetCacheConfig(
      qacc_FuncCachePreferNone));  // for smearing settings
}
...
inline void begin_quda_with_gpu()
{
  ...
  qlat::GeometryNode geon = qlat::get_geometry_node();
  for (Int i = 0; i < 4; i++) {
    mpi_layout[i] = geon.size_node[i];
  }
  //
...
inline void begin_Qlat_quda(int* argc, char** argv[], inputpara& in,
                            Int read_Lat = 0)
{
...
void quda_convert_gauge(Ta* qgf, Ty* gf, const Geometry& geo, Int dir = 0,
                        Int GPU = 0)
{
...
void quda_convert_gauge(qlat::vector<qlat::ComplexT<Ta>>& qgf,
                        GaugeFieldT<Td>& gf, Int dir = 0, Int GPU = 0)
{
  TIMER("quda_convert_gauge(qgf,gf)");
  ...
  //     //ColorMatrixT<T>& ms =
  //     gf.get_elem_offset(qlat_idx*gf.geo().multiplicity+mu);
                                    ...
                                    const QudaGaugeParam* gauge_param)
{
  ...
  cs_param->pc_type = inv_param->dslash_type == QUDA_DOMAIN_WALL_DSLASH
                          ? QUDA_5D_PC
                          : QUDA_4D_PC;
...
//         milc_out[(i * 4 + dir) * siteSize + j] =
//         static_cast<Out>(qdp_in[dir][i * siteSize + j]);
//       }
//     }
//   });
// }
...
void applyGaugeFieldScaling_long(
    Ty* gauge, Long Vh, QudaGaugeParam* param,
    QudaDslashType dslash_type = QUDA_STAGGERED_DSLASH)
{
  ...
  const MemType gmem =
      check_mem_type(gauge) != MemType::Cpu ? MemType::Acc : MemType::Cpu;
  //
...
void Ffield4d_to_quda_ff(T1* quda_ff, qlat::FermionField4dT<Td>& ff,
                         Int dir = 1)
{
...
#pragma omp parallel for
  for (Long qlat_idx_4d = 0; qlat_idx_4d < V; qlat_idx_4d++) {
    ...
    for (Int dc = 0; dc < 12; dc++) {
      ...
      if (dir == 1) {
        quda_ff[quda_idx * 12 + dc] = src[qlat_idx_4d * 12 + dc];
      }
      if (dir == 0) {
        src[qlat_idx_4d * 12 + dc] = quda_ff[quda_idx * 12 + dc];
      }
    }
  }
}
...
void qlat_cf_to_quda_cf_test(T1* quda_cf, Ty* src, const Geometry& geo, Int Dim,
                             Int dir = 1)
{
  TIMER("qlat_cf_to_quda_cf_test(qlat_cf, quda_cf)");
  Qassert(sizeof(T1) ==
          sizeof(ComplexT<double>));  // quda can only use double interface
  const Long V = geo.local_volume();
  const Long Vh = V / 2;
  const MemType mem0 = check_mem_type(quda_cf);
  const MemType mem1 = check_mem_type(src);
  const MemType mem_type = check_mem_operations(mem0, mem1);
  Qassert(mem_type == MemType::Cpu);  // ordering of dc inner on cpus
  qmem_for(qlat_idx_4d, V, mem_type, {
    const Coordinate xl = geo.coordinate_from_index(qlat_idx_4d);
    Int eo = (xl[0] + xl[1] + xl[2] + xl[3]) % 2;
    for (Int dc = 0; dc < Dim; dc++) {
      const Long quda_idx = eo * Vh + qlat_idx_4d / 2;
      if (dir == 1) {
        quda_cf[quda_idx * Dim + dc] = src[qlat_idx_4d * Dim + dc];
      }
      if (dir == 0) {
        src[qlat_idx_4d * Dim + dc] = quda_cf[quda_idx * Dim + dc];
      }
    }
  });
}
...
template <class Ty, class T1>
void quda_cf_to_qlat_cf_test(Ty* qlat_cf, T1* quda_cf, const Geometry& geo,
                             Int Dim)
{
...
// template <class Ty>
// void copy_color_prop(qlat::vector_gpu<Ty >& res, std::vector<colorFT >& src,
// Int dir = 1)
//{
//   Qassert(src.size() == 3);
...
//
// template <class Ty>
// void copy_to_color_prop(std::vector<colorFT >& res, qlat::vector_gpu<Ty >&
// src)
//{
...
// void qlat_cf_to_quda_cfT(T1*  quda_cf, Ty* src, const Int Dim, const
// Geometry& geo, qlat::vector<Long >& map_)
//{
...
//     for(Int dc = 0; dc < Dim; dc++)
//     {
//       //if(dir == 1){quda_cf[ quda_idx*Dim + dc] = src[qlat_idx_4d*Dim +
//       dc];}
//       //if(dir == 0){src[qlat_idx_4d*Dim + dc] = quda_cf[quda_idx*Dim + dc];}
//       if(dir == 1){quda_cf[(eo*Dim + dc)*Vh + qi] = src[qlat_idx_4d*Dim +
//       dc];} if(dir == 0){src[qlat_idx_4d*Dim + dc] = quda_cf[(eo*Dim + dc)*Vh
//       + qi];}
//     }
//   });
// }
...
// void qlat_cf_to_quda_cf(T1*  quda_cf, Ty* src, const Int Dim, const Geometry&
// geo, qlat::vector<Long >& map)
//{
//   qlat_cf_to_quda_cfT<T1, Ty, 1>(quda_cf, src, Dim, geo, map);
// }
//
// template <class T1, class Ty>
// void quda_cf_to_qlat_cf(Ty* res, T1*  quda_cf, const Int Dim, const Geometry&
// geo, qlat::vector<Long >& map)
//{
...
void qlat_cf_to_quda_cfT(quda::ColorSpinorField& x, Ty* src,
                         const Geometry& geo, qlat::vector<Long>& map)
{
  ...
  Qassert(Dtype == QUDA_DOUBLE_PRECISION or Dtype == QUDA_SINGLE_PRECISION or
          Dtype == QUDA_HALF_PRECISION);
  //
  ...
  if (x.IsComposite()) {
    for (Int di = 0; di < x.CompositeDim(); di++) {
      ...
      qlat_cf_to_quda_cfT<qlat::ComplexT<double>, Ty, dir>(
          (qlat::ComplexT<double>*)g->data(), &src[di * Vl], Ndata, geo, map);
      if (dir == 1) x.Component(di) = *g;
      //
      // if(Dtype == QUDA_DOUBLE_PRECISION){
      //   qlat_cf_to_quda_cfT<qlat::ComplexT<double>, Ty,
      //   dir>((qlat::ComplexT<double>* ) x.Component(di).data(), &src[di*Vl],
      //   Ndata, geo, map);
      // }else{
      //   if(dir == 0)*g = x.Component(di);
      //   qlat_cf_to_quda_cfT<qlat::ComplexT<double >, Ty,
      //   dir>((qlat::ComplexT<double >* ) g->data(), &src[di*Vl], Ndata, geo,
      //   map); if(dir == 1)x.Component(di) = *g;
      // }
      // if(Dtype == QUDA_SINGLE_PRECISION)
      // qlat_cf_to_quda_cfT<qlat::ComplexT<float >, Ty,
      // dir>((qlat::ComplexT<float >* ) x.Component(di).data(), &src[di*Vl],
      // Ndata, geo, map); if(Dtype == QUDA_HALF_PRECISION)
      //{
      // if(dir == 0)*g = x.Component(di);
      // qlat_cf_to_quda_cfT<qlat::ComplexT<float >, Ty,
      // dir>((qlat::ComplexT<float >* ) g->data(), &src[di*Vl], Ndata, geo,
      // map); if(dir == 1)x.Component(di) = *g;
      // }
    }
  } else {
    if (dir == 0) *g = x;
    qlat_cf_to_quda_cfT<qlat::ComplexT<double>, Ty, dir>(
        (qlat::ComplexT<double>*)g->data(), src, Ndata, geo, map);
    if (dir == 1) x = *g;
    // if(Dtype == QUDA_DOUBLE_PRECISION){
    //   qlat_cf_to_quda_cfT<qlat::ComplexT<double>, Ty,
    //   dir>((qlat::ComplexT<double>* ) x.data(), src, Ndata, geo, map);
    // }else{
    //   if(dir == 0)*g = x;
    //   qlat_cf_to_quda_cfT<qlat::ComplexT<double >, Ty,
    //   dir>((qlat::ComplexT<double >* ) g->data(), src, Ndata, geo, map);
    //   if(dir == 1)x = *g;
    // }
    // if(Dtype == QUDA_SINGLE_PRECISION)
    // qlat_cf_to_quda_cfT<qlat::ComplexT<float >, Ty,
    // dir>((qlat::ComplexT<float >* ) x.data(), src, Ndata, geo, map); if(Dtype
    // == QUDA_HALF_PRECISION)
    //{
    // if(dir == 0)*g = x;
    // qlat_cf_to_quda_cfT<qlat::ComplexT<float >, Ty,
    // dir>((qlat::ComplexT<float >* ) g->data(), src, Ndata, geo, map); if(dir
    // == 1)x = *g;
    // }
  }
  if (g != NULL) {
...
template <class Ty>
void qlat_cf_to_quda_cf(quda::ColorSpinorField& x, Ty* src, const Geometry& geo,
...
template <class Ty>
void quda_cf_to_qlat_cf(Ty* src, quda::ColorSpinorField& x, const Geometry& geo,
                        qlat::vector<Long>& map)
{
