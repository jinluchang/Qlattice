...
__global__ void baryon_sparse_global(Ty** p1, Ty** p2, Ty** p3, Ty** resP,
                                     ...
                                     const Int* ivP, const Int Nsrc,
                                     const Long Nvol, const Int Ngv)
{
...
__global__ void baryon_sparse_global_insertion(Ty** p1, Ty** p2, Ty** p3,
                                               Ty** resP, signed char** gPP,
                                               unsigned char** oPP,
                                               const Int* ivP, const Int Nsrc,
                                               const Long Nvol, const Int Ngv)
{
      ...
      for (Int idv = 0; idv < dv; idv++) {
        ...
        if (insertion >= 0) {
          ...
          for (Int bi = 0; bi < bfac; bi++) {
            b1[bi] += (t2[bi] * t3[bi] * gtem);
          }
          for (Int bi = 0; bi < bfac; bi++) {
            b2[bi] += (t1[bi] * t3[bi] * gtem);
          }
          for (Int bi = 0; bi < bfac; bi++) {
            b3[bi] += (t1[bi] * t2[bi] * gtem);
          }
          //
          // if(ins == 0){
          //   Ty* b1 = &buf[o1*bfac + 0];
...
void baryon_sparse_kernel(Ty** p1, Ty** p2, Ty** p3, Ty** resP,
                          signed char** gPP, unsigned char** oPP,
                          const Int* ivP, const Int Nsrc, const Long Nvol,
                          const Int Ngv)
{
  ...
  // Qassert(insertion == -1 or insertion == 0 or insertion == 1 or insertion ==
  // 2); qmessage("sites %5d %5d \n", int(Nbfac), int(bfac));
  //
  bool job_done = false;
//
#ifdef QLAT_USE_ACC
  ...
  */
  if (insertion < 0) {
    ...
    baryon_sparse_global<Ty, bfac, Blocks><<<dimGrid, dimBlock>>>(
        p1, p2, p3, resP, gPP, oPP, ivP, Nsrc, Nvol, Ngv);
    job_done = true;
  }
  ...
  //  dim3 dimBlock(    nt, bfac, 1);
  //  dim3 dimGrid(  Nbfac,  1, 1);
  //  baryon_sparse_global_insertion<Ty, bfac, insertion><<<dimGrid,
  //  dimBlock>>>(p1, p2, p3, resP, gPP, oPP, ivP, Nsrc, Nvol, Ngv);
  //}
  qacc_barrier(dummy);
#endif
  if (!job_done) {
  ...
  */
  if (clear) {
    for (Long si = 0; si < Nres; si++) {
      if (ins < 0) {
        zero_Ty(resP[si], Nvol, 1, QFALSE);
      }  // clear vectors
      if (ins >= 0) {
        zero_Ty(resP[si], Nvol * 144, 1, QFALSE);
      }  // clear props
    }
    qacc_barrier(dummy);
  }
  ...
  */
  ...
  Int count_flops = 0;
  {
    ...
    for (Int iv = 0; iv < Ngv; iv++) {
      ...
      if (ins >= 0) {
        count_flops += (3 * 2 * 6 + 3 * 2) * iv_size[iv];
      }
    }
    ivP = iv_size.data();
  }
...
#define baryon_macros(ba)                                                    \
  if (Nvol % ba == 0 and get == false) {                                     \
    get = true;                                                              \
    baryon_sparse_kernel<Ty, ba, ins>(p1, p2, p3, resP, gPP, oPP, ivP, Nsrc, \
                                      Nvol, Ngv);                            \
  }
//
#ifdef QLAT_USE_ACC
...
void baryon_sparseI(Ty** p1, Ty** p2, Ty** p3, Ty** resP, const Int Nsrc,
                    ga_M& A, ga_M& B, qlat::vector<Ty>& GV,
                    qlat::vector<Int>& mLV, const Long Nvol,
                    const bool clear = 1, const Int insertion = -1)
{
  if (insertion == -1)
    baryon_sparseP<Ty, -1>(p1, p2, p3, resP, Nsrc, A, B, GV, mLV, Nvol, clear);
  if (insertion == 1)
    baryon_sparseP<Ty, 1>(p1, p2, p3, resP, Nsrc, A, B, GV, mLV, Nvol, clear);
}

template <typename Ty>
void baryon_sparse(std::vector<FieldG<Ty>>& prop1,
                   std::vector<FieldG<Ty>>& prop2,
                   std::vector<FieldG<Ty>>& prop3, std::vector<FieldG<Ty>>& res,
                   ga_M& A, ga_M& B, qlat::vector<Ty>& G, qlat::vector<Int>& mL,
                   const bool clear = true, const Int insertion = -1)
{
  ...
  for (unsigned int si = 0; si < Nsrc; si++) {
    Qassert(prop1[si].initialized and prop2[si].initialized and
            prop3[si].initialized);
    Qassert(prop1[si].multiplicity == Ndc and prop2[si].multiplicity == Ndc and
            prop3[si].multiplicity == Ndc);
    Qassert(prop1[si].mem_order == QLAT_OUTTER and
            prop2[si].mem_order == QLAT_OUTTER and
            prop3[si].mem_order == QLAT_OUTTER);
  }
  ...
  for (Long ri = 0; ri < Nres; ri++) {
    ...
    if (insertion < 0) {
      Qassert(res[ri].multiplicity == 1);
    } else {
      Qassert(res[ri].multiplicity == 144);
    }
  }
  ...
  baryon_sparseI(p1.data(), p2.data(), p3.data(), rP.data(), Nsrc, A, B, G, mL,
                 Nvol, clear, insertion);
}

template <typename Ty>
void baryon_sparse(FieldG<Ty>& prop1, FieldG<Ty>& prop2, FieldG<Ty>& prop3,
                   std::vector<FieldG<Ty>>& res, ga_M& A, ga_M& B,
                   qlat::vector<Ty>& G, qlat::vector<Int>& mL,
                   const bool clear = true, const Int insertion = -1)
{
  ...
  Qassert(prop1.multiplicity == Ndc and prop2.multiplicity == Ndc and
          prop3.multiplicity == Ndc);
  Qassert(prop1.mem_order == QLAT_OUTTER and prop2.mem_order == QLAT_OUTTER and
          prop3.mem_order == QLAT_OUTTER);
...
*/
template <typename Ty>
void baryon_sparse_insertion(std::vector<FieldG<Ty>>& prop1,
                             std::vector<FieldG<Ty>>& prop2,
                             std::vector<FieldG<Ty>>& prop3,
                             std::vector<FieldG<Ty>>& res,
                             const vector<Int>& map_sec,
          ...
          for (Int prj = 0; prj < 4; prj++) {
            ...
            for (Int si = 0; si < 2; si++)  //// 1 + r4 sectors
              for (Int gd = 0; gd < 4; gd++) {
                //  const Int bind = gP[prj*2 + si].ind[gd]*4 + gd ; //   need
                //  reverse or not
                const Int bind = gd * 4 + gP[prj * 2 + si].ind[gd] -
                                 op_min;  //   need reverse or not
                if (bind < 0 or bind >= Ngroup) {
                  continue;
                }
                ...
                //  Ty* sP = (Ty*) qlat::get_data(bufV[(ins*16 + bind)*Nsrc +
                //  srci]).data();
                const Ty* sP = (Ty*)qlat::get_data(
                                   bufV[(ins * Ngroup + bind) * Nsrc + srci])
                                   .data();
                Ty* rP = (Ty*)qlat::get_data(res[srci * 2 * 4 + (ud * 4 + prj)])
                             .data();
                const Int tini = fd.init;
                //  if(seci == 1 and si == 1){sign_use = (-1.0) * sign;} //
