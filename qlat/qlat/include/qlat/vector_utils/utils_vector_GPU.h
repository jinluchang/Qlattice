...
struct vector_gpu {
  ...
  inline void set_zero(QBOOL dummy = QTRUE) { zero_Ty(p, n, GPU, dummy); }
  ...
  inline Ty norm2()
  {
    qlat::vector_gpu<Ty> copy;
    copy.resize(n, GPU);
    Ty* res = copy.data();
    Ty* src = p;
    // if(GPU){
    //   qacc_for(isp, Long(n),    {res[isp] = qlat::qconj(src[isp]) *
    //   src[isp];});
    // }
    // else{
    //   qthread_for(isp, Long(n), {res[isp] = qlat::qconj(src[isp]) *
    //   src[isp];});
    // }
    qGPU_for(isp, Long(n), GPU,
             { res[isp] = qlat::qconj(src[isp]) * src[isp]; });
    qlat::vector<Ty> tmp;
    tmp.resize(2, MemType::Uvm);
    tmp[0] = 0;
    ...
    // MPI_Allreduce(&tmp[0], &tmp[1], sizeof(Ty)/M_size, curr, MPI_SUM,
    // get_comm());
    return tmp[0];
  }
  ...
  const vector_gpu<Ty>& operator*=(const T& f)
  {
    Ty* data = p;
    const Int GPU_ = GPU;
    qGPU_for(isp, Long(n), GPU_, { data[isp] = f * data[isp]; });
    return *this;
  }
};
...
void qblas_xAXPY(vector_gpu<Ty>& y, vector_gpu<Ty>& x, T& alpha, T& beta = 0.0)
{
  ...
  const Int GPU = x.GPU;
  if (beta != 0.0) {
    qGPU_for(isp, N, GPU, { yp[isp] = alpha * xp[isp] + beta * yp[isp]; });
  } else {
    qGPU_for(isp, N, GPU, { yp[isp] = alpha * xp[isp]; });
  }
}
...
Ty qblas_dot(vector_gpu<Ty>& y, vector_gpu<Ty>& x, bool conj = true)
{
  ...
  buf.resize(N, GPU);
  if (conj) {
    qGPU_for(isp, N, GPU, { buf[isp] = qconj(xp[isp]) * yp[isp]; });
  } else {
    qGPU_for(isp, N, GPU, { buf[isp] = xp[isp] * yp[isp]; });
  }
  return buf.norm2();
}
...
inline bool operator<(const VectorGPUKey& x, const VectorGPUKey& y)
{
  ...
  //
  if (x.tag.compare(y.tag) < 0) {
    return true;
  }
  if (y.tag.compare(x.tag) < 0) {
    return false;
  }
  //
  return false;
...
template <typename Ty>
inline vector_gpu<Ty>& get_vector_gpu_plan(size_t vol, const std::string& info,
                                           const Int GPU)
{
...
template <typename Ty>
inline void safe_free_vector_gpu_plan(const std::string& info, const Int GPU,
                                      const bool zero = false)
{
...
inline void clear_vector_gpu_cache()
{
  ...
  // >(); Cache<VectorGPUKey, vector_gpu<float > >& c1 =
  // get_vector_gpu_cache<float >(); Cache<VectorGPUKey, vector_gpu<double > >&
  // c2 = get_vector_gpu_cache<double >(); Cache<VectorGPUKey,
  // vector_gpu<qlat::ComplexT<float > > >& c3 =
  // get_vector_gpu_cache<qlat::ComplexT<float> >(); Cache<VectorGPUKey,
  // vector_gpu<qlat::ComplexT<double> > >& c4 =
  // get_vector_gpu_cache<qlat::ComplexT<double> >(); c0.clear(); c1.clear();
  // c2.clear();
  // c3.clear();
  // c4.clear();
}
