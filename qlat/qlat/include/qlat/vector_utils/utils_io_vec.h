...
*/
template<typename Ty >
inline void load_gwu_cs_evecs(const char* filename, vector_cs<Ty >& eigen, vector<Ty >& evals_, vector<double>& err_,
  const Geometry& geo, const int N0=0, const int N1=-1,
  const bool single_file = false, const bool read = true)
{
  ...
  Qassert(n1 > 0);
  if(n1<n0 or n0<0){abort_r("Read number of eigen should be larger than 1. \n");}
  if(eigen.nvec < size_t(n1-n0)){abort_r("eigen vector size wrong!\n");}
  ...
  if(single_file == true){
    ...
    for(int iv=0;iv<n_vec;iv++){
      ...
      if(read==false){
        for(int ic=0;ic<ri;ic++)
        {
          eigen.copy_to((ComplexF*) &prop_E[ic*noden*12*2], count + ic, false);
        }
        ////Do not rotate source and sink
        gwu_to_cps_rotation_vec(&prop_E[0],ri,noden, false, false, false, read);
        /////single precision eigen vector in milc base
      ...
      if(read==true){
        ...
        for(int ic=0;ic<ri;ic++)
        {
          eigen.copy_from((ComplexF*) &prop_E[ic*noden*12*2], count + ic, false);
        }
      }
      count += ri;if(count >= n_vec){break;}
    }
    ...
    for(int iv=0;iv<n_vec;iv++){
      ...
      if(read==false){
        for(int ic=0;ic<ri;ic++)
        {
          eigen.copy_to((ComplexD*) &prop_E[ic*noden*12*2], count + ic, false);
        }
        gwu_to_cps_rotation_vec(&prop_E[0],ri,noden, false, true, true, read);
        rotate_gwu_vec_file(&prop_E[0],ri,noden, false, read);
      }
      ...
      if(read==true){
        ...
        for(int ic=0;ic<ri;ic++)
        {
          eigen.copy_from((ComplexD*) &prop_E[ic*noden*12*2], count + ic, false);
        }
      }
      count += ri;if(count >= n_vec){break;}
    }
