...
struct io_vec {
  ...
  inline FILE* io_read(const char* filename, const char* mode)
  {
    ...
    if (val != "") {
      do_thread_io = stringtonum(val);
    }
    //
    Int curr_threadio = threadio;
    Int Nv = omp_get_max_threads();
    if (curr_threadio < 1 or curr_threadio > Nv) {
      curr_threadio = Nv;
    }
    if (do_thread_io == 0) {
      curr_threadio = 1;
    ...
    if (do_thread_io > 0) {
      curr_threadio = do_thread_io;
    }
    if (curr_threadio * ionum > 64) {
      curr_threadio = 1;
    }
    ...
    if (node_ioL[rank] >= 0) {
      ...
      if (threadio != 1)
        for (Int i = 0; i < threadio; i++) {
          file_omp.push_back(fopen(filename, mode));
        }
  ...
  inline void ini_MPI(Int size_c0)
  {
    ...
    if (node_ioL[rank] >= 0) {
      ...
      if (tmp.size() != size_c0 * vol) {
        tmp.resize(size_c0 * vol);
      }
    }
    //
  ...
  ////fd = &fds;
  io_vec(const Geometry& geo, Int ionum_or, Int threadio_set = IO_THREAD,
         bool do_checksum_set = false)
  {
    ...
    rank = qlat::get_id_node();
    //
    // for(Int ri=0;ri<Nmpi;ri++)MPI_Bcast(&map_Nitoi[ri][0],
    // (noden/Nx)*sizeof(LInt), MPI_CHAR, ri, get_comm());
    // if(node_ioL[rank]==0){map_Nitoi[rank].resize(0);}
    //
    node_ioL.resize(Nmpi);
    if (ionum_or > 0 and ionum_or < Nmpi) {
      ionum = ionum_or;
    } else {
      ...
      if (val == "") {
        ...
        ionum = stringtonum(val);
      }
      if (ionum < 0 or ionum > Nmpi) {
        ionum = Nmpi;
      }
...
struct IOvecKey {
  ...
  void init(const Coordinate& total_site_, Int ionum_ = 0,
            bool do_checksum_set_ = false)
  {
  ...
  IOvecKey(const Coordinate& total_site_, Int ionum_ = 0,
           bool do_checksum_set_ = false)
  {
...
{
  if (x.total_site < y.total_site) {
    return true;
  }
  if (y.total_site < x.total_site) {
    return false;
  }
  if (x.ionum < y.ionum) {
    return true;
  ...
  if (x.do_checksum_set < y.do_checksum_set) {
    return true;
  }
  if (y.do_checksum_set < x.do_checksum_set) {
    return false;
  }
  return false;
}
...
inline void clear_io_vec_cache() { get_io_vec_cache().clear(); }

/////io_vec buffers related

inline void send_vec_kentucky(char* src, char* res, Int dsize, Int gN,
                              io_vec& io, bool read = true)
{
...
#pragma omp parallel for
  ...
  //
  {
    ...
    if (read == true) {
      MPI_Alltoallv_Send_Recv((char*)io.tmp.data(), &io.currsend[0],
                              &io.currspls[0], (char*)res, &io.currrecv[0],
                              &io.currrpls[0], tem_comm);
    }
    //
    if (read == false) {
      MPI_Alltoallv_Send_Recv((char*)res, &io.currrecv[0], &io.currrpls[0],
                              (char*)io.tmp.data(), &io.currsend[0],
                              &io.currspls[0], tem_comm);
    }
    //
    //}
    // else{
...
inline void read_kentucky_vector(FILE* file, char* props, Int Nvec, io_vec& io,
                                 bool Rendian = false, Int dsize = 8,
                                 bool single_file = false, Int gN = 1,
                                 bool read = true)
{
  TIMER("IO VECS");
  //
  ...
  if (io.res.size() != ionum * noden * gN * dsize) {
    io.res.resize(ionum * noden * gN * dsize);
  }
  char* res = &io.res[0];
  size_t sizec = vol * dsize;
  //
  size_t off_file = 0;  ////size_t sem=0;
  std::vector<size_t> semL;
  semL.resize(io.threadio);
  for (LInt i = 0; i < semL.size(); i++) {
    semL[i] = 0;
  }
  if (node_ioL[rank] >= 0) {
    off_file = ftell(file);
    ...
    //
    {
      ...
      if (node_ioL[rank] >= 0)
        if (curr_v + node_ioL[rank] * gN < Nvec) {
          ...
          //
          Int rN = gN;
          if (offv + rN >= size_t(Nvec)) {
            rN = Nvec - offv;
          }
          //
          /////Switch endian of the file write
          if (read == false) {
            Int dsize_single = sizeof(RealD);
            int fac = dsize / sizeof(RealD);
            if (single_file == true) {
              dsize_single = sizeof(float);
              fac = dsize / sizeof(float);
            }
            //////Write Prop and link
            if (Rendian == false)
              if (!is_big_endian_gwu())
                switchendian((char*)&buf[0], gN * vol * fac, dsize_single);
            //////Write source noise
            if (Rendian == true)
              if (is_big_endian_gwu())
                switchendian((char*)&buf[0], gN * vol * fac, dsize_single);
          ...
          if (rN > 0) {
            ...
            if (curr_threadio == 1) {
              ...
              if (semL[0] != 1) {
                printf("Reading/Writing error %zu 1 \n", semL[0]);
              }
            }
            //
            ////omp reading
...
#pragma omp parallel for
              for (Int tid = 0; tid < curr_threadio; tid++) {
                ...
                size_t currN = Group;
                if ((tid + 1) * Group > Nvec) {
                  currN = Nvec - tid * Group;
                }
                if (currN > 0) {
                  size_t iniN = tid * Group;  // size_t endN = tid*Group +
                                              // currN;
                  // FILE *file_omp = fdopen (dup (fileno (file)), "r");
                  fseek(io.file_omp[tid], off_file_local + iniN, SEEK_SET);
                  semL[tid] = file_operation(&buf[iniN], currN, 1,
                                             io.file_omp[tid], read);
                  // if(read==true ){semL[tid] =  fread(&buf[iniN], currN, 1,
                  // io.file_omp[tid]);} if(read==false){semL[tid] =
                  // fwrite(&buf[iniN], currN, 1, io.file_omp[tid]);}
                }
              }
              for (Int tid = 0; tid < curr_threadio; tid++) {
                if (semL[tid] != 1) {
            ...
            if (io.do_checksum) {
              ...
              crc32_tem = crc32_combine(
                  crc32_tem, 0, io.end_of_file - pos_file_cur - rN * sizec);
    ...
    //
    ////////Reordersrc buf
    /// if(gN!=1){if(node_ioL[rank]>=0)reorder_civ((char*)&buf[0],(char*)&buf[0],1,gN,vol,0
    /// ,sizeof(RealD));}
    /////
    //
    if (read == true) {
    ...
    if (read == true) {
      ...
      for (Int iou = 0; iou < ionum; iou++)
        for (Int gi = 0; gi < gN; gi++)
          if (curr_v + iou * gN + gi < Nvec) {
            Int offv = curr_v + iou * gN + gi;
            // memcpy(&props[(offv)*noden*dsize +
            // 0],&res[(iou*gN+gi)*noden*dsize + 0],noden*dsize);
            //
...
#pragma omp parallel for
  ...
  //
  if (node_ioL[rank] >= 0) {
    fseek(file, off_file + Nvec * sizec, SEEK_SET);
  }
  ...
  if (read == true)
    qmessage(
        "READ_TIMING: total %.3e s , MPI %.3e s, disk %.3f GB/s, eff %.3f GB/s "
        "\n",
        time0, mpi_t, total_R / (time0 - mpi_t), total_R / time0);
  if (read == false)
    qmessage(
        "WRIT_TIMING: total %.3e s , MPI %.3e s, disk %.3f GB/s, eff %.3f GB/s "
        "\n",
        time0, mpi_t, total_R / (time0 - mpi_t), total_R / time0);
  //
  MPI_Barrier(get_comm());
...
inline void save_txt_eigenvalues(std::vector<double>& values,
                                 std::vector<double>& errors,
                                 const char* filename, const char* sDescription)
{
  if (qlat::get_id_node() == 0) {
    ...
    fprintf(
        filew,
        "Each eigenvector is preceded by a line describing the eigenvalue.\n");
    fprintf(filew, "The residue is defined as norm(mat.vec-lambda.vec).\n");
    fprintf(filew,
            "The format is: a tag EIGV, the real and imaginary part of the "
            "eigenvalue and the residue.\n");
    //
    for (Int iv = 0; iv < nvec; iv++)
      fprintf(filew, "EIGV %+.15le\t%+.15le\t%.10le\n", values[iv * 2 + 0],
              values[iv * 2 + 1], errors[iv]);
    //
    fclose(filew);
  }
}
...
inline void load_txt_eigenvalues(std::vector<double>& values,
                                 std::vector<double>& errors,
                                 const char* filename)
{
  Int nvec = 0;
  ...
  if (qlat::get_id_node() == 0) {
    ...
    for (Int i = 0; i < 4; i++) {
      ...
      if (ftem == NULL) {
        qmessage("Read eigenvalues error!\n");
      }
    }
    //
    // while(!feof(filer))
...
void rotate_gwu_vec_file(Ty* src, Int n_vec, size_t noden, bool single_file,
                         bool read = true)
{
...
template <typename Ty>
void gwu_to_cps_rotation_vec(Ty* src, Int n_vec, size_t noden,
                             bool source = false, bool PS0 = true,
                             bool PS1 = true, bool read = true)
{
  TIMERB("Rotate gwu file vec");
  /// prop
  if (source == true)
    if (n_vec % 12 != 0) {
      qmessage("n_vec %8d.\n", n_vec);
      abort_r("source vector size wrong.\n");
    }
  // size_t noden =
  // geo.node_site[0]*geo.node_site[1]*geo.node_site[2]*geo.node_site[3];
  //
  Int signT = 1;
  if (read == false) {
    signT = -1;
...
#pragma omp parallel for
        ...
        d1 = 3;
#pragma omp parallel for
        for (size_t c = 0; c < 3 * Np; c++)
          q[dr * 3 * Np + c] =
              (-p[d0 * 3 * Np + c] * signT + p[d1 * 3 * Np + c]) / sqrt2;
        dr = 1;
        d0 = 0;
        d1 = 2;
#pragma omp parallel for
...
#pragma omp parallel for
        for (size_t c = 0; c < 3 * Np; c++)
          q[dr * 3 * Np + c] =
              (-p[d0 * 3 * Np + c] - p[d1 * 3 * Np + c] * signT) / sqrt2;
        dr = 3;
        d0 = 0;
        d1 = 2;
#pragma omp parallel for
...
#pragma omp parallel for
    for (size_t isp = 0; isp < noden; isp++) {
      ...
      //
      if (PS1 == true) {
        memcpy(&p[0], &src[(dc1 * noden + isp) * 12 * 2 + 0],
               sizeof(Ty) * p.size());
        dr = 0;
        d0 = 1;
        d1 = 3;
        for (Int c = 0; c < 6; c++)
          q[dr * 6 + c] = (-p[d0 * 6 + c] * signT + p[d1 * 6 + c]) / sqrt2;
        dr = 1;
        d0 = 0;
        d1 = 2;
        for (Int c = 0; c < 6; c++)
        ...
        d1 = 3;
        for (Int c = 0; c < 6; c++)
          q[dr * 6 + c] = (-p[d0 * 6 + c] - p[d1 * 6 + c] * signT) / sqrt2;
        dr = 3;
        d0 = 0;
        d1 = 2;
        for (Int c = 0; c < 6; c++)
        ...
        dr = 2;
        for (Int c = 0; c < 6; c++) q[dr * 6 + c] = -1.0 * q[dr * 6 + c];
        dr = 3;
        for (Int c = 0; c < 6; c++) q[dr * 6 + c] = -1.0 * q[dr * 6 + c];
      }
    }
...
inline Int test_single(const char* filename, io_vec& io_use, Int iv = 0)
{
  ...
  //
  {
    ...
    read_kentucky_vector(file, (char*)&prop_E[0], n_vec * 12 * 2, io_use, false,
                         sizeof(RealD), false, 12 * 2);
  ...
  //
  {
    ...
    //
    read_kentucky_vector(file, (char*)&prop_E[0], n_vec, io_use, true,
                         6 * 2 * sizeof(RealD), true);
    io_use.io_close(file);
    //
...
inline Int check_Eigen_file_type(const char* filename, io_vec& io_use, Int n1,
                                 bool check)
{
  size_t noden = io_use.noden;
  ...
  if (sizen < Fsize) {
    qmessage("%s \n", filename);
    abort_r("Eigen file size too small. \n");
  }
  if (sizen < 2 * Fsize) {
    if (n1 != 1) {
      abort_r("Eigen file size too small 1. \n");
    }
    /////read with single
    return 1;
  }
  ...
  if (check == false) {
    if (sizen < n1 * Fsize) {
      abort_r("Eigen file size too small 2. \n");
    }
    return 1;
  }
  //
  ...
  if (tmp == -1) {
    abort_r("Eigen system file norm not correct.\n");
  }
  //
  if (tmp == 1) {
    if (sizen < n1 * Fsize) {
  ...
  if (tmp == 0) {
    if (sizen < 2 * n1 * Fsize) {
      abort_r("Eigen file size too small 3. \n");
    }
    return 0;
  }
  //
                    ...
                    bool read_single = true)
{
  (void)check;
  if (n1 < n0 or n0 < 0) {
    abort_r("Read number of eigen should be larger than 1. \n");
  }
  if (resp.size() < size_t(n1 - n0)) {
    abort_r("Final point size wrong!\n");
  }
  if (sizeof(Ty) != sizeof(RealD) and sizeof(Ty) != sizeof(float)) {
    abort_r("Need double or float pointer! \n");
  }
...
#pragma omp parallel for
      ...
      //
      /////qmessage("WRITE!!!!!!\n");
      read_kentucky_vector(file, (char*)&prop_E[0], ri, io_use, true,
                           6 * 2 * sizeof(RealD), true, 1, read);
      fflush_MPI();
      //
      if (read == true) {
...
#pragma omp parallel for
      ...
      //
      read_kentucky_vector(file, (char*)&prop_E[0], ri * 12 * 2, io_use, false,
                           sizeof(RealD), false, 12 * 2, read);
      fflush_MPI();
      //
      if (read == true) {
...
// template<typename Ty>
// void load_gwu_eigen(FILE* file, std::vector<qlat::FieldM<Ty , 12> >
// res,io_vec &io_use,Int n0,Int n1,
...
//   \n");} Int n_vec = n1-n0; size_t noden = io_use.noden; if(read == true)
//   if(res.size() != n_vec)
//   {
//     res.resize(0);
//     res.resize(n_vec);
...
//   for(Int iv=0;iv<n_vec;iv++){resp[iv] = (Ty*)
//   qlat::get_data(res[iv]).data();} load_gwu_eigen(file, resp,
//   io_use,n0,n1,check, read, read_single);
// }

inline FILE* open_gwu_eigen(const char* filename, io_vec& io_use,
                            bool read = true)
{
  if (read == true) {
    return io_use.io_read(filename, "rb");
  }
  if (read == false) {
    return io_use.io_read(filename, "wb");
  }
  return NULL;
}

template <typename Ty>
void load_gwu_eigen(const char* filename, std::vector<Ty*> resp, io_vec& io_use,
                    Int n0, Int n1, bool check = true, bool read = true)
{
  /////FILE* file = NULL;
  bool read_single = true;
  if (read == true) {
    read_single = check_Eigen_file_type(filename, io_use, n1, check);
  }
...
template <typename Ty>
void load_gwu_eigen(FILE* file, std::vector<Ty>& res, io_vec& io_use, Int n0,
                    Int n1, bool check = true, bool read = true,
                    bool read_single = true)
{
  if (n1 < n0 or n0 < 0) {
...
template <typename Ty>
void load_gwu_eigen(const char* filename, std::vector<Ty>& res, io_vec& io_use,
                    Int n0, Int n1, bool check = true, bool read = true)
{
  /////FILE* file = NULL;
  bool read_single = true;
...
void load_gwu_eigen(const char* filename,
                    std::vector<qlat::FermionField4dT<Td>>& eigen,
                    io_vec& io_use, Int n0, Int n1, bool check = true,
                    bool read = true)
{
  if (n1 < n0 or n0 < 0) {
    abort_r("Read number of eigen should be larger than 1. \n");
  }
  if (sizeof(Td) != sizeof(double) and sizeof(Td) != sizeof(float)) {
...
template <typename Td>
void save_gwu_eigen(const char* filename,
                    std::vector<qlat::FermionField4dT<Td>>& eigen,
                    io_vec& io_use, Int n0, Int n1, bool check = true)
{
...
inline void load_gwu_eigen(const char* filename, EigenM& Mvec, io_vec& io_use,
                           Int n0, Int n1, bool check = true, bool read = true)
{
  if (n1 < n0 or n0 < 0) {
    abort_r("Read number of eigen should be larger than 1. \n");
  ...
  Int n_vec = n1 - n0;
  if (n_vec > int(Mvec.size())) {
    abort_r("Read number of eigen larger than memory. \n");
  }
  //
  Long Nsize = io_use.noden * 12;
  //
  std::vector<Ftype*> resp;
  resp.resize(n_vec);
  for (Int iv = 0; iv < n_vec; iv++) {
    if (Mvec[iv].size() != Nsize) {
      if (read == false) abort_r("Mvec not initialized! \n");
      if (read == true) {
        Mvec[iv].resize(Nsize);
        set_zero(Mvec[iv]);
      }
    }
...
inline void save_gwu_eigen(const char* filename, EigenM& Mvec, io_vec& io_use,
                           Int n0, Int n1, bool check = true)
{
  load_gwu_eigen(filename, Mvec, io_use, n0, n1, check, false);
}

template <typename Td>
void load_gwu_prop(const char* filename,
                   std::vector<qlat::FermionField4dT<Td>>& prop, io_vec& io_use,
                   bool read = true)
{
  if (sizeof(Td) != sizeof(double) and sizeof(Td) != sizeof(float)) {
    abort_r("Cannot understand the input format! \n");
  ...
  if (read == true) {
    size_t sizen = get_file_size_MPI(filename);
    //
    if (sizen != 2 * Fsize * 12 and sizen != Fsize * 12) {
      qmessage("File %s \n", filename);
      abort_r("prop size wrong! \n");
    }
    prop.resize(0);
    prop.resize(12);
  ...
  if (single == true) {
    ...
    if (read == false) {
      for (Int iv = 0; iv < 12; iv++) {
        qlat::ComplexT<Td>* res =
            (qlat::ComplexT<Td>*)qlat::get_data(prop[iv]).data();
        ComplexT<RealF>* src =
            (ComplexT<RealF>*)&prop_qlat[iv * noden * 12 * 2];
#pragma omp parallel for
        for (size_t isp = 0; isp < noden * 12; isp++)
          src[isp] = ComplexT<RealF>(res[isp].real(), res[isp].imag());
      }
      //
    ...
    if (read == true) file = io_use.io_read(filename, "rb");
    if (read == false) file = io_use.io_read(filename, "wb");
    read_kentucky_vector(file, (char*)&prop_qlat[0], 12, io_use, true,
                         6 * 2 * sizeof(RealD), true, 1, read);
    io_use.io_close(file);
    //
    if (read == true) {
...
#pragma omp parallel for
        for (size_t isp = 0; isp < noden * 12; isp++)
          res[isp] = qlat::ComplexT<Td>(src[isp].real(), src[isp].imag());
      }
    }
  }
  //
  if (single == false) {
    ...
    //
    file = io_use.io_read(filename, "rb");
    read_kentucky_vector(file, (char*)&prop_qlat[0], 12 * 2 * 12, io_use, false,
                         sizeof(RealD), false, 1);
    io_use.io_close(file);
    //
    /// double precision eigen vector in ps base
    ...
    for (Int iv = 0; iv < 12; iv++) {
      qlat::ComplexT<Td>* res =
          (qlat::ComplexT<Td>*)qlat::get_data(prop[iv]).data();
      ComplexT<double>* src =
          (ComplexT<double>*)&prop_qlat[iv * noden * 12 * 2];
      for (size_t isp = 0; isp < noden * 12; isp++)
        res[isp] = qlat::ComplexT<Td>(src[isp].real(), src[isp].imag());
    }
  }
...
template <typename Td>
void save_gwu_prop(const char* filename,
                   std::vector<qlat::FermionField4dT<Td>>& prop, io_vec& io_use)
{
...
template <typename Td>
void load_gwu_prop(const char* filename,
                   qlat::FieldM<qlat::ComplexT<Td>, 12 * 12>& res,
                   io_vec& io_use, bool read = true)
{
...
void load_gwu_link(const char* filename, GaugeFieldT<Td>& gf, bool read = true)
{
  ...
  if (read == true) file = io_use.io_read(filename, "rb");
  if (read == false) file = io_use.io_read(filename, "wb");
  read_kentucky_vector(file, (char*)&link_qlat[0], 4 * 9 * 2, io_use, false,
                       sizeof(RealD), false, 9 * 2, read);
  io_use.io_close(file);
  //
  //////double precision eigen vector in ps base
  /// rotate_gwu_vec_file(&prop_qlat[0], 12,noden, false);
      ...
      for (Int dir = 0; dir < 4; dir++) {
        for (Int c0 = 0; c0 < 3; c0++)
          for (Int c1 = 0; c1 < 3; c1++) {
            ...
            Int dir1R = ((dir * 3 + c1) * 3 + c0) * 2 + 0;
            Int dir1I = ((dir * 3 + c1) * 3 + c0) * 2 + 1;
            res.p[dir0] = qlat::ComplexT<Td>(link_qlat[dir1R * noden + index],
                                             link_qlat[dir1I * noden + index]);
          }
      }
    }
}
...
template <typename Ty>
void load_gwu_noies(const char* filename,
                    std::vector<qlat::FieldM<Ty, 1>>& noises, bool read = true)
{
  if (sizeof(Ty) != 2 * sizeof(double) and sizeof(Ty) != 2 * sizeof(float) and
      IsTypeComplex<Ty>() == 0) {
    abort_r("Cannot understand the input format! \n");
  }
  if (noises.size() == 0 and read == false) {
    return;
  }
  if (noises.size() == 0 and read == true) {
    abort_r("Initialize your vectors");
  }
  //
  ...
  for (Long iv = 0; iv < Nnoi; iv++) {
    ...
    if (read == true) {
      if (iv == 0) {
        size_t sizen = get_file_size_MPI(filename);
        if (sizen != Nnoi * 2 * Fsize) {
          abort_r("noise size wrong! \n");
        }
      }
...
#pragma omp parallel for
      ...
      mv_civ.dojob((char*)&prop_noi[0], (char*)&prop_noi[0], 1, 2, noden, 1,
                   sizeof(RealD), false);
    }
    //
    read_kentucky_vector(file, (char*)&prop_noi[0], 2, io_use, true,
                         sizeof(RealD), false, 1, read);
    //
    /////Copy noise vectors
    if (read == true) {
      // reorder_civ((char*) &prop_noi[0],(char*) &prop_noi[0], 1, 2, noden,
      // 0,sizeof(RealD)); io_use.mv_civ.dojob((char*) &prop_noi[0],(char*)
      // &prop_noi[0], 1, 2, noden, 0,sizeof(RealD), false);
      mv_civ.dojob((char*)&prop_noi[0], (char*)&prop_noi[0], 1, 2, noden, 0,
                   sizeof(RealD), false);
      ComplexT<double>* src = (ComplexT<double>*)&prop_noi[0];
      Ty* res = (Ty*)qlat::get_data(noi).data();
#pragma omp parallel for
...
void load_gwu_noiP(const char* filename, Fieldy& noi, bool read = true,
                   bool GPU = false)
{
  TIMER("load_gwu_noi");
  Qassert(GetBasicDataType<Fieldy>::get_type_name() !=
  ...
  if (read == false) {
    ...
    // #pragma omp parallel for
    // for(size_t isp=0;isp<noden;isp++)src[isp] =
    // ComplexT<double>(res[isp].real(),res[isp].imag()); reorder_civ((char*)
    // &prop_noi[0],(char*) &prop_noi[0], 1, 2, noden, 1,sizeof(RealD));
    // io_use.mv_civ.dojob((char*) &prop_noi[0],(char*) &prop_noi[0], 1, 2,
    // noden, 1,sizeof(RealD), false);
    mv_civ.dojob((char*)&prop_noi[0], (char*)&prop_noi[0], 1, 2, noden, 1,
                 sizeof(RealD), false);
  }
  //
  ...
  if (read == true) {
    ...
    // #pragma omp parallel for
    // for(size_t isp=0;isp<noden;isp++)res[isp]=
    // Ty(src[isp].real(),src[isp].imag());
  }
}
...
void save_gwu_noiP(const char* filename, Propagator4dT<Td>& prop)
{
  ...
  qthread_for(index, noden, {
    ...
    //
    if (sum > 1e-8) {
      noi.get_elem_offset(index) = 1.0 * phase;
    }
  });
  //
...
template <typename Td>
void noi_to_propP(qlat::FieldM<qlat::ComplexD, 1>& noi, Propagator4dT<Td>& prop,
                  Int dir = 0)
{
  for (Long index = 0; index < prop.geo().local_volume(); ++index) {
    ...
    if (dir == 0)
      for (Int d0 = 0; d0 < 12; d0++) {
        res(d0, d0) = noi.get_elem_offset(index);
      }
    if (dir == 1)
      for (Int d0 = 0; d0 < 12; d0++) {
        noi.get_elem_offset(index) = res(d0, d0);
      }
  }
}
...
void copy_noise_to_vec(T* noiP, Ty* buf, const Geometry& geo, const Long bfac,
                       const Int dir = 1, const QMEM Gmem = QMCPU)
{
  TIMERB("copy_noise_to_vec");
  const Long Nd = geo.local_volume() * bfac;
  if (dir == 1) {
    cpy_GPU(buf, noiP, Nd, QMCPU, Gmem);
  }
  if (dir == 0) {
    cpy_GPU(noiP, buf, Nd, Gmem, QMCPU);
  }
  //
  // #pragma omp parallel for
...
inline void open_file_qlat_noisesT(
    const char* filename, Int bfac, inputpara& in, bool read = true,
    bool single_file = true, Int N_noi = -1,
    const std::string& VECS_TYPE = std::string("NONE"),
    const std::string& INFO_LIST = std::string("NONE"), bool rotate_bfac = true)
{
  ...
  if (in.read == true) {
    ...
    if (in.VECS_TYPE != VECS_TYPE) {
      qmessage("Noise type wrong, file %s, input %s, expect %s \n", filename,
               VECS_TYPE.c_str(), in.VECS_TYPE.c_str());
      abort_r("");
    }
    if (in.nvec <= 0) {
      qmessage("%s \n", filename);
      abort_r("File noise vector size Wrong! \n");
    }
    if (in.bfac != 1) {
      Qassert(in.bfac == bfac);
    }
    ...
    //////Check file sizes
    size_t sizen =
        get_file_size_MPI(filename) -
        in.off_file;  // Qassert(sizen == string_to_size(in.total_size));
    ...
    size_t Fsize =
        (in.N_noi + 0) * Vsize *
        in.bsize;  // Qassert(Fsize <= string_to_size(in.total_size));
    if (Fsize > string_to_size(in.total_size)) {
      abort_r("FILE size too small for vectors read !\n");
    }
    if (Fsize != string_to_size(in.total_size)) {
      in.do_checksum = false;
    }
    //
    //////Check file sizes
  ...
  if (in.read == false) {
    ...
    if (in.N_noi <= 0) {
      qmessage("write noises size zero \n");
      return;
    }
    ...
    in.bfac = in.bfac_write;
    in.checksum = 0;
    if (in.single_file == false) {
      in.bsize = sizeof(RealD);
      in.save_type = std::string("Double");
    }
    if (in.single_file == true) {
      in.bsize = sizeof(float);
      in.save_type = std::string("Single");
    }
    //
    size_t Fsize =
        size_t(in.N_noi) * in.nx * in.ny * in.nz * in.nt * size_t(bfac * 2);
    Fsize = Fsize * in.bsize;
    in.total_size = print_size(Fsize);
    ////qmessage("size of file %zu \n", Fsize);
    vecs_head_write(in, filename, true);
    in.end_of_file = Fsize + in.off_file;
  }
  //
  if (in.bfac_write == bfac) {
    in.rotate_bfac = false;
  }
}
...
template <class Ty>
void load_qlat_noisesT_core(FILE* file, std::vector<Ty*>& noises,
                            const Int bfac, const QMEM Gmem,
                            const Geometry& geo, io_vec& io_use, inputpara& in,
                            Int n0 = 0, Int n1 = -1)
{
  bool read = in.read;
  ...
  if (n1 <= n0 or n0 < 0) {
    qmessage("Need read more vectors, n0 %d, n1 %d !\n", n0, n1);
    abort_r();
  }
  if (n1 > in.N_noi) {
    qmessage("Need set input more vectors, n1 %d, N_noi %d !\n", n1, in.N_noi);
    abort_r();
  }
  in.ncur = n1;
  Int nread = n1 - n0;
  ...
  if (in.FILE_ENDIAN == std::string("BIGENDIAN")) {
    Rendian = false;
  }
  if (in.FILE_ENDIAN == std::string("LITTLEENDIAN")) {
    Rendian = true;
  }
  //
  ...
  qlat::vector<char> buf_vec;
  buf_vec.resize(bufN * bfac * io_use.noden * 2 * bsize, MemType::Cpu);
  ...
  for (Int ni = 0; ni < nread; ni++) {
    if (read == false) {
      if (!single_file)
        copy_noise_to_vec(noises[ni], &bufD[bi * bfac * io_use.noden], geo,
                          bfac, 1, Gmem);
      if (single_file)
        copy_noise_to_vec(noises[ni], &bufF[bi * bfac * io_use.noden], geo,
                          bfac, 1, Gmem);
    }
    bi = bi + 1;
    //
    if (bi == bufN or ni == (nread - 1)) {
      if (read == false)
        if (rotate_bfac)
          mv_civ.dojob((char*)buf, (char*)buf, bi, bfac, io_use.noden, 1,
                       2 * bsize, false);
      //
      read_kentucky_vector(file, (char*)buf, bi * bfac / bfac_write, io_use,
                           Rendian, bfac_write * bsize * 2, single_file, io_gn,
                           read);
      //
      if (read == true)
        if (rotate_bfac)
          mv_civ.dojob((char*)buf, (char*)buf, bi, bfac, io_use.noden, 0,
                       2 * bsize, false);
      if (read == true)
        for (Int nbi = 0; nbi < bi; nbi++) {
          int na = ni - bi + 1;
          {
            if (!single_file)
              copy_noise_to_vec(noises[na + nbi],
                                &bufD[nbi * bfac * io_use.noden], geo, bfac, 0,
                                Gmem);
            if (single_file)
              copy_noise_to_vec(noises[na + nbi],
                                &bufF[nbi * bfac * io_use.noden], geo, bfac, 0,
                                Gmem);
          }
        }
      //
...
template <class Ty>
void initialize_Field(std::vector<qlat::FieldG<Ty>>& noises, const Int bfac,
                      const Int nread, inputpara& in)
{
...
inline Int load_qlat_noisesT_ini(std::vector<Ty*>& bufP,
                                 std::vector<qlat::FieldM<Ty, bfac>>& noises,
                                 const Int bc, inputpara& in, Int n0,
                                 Int n1 = -1)
{
  Qassert(bc == bfac);
  if (in.read == true) {
...
template <class Ty>
inline Int load_qlat_noisesT_ini(std::vector<Ty*>& bufP,
                                 std::vector<qlat::FieldG<Ty>>& noises,
                                 const Int bfac, inputpara& in, Int n0,
                                 Int n1 = -1)
{
  if (in.read == true) {
    if (n1 == -1) {
  ...
  // dc inside vol
  if (bfac != 1) {
    Qassert(noises[0].mem_order == QLAT_DEFAULT);
  }
  bufP.resize(noises.size());
  for (unsigned int iv = 0; iv < noises.size(); iv++) {
    bufP[iv] = (Ty*)qlat::get_data(noises[iv]).data();
...
*/
template <class Ty, Int bfac>
void load_qlat_noisesT(FILE* file, std::vector<qlat::FieldM<Ty, bfac>>& noises,
                       io_vec& io_use, inputpara& in, Int n0 = 0, Int n1 = -1)
{
  std::vector<Ty*> bufP;
  n1 = load_qlat_noisesT_ini(bufP, noises, bfac, in, n0, n1);
  load_qlat_noisesT_core(file, bufP, bfac, QMCPU, noises[0].geo(), io_use, in,
                         n0, n1);
}

////initialize the instruct and end of file
template <class Ty>
Geometry load_qlat_noisesT_file_ini(
    const char* filename, const Int N_noi, const Int bfac, inputpara& in,
    const Geometry& geo, bool read = true, bool single_file = true,
    const std::string& VECS_TYPE = std::string("NONE"),
    const std::string& INFO_LIST = std::string("NONE"),
    const bool rotate_bfac = true)
{
  if (sizeof(Ty) != 2 * sizeof(double) and sizeof(Ty) != 2 * sizeof(float) and
      IsTypeComplex<Ty>() == 0) {
    abort_r("Cannot understand the input format! \n");
  ...
  //
  open_file_qlat_noisesT(filename, bfac, in, read, single_file, N_noi,
                         VECS_TYPE, INFO_LIST, rotate_bfac);
  //
...
template <class Ty>
void load_qlat_noisesT(const char* filename, std::vector<Ty*>& noises,
                       const Int bfac, inputpara& in, const Geometry& geo,
                       bool read = true, bool single_file = true,
                       const std::string& VECS_TYPE = std::string("NONE"),
                       const std::string& INFO_LIST = std::string("NONE"),
                       Int n0 = 0, Int n1 = -1, bool rotate_bfac = true)
{
  ...
  Geometry geo_copy = load_qlat_noisesT_file_ini<Ty>(
      filename, N_noi, bfac, in, geo, read, single_file, VECS_TYPE, INFO_LIST,
      rotate_bfac);
  //
  if (in.read == true) {
    if (n1 == -1) {
...
template <class Ty, Int bfac>
void load_qlat_noisesT(const char* filename,
                       std::vector<qlat::FieldM<Ty, bfac>>& noises,
                       bool read = true, bool single_file = true,
                       const std::string& VECS_TYPE = std::string("NONE"),
                       const std::string& INFO_LIST = std::string("NONE"),
                       Int n0 = 0, Int n1 = -1, bool rotate_bfac = true)
{
  ...
  //
  geo = load_qlat_noisesT_file_ini<Ty>(filename, N_noi, bfac, in, geo, read,
                                       single_file, VECS_TYPE, INFO_LIST,
                                       rotate_bfac);
  //
  std::vector<Ty*> bufP;
  n1 = load_qlat_noisesT_ini(bufP, noises, bfac, in, n0, n1);
...
*/
template <class Ty>
void load_qlat_noisesG(const char* filename,
                       std::vector<qlat::FieldG<Ty>>& noises, const Int bfac,
                       bool read = true, bool single_file = true,
                       const std::string& VECS_TYPE = std::string("NONE"),
                       const std::string& INFO_LIST = std::string("NONE"),
                       Int n0 = 0, Int n1 = -1, bool rotate_bfac = true)
...
void load_qlat_eigen(const char* filename,
                     std::vector<qlat::FieldM<T, civ>>& noises, bool read,
                     bool single_file = true,
                     const std::string& INFO_LIST = std::string("NONE"),
                     Int n0 = 0, Int n1 = -1, std::string info = "NONE")
{
...
template <class T, Int civ>
void load_qlat_eigen(const char* filename,
                     std::vector<qlat::FieldM<T, civ>>& noises, Int n0 = 0,
                     Int n1 = -1, std::string info = "NONE")
{
  bool read = true;
  bool single_file = true;
...
template <class T, Int civ>
void save_qlat_eigen(const char* filename,
                     std::vector<qlat::FieldM<T, civ>>& noises,
                     bool single_file = true,
                     const std::string& INFO_LIST = std::string("NONE"),
...
inline Int check_qlat_eigen_file_type(const char* filename, io_vec& io_use,
                                      Int n1, inputpara& in)
{
...
void load_qlat_noises(const char* filename,
...
template <class T, Int bfac>
void save_qlat_noises(const char* filename,
                      std::vector<qlat::FieldM<T, bfac>>& noises,
                      bool single_file = true,
                      const std::string& INFO_LIST = std::string("NONE"))
{
...
template <class T, Int bfac>
void save_qlat_noises(const std::string& filename,
                      std::vector<qlat::FieldM<T, bfac>>& noises,
                      bool single_file = true,
                      const std::string& INFO_LIST = std::string("NONE"))
{
...
template <class T, Int bfac>
void load_qlat_noise(const char* filename, qlat::FieldM<T, bfac>& noise,
                     bool read = true, bool single_file = true,
                     const std::string& INFO_LIST = std::string("NONE"))
{
  std::vector<qlat::FieldM<T, bfac>> noises;
...
template <class T, Int bfac>
void save_qlat_noise(const char* filename, qlat::FieldM<T, bfac>& noise,
                     bool single_file = true,
                     const std::string& INFO_LIST = std::string("NONE"))
{
...
template <class T>
void load_qlat_noises(const char* filename,
                      std::vector<qlat::FieldG<T>>& noises, bool read = true,
                      bool single_file = true,
                      const std::string& INFO_LIST = std::string("NONE"),
                      Int n0 = 0, Int n1 = -1, const Int bfac = 1)
{
  TIMERB("load/save qlat noises");
  std::string VECS_TYPE = std::string("Noise_Vectors");
  load_qlat_noisesG(filename, noises, bfac, read, single_file, VECS_TYPE,
                    INFO_LIST, n0, n1);
}

template <class T>
void save_qlat_noises(const char* filename,
                      std::vector<qlat::FieldG<T>>& noises,
                      bool single_file = true,
                      const std::string& INFO_LIST = std::string("NONE"),
                      const Int bfac = 1)
{
  load_qlat_noises(filename, noises, false, single_file, INFO_LIST, 0, -1,
                   bfac);
}
...
void save_qlat_noises(const std::string& filename,
                      ...
                      const Int bfac = 1)
{
  load_qlat_noises(filename.c_str(), noises, false, single_file, INFO_LIST, 0,
                   -1, bfac);
}

//////Assume memory allocated already
template <class T, typename Td>
void copy_noise_to_prop(qlat::FieldM<T, 12 * 12>& noise,
                        Propagator4dT<Td>& prop, Int dir = 1)
{
  ...
  for (Long index = 0; index < prop.geo().local_volume(); ++index) {
    ...
    for (Int d0 = 0; d0 < 12; d0++) {
      for (Int d1 = 0; d1 < 12; d1++) {
        //////copy to prop
        if (dir == 0) {
          src[d1 * 12 + d0] = res[d0 * 12 + d1];
        }
        //////copy to buf
        if (dir == 1) {
          res[d0 * 12 + d1] = src[d1 * 12 + d0];
        }
      }
    }
  }
...
//////Assume memory allocated already
template <class T, typename Td>
void copy_noises_to_prop(std::vector<qlat::FieldM<T, 12 * 12>>& noises,
                         Propagator4dT<Td>& prop, Int dir = 1)
{
  ...
  if (dir == 0) {
    if (noises.size() != 1 or !noises[0].initialized or
        noises[0].geo() != prop.geo()) {
      noises.resize(0);
      noises.resize(1);
      noises[0].init(prop.geo());
    }
  }
...
template <typename Td>
void load_qlat_prop(const char* filename, Propagator4dT<Td>& prop,
                    bool read = true, bool single_file = true)
{
  ...
  if (read == false) {
    copy_noises_to_prop(noises, prop, 0);
  }
  load_qlat_noisesT(filename, noises, read, single_file, VECS_TYPE, INFO_LIST);
  if (read == true) {
    copy_noises_to_prop(noises, prop, 1);
  }
}

template <typename Td>
void load_qlat_prop(const std::string& filename, Propagator4dT<Td>& prop,
                    bool read = true, bool single_file = true)
{
...
template <typename Td>
void save_qlat_prop(const char* filename, Propagator4dT<Td>& prop,
                    bool single_file = true)
{
...
template <typename Td>
void save_qlat_prop(const std::string& filename, Propagator4dT<Td>& prop,
                    bool single_file = true)
{
...
// need some cleaning .........
template <typename Td>
void load_qlat_prop_buf(
    const char* filename, Propagator4dT<Td>& prop,
    std::vector<qlat::FieldM<qlat::ComplexT<Td>, 12 * 12>>& noises,
    bool read = true, bool single_file = true)
{
  std::string VECS_TYPE = std::string("Propagator");
...
template <typename Td>
void load_qlat_link(const char* filename, GaugeFieldT<Td>& gf, bool read = true,
                    bool single_file = false)
{
  ...
  //
  std::vector<qlat::ComplexT<Td>*> src;
  src.resize(1);
  src[0] = (qlat::ComplexT<Td>*)qlat::get_data(gf).data();
  //
  load_qlat_noisesT<qlat::ComplexT<Td>>(filename, src, 4 * 9, in, geo, read,
                                        single_file, VECS_TYPE, INFO_LIST);
}

template <typename Td>
void load_qlat_link(const std::string& filename, GaugeFieldT<Td>& gf,
                    bool read = true, bool single_file = false)
{
  load_qlat_link(filename.c_str(), gf, read, single_file);
}

template <typename Td>
void save_qlat_link(const char* filename, GaugeFieldT<Td>& gf,
                    bool single_file = false)
{
...
template <typename Td>
void save_qlat_link(const std::string& filename, GaugeFieldT<Td>& gf,
                    bool single_file = false)
{
...
inline FILE* open_eigensystem_file(const char* filename, Int nini, Int nvec,
                                   bool read, io_vec& io_use, inputpara& in,
                                   Int save_type = 3)
{
  in.single_file = true;
  in.read = read;
  ...
  //
  if (read == true) {
    in.file_type = check_qlat_eigen_file_type(filename, io_use, nvec, in);
  }
  if (read == false) {
    if (save_type != 0 and save_type != 1 and save_type != 2 and
        save_type != 3) {
      abort_r("Save type unknown! \n ");
    }
    in.file_type = save_type;
  }
  //
  if (in.file_type == 0 or in.file_type == 2) {
    in.single_file = false;
  }
  if (in.file_type == 1 or in.file_type == 3) {
    in.single_file = true;
  }
  //
  ...
  if (in.file_type == 2 or in.file_type == 3) {
    if (read == false) {
      in.read_geo(io_use.geo());
    }
    Int bfac_eigen = 12;
    std::string VECS_TYPE = std::string("Eigen_system_nvec.12.tzyx.R/I");
    std::string INFO_LIST = std::string("NONE");
    bool rotate_bfac = true;
    open_file_qlat_noisesT(filename, bfac_eigen, in, read, in.single_file, nvec,
                           VECS_TYPE, INFO_LIST, rotate_bfac);
    //
    if (io_use.end_of_file != 0) {
      abort_r("Please close file first! \n");
    }
    //
    io_use.end_of_file = in.end_of_file;
  ...
  // io_use.nz, io_use.nt, in.nx, in.ny, in.nz, in.nt);
  if (in.nx != 0) {
    Qassert(io_use.nx == in.nx);
  }
  if (in.ny != 0) {
    Qassert(io_use.ny == in.ny);
  }
  if (in.nz != 0) {
    Qassert(io_use.nz == in.nz);
  }
  if (in.nt != 0) {
    Qassert(io_use.nt == in.nt);
  }
  //
  FILE* file = NULL;
  if (read == true) file = io_use.io_read(in.filename.c_str(), "rb");
  ...
  //////shift the file to nini position
  {
    ...
    size_t Vsize = size_t(io_use.nx) * io_use.ny * io_use.nz * io_use.nt *
                   size_t(bfac * 2);
...
template <class Ty>
void load_eigensystem_vecs(FILE* file,
                           std::vector<qlat::FieldM<Ty, 12>>& noises,
                           io_vec& io_use, inputpara& in, Int n0 = 0,
                           Int n1 = -1)
{
  if (in.file_type == 0 or in.file_type == 1) {
    if (n1 < n0 or n0 < 0) {
    ...
    if (in.read == true) {
      if (noises.size() < (LInt)n_vec) {
        ...
        noises.resize(n_vec);
        for (Int iv = 0; iv < n_vec; iv++) {
          noises[iv].init(io_use.geo());
        }
      }
    }
    //
    if (sizeof(Ty) == 2 * sizeof(RealD)) {
      std::vector<double*> respD;
      respD.resize(n_vec);
      for (Int iv = 0; iv < n_vec; iv++) {
        respD[iv] = (double*)qlat::get_data(noises[iv]).data();
      }
      load_gwu_eigen(file, respD, io_use, n0, n1, check, in.read,
                     in.single_file);
    }
    if (sizeof(Ty) == 2 * sizeof(float)) {
      std::vector<RealF*> respF;
      respF.resize(n_vec);
      for (Int iv = 0; iv < n_vec; iv++) {
        respF[iv] = (RealF*)qlat::get_data(noises[iv]).data();
      }
      load_gwu_eigen(file, respF, io_use, n0, n1, check, in.read,
                     in.single_file);
    }
  }
  //
...
template <class Td>
void load_qlat_vecs(const char* filename, Td* prop, const Int nvec,
                    io_vec& io_use, const bool single = false,
                    bool Rendian = false)
{
  ////if(sizeof(Td) != sizeof(double ) and sizeof(Td) != sizeof(float
  ///)){abort_r("Cannot understand the input format! \n");}
  ...
  size_t sizen = get_file_size_MPI(filename);
  //
  if (sizen != 2 * Fsize and sizen != Fsize) {
    qmessage("File %s \n", filename);
    abort_r("File size wrong! \n");
  }
  const Int ncomplex = sizen / Fsize;
  Int s_inner = sizeof(float);
  if (single == false) {
    s_inner = sizeof(RealD);
  }
  s_inner = s_inner * ncomplex;
  //
  FILE* file = io_use.io_read(filename, "rb");
  read_kentucky_vector(file, (char*)prop, nvec, io_use, Rendian, s_inner,
                       single, 1);
  io_use.io_close(file);
}

// template<typename Ty, Int dir>
// void copy_eo_cs_to_fieldM(qlat::vector_gpu<Ty >& res, const Int civ, const
// Geometry& geo, vector_cs<Ty >& even, vector_cs<Ty >& odd,
...
template <typename Ty>
inline void load_eo_evecs(const char* filename, vector_cs<Ty>& even,
                          qlat::vector<Ty>& evals, std::vector<double>& err,
                          const Geometry& geo, const Int N0 = 0,
                          const Int N1 = -1, double mass = 0.0, Int mode_c = 0,
                          const bool single_file = true, const bool read = true,
                          std::string VECS_TYPE = std::string("EO_Eigensystem"),
                          const Int n_off_file = 0)
{
  ...
  io_use.ini_crc(true);
  const Int DIM = 3;
  const bool rotate_bfac =
      true;  ////default rotate color to outside to save memory
  //
  // char fileE[600];
  ...
  if (read == true) {
    ...
    if (get_file_size_MPI(fileE.c_str(), true) > 0) {
      ...
      if (2 * nhalf >= Long(values.size())) {
        nvec = values.size();
      }  ////DIM 3 may be needed here for check
      else {
        nvec = 2 * nhalf;
      }
      Qassert(
          nvec <=
          Long(values.size()));  /// nhalf*2 could be larger than values.size()
      if (nvec > Nmax) {
        nvec = Nmax;
        nhalf = (nvec + 1) / 2;
      }
  ...
  if (read == false) {
    ...
    if (evals.size() > 0) {
      ...
      for (Int n = 0; n < nvec; n++) {
        values[n * 2 + 0] =
            evals[n].real() - 4.0 * mass * mass;  /// subtract to zero mass
        values[n * 2 + 1] = 0.0;
      }
  ...
  std::vector<Ty*> noises;
  noises.resize(Ngroup);
  std::vector<qlat::vector_gpu<Ty>> eig;
  eig.resize(Ngroup);
  for (Int iv = 0; iv < Ngroup; iv++) {
    eig[iv].resize(V * DIM, QMSYNC);  /// default on SYNC
  }
  for (Int iv = 0; iv < Ngroup; iv++) {
    noises[iv] = (Ty*)qlat::get_data(eig[iv]).data();
  }
  vector_cs<Ty> tmp_end;
  tmp_end.resize(1, QMSYNC, even);
  qmessage("after io mem allocate");
  print_mem_info();
  //
  ////load evecs
  {
    ...
    for (LInt jobi = 0; jobi < jobA.size() / 2; jobi++) {
      if (read == true) {
        if (jobi == 0) {
          nini_off = n_off_file / 2;
        }
      }
      if (jobi != 0) {
        nini_off = 0;
      }  /// default zero offset, by default it will always read next rounds
      ...
      // qmessage("load %5d, dN %5d, N %5d, inioff %5d ", int(n0), int(ncut),
      // int(nhalf), int(nini_off));
      //
      if (read == false)
        for (Int iv = 0; iv < ncut; iv++) {
          ...
          if (ne + 2 <= even.nvec) {
            ...
            copy_eo_cs_to_fieldM(eig[iv], 3, geo_copy, even, tmp_end, ne,
                                 ne + 1, 0, 1, map, mode_c);
          }
        }
      //
      /////will shift file from current position
      load_qlat_noisesT_core(file, noises, DIM, QMCPU, geo_copy, io_use, in,
                             nini_off, ncut + nini_off);
      ////print_mem_info();
      //
      if (read == true)
        for (Int iv = 0; iv < ncut; iv++) {
          ...
          const Int ne = (n0 + iv) * 2 + nini;
          if (ne + 2 <= even.nvec) {
            copy_fieldM_to_eo_cs(even, even, eig[iv], 3, geo_copy, ne, ne + 1,
                                 ne + 1, ne + 2, map, mode_c);
          } else {
            copy_fieldM_to_eo_cs(even, tmp_end, eig[iv], 3, geo_copy, ne,
                                 ne + 1, 0, 1, map, mode_c);
          }
        }
    }
...
template <typename Ty>
inline void load_eo_evecs(const std::string& filename, vector_cs<Ty>& even,
                          qlat::vector<Ty>& evals, std::vector<double>& err,
                          const Geometry& geo, const Int N0 = 0,
                          const Int N1 = -1, double mass = 0.0, Int mode_c = 0,
                          const bool single_file = true, const bool read = true,
                          ...
                          const Int n_off_file = 0)
{
  load_eo_evecs(filename.c_str(), even, evals, err, geo, N0, N1, mass, mode_c,
                single_file, read, VECS_TYPE, n_off_file);
}

template <typename Ty>
inline void save_eo_evecs(const char* filename, vector_cs<Ty>& even,
                          qlat::vector<Ty>& evals, std::vector<double>& err,
                          const Geometry& geo, const Int N0 = 0,
                          const Int N1 = -1, double mass = 0.0, Int mode_c = 0,
                          ...
                          std::string VECS_TYPE = std::string("EO_Eigensystem"))
{
  load_eo_evecs(filename, even, evals, err, geo, N0, N1, mass, mode_c,
                single_file, false, VECS_TYPE);
}

template <typename Ty>
inline void save_eo_evecs(const std::string& filename, vector_cs<Ty>& even,
                          qlat::vector<Ty>& evals, std::vector<double>& err,
                          const Geometry& geo, const Int N0 = 0,
                          const Int N1 = -1, double mass = 0.0, Int mode_c = 0,
                          const bool single_file = true,
...
/*
  save vector_cs eigensystem to disc with gwu format
*/
template <typename Ty>
inline void load_gwu_cs_evecs(const char* filename, vector_cs<Ty>& eigen,
                              vector<Ty>& evals_, vector<double>& err_,
                              const Geometry& geo, const int N0 = 0,
                              const int N1 = -1, const bool single_file = false,
                              const bool read = true)
{
  ...
  if (eigen.nvec < size_t(n1 - n0)) {
    abort_r("eigen vector size wrong!\n");
  }
  //
  ...
  if (read_single == true) {
    ...
    for (int iv = 0; iv < n_vec; iv++) {
      ...
      if (read == false) {
        for (int ic = 0; ic < ri; ic++) {
          eigen.copy_to((ComplexF*)&prop_E[ic * noden * 12 * 2], count + ic,
                        false);
        }
        ////Do not rotate source and sink
        gwu_to_cps_rotation_vec(&prop_E[0], ri, noden, false, false, false,
      ...
      if (read == true) {
        ...
        gwu_to_cps_rotation_vec(&prop_E[0], ri, noden, false, false, false,
                                read);
        for (int ic = 0; ic < ri; ic++) {
          eigen.copy_from((ComplexF*)&prop_E[ic * noden * 12 * 2], count + ic,
                          false);
        }
      }
      count += ri;
    ...
    for (int iv = 0; iv < n_vec; iv++) {
      ...
      if (read == false) {
        for (int ic = 0; ic < ri; ic++) {
          eigen.copy_to((ComplexD*)&prop_E[ic * noden * 12 * 2], count + ic,
                        false);
        }
        gwu_to_cps_rotation_vec(&prop_E[0], ri, noden, false, true, true, read);
        rotate_gwu_vec_file(&prop_E[0], ri, noden, false, read);
      ...
      if (read == true) {
        ...
        gwu_to_cps_rotation_vec(&prop_E[0], ri, noden, false, true, true, read);
        for (int ic = 0; ic < ri; ic++) {
          eigen.copy_from((ComplexD*)&prop_E[ic * noden * 12 * 2], count + ic,
                          false);
        }
      }
      count += ri;
  ...
  std::string fileE = ssprintf("%s.eigvals", filename);
  if (read == false) {
    values.resize((n1 - n0) * 2);
    err.resize(n1 - n0);
    for (int ni = n0; ni < n1; ni++) {
      values[ni * 2 + 0] = evals_[ni - n0].real();
      values[ni * 2 + 1] = evals_[ni - n0].imag();
      err[ni] = err_[ni - n0];
    }
    save_txt_eigenvalues(values, err, fileE.c_str(), "H_wilson");
  } else {
    load_txt_eigenvalues(values, err, fileE.c_str());
    evals_.resize(n1 - n0);
    err_.resize(n1 - n0);
    for (int ni = n0; ni < n1; ni++) {
      evals_[ni - n0] = ComplexD(values[ni * 2 + 0], values[ni * 2 + 1]);
      err_[ni - n0] = err[ni];
    }
  }
  //
