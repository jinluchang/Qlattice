...
inline void get_mom_single_nodeA(
    qlat::vector<Long>& mapA, const Geometry& geo, const Int mom_cut,
    const std::vector<Coordinate>& mom_off = std::vector<Coordinate>(0))
{
  ...
  qlat::vector<long> momL;
  momL.resize(mom_off.size() * 4);
  for (unsigned long momG = 0; momG < mom_off.size(); momG++) {
    for (Int i = 0; i < 4; i++) {
  ...
  qacc_for(isp, geo.local_volume(), {
    ...
    // if(flag == true ){ Bi[isp] = ((mom[3]*mc + mom[2])*mc + mom[1])*mc+
    // mom[0]; }
    if (Nmom_flag != 0) {
      Bi[isp] = 1;
    }
...
inline void get_mom_single_nodeB(
    qlat::vector<Long>& mapA, qlat::vector<Long>& mapB, const Geometry& geo,
    const Int mom_cut, const Coordinate& mom_off = Coordinate(0, 0, 0, 0))
{
  ...
  qacc_for(isp, Mvol, {
    ...
    if (flag == true) {
      Bi[isp] = ((mom[3] * mc + mom[2]) * mc + mom[1]) * mc + mom[0];
    }
    if (flag == false) {
      Bi[isp] = -1;
    }
...
struct momentum_dat {
  ...
  void pick_mom_from_vecs(qlat::vector_gpu<Ty>& resF,
                          qlat::vector_gpu<Ty>& resV)
  {
  ...
  Int copy_momF_to_sf(qlat::SelectedField<Ty1>& sf_, qlat::vector_gpu<Ty>& srcF,
                      Int dir = 0)
  {
    ...
    sum_value_mpi(nvec);
    nvec_copy = nvec;
    if (!sf_.initialized or sf_.multiplicity != nvec) {
      sf_.init(fsel, nvec);
    }
    //
    Ty* src = srcF.data();
  ...
  template <typename Ty>
  void write(qlat::vector_gpu<Ty>& srcF, const std::string& nameQ,
             const std::string& tag_ = std::string("-1"),
             const bool clean = false)
  {
    TIMERA("Qlat write mdat");
    // const Long Mvol =  mapA.size();
    // printf("Long %d %ld %ld %ld \n",  qlat::get_id_node(), srcF.size()
    // ,mapA.size(), srcF.size()%mapA.size()); Qassert(srcF.size() % Mvol == 0);
    if (mapA.size() != 0) {
      Qassert(srcF.size() % mapA.size() == 0);
    }
    //
  ...
  /////template<typename Ty >
  // Long read(qlat::SelectedField<TWOPT_TYPE  >& sf_, const Int nvec, const
  // std::string& nameQ, const std::string& tag_){
  ...
  template <typename Ty>
  void shift_t(qlat::vector_gpu<Ty>& s1, qlat::vector_gpu<Ty>& s0,
               const Coordinate& shift_)
  {
    TIMERA("shift_t");
    ////Qassert(t0 < nv[3]);
    //
    ...
    //   TWOPT_TYPE* sfP = (TWOPT_TYPE*)
    //   qlat::get_data(sf_1.get_elems(si)).data(); Ty* reP = (Ty*)
    //   &src[isp*nvec_copy + 0];
  ...
  template <typename Ty>
  Int read_momcut(qlat::vector_gpu<Ty>& srcF, const std::string& nameQ,
  ...
  inline void get_mom_pos(const std::vector<Coordinate>& mom_pick,
  ...
  template <typename Ty>
  void update_phases(const Coordinate& src,
                     const Coordinate& shift = Coordinate(0, 0, 0, 0),
                     const Int sign = -1)
  {
    ...
    if (cur_pos == src and Long(phases.size()) == Mvol * fac and
        cur_shift == shift) {
      return;
    }
    cur_shift = shift;
    phases.resize(Mvol * fac);
    Ty* resP = (Ty*)phases.data();
    ...
    if (Mvol != 0)
      qacc_for(isp, Mvol, {
        ...
        double v0 = 0.0;
        for (Int i = 0; i < 3; i++) {
          v0 += (2.0 * QLAT_PI_LOCAL * src[i] *
                 ((mom[i] + shift[i] + Lat[i]) % (Lat[i])) / Lat[i]);
        }
  ...
  /////src phases e^{-i p ( x - y)}
  template <typename Ty>
  void apply_src_phases(qlat::vector_gpu<Ty>& vec, const Coordinate& src,
                        const Coordinate& shift = Coordinate(0, 0, 0, 0),
                        const Int sign = -1)
  {
...
template <typename Ty>
void fft_local_to_global(qlat::vector_gpu<Ty>& FG, qlat::vector_gpu<Ty>& FL,
                         momentum_dat& mdat,
                         const Coordinate& mom_off = Coordinate(0, 0, 0, 0))
{
    ...
    qacc_for(isp, NmapB, {
      ...
      if (i1 >= 0) {
        for (Long iv = 0; iv < nvec; iv++) {
          PFG[iv * Mvol + i1] = PFL[isp * nvec + iv];
        }
      }
    });
  //
...
void copy_sparse_fields(qlat::SelectedField<Ty>& res,
                        qlat::SelectedField<Ta>& src, const Int Ndc, Int nr = 0,
                        const Int ns = 0)
{
