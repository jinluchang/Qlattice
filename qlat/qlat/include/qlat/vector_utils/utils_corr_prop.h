...
void init_qpropT(std::vector<qpropT>& res, const unsigned int size,
                 const Geometry& geo)
{
  if (res.size() != size) {
    res.resize(size);
      ...
      for (Int d0 = 0; d0 < 4; d0++)
        for (Int c1 = 0; c1 < 3; c1++)
          for (Int d1 = 0; d1 < 4; d1++) {
            if (rotate == 1) {
              v0(d0 * 3 + c0, d1 * 3 + c0) =
                  qlat::qconj(v1(d1 * 3 + c1, d0 * 3 + c0));
            }
            if (rotate == 0) {
              v0(d0 * 3 + c0, d1 * 3 + c0) =
                  qlat::qconj(v1(d0 * 3 + c0, d1 * 3 + c1));
            }
          }
  });
}
  ...
  qacc_for(isp, Long(geo.local_volume()), {
    ...
    for (Int d0 = 0; d0 < 4; ++d0) {
      ...
      if (dir == 0)
        for (Int c0 = 0; c0 < 3; c0++) {
          if (!conj) {
            v[ga.ind[d0] * 3 + c0] = ga.g[d0] * buf[d0 * 3 + c0];
          }
          if (conj) {
            v[ga.ind[d0] * 3 + c0] = qlat::qconj(ga.g[d0] * buf[d0 * 3 + c0]);
          }
        }
      /////Sink   multiply
      if (dir == 1)
        for (Int c0 = 0; c0 < 3; c0++) {
          if (!conj) {
            v[d0 * 3 + c0] = ga.g[d0] * buf[ga.ind[d0] * 3 + c0];
          }
          if (conj) {
            v[d0 * 3 + c0] = qlat::qconj(ga.g[d0] * buf[ga.ind[d0] * 3 + c0]);
          }
        }
    }
  });
            ...
            for (Int c1 = 0; c1 < 3; c1++) {
              if (!conj) {
                v0(s * 3 + c0, ga.ind[d0] * 3 + c1) =
                    ga.g[d0] * v1(s * 3 + c0, d0 * 3 + c1);
              }
              if (conj) {
                v0(s * 3 + c0, ga.ind[d0] * 3 + c1) =
                    qlat::qconj(ga.g[d0] * v1(s * 3 + c0, d0 * 3 + c1));
              }
            }
          // Sink multiply
          if (dir == 1)
            for (Int c1 = 0; c1 < 3; c1++) {
              if (!conj) {
                v0(d0 * 3 + c0, s * 3 + c1) =
                    ga.g[d0] * v1(ga.ind[d0] * 3 + c0, s * 3 + c1);
              }
              if (conj) {
                v0(d0 * 3 + c0, s * 3 + c1) =
                    qlat::qconj(ga.g[d0] * v1(ga.ind[d0] * 3 + c0, s * 3 + c1));
              }
            }
        }
  });
...
{
  if (!conj) {
    prop4d_src_gammaT<Td, 0, false>(prop, ga);
  }
  if (conj) {
    prop4d_src_gammaT<Td, 0, true>(prop, ga);
  }
}
template <typename Td>
...
{
  if (!conj) {
    prop4d_src_gammaT<Td, 1, false>(prop, ga);
  }
  if (conj) {
    prop4d_src_gammaT<Td, 1, true>(prop, ga);
  }
}
      ...
      for (Int c1 = 0; c1 < 3; c1++)
        for (Int d0 = 0; d0 < 4; d0++)
          for (Int c0 = 0; c0 < 3; c0++) {
            /////Source multiply
            if (dir == 0) {
              buf[(ga.ind[d1] * 3 + c1) * 12 + d0 * 3 + c0] =
                  ga.g[d1] * src[(d1 * 3 + c1) * 12 + d0 * 3 + c0];
            }
            /////Sink multiply
            if (dir == 1) {
              buf[(d1 * 3 + c1) * 12 + d0 * 3 + c0] =
                  ga.g[d0] * src[(d1 * 3 + c1) * 12 + ga.ind[d0] * 3 + c0];
            }
          }
    //
    for (Int i = 0; i < 12 * 12; i++) {
      if (!conj) {
        res[i * Nsize + isp] = buf[i];
      }
      if (conj) {
        res[i * Nsize + isp] = qlat::qconj(buf[i]);
      }
    }
  });
}
...
void Gprop_src_gamma(EigenTy& prop, ga_M& ga, bool conj = false)
{
  for (unsigned long iv = 0; iv < prop.size(); iv++) {
    ...
    if (!conj)
      qprop_src_gamma_T<Ty, 0, false>(res, ga, prop[iv].size() / (12 * 12));
    if (conj)
      qprop_src_gamma_T<Ty, 0, true>(res, ga, prop[iv].size() / (12 * 12));
  }
}
...
void Gprop_sink_gamma(EigenTy& prop, ga_M& ga, bool conj = false)
{
  for (unsigned long iv = 0; iv < prop.size(); iv++) {
    Ty* res = (Ty*)qlat::get_data(prop[iv]).data();
    if (!conj)
      qprop_src_gamma_T<Ty, 1, false>(res, ga, prop[iv].size() / (12 * 12));
    if (conj)
      qprop_src_gamma_T<Ty, 1, true>(res, ga, prop[iv].size() / (12 * 12));
  }
}

// can be expanded fields and selected fields
template <class Fieldy>
void fieldG_src_gamma(Fieldy& prop, ga_M& ga, bool conj = false,
                      const bool srcG = true)
{
...
template <class Ty>
void fieldG_src_gammaG(std::vector<FieldG<Ty>>& prop, ga_M& ga,
                       bool conj = false)
{
...
template <class Ty>
void fieldG_sink_gammaG(std::vector<FieldG<Ty>>& prop, ga_M& ga,
                        bool conj = false)
{
  for (unsigned int i = 0; i < prop.size(); i++) {
...
void qprop_move_dc_in(Ty* src, const qlat::Geometry& geo, const Int dir = 1)
{
  ...
  //
  if (dir == 1) {
    mv_civ.move_civ_in(src, src, 1, 12 * 12, sizeF, 1, true);
  }
  if (dir == 0) {
    mv_civ.move_civ_out(src, src, 1, sizeF, 12 * 12, 1, true);
  }
}
...
template <typename Ty>
void qprop_sub_add(std::vector<qpropT>& res, std::vector<qpropT>& s0,
                   const Ty f0, const Ty f1)
{
  if (s0.size() == 0) {
    res.resize(0);
    return;
  ...
  for (Int vi = 0; vi < Nvec; vi++) {
    ...
    for (Int dc = 0; dc < 12 * 12; dc++) {
      qacc_for(isp, geo.local_volume(), {
        r0[dc * Nvol + isp] =
            r0[dc * Nvol + isp] * f0 + p0[dc * Nvol + isp] * f1;
      });
    }
  }
}
...
template <typename Ty>
void qprop_sub_add(std::vector<qpropT>& res, std::vector<qpropT>& s0,
                   std::vector<qpropT>& s1, const Ty f0, const Ty f1)
{
  if (s0.size() == 0) {
    res.resize(0);
    return;
  ...
  for (Int vi = 0; vi < Nvec; vi++) {
    ...
    for (Int dc = 0; dc < 12 * 12; dc++) {
      qacc_for(isp, geo.local_volume(), {
        r0[dc * Nvol + isp] =
            (p0[dc * Nvol + isp] + p1[dc * Nvol + isp] * f0) * f1;
      });
    }
  }
}
...
*/
template <typename Ty>
void shift_vecs_cov_qpropT(std::vector<std::vector<qpropT>>& res,
                           std::vector<qpropT>& s0, shift_vec& svec,
                           std::vector<std::vector<qpropT>>& buf)
{
  ...
  for (Int nu = 0; nu < 4; nu++) {
    ...
    qprop_sub_add(res[1 + nu], buf[0], buf[1], Ty(-1.0, 0.0),
                  Ty(1.0 / 2.0, 0.0));  ////equal
  }
}
  ...
  qacc_for(isp, prop.geo().local_volume(), {
    ...
    for (Int s0 = 0; s0 < 4; ++s0)
      for (Int c0 = 0; c0 < 3; c0++) {
        ...
        d1 = 3;
        for (Int c1 = 0; c1 < 3; c1++)
          v0(s0 * 3 + c0, dr * 3 + c1) = (-v1(s0 * 3 + c0, d0 * 3 + c1) * sn +
                                          v1(s0 * 3 + c0, d1 * 3 + c1)) /
                                         sqrt2;
        dr = 1;
        d0 = 0;
        ...
        d1 = 3;
        for (Int c1 = 0; c1 < 3; c1++)
          v0(s0 * 3 + c0, dr * 3 + c1) = (-v1(s0 * 3 + c0, d0 * 3 + c1) -
                                          v1(s0 * 3 + c0, d1 * 3 + c1) * sn) /
                                         sqrt2;
        dr = 3;
        d0 = 0;
        d1 = 2;
    ...
    for (Int c0 = 0; c0 < 3; c0++)
      for (Int s0 = 0; s0 < 4; ++s0) {
        ...
        d1 = 3;
        for (Int c1 = 0; c1 < 3; c1++)
          v0(dr * 3 + c0, s0 * 3 + c1) = (-v1(d0 * 3 + c0, s0 * 3 + c1) * sn +
                                          v1(d1 * 3 + c0, s0 * 3 + c1)) /
                                         sqrt2;
        dr = 1;
        d0 = 0;
        ...
        d1 = 3;
        for (Int c1 = 0; c1 < 3; c1++)
          v0(dr * 3 + c0, s0 * 3 + c1) = (-v1(d0 * 3 + c0, s0 * 3 + c1) -
                                          v1(d1 * 3 + c0, s0 * 3 + c1) * sn) /
                                         sqrt2;
        dr = 3;
        d0 = 0;
        d1 = 2;
...
template <typename Td>
void get_corr_pion(std::vector<qlat::FermionField4dT<Td>>& prop,
                   const Coordinate& x_ini, std::vector<double>& write)
{
  ...
  Int tini = x_ini[3];
  //
  qlat::vector<qlat::ComplexT<Td>> res;
  res.resize(Nvol);
  //
  qacc_for(isp, Long(Nvol), {
    ...
    for (Int dc2 = 0; dc2 < 12; dc2++) {
      qlat::ComplexT<Td>* a =
          (qlat::ComplexT<Td>*)&(prop[dc2].get_elem_offset(isp));
...
void get_src_phase(Ty& phase, const qlat::vector<Int>& nv,
                   const Coordinate& pos = Coordinate(),
                   const Coordinate& mom = Coordinate())
{
  double p0[3] = {2 * QLAT_PI_LOCAL / nv[0], 2 * QLAT_PI_LOCAL / nv[1],
                  2 * QLAT_PI_LOCAL / nv[2]};
  double theta = mom[0] * p0[0] * pos[0] + mom[1] * p0[1] * pos[1] +
                 mom[2] * p0[2] * pos[2];
  phase = Ty(std::cos(theta), -1.0 * std::sin(theta));
}

template <typename Ty>
void vec_corrE(Ty* srcE, qlat::vector<Ty>& res, qlat::fft_desc_basic& fd,
               const Int nvec, const Int clear = 0,
               ...
               const Int t0 = 0)
{
  ...
  if (mom != Coordinate()) {
    ...
    src = bufE.data();
    //
    qlat::vector<double> p0;
    p0.resize(3);
    for (Int i = 0; i < 3; i++) {
      p0[i] = 2 * QLAT_PI_LOCAL / fd.nv[i];
    }
    //
    qlat::vector<Ty> phaseEG;
    phaseEG.resize(Nxyz);
    ...
    qthread_for(xi, Long(Nxyz), {
      ...
      //
      double theta = mom[0] * p0[0] * pi[0] + mom[1] * p0[1] * pi[1] +
                     mom[2] * p0[2] * pi[2];
    ...
    qacc_for(i, Long(Ns), {
      ...
      LInt xi = i % (Nxyz);
      src[(mi * NTt + ti) * Nxyz + xi] =
          src[(mi * NTt + ti) * Nxyz + xi] * phaseEG[xi];
    });
    /////#endif
  }
  //
  ...
  if (clear == 0) {
    if (res.size() != nvec * nt) {
      qmessage("res size wrong for corr.\n");
      Qassert(false);
    }
  }
  //
  qlat::vector<Ty> tmp;
  tmp.resize(nvec * NTt);
  qlat::set_zero(tmp);  // tmp.set_zero();
  ...
  cpy_data_thread((Ty*)qlat::get_data(res).data(), RES.data(), RES.size(), 1,
                  QTRUE, 1.0);
}

template <typename Ty>
void vec_corrE(qlat::vector_gpu<Ty>& resE, qlat::vector<Ty>& res,
               qlat::fft_desc_basic& fd, const Int clear = 0,
               const Coordinate& mom = Coordinate(), const Ty& src_phase = 1.0,
               Int t0 = 0)
{
  Int NTt = fd.Nv[3];
...
template <typename Ty>
void vec_corrE(qlat::vector<Ty>& resE, qlat::vector<Ty>& res,
               qlat::fft_desc_basic& fd, const Int clear = 0,
               const Coordinate& mom = Coordinate(), const Ty& src_phase = 1.0,
               Int t0 = 0)
{
...
void shift_result_t(qlat::vector<Ty>& Esrc, Int nt, Int tini)
{
  ...
  if (Ntotal % (nt) != 0) {
    abort_r("Correlation function size wrong!\n");
  }
  qlat::vector<Ty> tmp;
  tmp.resize(Ntotal);
  qacc_for(i, Ntotal, {
...
// void ini_propE(EigenMTa &prop,Int nmass, qlat::fft_desc_basic &fd, bool clear
// = true){
//   LInt Nxyz = fd.Nv[0]*fd.Nv[1]*fd.Nv[2];
//   Int NTt  = fd.Nv[3];
//   Int do_resize = 0;
//   if(prop.size() != (LInt) (nmass*12*12*NTt)){do_resize = 1;}
//   for(unsigned int i=0;i<prop.size();i++){if((LInt) prop[i].size() !=
//   Nxyz){do_resize=1;}}
//
//   if(do_resize == 1)
//   {
//     for(unsigned int
//     i=0;i<prop.size();i++){prop[i].resize(0);}prop.resize(0);
...
// void copy_prop4d_to_propE(EigenMTa &prop, std::vector<Propagator4dT<Ty > >
// &pV1, qlat::fft_desc_basic &fd){
//   copy_propE(pV1, prop, fd, 0);
// }
// template<typename Ty, typename Ta>
// void copy_propE_to_prop4d(std::vector<Propagator4dT<Ty > > &pV1, EigenMTa
// &prop, qlat::fft_desc_basic &fd){
//   copy_propE(pV1, prop, fd, 1);
// }
//
// template<typename Ty, typename Ta>
// void copy_prop4d_to_propE(EigenMTa &prop, Propagator4dT<Ty > &pV1,
// qlat::fft_desc_basic &fd){
//   copy_propE(pV1, prop, fd, 0);
// }
// template<typename Ty, typename Ta>
// void copy_propE_to_prop4d(Propagator4dT<Ty > &pV1, EigenMTa &prop,
// qlat::fft_desc_basic &fd){
//   copy_propE(pV1, prop, fd, 1);
// }
//
...
// template<typename Ty>
// void ini_propG(EigenTy& prop, const Long nmass, size_t Nsize, bool clear =
// true){
...
template <typename Ty>
void copy_qprop_to_propG(EigenTy& res,
                         std::vector<qlat::FieldM<Ty, 12 * 12>>& src,
                         const qlat::Geometry& geo, Int GPU = 1, Int dir = 1)
{
  Int nvec = 0;
  if (dir == 1) {
    nvec = src.size();
  ...
  for (Int ni = 0; ni < nvec; ni++) {
    if (dir == 1) {
      res[ni].copy_from((Ty*)qlat::get_data(src[ni]).data(),
                        12 * 12 * geo.local_volume(), GPU);
    }
    if (dir == 0) {
      res[ni].copy_to((Ty*)qlat::get_data(src[ni]).data(), GPU);
    }
  }
}

template <typename Ty>
void copy_propG_to_qprop(std::vector<qpropT>& res, EigenTy& src,
                         const qlat::Geometry& geo, Int GPU = 1)
{
...
void ini_resE(qlat::vector<Ty>& res, Int nmass, qlat::fft_desc_basic& fd)
{
  ...
  LInt Nxyz = fd.Nv[0] * fd.Nv[1] * fd.Nv[2];
  Int do_resize = 0;
  if ((LInt)res.size() != (LInt)nmass * NTt * Nxyz) {
    do_resize = 1;
  }
  if (do_resize == 1) {
    res.resize(0);
    res.resize(nmass * NTt * Nxyz);
  }
  clear_qv(res);
}

inline Coordinate get_src_pos(std::string src_n, Coordinate& off_L,
                              const Geometry& geo)
{
...
*/
template <typename Ty>
void get_phases(std::vector<vector_gpu<Ty>>& phases,
                const std::vector<Coordinate>& momL, const Geometry& geo,
                const int8_t sign = 1, const Coordinate& offset = Coordinate())
{
  TIMER("get_phases");
  Int Nmom = momL.size();
  phases.resize(Nmom);
  ...
  p0.resize(3);
  for (Int i = 0; i < 3; i++) {
    p0[i] = 2 * QLAT_PI_LOCAL / nv[i];
  }
  //
  ////copy momentum to gpu memery
  qlat::vector<Int> momLV;
  momLV.resize(momL.size() * 3);
  for (Int momi = 0; momi < Nmom; momi++)
    for (Int i = 0; i < 3; i++) {
      momLV[momi * 3 + i] = momL[momi][i];
    }
  ...
  qacc_for(xi, Long(vol), {
    ...
    for (Int momi = 0; momi < Nmom; momi++) {
      double theta = 0.0;
      for (Int i = 0; i < 3; i++) {
        theta += (p0[i] * momLP[momi * 3 + i] * (pos[i] - offset[i]));
      }
...
void get_phases(vector_gpu<Ty>& phases, const Coordinate& mom,
                const Geometry& geo, const int8_t sign = 1,
                const Coordinate& offset = Coordinate())
{
  TIMER("get_phases");
  std::vector<Coordinate> momL;
  momL.resize(1);
  momL[0] = mom;
...
void apply_phases(qlat::FieldM<Ty, civ>& src, qlat::FieldM<Ty, civ>* res,
                  vector_gpu<Ty>* phases, const unsigned int Nmom)
{
  Qassert(src.initialized);
  ...
  qlat::vector<Ty*> Pphase;
  Pphase.resize(Nmom);
  for (unsigned int mi = 0; mi < Nmom; mi++) {
    Pphase[mi] = (Ty*)qlat::get_data(phases[mi]).data();
  }
    ...
    for (unsigned int momi = 0; momi < Nmom; momi++) {
      ...
      for (Int ti = 0; ti < Nt; ti++)
        for (Int ci = 0; ci < civ; ci++) {
          pres[momi][(ti * vol + xi) * civ + ci] =
              psrc[(ti * vol + xi) * civ + ci] * ph;
        }
    }
  });
}
...
template <class Ty, Int civ>
void apply_phases(qlat::FieldM<Ty, civ>& src,
...
void apply_phases(Propagator4dT<Td>& src, Propagator4dT<Td>& res,
                  qlat::vector_gpu<Ty>& phases)
{
  Qassert(src.initialized);
  const Geometry& geo = src.geo();
  if (!res.initialized) {
...
// template <class Ty, Int civ>
// void apply_phases(qlat::FieldM<Ty, civ >& src, std::vector<qlat::FieldM<Ty,
// civ>>& res, std::vector<vector_gpu<Ty >>& phases){
//   Qassert(src.initialized);
//   const Geometry& geo = src.geo();
//   const unsigned int Nmom = phases.size();
//
...
template <class Ty, Int civ>
void multi_factor(qlat::FieldM<Ty, civ>& src, qlat::FieldM<Ty, civ>& res,
                  const Ty& phases)
{
  Qassert(src.initialized);
  const Geometry& geo = src.geo();
...
template <typename Ty>
void get_phases(std::vector<Ty>& phases, Coordinate& pL, const Coordinate& src,
...
#pragma omp parallel for
  for (Long isp = 0; isp < vol; isp++) {
    ...
    double v0 = 0.0;
    for (Int i = 0; i < 3; i++) {
      v0 += (2.0 * QLAT_PI_LOCAL * src[i] * pos[i] / Lat[i]);
    }
    //
...
template <typename Ty>
void copy_qprop_to_propE(std::vector<qlat::vector<Ty>>& Eprop,
                         std::vector<qpropT>& src, Int dir = 1)
{
  ...
  for (Int mi = 0; mi < nmass; mi++)
    for (Long i = 0; i < nvec; i++) {
      if (dir == 1)
        cpy_data_thread(Eprop[mi * nvec + i].data(), &ps[mi][i * sizeF], sizeF,
                        1, false);
      if (dir == 0)
        cpy_data_thread(&ps[mi][i * sizeF], Eprop[mi * nvec + i].data(), sizeF,
                        1, false);
    }
  qacc_barrier(dummy);
}
...
template <typename Ty>
void copy_propE_to_qprop(std::vector<qpropT>& src,
                         std::vector<qlat::vector<Ty>>& Eprop)
{
...
template <class Ty, Int civ>
void copy_FieldM(std::vector<qlat::FieldM<Ty, civ>>& res,
                 const std::vector<qlat::FieldM<Ty, civ>>& src)
{
  if (src.size() == 0) {
    res.resize(0);
    return;
