...
enum QMEM { QMSYNC = -1, QMCPU = 0, QMGPU = 1 };
...
inline QMEM get_type_mem(QMEM g) { return g; }
...
// Return only Cpu, Acc
{
  ...
  if (a == MemType::Uvm) {
    if (b == MemType::Uvm or b == MemType::Acc) {
      return MemType::Acc;
    } else {
      return MemType::Cpu;
    }
  ...
  if (b == MemType::Uvm) {
    if (a == MemType::Uvm or a == MemType::Acc) {
      return MemType::Acc;
    } else {
      return MemType::Cpu;
    }
...
#define gpuMalloc(bres, bsize, Ty, GPU)                           \
  {                                                               \
    if (int(GPU) == -1) {                                         \
      qacc_Errchk(qacc_MallocManaged(&bres, bsize * sizeof(Ty))); \
    }                                                             \
    if (int(GPU) == 0) {                                          \
      bres = (Ty*)aligned_alloc_no_acc(bsize * sizeof(Ty));       \
    }                                                             \
    if (int(GPU) == 1) {                                          \
      qacc_Errchk(qacc_Malloc(&bres, bsize * sizeof(Ty)));        \
    }                                                             \
  }
#else
#define gpuMalloc(bres, bsize, Ty, GPU)                   \
...
// inline std::complex<RealD> operator*(const std::complex<float> &a, const
// double &b) {
...
inline std::complex<RealD> operator*(const std::complex<float>& a,
                                     const std::complex<RealD>& b)
{
  return std::complex<RealD>(a.real() * b.real() - a.imag() * b.imag(),
                             a.imag() * b.real() + a.real() * b.imag());
}

inline std::complex<RealD> operator*(const std::complex<RealD>& b,
...
inline std::complex<RealD> operator-(const std::complex<RealD>& a,
...
template <typename T>
struct qlat_is_pointer {
  static const bool value = false;
};

template <typename T>
struct qlat_is_pointer<T*> {
...
template <typename Ty>
inline crc32_t quick_checksum(Ty* buf, size_t Nsize, const Long Nsum = 11,
                              const Long hits = 13)
{
  TIMERA("quick_checksum");
  crc32_t sum = 0;
  if (hits < 0) {
    sum = crc32_par((void*)buf, Nsize * sizeof(Ty));
  }
  if (hits > 0) {
    Long Nuse = Nsum;
    if (Long(Nsize / hits) < Nuse) {
      Nuse = Nsize / hits;
    }
