...
__global__ void gauss_smear_global4(T* pres, const T* psrc, const T* gf,
                                    const T bw, const T norm, const Long Nvol,
                                    const Long* map_bufD, const Long* map_final)
{
  ...
  if (count < Nvol) {
    ...
    unsigned int off = tid;
    {
      ...
      while (off < (2 * dirL) * 9) {
        ...
        // ls[(c/3)*(2*dirL)*3 + dir*3 + c%3] = gf[count*(2*dirL)*9 + off];off
        // += ns; ls[off] = gf_t[off];
        const Int dir = off / 9;
        const Int c0 = (off / 3) % 3;
        const Int c1 = off % 3;
        const Int dirM = dir - dirL;
        // ls[c1*(2*dirL)*3 + dir*3 + c0 ] = gf_t[(dirM + dir_max)*9 + c1*3 +
        // c0];
...
inline void get_mapvq_each(const std::vector<CommPackInfo>& pack_infos,
                           std::vector<qlat::vector<Long>>& mapvq, Int dir = 0)
{
  ...
  //
  // mapvq.resize(2);for(unsigned int i=0;i<2;i++){mapvq[i].copy_from(mapv[i],
  // 1);}
}
...
inline void get_maps_hoppings(
    ...
    qlat::vector<Long>& map_index_typeA0, qlat::vector<Long>& map_index_typeA1,
    qlat::vector<Long>& map_index_typeAL, qlat::vector<Long>& map_bufD_typeA,
    ...
    std::vector<Long>& pos_typeA)
{
  ...
  //
  /////const Int dir_max = 4;
  qlat::vector<Long> map_index_typeO_0;
  map_index_typeO_0.resize(Nvol_ext);
  qlat::vector<Long> map_index_typeO_1;
  map_index_typeO_1.resize(Nvol);
  for (Long i = 0; i < Nvol_ext; i++) {
    map_index_typeO_0[i] = -1;
  }
  for (Long i = 0; i < Nvol; i++) {
    map_index_typeO_1[i] = -1;
  }
  //
...
#pragma omp parallel for
  ...
  //
  std::vector<Long> need_pos;
  need_pos.resize(Nvol_ext);  ////for checkings only
...
#pragma omp parallel for
    for (Long index = 0; index < Nvol; index++) {
      ...
      //{
      //   ////const Coordinate xl = geo_ext.coordinate_from_index(i);///this
      //   function is wrong! if(xl[3] == 0){
      //     xlE[xl1[2]] += 1;
      //     //qmessage("rank %3d, x y z t, %3d %3d %3d %3d, ext %3d %3d %3d %3d
      //     , index %ld ext %ld\n", qlat::get_id_node(),
      //     //  xl[0], xl[1], xl[2], xl[3],
  ...
  //}
  //
  std::vector<std::vector<Long>> check_send;
  check_send.resize(Nvol_ext);
  std::vector<std::vector<Long>> check_recv;
  check_recv.resize(Nvol_ext);
  // for(Long i=0;i<Nvol_ext;i++){
  ...
  //
  std::vector<Long> local_map_typeA0_e;
  local_map_typeA0_e.resize(
      Nvol_ext);  ////local_map0[count] == original positions
  std::vector<Long> local_map_typeA0_o;
  local_map_typeA0_o.resize(
      Nvol_ext);  ////local_map0[count] == original positions
...
#pragma omp parallel for
  ...
  local_map_typeA0.resize(
      Nvol_sum);  ////local_map0[count] == original positions
...
#pragma omp parallel for
  ...
  std::vector<Long> sortL =
      get_sort_index(copy_extraL.data(), copy_extraL.size());
  ////std::vector< qlat::vector<Long > > copy_extra_index;
  ...
  //
  // for(unsigned long
  // isp=0;isp<copy_extra_index0.size();isp++){copy_extra_index[isp] =
  // copy_extra_index0[isp];} for(unsigned long
  // isp=0;isp<copy_extra_posL0.size() ;isp++){copy_extra_posL[ isp] =
  // copy_extra_posL0[ isp];}
  //
  Long counti = 0;
  ...
  //{
  //   if(copy_extra[i].size() > 1){
  //     copy_extra_index0.push_back(copy_extra[i][0]);///index of the initial
  //     value
  ...
  // \n",
  //   pos_send, pos_recv - pos_send, pos_recv, count_end - pos_recv, count0,
  //   count, count_end, Nvol_ext);
  ////Qassert(false);
  //
...
#pragma omp parallel for
  for (Long i = 0; i < Nvol; i++) {
    map_index_typeA0[i] = -1;
  }
#pragma omp parallel for
  for (Long i = 0; i < Nvol_sum; i++) {
    map_index_typeA1[i] = -1;
  }
#pragma omp parallel for
  for (Long i = 0; i < Nvol_sum; i++) {
    map_index_typeAL[i] = -1;
  }
//
#pragma omp parallel for
  for (Long index = 0; index < Nvol; index++) {
    ...
    Long index_typeA =
        local_map_typeA0[index_ext];  ////memory positions in typeA format
...
#pragma omp parallel for
  for (Long i = 0; i < Long(map_bufD_typeA.size()); i++) {
    map_bufD_typeA[i] = -1;
  }
  ////count = index_typeAL
//
#pragma omp parallel for
  for (Long count = 0; count < Nvol; count++) {
    ...
    for (Int dir = -dirL; dir < dirL; dir++) {
      Long index_ext = map_bufD[index * dirL * 2 +
                                (dir + dirL)];  ////memory postions in index_ext
...
//
#pragma omp parallel for
  for (Long i = 0; i < Nvol * dirL * 2; i++) {
    Qassert(map_bufD_typeA[i] != -1);
  }
  // count = 0;
  // for(Long i=0;i<Nvol_ext;i++){
...
struct smear_fun {
  ...
  // template <class Td>
  // void setup(const GaugeFieldT<Td >& gf, const CoordinateD& mom, const bool
  // smear_in_time_dir){
  ...
  inline void check_setup()
  {
    ...
    const Int GPU = 1;
    qlat::vector_gpu<Ty>& gauge_buf =
        get_vector_gpu_plan<Ty>(0, gauge_buf_name, GPU);
    if (gauge_buf.size() == 0) {
      Qassert(false);
    }
    if (gauge_check == NULL) {
                        ...
                        std::vector<qlat::vector_gpu<Long>>& mapvq, Int dir = 0)
  {
    ...
    for (Long i = 0; i < (Long)pack_infos.size(); ++i) {
      ...
      //   Long fi   = cpi.offset     + j;
      //   qmessage("size %d, group %d, bufi %d, fi %d \n", int(i),
      //   int(cpi.size), int(bufi), int(fi)); if(dir ==
      //   0){mapv[0].push_back(bufi);mapv[1].push_back(  fi);} if(dir ==
      //   1){mapv[0].push_back(  fi);mapv[1].push_back(bufi);}
      // }
    }
    //
    ...
    for (unsigned int i = 0; i < 3; i++) {
      mapvq[i].copy_from(mapv_copy[i], 1);
    }
  }
  ...
  {
    if (geo == geo_ and smear_in_time_dir == smear_in_time_dir_ and
        mem_setup_flag == true) {
      return;
    }
    geo = geo_;
    ...
    geo.eo = 0;
    Geometry geo1 = geo;
    if (!smear_in_time_dir) {
      geo1.resize(Coordinate(1, 1, 1, 0), Coordinate(1, 1, 1, 0));
    }
    if (smear_in_time_dir) {
      geo1.resize(Coordinate(1, 1, 1, 1), Coordinate(1, 1, 1, 1));
    }
    geo_ext = geo1;
    QLAT_PUSH_DIAGNOSTIC_DISABLE_DANGLING_REF;
    const CommPlan& plan = get_comm_plan(set_marks_field_1, "", geo_ext, 1);
    ...
    get_maps_hoppings(geo, geo_ext, dirL, local_map_typeA0, local_map_typeA1,
                      map_index_typeA0, map_index_typeA1, map_index_typeAL,
                      map_bufD_typeA, copy_extra_index, pos_typeA);
    //
    ...
    //   const Coordinate xl = geo.coordinate_from_index(index);
    //   const Coordinate xl1 = coordinate_shifts(xl, dir);
    //   map_bufD[index*dirL*2 + (dir+dirL)] =
    //   geo_ext.offset_from_coordinate(xl1, 1);
    // }
    //
    mem_setup_flag = true;
  }
  ...
  inline void init_distribute()
  {
    ...
    qlat::vector_gpu<Ty> gfT;
    gfT.resize(NVmpi * gauge_buf.size());
    qlat::vector_gpu<Ty> gfT_buf;
    gfT_buf.resize(NVmpi * gauge_buf.size());
    const Int dir_max = 4;
    const size_t Ndata = gauge_buf.size();
    //
    for (Long vi = 0; vi < NVmpi; vi++) {
      cpy_data_thread(&(gfT.data()[vi * Ndata]), gauge_buf.data(), Ndata);
    }
    //
...
#pragma omp parallel for
      for (Long index = 0; index < Nvol; index++) {
        ...
        // const Coordinate xl1 = coordinate_shifts(xl, dir);
        //
        /// map_bufD[(dir+4)*Nvol + index] = ((xl[3]*Nn[2]+xl[2])*Nn[1] +
        /// xl[1])*Nn[0] + xl[0];
        // map_bufD[index*dirL*2 + (dir+dirL)] =
        // geo_ext.offset_from_coordinate(xl1, 1);
        const Long pos =
            ((xl[3] * Nn[2] + xl[2]) * Nn[1] + xl[1]) * Nn[0] + xl[0];
...
//
    ...
    //
    // cpy_data_from_group(send_buffer.data(), f , mapvq_send[0].data(),
    // mapvq_send[1].data(), mapvq_send[2].data(), mapvq_send[0].size(), bfac,
    // GPU);
    {
...
//
    ...
    //
    ////sync_node();
    // cpy_data_from_group(f, recv_buffer.data(), mapvq_recv[0].data(),
    // mapvq_recv[1].data(), mapvq_recv[2].data(), mapvq_recv[0].size(), bfac,
    // GPU);
  }

  inline void gauge_buf_name_append(Int buf)
  {
    if (gauge_dup == buf or buf == -1) {
      return;
    }
    // char tmp[100];
    ...
    gauge_dup = buf;
    // qlat::vector_gpu<Ty >& gauge_buf = get_vector_gpu_plan<Ty >(0,
    // gauge_buf_name, GPU);
  }

  // void gauge_setup(qlat::vector_gpu<T >& gfE, const GaugeFieldT<Tg >& gf,
  //            const qlat::vector<qlat::ComplexD >& momF =
  //            qlat::vector<qlat::ComplexD >(), bool force_update = false);
  template <class Td>
  void gauge_setup(const GaugeFieldT<Td>& gf, const CoordinateD& mom_,
                   const Int force_update = 0, const Int use_gauge_mapping_ = 1)
  {
    ...
    if (gauge_check != (void*)qlat::get_data(gf).data()) {
      update = true;
    }
    if (mom_factors.size() != 8) {
      update = true;
    } else {
      for (Int i = 0; i < momF.size(); i++) {
        if (momF[i] != mom_factors[i]) {
          update = true;
        }
      }
    }
    if (use_gauge_mapping != use_gauge_mapping_) {
      update = true;
    }
    //
    ...
    if (force_update == 0) {
      crc32_t tmp_gauge_checksum =
          quick_checksum((Td*)qlat::get_data(gf).data(), gf_data_size);
      if (gauge_checksum != tmp_gauge_checksum) {
        update = true;
      }
    }
    //
    if (update) {
      ...
      // std::sscanf(gauge_buf_name.c_str(), "%s_%X",
      // gauge_buf_name_base.c_str(), &tmp_gauge_checksum);
      std::string tem = ssprintf("%X", tmp_gauge_checksum);
      gauge_buf_name =
          gauge_buf_name_base + std::string("_") + std::string(tem);
      qlat::vector_gpu<Ty>& gauge_buf =
          get_vector_gpu_plan<Ty>(0, gauge_buf_name, GPU);
      //
  ...
  inline void clear_buf_mem()
  {
    ...
    gauge_buf.resize(0);
    //
    qlat::vector_gpu<Ty>& prop_buf0 =
        get_vector_gpu_plan<Ty>(0, prop_buf0_name, GPU);
    qlat::vector_gpu<Ty>& prop_buf1 =
        get_vector_gpu_plan<Ty>(0, prop_buf1_name, GPU);
    prop_buf0.resize(0);
    prop_buf1.resize(0);
  }
...
inline bool operator<(const SmearPlanKey& x, const SmearPlanKey& y)
{
  ...
  //
  if (x.smear_in_time_dir < y.smear_in_time_dir) {
    return true;
  }
  if (y.smear_in_time_dir < x.smear_in_time_dir) {
    return false;
  }
  //
  if (x.bfac < y.bfac) {
...
struct smear_fun_copy {
  ...
  const smear_fun_copy& operator=(const smear_fun_copy& smf)
  {
    ...
    if (prec == ComplexD_TYPE) {
      ...
      qmessage("Only ComplexD and ComplexF supported for smearing! \n");
      Qassert(false);
    }
    //
    /////swap(*this, smf);
  ...
  inline void delete_pointer()
  {
    if (smfP != NULL) {
      if (prec == ComplexD_TYPE) {
        delete ((smear_fun<ComplexD>*)smfP);
      }
      if (prec == ComplexF_TYPE) {
        delete ((smear_fun<ComplexF>*)smfP);
      }
      smfP = NULL;
    }
  }
...
inline smear_fun_copy make_smear_plan(const Geometry& geo,
                                      const bool smear_in_time_dir,
                                      const DATA_TYPE prec)
{
  TIMER_VERBOSE("make_smear_plan");
  smear_fun_copy st;
  st.prec = prec;
  if (prec == ComplexD_TYPE) {
    st.smfP = (void*)(new smear_fun<ComplexD>(geo, smear_in_time_dir));
  } else if (prec == ComplexF_TYPE) {
    st.smfP = (void*)(new smear_fun<ComplexF>(geo, smear_in_time_dir));
  } else {
    qmessage("Only ComplexD and ComplexF supported for smearing! \n");
    Qassert(false);
  }
...
inline void clear_smear_plan_cache() { get_smear_plan_cache().clear(); }
...
inline SmearPlanKey get_smear_plan_key(const Geometry& geo,
                                       const bool smear_in_time_dir,
                                       Int dup = -1)
{
  SmearPlanKey skey;
...
void extend_links_to_vecs(
    T* resE, const GaugeFieldT<Td>& gf,
    const qlat::vector<qlat::ComplexD>& mom_factors =
        qlat::vector<qlat::ComplexD>(),
    const qlat::vector<Long>& index_map = qlat::vector<Long>(0))
{
  ...
  qlat::vector<qlat::ComplexD> buf_mom;
  buf_mom.resize(8);
  for (Int i = 0; i < buf_mom.size(); i++) {
    buf_mom[i] = 1;
  }
  if (mom_factors.size() == 0) {
    momF = (qlat::ComplexD*)qlat::get_data(buf_mom).data();
  }
  if (mom_factors.size() != 0) {
  ...
  qacc_for(count, geo.local_volume(), {
    ...
    for (Int dir = -dir_limit; dir < dir_limit; ++dir) {
      ...
      for (Int ci = 0; ci < 9; ci++) {
        // resE[(index*dir_limit*2+ (dir+dir_limit))*9 + (ci%3)*3 + ci/3] =
        // momF[dir + 4] * link.p[ci];
        resE[(count * dir_limit * 2 + (dir + dir_limit)) * 9 + ci] =
            momF[dir + 4] * link.p[ci];
      }
    }
  })
...
template <class T, class Td>
void extend_links_to_vecs(qlat::vector_gpu<T>& gfE, const GaugeFieldT<Td>& gf,
                          const qlat::vector<qlat::ComplexD>& mom_factors =
                              qlat::vector<qlat::ComplexD>())
{
...
template <class T>
void rotate_Vec_prop(Propagator4dT<T>& prop,
                     qlat::vector_gpu<ComplexT<T>>& propT, unsigned int NVmpi,
                     unsigned int groupP, Int dir = 0)
{
  TIMER("Rotate Vec prop");
  /// unsigned int NVmpi = fd.mz*fd.my*fd.mx;
  /// int groupP = (12+NVmpi-1)/NVmpi;
  Qassert(groupP <= 12);
  Qassert(groupP * NVmpi >= 12);
  Qassert(groupP > 0);
  //
      ...
      for (Int d1 = 0; d1 < 4; d1++)
        for (Int c0 = 0; c0 < 3; c0++)
          for (Int d0 = 0; d0 < 4; d0++) {
            ...
            if (dir == 0) {
              propT[offP] = v0(d0 * 3 + c0, d1 * 3 + c1);
            }
            if (dir == 1) {
              v0(d0 * 3 + c0, d1 * 3 + c1) = propT[offP];
            }
          }
  });
}
...
void smear_propagator_box_kernel(qlat::vector_gpu<T>& prop,
                                 qlat::vector_gpu<T>& vp,
                                 qlat::vector_gpu<T>& vm, const Int Bsize,
                                 const Int dir, shift_vec& svec)
{
  // return ;
  TIMERC("Shift Kernel");
  ...
  for (Int bi = 0; bi < Bsize; bi++) {
    iDir[dir] = 1;
    svec.shift_vecP(vp.p, vp.p, iDir, bfac * 3 * civ);
    iDir[dir] = -1;
    svec.shift_vecP(vm.p, vm.p, iDir, bfac * 3 * civ);
    prop += vp;
    prop += vm;
  }
}
...
void smear_propagator_box(T* src, const Int Bsize, smear_fun<T>& smf)
{
  ...
  //
  const Int GPU = 1;
  qlat::vector_gpu<T>& gauge_buf =
      get_vector_gpu_plan<T>(0, smf.gauge_buf_name, GPU);
  //
  ...
  //
  for (unsigned int i = 0; i < smf.vL.size(); i++) {
    smf.vL[i].resize(Ndata);
    smf.vL[i].set_zero();
  }
  ...
  // std::vector<qlat::vector_gpu<T >  > vL(3);
  // for(unsigned int
  // i=0;i<vL.size();i++){vL[i].resize(Ndata);vL[i].set_zero();}
  //
  ...
  vprop.set_zero();
  for (Int dir = 0; dir < dir_limit; ++dir) {
    smear_propagator_box_kernel<T, bfac, civ>(*vL[dir], vp, vm, Bsize, dir,
                                              svec);
    vprop += (*vL[dir]);
  }
  //
  ...
  qacc_for(index, Nvol, {
    for (Int c = 0; c < nsites; c++) {
      src[index * nsites + c] = data[index * nsites + c] * (1.0 / 6.0);
    }
  });
  //
  smf.clear_mem();
...
template <class T, Int bfac, Int civ>
void gauss_smear_kernel(T* src, const double width, const Int step,
                        const T norm, smear_fun<T>& smf)
{
  double bw_fac = width * width / (4.0 * step - 6.0 * width * width);
  ////match convention to qlat
  if (smf.dirL == 4) {
    bw_fac = bw_fac * 3.0 / 4.0;
  }
  const T bw = bw_fac;
  //
  ...
  qlat::vector_gpu<T>& gauge_buf =
      get_vector_gpu_plan<T>(0, smf.gauge_buf_name, GPU);
  qlat::vector_gpu<T>& prop_buf0 =
      get_vector_gpu_plan<T>(0, smf.prop_buf0_name, GPU);
  qlat::vector_gpu<T>& prop_buf1 =
      get_vector_gpu_plan<T>(0, smf.prop_buf1_name, GPU);
  //
  smf.check_setup();
  //
  const T* gf = (T*)gauge_buf.data();
  const Long Nvol_sum =
      smf.map_index_typeAL.size();  ////only Nvol have data, just save Nvol_sum
...
//
  ...
  if (smf.redistribution_copy == 0) {
    cpy_data_from_index(prop_src, src, &smf.map_index_typeA0[0],
                        &smf.map_index_typeI[0], Nvol, nsites, GPU);
    // cpy_data_from_index(prop_src, src, &smf.map_bufV[0][0],
    // &smf.map_bufV[1][0], Nvol, nsites, GPU);
  }
  //
  // const Long Ndata = smf.Nvol * nsites;
  ////cpy_data_from_index(prop_res, src, &smf.map_bufV[1][0],
  ///&smf.map_bufV[1][0], Nvol, nsites, GPU);
  // size_t Ndata = smf.Nvol * bfac * 3* civ;
  ...
  for (Int i = 0; i < step; ++i) {
    //{TIMERC("Copy prop");
    // cpy_data_from_index(prop_src, prop_res, &smf.map_bufV[0][0],
    // &smf.map_bufV[0][0], Nvol, nsites, GPU);}
    //
    {
...
#ifdef QLAT_USE_ACC
      if (smf.dirL == 3) {
        gauss_smear_global4<T, bfac, civ, 3><<<dimGrid, dimBlock>>>(
            prop_res, prop_src, gf, bw, norm, Nvol, Pdir1, map_final);
      }
      if (smf.dirL == 4) {
        gauss_smear_global4<T, bfac, civ, 4><<<dimGrid, dimBlock>>>(
            prop_res, prop_src, gf, bw, norm, Nvol, Pdir1, map_final);
      }
      qacc_barrier(dummy);
#else
      ...
      qthread_for(count, Nvol, {
        ...
        for (Int dir = -dir_limit; dir < dir_limit; ++dir) {
          ...
          for (Int bi = 0; bi < bfac; bi++) {
            Eigen::Matrix<T, 3, 3, Eigen::ColMajor>& lE =
                *((Eigen::Matrix<T, 3, 3, Eigen::ColMajor>*)lp);
            // Eigen::Matrix<T, 3, 3, Eigen::RowMajor>&     lE =
            // *((Eigen::Matrix<T, 3, 3, Eigen::RowMajor>*) lp);
            Eigen::Matrix<T, civ, 3, Eigen::ColMajor>& wmE = *(
                (Eigen::Matrix<T, civ, 3, Eigen::ColMajor>*)&buf[bi * 3 * civ]);
            //
            // Eigen::Matrix<T, civ, 3, Eigen::ColMajor>&  pE =
            // *((Eigen::Matrix<T, civ, 3, Eigen::ColMajor>*) &wm1p[bi*3*civ]);
            // wmE += ( pE * lE);
            //
            if (civ != 1) {
              Eigen::Matrix<T, civ, 3, Eigen::ColMajor>& pE =
                  *((Eigen::Matrix<T, civ, 3, Eigen::ColMajor>*)&wm1p[bi * 3 *
                                                                      civ]);
              wmE += (pE * lE);
            }
            if (civ == 1) {
              Eigen::Matrix<T, civ, 3, Eigen::RowMajor>& pE =
                  *((Eigen::Matrix<T, civ, 3, Eigen::RowMajor>*)&wm1p[bi * 3 *
                                                                      civ]);
              wmE += (pE * lE);
...
//
  ...
  //
  if (smf.redistribution_copy == 0) {
    cpy_data_from_index(src, prop_src, &smf.map_index_typeI[0],
                        &smf.map_index_typeA0[0], Nvol, nsites, GPU);
    // cpy_data_from_index(src, prop_src, &smf.map_bufV[1][0],
    // &smf.map_bufV[0][0], Nvol, nsites, GPU);
  }
  //
  smf.clear_mem();
...
template <class T, Int bfac, Int civ>
void smear_kernel_sort(T* src, const double width_in, const Int step,
                       smear_fun<T>& smf)
{
  ...
  //
  if (step == -1) {
    if (smf.smear_in_time_dir == true) {
      abort_r("Box smearing not supported for t smearings \n");
    }
...
template <class T>
void smear_kernel(T* src, const double width, const Int step, smear_fun<T>& smf,
                  Int bfac, Int civ)
{
  bool cfind = false;
  {
    TIMERC("==smear_kernel");
#define smear_macros(ba, da)                             \
  if (bfac == ba and civ == da and cfind == false) {     \
    cfind = true;                                        \
...
#undef smear_macros
  ...
  if (cfind == false) {
    qmessage("Case bfac %5d, civ %5d \n", bfac, civ);
    abort_r("smear kernel not found!\n");
  }
  ////Qassert(cfind);
...
void smear_propagator_gwu_convension_inner(
    Ty* prop, const GaugeFieldT<Td>& gf, const double width, const Int step,
    const CoordinateD& mom = CoordinateD(),
    const bool smear_in_time_dir = false, const Int mode = 1,
    const Int dup = -1, const Int force_update = 0)
{
...
#ifdef QLAT_USE_ACC
  if (c0 * d0 > 48) {
    abort_r("d0 should be smaller than 48 for gpu mem. \n");
  }
#endif
  //
  ...
  //
  SmearPlanKey skey =
      get_smear_plan_key<Ty, c0, d0>(geo, smear_in_time_dir, dup);
...
////do this check only on GPU, CPU set it by hand
  ...
  //
  if (ini_check and smear_in_time_dir == false and mode == 1) {
    reorder = true;
  }
  ...
  const Int GPU = 1;
  qlat::vector_gpu<Ty>& prop_buf0 =
      get_vector_gpu_plan<Ty>(0, smf.prop_buf0_name, GPU);
  qlat::vector_gpu<Ty>& prop_buf1 =
      get_vector_gpu_plan<Ty>(0, smf.prop_buf1_name, GPU);
  //
  if (reorder) {
    use_gauge_mapping = 0;
  }  ////hack for reordering
  ...
  if (reorder) {
    ...
    cpy_GPU2D(res, src, Long(d0), Long(Nvol_pre * c0 * 3),
              Long(smf.NVmpi * groupP), Long(d0), QMGPU, QMGPU);
    // void copy_buffers_vecs(Ty *res, Ty *src,Long N0, Long N1, Long Ncopy,
    // size_t Vol, Int GPU = 1) copy_buffers_vecs(res, src, smf.NVmpi*groupP,
    // d0, d0, Nvol_pre*c0*3, 1);
    smf.mv_idx.dojob(res, res, 1, smf.NVmpi, Nvol_pre * c0 * 3, 1, groupP,
                     true);
    {
      TIMERC("Vec prop");
      smf.vec_rot->reorder(res, prop_buf1.data(), 1, c0 * 3 * groupP, 0);
    }
    src = prop_buf0.data();
  }
  //
  ...
  if (reorder) {
    ...
    {
      TIMERC("Vec prop");
      smf.vec_rot->reorder(res, prop_buf1.data(), 1, c0 * 3 * groupP, 100);
    }
    smf.mv_idx.dojob(res, res, 1, smf.NVmpi, Nvol_pre * c0 * 3, 0, groupP,
                     true);
    cpy_GPU2D(prop, prop_buf0.data(), Long(d0), Long(Nvol_pre * c0 * 3),
              Long(d0), Long(smf.NVmpi * groupP), QMGPU, QMGPU);
    ////cpy_data_thread(prop, smf.prop.data(), smf.prop.size(), 1);
    // copy_buffers_vecs(prop, prop_buf0.data(), d0, smf.NVmpi*groupP, d0,
    // Nvol_pre*c0*3, 1);
  }
  /////rotate_prop(prop, 1);
  //
  ...
  // \n", c0 , d0, smf.NVmpi, groupP, int(reorder)); #endif
}
...
void smear_propagator_gwu_convension(FieldG<Ty>& prop,
                                     const GaugeFieldT<Td>& gf,
                                     ...
                                     const Int force_update = 0)
{
  ...
  Qassert(prop.initialized and prop.multiplicity == 12 * 12 and
          prop.mem_order == QLAT_OUTTER);
  const Long Nvol = prop.geo().local_volume();
  Ty* src = (Ty*)qlat::get_data(prop).data();
  move_index mv_civ;
  int flag = 0;
  flag = 0;
  mv_civ.dojob(src, src, 1, 12 * 12, Nvol, flag, 1, true);
  //
  qacc_for(isp, Nvol, {
    Ty buf[12 * 12];
    for (Int i = 0; i < 12 * 12; i++) {
  ...
  //
  smear_propagator_gwu_convension_inner<Ty, 1, 12 * 4, Td>(
      src, gf, width, step, mom, smear_in_time_dir, mode, dup, force_update);
  //
  qacc_for(isp, Nvol, {
    Ty buf[12 * 12];
    for (Int i = 0; i < 12 * 12; i++) {
                                     ...
                                     const Int force_update = 0)
{
  ...
  smear_propagator_gwu_convension(tmp_prop, gf, width, step, mom,
                                  smear_in_time_dir, mode, dup, force_update);
}

template <class Ty, class Td>
void smear_propagator_gwu_convension(qlat::FieldM<ComplexT<Ty>, 12>& prop,
                                     const GaugeFieldT<Td>& gf,
                                     const double width, const Int step,
                                     const CoordinateD& mom = CoordinateD(),
                                     const bool smear_in_time_dir = false,
                                     ...
                                     const Int force_update = 0)
{
  ...
  //
  smear_propagator_gwu_convension_inner<ComplexT<Ty>, 1, 4, Td>(
      src, gf, width, step, mom, smear_in_time_dir, mode, dup, force_update);
  qacc_for(isp, prop.geo().local_volume(), {
    ComplexT<Ty> buf[12];
    for (Int i = 0; i < 12; i++) {
...
template <class Ty, class Td>
void smear_propagator_gwu_convension(Propagator4dT<Ty>& prop,
                                     const GaugeFieldT<Td>& gf,
                                     const double width, const Int step,
                                     const CoordinateD& mom = CoordinateD(),
                                     const bool smear_in_time_dir = false,
                                     ...
                                     const Int force_update = 0)
{
  ...
  rotate_prop(prop, 0);
  ComplexT<Ty>* src = (ComplexT<Ty>*)qlat::get_data(prop).data();
  smear_propagator_gwu_convension_inner<ComplexT<Ty>, 1, 12 * 4, Td>(
      src, gf, width, step, mom, smear_in_time_dir, mode, dup, force_update);
  rotate_prop(prop, 1);
}

template <class Ty, class Td>
void smear_propagator_wuppertal_convension(
    Propagator4dT<Ty>& prop, const GaugeFieldT<Td>& gf, const double width,
    const Int step, const CoordinateD& mom = CoordinateD(),
    const bool smear_in_time_dir = false, const Int mode = 1,
    const Int dup = -1, const Int force_update = 0)
{
  if (0 == step) {
  ...
  smear_propagator_gwu_convension_inner<ComplexT<Ty>, 1, 12 * 4, Td>(
      src, gf, factor_sigma, step, mom, smear_in_time_dir, mode, dup,
      force_update);
  rotate_prop(prop, 1);
}
