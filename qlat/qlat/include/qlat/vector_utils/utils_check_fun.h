  ...
  for (Long index = 0; index < p0.geo().local_volume(); ++index) {
    ...
    for (Int d0 = 0; d0 < 12; d0++)
      for (Int d1 = 0; d1 < 12; d1++) {
        ...
        bool checknan = false;
        if (std::isnan(p0.real()) or std::isnan(p0.imag())) {
          checknan = true;
        }
        if (std::isnan(p1.real()) or std::isnan(p1.imag())) {
          checknan = true;
        }
        //
        if (n1 > 1e-18) {
          diff = std::fabs(qlat::qnorm(p0 - p1) / n1);
        } else {
          if (n0 > 1e-18) {
            diff = std::fabs(qlat::qnorm(p0 - p1) / n0);
          }
        }
        diffp += diff;
        if ((diff > err or checknan) and countp < MAX_COUNT) {
          printf(
              "rank %5d, x %5d, y %5d, z %5d, t %5d, d %5d, value %+.3e %+.3e, "
              "%+.3e %+.3e, %+.3e \n",
              rank, xg0[0], xg0[1], xg0[2], xg0[3], d0 * 12 + d1, p0.real(),
              p0.imag(), p1.real(), p1.imag(), diff);
          countp += 1;
        }
      }
  }
...
template <class Ty, Int civ>
void diff_propT(qlat::FieldM<Ty, civ>& p0, qlat::FieldM<Ty, civ>& p1,
                double err = 1e-15, Long MAX_COUNT = 64)
{
  Int rank = qlat::get_id_node();
  /////Long MAX_COUNT = 64;
  double diffp = 0.0;
  ...
  for (Long index = 0; index < p0.geo().local_volume(); ++index) {
    ...
    for (Int c0 = 0; c0 < civ; c0++) {
      ...
      bool checknan = false;
      if (std::isnan(pa.real()) or std::isnan(pa.imag())) {
        checknan = true;
      }
      if (std::isnan(pb.real()) or std::isnan(pb.imag())) {
        checknan = true;
      }
      //
      if (n1 > 1e-18) {
        diff = std::fabs(qlat::qnorm(pa - pb) / n1);
      } else {
        if (n0 > 1e-18) {
          diff = std::fabs(qlat::qnorm(pa - pb) / n0);
        }
      }
      diffp += diff;
      if ((diff > err or checknan) and countp < MAX_COUNT) {
        printf(
            "rank %5d, x %5d, y %5d, z %5d, t %5d, d %5d, value %+.8e %+.8e, "
            "%+.3e %+.3e, %+.3e \n",
            rank, xg0[0], xg0[1], xg0[2], xg0[3], c0, pa.real(), pa.imag(),
            pb.real(), pb.imag(), diff);
        countp += 1;
      }
    }
  }
...
void diff_EigenM(qlat::vector<T>& a, qlat::vector<T>& b, std::string lab)
{
  double diff = 0.0;
  if (a.size() != b.size()) {
    qmessage("%s size not equal %d, %d\n", lab.c_str(), int(a.size()),
             int(b.size()));
    abort_r("");
  }
  //
...
void random_point_src(Propagator4dT<T>& prop, Int seed = 0)
{
  ...
  qlat::displayln_info(
      qlat::ssprintf("===Point src x %5d, y %5d, z %5d, t %5d \n", xg[0], xg[1],
                     xg[2], xg[3]));
...
void get_mom_apply(qlat::FieldM<Ty0, civ>& src, std::vector<Int>& mom,
                   std::vector<Ty1>& dat, bool dir = true, bool ft4D = false)
{
  ...
  for (Long di = 0; di < Long(dat.size()); di++) {
    dat[di] = 0.0;
  }
  //
...
template <typename Ty0, typename Ty1, Int civ>
void get_mom_fft(qlat::FieldM<Ty0, civ>& src, std::vector<Int>& mom,
                 std::vector<Ty1>& dat, bool ft4D = false)
{
  Geometry& geo = src.geo();
  std::vector<Int> Nv, nv, mv;
  Nv.resize(4);
  ...
  for (Long isp = 0; isp < geo.local_volume(); isp++) {
    ...
    Coordinate xg = geo.coordinate_g_from_l(xl);
    if (ft4D)
      if (xg[0] == mom[0] and xg[1] == mom[1] and xg[2] == mom[2] and
          xg[3] == mom[3]) {
        for (Int di = 0; di < civ; di++) {
          dat[di] += P0[isp * civ + di];
        }
      }
    if (!ft4D)
      if (xg[0] == mom[0] and xg[1] == mom[1] and xg[2] == mom[2]) {
        for (Int di = 0; di < civ; di++) {
          dat[xg[3] * civ + di] += P0[isp * civ + di];
        }
      }
  }
  //
...
void print_sum(const Ty* a, size_t size, std::string stmp = std::string(""),
               Int GPU = 1)
{
...
double check_sum_FieldM(qlat::Field<Ty>& p0)
{
  ...
  for (Long index = 0; index < Nvol; ++index) {
    ...
    Coordinate xg = geo.coordinate_g_from_l(xl);
    double cor =
        xg[0] * 0.5 + xg[1] * 1.7 + xg[2] * xg[2] * 2.5 + xg[3] * xg[3] * 0.2;
...
double check_sum_prop(Propagator4dT<Td>& p0)
{
  ...
  for (Long index = 0; index < Nvol; ++index) {
    ...
    for (Long dc = 0; dc < 12 * 12; dc++) {
      ComplexT<Td> fac =
          ComplexT<Td>(std::cos(cor + dc), std::sin(cor * 5.7 + dc * dc));
...
double diff_FieldM(qlat::FieldM<T, civ>& prop0, qlat::FieldM<T, civ>& prop1,
                   double err = 1e-15)
{
  Int rank = qlat::get_id_node();
  Long MAX_COUNT = 80;
    ...
    for (Int d0 = 0; d0 < civ; d0++) {
      ...
      if ((diff > err or checknan) and countp < MAX_COUNT) {
        printf(
            "rank %5d, x %5d, y %5d, z %5d, t %5d, d %5d, value %+.3e %+.3e, "
            "%+.3e %+.3e, %+.3e \n",
            rank, xg0[0], xg0[1], xg0[2], xg0[3], d0, p0.real(), p0.imag(),
            p1.real(), p1.imag(), diff);
        countp += 1;
      }
    }
  }
