...
inline std::vector<Int> get_map_sec(Int dT, Int nt)
{
  ...
  Qassert(nt % dT == 0);
  Int secN = 2 * nt / dT;
  double lensec = nt / (1.0 * secN);
  Int tcount = 0;
  Int t0 = 0;
  for (Int si = 0; si < secN; si++) {
    for (Int t = t0; t < (si + 1) * lensec;
         t++)  /// boundary with the same sector?
    {
      Qassert(t < nt);
      map_sec[t] = si;
      tcount = tcount + 1;
...
inline void get_map_sec(vector<Int>& map_sec, Int tini, Int dT, Int nt,
                        bool message = false)
{
  TIMER("get_map_sec");
...
inline void get_src_times(vector<Int>& src_t, vector<Int>& src_t_order,
                          vector<Int>& map_sec, const Int tini, const Int dT)
{
  TIMER("get_src_times");
  ...
  for (Int t = 0; t < nt; t++) {
    ...
    const Int secj =
        (seci - sec0 + Nsec) % Nsec;  // sector number shift to zero
...
struct sec_list {
  ...
  Int Nsec;
  bool antiP;
  std::vector<Int> Loop_sec;  // defines the calculation order of sections start
                              // from 0, gap of Nsec / 2
  /*
  ...
  */
  ...
  inline std::vector<Int> get_sinkt(const Int sep)
  {
    ...
    for (Int si = 0; si < Nsec; si++) {
      Int tsink = 0;
      if (si % 2 == 0) {
        tsink = (tini + dT * (si / 2) + sep) % nt;
      }
      if (si % 2 == 1) {
        tsink = (tini + dT * (si / 2 + 1) - sep) % nt;
      }
  ...
  inline void setup()
  {
    ...
    for (Int t = 0; t < nt; t++) {
      ...
      if (antiP) {
        ...
        if (src_time == tk and t > tk and t >= tk + int(dT / 2)) {
          anti_sign[t] = -1;
        }
        //
        // if src is last time and time cross boundary
        if (src_time == tl and t < tl and t < tl - int(dT / 2)) {
          anti_sign[t] = -1;
        }
      }
    }
  ...
  inline void init(const Geometry& geo_, const Int tini_, const Int dT_,
                   const bool antiP_ = false, const bool message = true)
  {
  ...
  inline void update(Int tini_, const bool message = true)
  {
    ...
    // }
    has_sec.resize(
        Nsec);  //// local variable to check whether a sector is within nodes
    for (Int si = 0; si < Nsec; si++) {
      has_sec[si] = 0;
      for (Int t0 = 0; t0 < fd.Nt; t0++) {
        if (map_sec[t0 + fd.init] == si) {
          has_sec[si] = 1;
        }
      }
    }
  ...
  */
  ...
  void bcast_sink_vecs(std::vector<qlat::vector_gpu<Ty>>& data,
                       const std::vector<Int>& host_ti)
  {
    ...
    */
    for (size_t i = 0; i < data.size(); i++) {
      ...
      if (host_ti[i] / fd.Nt ==
          fd.init / fd.Nt)  /// host t could be not the initial time of the node
      {
    ...
    */
    for (size_t i = 0; i < data.size(); i++) {
      ...
      const Int g_rank =
          fd.mi_list[tmi0][fd.get_xyzmi_curr()];  /// each xyz do the bcast to
                                                  /// each other
...
*/
template <typename Ty>
inline void get_src_phases(std::vector<vector<Ty>>& src_phases,
                           const std::vector<std::vector<Coordinate>>& pos_new,
                           const std::vector<Coordinate>& sink_momL,
                           const int sign_sink, const int NsecT,
                           const fft_desc_basic& fd)
{
  ...
  for (int srci = 0; srci < Npsrc; srci++) {
    for (int momi = 0; momi < Nmom; momi++) {
      ...
      for (int si = 0; si < NsecT; si++) {
        ...
        const Coordinate& sp =
            Npsec == 1 ? pos_new[srci][0] : pos_new[srci][si];
        double theta = 0.0;
        // TODO check extra minus sign here
        for (int i = 0; i < 3; i++) {
          theta += (p0[i] * sink_momL[momi][i] * (-1 * sp[i]));
        }
        src_phases[srci * Nmom + momi][si] =
            Ty(cos(theta), sign_sink * sin(theta));
      }
    }
  }
}
...
*/
template <typename Ty>
void shift_results_with_phases(
    std::vector<FieldG<Ty>>& curr_res, const Long Nops,
    const std::vector<std::vector<Coordinate>>& posL,
    const std::vector<Coordinate>& sink_momL, sec_list& sec,
    std::vector<std::vector<FieldG<Ty>>>& curr_shift_buf,
    std::vector<bool>& baryon_list, const int with_phases = 1)
{
  ...
  const int Nsec = sec.Nsec;
  Qassert(Nsec % 2 == 0);
  // const vector<int>& map_sec = sec.map_sec;
  if (with_phases == 1) {
    std::vector<vector<Ty>> src_phases;
    src_phases.resize(0);
      ...
      for (Long srci = 0; srci < Nsrc_tot; srci++) {
        ...
        for (Long momi = 0; momi < Nmom; momi++) {
          ...
          for (Long ri = 0; ri < Nrest; ri++) {
            resP[ri] =
                (Ty*)get_data(curr_res[srci * Nops + momi * Nrest + ri]).data();
          }
          //
          qacc_for(isp, Nvol, {
  ...
  //
  std::vector<FieldG<Ty>> shift_res;
  shift_res.resize(Nops);
  if (need_buf) {
  ...
  for (Long si = 0; si < Nsrc_tot; si++) {
    ...
    if (pos_new[si].size() == 1) {
      ...
      // vector<int>& src_t = sec.src_t;
      //
      // const int stime = pos_new[si][0][3];//shift only first time slice to
      // zero
      ...
      //     const Coordinate xl = geo.coordinate_from_index(isp);
      //     const int ti      = xl[3] + init;
      //     //const int seci    = map_sec[(ti + stime)%nt];// time already
      //     shifted
      //     //const int src_num = ((seci + 1) / 2) % (Nsrc_perT);
      //     //if(src_num == srci)
...
// 2pt shifts
template <typename Ty>
void shift_results_with_phases_2pt(
    qlat::vector_gpu<Ty>& res, const Geometry& geo,
    std::vector<Coordinate>& pos_src, sec_list& sec,
    std::vector<std::vector<FieldG<Ty>>>& shift_buf)
{
  ...
  std::vector<bool> baryon_list;
  baryon_list.resize(32);
  for (Int i = 0; i < 16; i++) {
    baryon_list[i] = false;
  }
  for (Int i = 16; i < 32; i++) {
  ...
  //
  std::vector<std::vector<Coordinate>> posL;
  posL.resize(1);
  for (Int i = 0; i < 1; i++) {
  ...
  std::vector<Coordinate> sink_momL;
  sink_momL.resize(1);
  sink_momL[0] = Coordinate(0, 0, 0, 0);
  //
  std::vector<FieldG<Ty>> curr;
  curr.resize(32 * nmass);
  ...
  std::vector<bool> baryon_list_;
  baryon_list_.resize(1);
  baryon_list_[0] = false;
  shift_results_with_phases(curr, Nops, posL, sink_momL, sec, shift_buf,
                            baryon_list_, 0);
}

template <typename Ty>
void shift_results_with_phases_2pt(
    qlat::vector_gpu<Ty>& res, const Geometry& geo, qlat::PointsSelection& pL,
    sec_list& sec, std::vector<std::vector<FieldG<Ty>>>& shift_buf)
{
  std::vector<Coordinate> pos_src;
  pos_src.resize(pL.size());
  for (unsigned int i = 0; i < pos_src.size(); i++) {
    pos_src[i] = pL[i];
  }
