...
void clear_fields(Fieldy& pr, Int GPU = 1)
{
  ...
  qGPU_for(isp, Ndata, GPU, { r0[isp] = 0.0; });
}
...
template <class Fieldy>
void init_fieldsG(std::vector<Fieldy>& res, Fieldy& src,
...
void copy_fields(T1* pr, const T2* p0, const Int civ, const Geometry& geor,
                 const Geometry& geo0)
{
  TIMER("copy_fields");
  ...
  qacc_for(isp, geor.local_volume(), {
    ...
    const void* src =
        (void*)&p0[d0];  // qlat::get_data(p0.get_elems(xl)).data();
...
template <class T1, class T2>
void copy_fieldsG(std::vector<qlat::FieldG<T1>>& pr,
                  const std::vector<qlat::FieldG<T2>>& p0)
{
  if (pr.size() != p0.size()) {
    pr.resize(0);
    pr.resize(p0.size());
...
void copy_fieldsG(SelectedFieldG<T1>& pr, const SelectedFieldG<T2>& p0)
{
  Qassert(p0.initialized);
  if (!pr.initialized) {
    pr.init(p0.geo(), p0.n_elems, p0.multiplicity,
            get_type_mem(p0.field_gpu.GPU));
  }
  //
  T1* res = (T1*)qlat::get_data(pr).data();
  const T2* src = (T2*)qlat::get_data(p0).data();
...
// norm2 = f^\dagger f
template <class Fieldy>
double fields_quick_checksum(Fieldy& fs, const Long block = 128,
                             const bool log = true)
{
  ...
  res = vec_norm2((qlat::ComplexT<D>*)src, (qlat::ComplexT<D>*)src, Ndata,
                  QMGPU, block);
  if (log) {
...
void fields_operationsGT(std::vector<FieldG<T1>>& pr,
                         std::vector<FieldG<T2>>& p0,
                         std::vector<FieldG<T3>>& p1,
                         const T1 fr = T1(1.0, 0.0), const T1 f0 = T1(1.0, 0.0),
                         const T1 f1 = T1(1.0, 0.0))
{
  TIMER_FLOPS("fields_operationsG");
  Qassert(IsTypeComplex<T1>() and IsTypeComplex<T1>() and IsTypeComplex<T1>());
  const Long nvec = p1.size();
  Qassert(nvec > 0);
  Qassert(p0.size() == p1.size() and p0.size() == pr.size());
  ...
  for (Long iv = 0; iv < nvec; iv++) {
    ...
    Qassert(civ == p0[iv].multiplicity and civ == p1[iv].multiplicity and
            civ == pr[iv].multiplicity);
    if (civ != 1) {
      Qassert(mem_order == p0[iv].mem_order and
              mem_order == p1[iv].mem_order and mem_order == pr[iv].mem_order);
    }
  }
  ...
  long long Tfloat =
      3 * V * civ * sizeof(T1);  // only count the memory bandwith
  timer.flops += Tfloat;
  //
  ...
  qacc_for(isp, V, {
    ...
    if (mem_order == QLAT_DEFAULT) {
      for (Int i = 0; i < nvec; i++)
        for (Int j = 0; j < civ; j++) {
          Pr[i][ir * civ + j] = fr * Pr[i][ir * civ + j] +
                                f0 * P0[i][i0 * civ + j] +
                                f1 * P1[i][i1 * civ + j];
        }
    }
...
//   const Ty f0 = Ty(1.0, 0.0), const Ty f1 = Ty(1.0, 0.0), const Ty f2 =
//   Ty(1.0, 0.0))
//{
...
//
//   Qassert(GetBasicDataType<Fd1>::get_type_name() !=
//   std::string("unknown_type"));
//   Qassert(GetBasicDataType<Fd2>::get_type_name() !=
//   std::string("unknown_type"));
//   Qassert(GetBasicDataType<Fd3>::get_type_name() !=
//   std::string("unknown_type")); using D1 = typename
//   GetBasicDataType<Fd1>::ElementaryType; using D2 = typename
//   GetBasicDataType<Fd2>::ElementaryType; using D3 = typename
//   GetBasicDataType<Fd3>::ElementaryType; Qassert(IsBasicTypeReal<D1>() and
//   IsBasicTypeReal<D2>() and IsBasicTypeReal<D3>());
//
//   // geometry can be extended, but only operated on local numbers
...
template <class T1, class T2, class T3>
void fields_operations(std::vector<FieldG<T1>>& pr, std::vector<FieldG<T2>>& p0,
                       std::vector<FieldG<T3>>& p1, const T1 f0 = T1(1.0, 0.0),
                       const T1 f1 = T1(1.0, 0.0), const T1 f2 = T1(1.0, 0.0))
{
  fields_operationsGT(pr, p0, p1, f0, f1, f2);
}

template <class T1, class T2, class T3, Int civ>
void fields_operations(std::vector<qlat::FieldM<T1, civ>>& pr,
                       std::vector<qlat::FieldM<T2, civ>>& p0,
                       std::vector<qlat::FieldM<T3, civ>>& p1,
                       const T1 f0 = T1(1.0, 0.0), const T1 f1 = T1(1.0, 0.0),
                       const T1 f2 = T1(1.0, 0.0))
{
...
template <class T1, class T2, class T3>
void fields_operations(FieldG<T1>& pr, FieldG<T2>& p0, FieldG<T3>& p1,
                       const T1 f0 = T1(1.0, 0.0), const T1 f1 = T1(1.0, 0.0),
                       const T1 f2 = T1(1.0, 0.0))
{
  std::vector<FieldG<T1>> prG;
...
template <class T1, class T2, class T3, Int civ>
void fields_operations(FieldM<T1, civ>& pr, FieldM<T2, civ>& p0,
                       FieldM<T3, civ>& p1, const T1 f0 = T1(1.0, 0.0),
                       const T1 f1 = T1(1.0, 0.0), const T1 f2 = T1(1.0, 0.0))
{
  std::vector<FieldG<T1>> prG;
  prG.resize(1);
...
// template <class Fd1, class Fd2, class Fd3, class Ty>
// void fields_operations(std::vector<Fd1 >& pr, std::vector<Fd2 >& p0,
// std::vector<Fd3 >& p1, const Ty f0 = Ty(1.0, 0.0), const Ty f1 = Ty(1.0,
// 0.0), const Ty f2 = Ty(1.0, 0.0))
//{
...
// void fields_operationsG(std::vector<qlat::FieldG<T1> >& pr,
// std::vector<qlat::FieldG<T2> >& p0, std::vector<qlat::FieldG<T3> >& p1, const
// Ty f0 = Ty(1.0, 0.0), const Ty f1 = Ty(1.0, 0.0), const Ty f2 = Ty(1.0, 0.0))
//{
//   Qassert(p0.size() == p1.size());
//   if(p0.size() == 0){return ;}
//   //const Geometry& geo = p0[0].geo();
...
template <class T1, class T2, class T3, Int civ>
void fields_additions(std::vector<qlat::FieldM<T1, civ>>& pr,
                      std::vector<qlat::FieldM<T2, civ>>& p0,
                      std::vector<qlat::FieldM<T3, civ>>& p1)
{
...
template <class T1, class T2, class T3, Int civ>
void fields_subtractions(std::vector<qlat::FieldM<T1, civ>>& pr,
                         std::vector<qlat::FieldM<T2, civ>>& p0,
                         std::vector<qlat::FieldM<T3, civ>>& p1)
{
  fields_operations(pr, p0, p1, T1(0.0, 0.0), T1(1.0, 0.0), T1(-1.0, 0.0));
...
template <class T1, class T2, class T3>
void fields_additionsG(std::vector<FieldG<T1>>& pr, std::vector<FieldG<T2>>& p0,
                       std::vector<FieldG<T3>>& p1)
{
...
template <class T1, class T2, class T3>
void fields_subtractionsG(std::vector<FieldG<T1>>& pr,
                          std::vector<FieldG<T2>>& p0,
                          std::vector<FieldG<T3>>& p1)
{
  fields_operationsGT(pr, p0, p1, T1(0.0, 0.0), T1(1.0, 0.0), T1(-1.0, 0.0));
...
// void fields_additionsG(std::vector<Fd1 >& pr, std::vector<Fd2 >& p0,
// std::vector<Fd3 >& p1)
//{
...
//
// template <class Fd1, class Fd2, class Fd3>
// void fields_subtractionsG(std::vector<Fd1 >& pr, std::vector<Fd2 >& p0,
// std::vector<Fd3 >& p1)
//{
//   using Ty = ComplexT<double>;
//   fields_operations(pr, p0, p1, Ty(0.0, 0.0), Ty( 1.0, 0.0), Ty(-1.0, 0.0));
...
void fields_equal(std::vector<qlat::FieldM<T1, civ>>& pr,
                  std::vector<qlat::FieldM<T2, civ>>& ps)
{
...
void switch_orders(M& f, const QMEM_ORDER order_)
{
  ...
  if (f.mem_order == QLAT_DEFAULT and order_ == QLAT_OUTTER) {
    mv_idx.move_civ_out(f.field_gpu.p, f.field_gpu.p, 1, Nd, Dim, 1,
                        f.field_gpu.GPU);
  ...
  if (f.mem_order == QLAT_OUTTER and order_ == QLAT_DEFAULT) {
    mv_idx.move_civ_in(f.field_gpu.p, f.field_gpu.p, 1, Dim, Nd, 1,
                       f.field_gpu.GPU);
...
void fields_operations_localG(std::vector<qlat::FieldG<T1>>& pr,
                              std::vector<qlat::FieldG<T2>>& ps,
                              const Ty f0 = Ty(1.0, 0.0))
{
