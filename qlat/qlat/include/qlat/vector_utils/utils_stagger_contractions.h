...
struct stag_inv_buf {
  ...
  inline void init() { free_buf(); }
};

////tr[cf cf^\dagger]
template <class Ty>
void cf_simple_pion(std::vector<colorFT>& cf0, std::vector<colorFT>& cf1,
                    EigenV& corr, qlat::fft_desc_basic& fd, Int clear = 1,
                    bool print = false, double factor = 1.0)
{
  ...
  EigenV resV;
  ini_resE(resV, 1, fd);
  if (resV.size() % NTt != 0 or resV.size() == 0) {
  ...
  d1.resize(Dim);
  for (Int c = 0; c < Dim; c++) {
    d0[c] = (Ty*)qlat::get_data(cf0[c]).data();
    d1[c] = (Ty*)qlat::get_data(cf1[c]).data();
  }
  //
  qacc_for(isp, geo.local_volume(), {
    for (Int c0 = 0; c0 < Dim; c0++)
      for (Int c1 = 0; c1 < Dim; c1++) {
        resV[isp] +=
            d0[c0][isp * Dim + c1] * qlat::qconj(d1[c0][isp * Dim + c1]);
      }
  });
  //
