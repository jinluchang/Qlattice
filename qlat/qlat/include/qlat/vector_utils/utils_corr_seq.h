...
void local_sequential_source(qlat::FieldM<Ty, civ>& src,
                             const qlat::vector<Int>& tseq,
                             const Int order_data = 0)
{
...
template <class Td>
void local_sequential_source(Propagator4dT<Td>& res, Propagator4dT<Td>& src,
                             const qlat::vector<Int>& tseq,
                             const Int gammai = -1)
{
  TIMERA("local_sequential_source");
  ...
  qacc_for(xi, long(Nvol), {
    for (Int ti = 0; ti < Nt; ti++) {
      ...
      for (long tj = 0; tj < tseq.size(); tj++) {
        if (pos[3] == (tseq[tj]) % nt) {
          for (Int ic = 0; ic < Dim; ic++) {
            resP[isp * Dim + ic] = srcP[isp * Dim + ic];
          }
        }
      }
    }
