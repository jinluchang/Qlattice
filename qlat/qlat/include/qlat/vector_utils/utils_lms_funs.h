...
struct lms_para {
  ...
  std::vector<qlat::FieldG<Ty>>
      buf_res_props;  // buffer for sink to current results, size Ngroup * Nlh
  ...
  */
  ...
  ////buffers
  qlat::vector<Ty> EresH;
  qlat::vector<Ty> EresL;
  qlat::vector<Ty> EresA;
  ...
  inline void free_buf()
  {
    ...
    buf_res_props.resize(
        0);  // buffer for sink to current results, size Ngroup * Nlh
  ...
  inline void init()
  {
    ...
    //
    if (ionum == 8) {
      new_size_node = Coordinate(1, 2, 2, 2);
    }
    if (ionum == 16) {
      new_size_node = Coordinate(1, 2, 2, 4);
    }
    if (ionum == 32) {
      new_size_node = Coordinate(2, 2, 2, 4);
    }
    //
  ...
  inline void set_eig(eigen_ov* eig, const double mass, const Int mode_sm,
                      const Int one_minus = 1)
  {
    Qassert(mass >= 0.0);
    Qassert(mode_sm >= 0);
    Qassert(one_minus == 1);
  ...
  inline void print()
  {
    ...
    qmessage("  init %5d %5d %5d %5d, grid %5d %5d %5d %5d \n", ini_pos[0],
             ini_pos[1], ini_pos[2], ini_pos[3], off_L[0], off_L[1], off_L[2],
             off_L[3]);
    //
    Int save_vecs = 0;
    if (name_mom_vecs != std::string("NONE")) {
      save_vecs = 1;
    }
    qmessage(
        "eigen mode sm %1d, src %5d %.3f, sink %5d %.3f , save_low %2d, "
        "mom_cut %5d , saveV %1d \n",
        mode_eig_sm, src_smear_iter, src_smear_kappa, sink_smear_iter,
        sink_smear_kappa, do_all_low, mom_cut, save_vecs);
    // qmessage("  mom vecs %s \n" ,name_mom_vecs);
...
//   const Int nvec, qlat::vector<Long >& mapA, qlat::vector<Long >& mapB, const
//   Geometry& geo)
//{
//   TIMER("save FFT");
...
template <typename Ty>
void save_sink_vecs(qlat::vector_gpu<Ty>& src, const Geometry& geo,
                    const Int nmass, lms_para<Ty>& srcI,
                    const std::string& GRID_INFO, const std::string tag = "")
{
  ...
  if (Long(Vzero_data.size()) != nvec) {
    Vzero_data.resize(0);
    Vzero_data.resize(nvec);
    for (Long iv = 0; iv < nvec; iv++) {
      if (!Vzero_data[iv].initialized) {
        Vzero_data[iv].init(geo);
      }
    }
  }
  for (Long iv = 0; iv < nvec; iv++) {
...
template <typename Ty>
void Save_sparse_prop(std::vector<qlat::vector_gpu<Ty>>& src,
                      lms_para<Ty>& srcI, const std::string& tag,
                      const Int count, const bool save_files = false)
{
  ...
  if (save_files) {
    ...
    for (Int mi = 0; mi < Nmass; mi++) {
      const std::string nameQ =
          ssprintf("%s.mass%02d", srcI.name_sparse_prop.c_str(), mi);
...
void point_corr(qnoiT& src, std::vector<qpropT>& propH,
                std::vector<double>& massL, eigen_ov& ei, fft_desc_basic& fd,
                corr_dat<Ta>& res, lms_para<Ty>& srcI, momentum_dat& mdat,
                sec_list& sec, Int shift_t = 1, const Int Zn = 3)
{
  TIMER("point corr");
  print_time();
  print_mem_info("Before point_corr");
  if (propH.size() != massL.size()) {
    abort_r("prop size, mass size not match!\n");
  }
  ////int Ns = src.size();
  ...
  std::vector<std::vector<Coordinate>> Ngrid;
  {
    ...
    if (srcI.sparse_src == 0) {
      ...
      /*
        ...
        For somewhere else when sparsening is used for both sink and current,
        src should be sorted by grid to have well defined source momentum
        projections
      */
      ...
      for (LInt gi = 0; gi < grids.size(); gi++) {
        ...
        if (std_find(tlist, tini) < 0) {
          tlist.push_back(tini);
        }
      }
      // sparsen one could not do  combineT
      if (srcI.combineT == 1) {
        Qassert(tlist.size() == 1)
      }
    }
  }
  ...
  std::vector<Int> tsrcL;
  tsrcL.resize(0);
  const int lms_show_all =
      qlat::get_env_long_default(std::string("qlat_lms_show_all_points"), 0);
  ...
  bool saveFFT = false;
  bool savezero = false;
  bool save_zero_corr = false;
  if (srcI.name_mom_vecs != std::string("NONE")) {
    saveFFT = true;
  }
  if (srcI.name_zero_vecs != std::string("NONE")) {
    savezero = true;
  }
  if (srcI.save_zero_corr == 1) {
    save_zero_corr = true;
  }
  ...
  if (srcI.check_prop_norm) {
    qmessage("===high norm 0 %ld , ", long(high_prop.size()));
  ...
  // qlat::vector_gpu<Ty > FFT_data;Long Nfdata =
  // Long(32)*massL.size()*fd.nt*mc*mc*mc ; qlat::vector_gpu<Ty > FFT_data_high;
  ...
  if (srcI.name_sparse_prop != std::string("NONE")) {
    ...
    if (!srcI.sf_single.initialized) {
      ini = true;
    }
    if (srcI.sfL.size() == 0) {
      ini = true;
    } else {
      if (long(srcI.sfL.size()) != nmass) {
        ini = true;
      }
      for (Int mi = 0; mi < nmass; mi++) {
        if (srcI.sfL[mi].multiplicity != Nsparse * Ndc) {
          ini = true;
        }
      }
    }
  ...
  if (srcI.SRC_PROP_WITH_LOW == 1) {
    ...
    if (srcI.check_prop_norm) {
      qmessage("===src  norm 0 %ld , ", long(stmp.size()));
      stmp.print_norm2();
    }
    prop_L_device(ei, stmp.data(), low_prop.data(), 12, massL, srcI.mode_eig_sm,
                  tsrcL);
    if (srcI.check_prop_norm) {
      qmessage("===low  norm 0 %ld , ", long(low_prop.size()));
      low_prop.print_norm2();
    }
    high_prop -= low_prop;
  }
  //
  if (srcI.check_prop_norm) {
    qmessage("===high norm 1 %ld , ", long(high_prop.size()));
    high_prop.print_norm2();
  }
  // qmessage("===check norm");low_prop.print_norm2();high_prop.print_norm2();
  /////reduce the high prop
  ...
  */
  if (do_hadron_contra) {
    ...
    if (srcI.check_prop_norm) {
      qmessage("===resTa norm 0 %ld , ", long(resTa.size()));
      resTa.print_norm2();
    }
    if (savezero) {
  ...
  POS_CUR = std::string("");
  write_pos_to_string(POS_CUR, pos);
  POS_LIST += POS_CUR;
  //
  print_mem_info();
  //
  for (Int gi = 0; gi < Nlms; gi++) {
    ...
    //
    POS_CUR = std::string("");
    write_pos_to_string(POS_CUR, Ngrid[gi][0]);
    POS_LIST += POS_CUR;
    ...
    if (srcI.do_all_low == 1) {
      ...
      copy_eigen_prop_to_EigenG(Eprop, low_prop.data(), ei.b_size, nmass, fd,
                                GPU);
      std::string sparse_tag = "";
      if (gi == Nlms - 1) {
        sparse_tag = ssprintf("Low%04d", Nlms);
      }
      ...
      if (do_hadron_contra) {
        ...
        if (saveFFT and srcI.save_low_fft == 1) {
          ...
          if (srcI.average_all_2pt != 1) {
            ...
            shift_results_with_phases_2pt(resTa, geo, Ngrid[gi], sec,
                                          srcI.curr_shift_buf);
            if (resLL.size() == 0) {
    ...
    // qmessage("===check norm");low_prop.print_norm2();high_prop.print_norm2();
    if (srcI.check_prop_norm) {
      qmessage("===low  norm 1 %ld , ", long(low_prop.size()));
      low_prop.print_norm2();
    }
    //
    //////format suitable for contractions
  ...
  if (srcI.average_all_2pt == 1 and saveFFT) {
    ...
    //
    shift_results_with_phases_2pt(resHH, geo, Ngrid[0], sec,
                                  srcI.curr_shift_buf);
  ...
  // save info
  {
    ...
    {
      key_T = ssprintf("%d  %d  %d %d %d %d", int(massL.size()), fd.nt, mc, mc,
                       mc, 2);
  ...
  if (do_hadron_contra) {
    if (savezero and srcI.save_full_vec == 0) {
      ...
      // for(Long iv=0;iv<nvec;iv++){
      //   Ty* resP = (Ty*) qlat::get_data(Vzero_data[iv]).data();
      //   cpy_data_thread(resP, &resZero[iv*Vol], geo.local_volume(), 1,
      //   QTRUE);
      // }
      //
    ...
    if (save_zero_corr) {
      ...
      cpy_data_thread(EresA.data(), EresH.data(), EresA.size(), 1, QTRUE,
                      -1.0 * Nlms);
...
template <typename Ty>
void test_all_prop_corr(std::vector<double>& massL, eigen_ov& ei,
                        fft_desc_basic& fd, corr_dat<Ty>& res, Int mode_sm = 0)
{
  ...
  for (Int i = 0; i < Eres.size(); i++) {
    Eres[i] = Eres[i] / (Ftype(fd.nx * fd.ny * fd.nz * 1.0));
  }
