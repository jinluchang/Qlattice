...
struct FFT_Vecs {
  ...
  void set_plan(std::vector<Int>& nv_set, Int civ_set = 2,
                std::vector<size_t> MPI_para_set = std::vector<size_t>());
...
template <typename Ty>
void FFT_Vecs::set_plan(std::vector<Int>& nv_set, Int civ_set,
                        std::vector<size_t> MPI_para_set)
{
  ...
  Int do_clear = 0;
  if (do_clear == 0 and nv_set != nv) {
    do_clear = 1;
  }
  /// if(do_clear == 0 and biva_set != biva){do_clear = 1;}
  if (do_clear == 0 and civ_set != civ) {
    do_clear = 1;
  }
  if (do_clear == 0 and sizeof(Ty) != bsize) {
    do_clear = 1;
  }
  if (do_clear == 0 and MPI_para_set != MPI_para) {
    do_clear = 1;
  }
  //
  if (flag_mem_set == true and do_clear == 0) {
    return;
  } else {
    if (do_clear == 1) {
      clear_plan();
...
#ifdef QLAT_USE_ACC
    if (MPI_para.size() != 0) {
      abort_r("GPU FFT MPI version not supported. \n");
    }
#endif
    //
  ...
  Int istride = howmany * istride_fac_default;
  int idist = idist_default;
  ...
  if (GPU) {
    ...
    if (dim == 4) {
      abort_r("dim 4 on GPU not supported! \n");
    }
    //
    ...
    //
    qacc_FFT_CALL(qacc_fftPlanMany(&plan_gpu, dim, &nv[0], &nv[0], istride,
                                   idist, &nv[0], istride, idist, cutype,
                                   howmany));
//
/////====GPU parts
#endif
    ...
    if (MPI_para.size() == 3) {
      ...
      if (single_type == 0) {
        alloc_local = fftw_mpi_local_size_many(
            dim, nrank, howmany, block0, fft_comm, &local_n0, &local_0_start);
        fft_dat = fftw_malloc(alloc_local * bsize);
        plan_cpuD0 = fftw_mpi_plan_many_dft(
            dim, nrank, howmany, block0, block0, (fftw_complex*)fft_dat,
            (fftw_complex*)fft_dat, fft_comm, FFTW_FORWARD, fftw_plan_flag);
        plan_cpuD1 = fftw_mpi_plan_many_dft(
            dim, nrank, howmany, block0, block0, (fftw_complex*)fft_dat,
            (fftw_complex*)fft_dat, fft_comm, FFTW_BACKWARD, fftw_plan_flag);
      }
      //
      if (single_type == 1) {
        ...
        plan_cpuF0 = fftwf_mpi_plan_many_dft(
            dim, nrank, howmany, block0, block0, (fftwf_complex*)fft_dat,
            (fftwf_complex*)fft_dat, fft_comm, FFTW_FORWARD, fftw_plan_flag);
        plan_cpuF1 = fftwf_mpi_plan_many_dft(
            dim, nrank, howmany, block0, block0, (fftwf_complex*)fft_dat,
            (fftwf_complex*)fft_dat, fft_comm, FFTW_BACKWARD, fftw_plan_flag);
      }
      //
      /////each node has data vol/("Nv[0]")
      MPI_datasize = datasize / (nrank[0] / block0);
      if (local_0_start != ranku or local_n0 != block0) {
        abort_r("fft_mpi not correct !\n");
      }
#else
      ...
      if (single_type == 0) {
        plan_cpuD0 = fftw_plan_many_dft(
            ...
            idist, (fftw_complex*)fft_dat, &nv[0], istride, idist, FFTW_FORWARD,
            fftw_plan_flag);
        plan_cpuD1 = fftw_plan_many_dft(
            dim, &nv[0], howmany, (fftw_complex*)fft_dat, &nv[0], istride,
            idist, (fftw_complex*)fft_dat, &nv[0], istride, idist,
            FFTW_BACKWARD, fftw_plan_flag);
      }
      //
      if (single_type == 1) {
        plan_cpuF0 = fftwf_plan_many_dft(
            dim, &nv[0], howmany, (fftwf_complex*)fft_dat, &nv[0], istride,
            idist, (fftwf_complex*)fft_dat, &nv[0], istride, idist,
            FFTW_FORWARD, fftw_plan_flag);
        plan_cpuF1 = fftwf_plan_many_dft(
            dim, &nv[0], howmany, (fftwf_complex*)fft_dat, &nv[0], istride,
            idist, (fftwf_complex*)fft_dat, &nv[0], istride, idist,
            FFTW_BACKWARD, fftw_plan_flag);
      }
...
void FFT_Vecs::do_fft(Ty* inputD, bool fftdir, bool dummy)
{
  ...
  if (GPU == true) {
    ...
    if (single_type == 0) {
      if (fftdir == true)
        qacc_FFT_CALL(qacc_fftExecZ2Z(plan_gpu, (qacc_fftDoubleComplex*)fft_dat,
                                      (qacc_fftDoubleComplex*)fft_dat,
                                      qacc_FFT_FORWARD));
      if (fftdir == false)
        qacc_FFT_CALL(qacc_fftExecZ2Z(plan_gpu, (qacc_fftDoubleComplex*)fft_dat,
                                      (qacc_fftDoubleComplex*)fft_dat,
    ...
    if (single_type == 1) {
      if (fftdir == true)
        qacc_FFT_CALL(qacc_fftExecC2C(plan_gpu, (qacc_fftComplex*)fft_dat,
                                      (qacc_fftComplex*)fft_dat,
                                      qacc_FFT_FORWARD));
      if (fftdir == false)
        qacc_FFT_CALL(qacc_fftExecC2C(plan_gpu, (qacc_fftComplex*)fft_dat,
                                      (qacc_fftComplex*)fft_dat,
...
//
    ...
    //
      ...
      if (single_type == 0) {
        if (fftdir == true)
          fftw_execute_dft(plan_cpuD0, (fftw_complex*)fft_dat,
                           (fftw_complex*)fft_dat);
        if (fftdir == false)
          fftw_execute_dft(plan_cpuD1, (fftw_complex*)fft_dat,
                           (fftw_complex*)fft_dat);
      }
      //
      if (single_type == 1) {
        if (fftdir == true)
          fftwf_execute_dft(plan_cpuF0, (fftwf_complex*)fft_dat,
                            (fftwf_complex*)fft_dat);
        if (fftdir == false)
          fftwf_execute_dft(plan_cpuF1, (fftwf_complex*)fft_dat,
                            (fftwf_complex*)fft_dat);
      }
    }
    //
    if (MPI_para.size() != 0)
      cpy_data_thread(inputD, (Ty*)fft_dat, MPI_datasize / bsize, GPU);
  }
  //
...
struct customLess {
                         ...
                         const std::vector<Int>& b) const
  {
    ...
    if (a[3] == b[3] and a[2] == b[2] and a[0] < b[0]) {
      flag = true;
    }
    //////need copy or not
    ...
    // if(a[3] == b[3] and a[4] == b[4] and a[2] == b[2] and a[0] < b[0]){flag =
    // true;}
    return flag;
  }
};

inline std::vector<Int> get_factor_jobs(Int nvec, Int civ, Int N0 = -1,
                                        Int N1 = -1, Int maxN0 = 16,
                                        Int dataB = 1)
{
  if (nvec < 0 or civ < 0 or maxN0 <= 0) {
    abort_r("Vectors wrong. \n");
  }
  if (N1 != -1) {
    if (N0 < N1 or N0 % N1 != 0) {
      abort_r("Vectors wrong. \n");
  ...
  if (jobL.size() == 0) {
    abort_r("input errors for schedule! \n");
  }
  //
...
struct fft_schedule {
  ...
  fft_schedule(fft_desc_basic& fd_set, bool GPU_set = true)
      : fd(), rot(fd_set, GPU_set), fft(GPU_set), fft_gpu(GPU_set)
  {
#ifndef QLAT_USE_ACC
    GPU = false;
#else
  ...
  template <typename Ty>
  void set_mem(const Int nvec_set, const Int civ_set,
               const std::vector<Int>& dimN_set = std::vector<Int>(),
               Int default_MPI_set = -1, Int maxN_set = 16, Int dataB_set = 1)
  {
    TIMERA("fft_schedule set mem");
    if (nvec == nvec_set and civ == civ_set and
        default_MPI == default_MPI_set and maxN == maxN_set and
        dataB == dataB_set) {
      if (dimN_set.size() == 0 and dimN.size() == 0) {
        abort_r("Dimension not set. \n");
      }
      if (dimN_set.size() == 0 or dimN == dimN_set) {
        return;
      }
    }
    bsize = sizeof(Ty);
    ...
    dim = dimN.size();
    if (dim < 3 or dim > 4) {
      abort_r("Dimension not supported. \n");
    }
    //
    civ = civ_set;
    nvec = nvec_set;
    default_MPI = default_MPI_set;
    if (!(default_MPI >= -3 and default_MPI < 2)) {
      abort_r("Wrong default_MPI ! \n");
    };
    ////Long total  = nvec*civ;
    N_extra = -1;
    int N0 = -1;
    ...
    if (dim == 3) {
      N0 = fd.mz * fd.my * fd.mx;
      N1 = 1;
    }
    if (dim == 4) {
      N0 = fd.mt * fd.mz * fd.my * fd.mx;
      N1 = fd.mz * fd.my * fd.mx;
    }
    //
    /////====Set up b0, c0
    ...
    if (default_MPI == -2 or default_MPI == -3) {
      ...
      //
      if (default_MPI == -1 and dim == 3 and fd.my * fd.mx != 1) {
        enable_MPI = 0;
      }
      //
      if (enable_MPI == -1) {
        if (GPU) {
          ...
          if (default_MPI == -1) {
            job = get_factor_jobs(nvec, civ, N0, N1, maxN, dataB);
            enable_MPI = job[0];
          } else {
    ...
    ////qmessage("job size %d \n", int(job.size()));
    if (enable_MPI == 1 and dim == 3 and fd.my * fd.mx != 1) {
      abort_r("mode not supported ! \n");
    }
    if (!(enable_MPI == 0 or enable_MPI == 1)) {
      abort_r("enable_MPI not set yes! \n");
    };
    //
    b0 = job[1];
...
//
    ...
    //
    if (GPU) {
      Qassert(enable_MPI == 0);
    }
    NEED_COPY = 0;
    if (civ % (N_extra * c0) != 0 or GPU) {
      NEED_COPY = 1;
    }
    //
    Int mode_rot = -2;
    ...
    if (enable_MPI == 1) {
      ...
      if (dim == 4) {
        MPI_para[0] = fd.Nt;
        MPI_para[1] =
            (fd.iniz * fd.ny + fd.iniy) * fd.nx + fd.inix;  ////colorxyz for MPI
  ...
  inline void print_info()
  {
    ...
    qmessage(
        "==N_extra %d, enable_MPI %d, GPU %d, b0 %d, c0 %d, mode_rot %d, need "
        "copy %d. \n",
  ...
  void call_fft(Ty* src, bool fftdir = true)
  {
    ...
    if (GPU == true and dim == 4) {
      flag_GPU_4d = true;
    }
    if (!flag_GPU_4d) {
      if (dim == 3) {
        for (Int ti = 0; ti < fd.Nt; ti++) {
          tsrc = &src[ti * (rot.vol_buf / fd.Nt) * c0];
          fft.do_fft(tsrc, fftdir, false);
        }
        qacc_barrier(dummy);
      }
      if (dim == 4) {
        tsrc = src;
        fft.do_fft(tsrc, fftdir);
      }
      ////if(dim==4){for(Int ti=0;ti<fd.nt;ti++){tsrc=&src[ti *
      ///(fd.nz*fd.ny*fd.nx) *c0]; fft.do_fft(tsrc, fftdir, false);}}
    }
    //
    if (flag_GPU_4d) {
      for (Int ti = 0; ti < fd.nt; ti++) {
        tsrc = &src[ti * (fd.nz * fd.ny * fd.nx) * c0];
        fft.do_fft(tsrc, fftdir, false);
      }
      qacc_barrier(dummy);
      tsrc = src;
      fft_gpu.do_fft(tsrc, fftdir);
    }
    tsrc = NULL;
  }
  ...
  void dojob(std::vector<Ty*>& data, bool fftdir = true, Int civ_set = -1)
  {
    ...
    ////update nvec, civ
    nvec = bN;
    if (civ_set != -1) civ = civ_set;
    //
    /////if(civ_set != -1){set_mem<Ty>(bN, civ);}
    if (!flag_mem_set or b0 < 0 or c0 < 0) {
      abort_r("Memory not set for fft_schedule! \n");
    }
    if (rot.flag_mem_set != true or fft.flag_mem_set != true) {
      abort_r("Memory not set for fft_schedule! \n");
    }
    if (civ % c0 != 0) {
      abort_r("civ cannot by divided by c0 ! \n");
    }
    //
    Int cN = civ / c0;
    ...
    if (civ != c0) {
      ...
      for (Int bi = 0; bi < bN; bi++)
        mv_civ.dojob((Ty*)&data[bi][0], (Ty*)&data[bi][0], 1, cN, fd.noden, 1,
                     c0, GPU);
    }
    //
    NEED_COPY = 0;
    ...
    for (Int ji = 0; ji < jobN; ji++) {
      ...
      if (NEED_COPY == 1) {
        ...
        for (Int bi = 0; bi < b0 * N_extra; bi++) {
          ...
          cpy_data_thread(&src[bi * fd.noden * c0], dp, fd.noden * c0, GPU,
                          QFALSE);
          cma += c0;
          if (cma == civ) {
            cma = 0;
        ...
        for (Int bi = 0; bi < b0 * N_extra; bi++) {
          ...
          cpy_data_thread(dp, &src[bi * fd.noden * c0], fd.noden * c0, GPU,
                          QFALSE);
          cm += c0;
          if (cm == civ) {
            cm = 0;
    ...
    if (civ != c0) {
      TIMERB("fft mem reorder");
      for (Int bi = 0; bi < bN; bi++)
        mv_civ.dojob((Ty*)&data[bi][0], (Ty*)&data[bi][0], 1, cN, fd.noden, 0,
                     c0, GPU);
    }
  }
  ...
  ~fft_schedule() { clear_mem(); }
};

template <class Ty, Int civ>
void fft_fieldM_test(std::vector<qlat::FieldM<Ty, civ>>& src,
                     bool fftdir = true, bool GPU = true, bool fft4D = false)
{
  TIMERD("fft fieldM");
  ...
  for (Int si = 0; si < nvec; si++) {
    data[si] = (Ty*)qlat::get_data(src[si]).data();
  }
  /////3D
  if (!fft4D) {
    dimN.resize(3);
...
template <class Ty, Int civ>
void fft_fieldM(fft_schedule& fft, std::vector<qlat::FieldM<Ty, civ>>& src,
                bool fftdir = true)
{
#if PRINT_TIMER > 4
...
struct fft_gpu_copy {
  ...
  const fft_gpu_copy& operator=(const fft_gpu_copy& fft)
  {
    ...
    //
    if (prec == "RealD") {
      fftP->set_mem<ComplexD>(nvec, civ, dimN, -1);
    } else if (prec == "RealF") {
      fftP->set_mem<ComplexF>(nvec, civ, dimN, -1);
    } else {
      qmessage("Only ComplexD and ComplexF supported for fft on GPU! \n");
      Qassert(false);
    }
    /// fft.fftP->print_info();
    /// fftP->print_info();
  ...
  inline void clear()
  {
    if (fftP != NULL and is_copy == false) {
      delete (fftP);
      fftP = NULL;
    }
  }
...
{
  if (x.geo.total_site() < y.geo.total_site()) {
    return true;
  }
  if (y.geo.total_site() < x.geo.total_site()) {
    return false;
  }
  //
  if (x.GPU < y.GPU) {
...
inline fft_gpu_copy make_fft_gpu_plan(const Geometry& geo, Int nvec, Int civ,
                                      bool GPU, bool fft4D,
                                      const std::string& prec)
{
  ...
  //
  if (prec == "RealD") {
    ft.fftP->set_mem<ComplexD>(nvec, civ, dimN, -1);
  } else if (prec == "RealF") {
    ft.fftP->set_mem<ComplexF>(nvec, civ, dimN, -1);
  } else {
    qmessage("Only ComplexD and ComplexF supported for fft on GPU! \n");
    Qassert(false);
  }
  ...
  // qlat::get_data(src[si]).data();} fft.dojob(data, fftdir);
  //
  return ft;
}
...
{
  return make_fft_gpu_plan(fkey.geo, fkey.nvec, fkey.civ, fkey.GPU, fkey.fft4D,
                           fkey.prec);
}
...
inline FFTGPUPlanKey get_fft_gpu_plan_key(
    std::vector<qlat::FieldM<Ty, civ>>& src, bool fft4d = false)
{
  Qassert(src.size() > 0);
  FFTGPUPlanKey fkey;
  fkey.geo = src[0].geo();
...
inline bool check_fft_mode(const Int nfft, const Geometry& geo,
                           const bool fft4d)
{
  bool use_qlat = false;
  std::vector<Int> nv, Nv, mv;
...
template <class Ty>
void fft_fieldM(std::vector<Ty*>& data, Int civ, const Geometry& geo,
                bool fftdir = true, bool fft4d = false)
{
...
template <class Ty>
void fft_fieldM(Ty* src, Int nvec, Int civ, const Geometry& geo,
                bool fftdir = true, bool fft4d = false)
{
  std::vector<Ty*> data;
  data.resize(nvec);
...
template <class Ty, Int civ>
void fft_fieldM(std::vector<qlat::FieldM<Ty, civ>>& src, bool fftdir = true,
                bool fft4d = false)
{
  if (src.size() < 1) return;
  //
...
template <class M>
void fft_fieldM(std::vector<Handle<qlat::Field<M>>>& src, bool fftdir = true,
                bool fft4d = false)
{
  if (src.size() < 1) return;
  const Geometry& geo = src[0]().geo();
  ...
  // if( is_double){prec = ComplexD_TYPE ; civ = multiplicity *
  // sizeof(M)/sizeof(ComplexD ); } if(!is_double){prec = ComplexF_TYPE; civ =
  // multiplicity * sizeof(M)/sizeof(ComplexF); }
  //
  ...
  //
  {
    ...
    TIMER_FLOPS("fft fieldM");
    timer.flops +=
        src.size() * qlat::get_data(src[0]()).data_size() / (sizeof(M)) * 64;
#else
    TIMER("fft fieldM");
#endif
    //
    ...
    if (is_double) {
      ...
      data.resize(nvec);
      for (Int si = 0; si < nvec; si++) {
        data[si] = (ComplexD*)qlat::get_data(src[si]()).data();
      }
      get_fft_gpu_plan(fkey).fftP->dojob(data, fftdir);
    }
    //
    if (!is_double) {
      std::vector<ComplexF*> data;
      data.resize(nvec);
      for (Int si = 0; si < nvec; si++) {
        data[si] = (ComplexF*)qlat::get_data(src[si]()).data();
      }
      get_fft_gpu_plan(fkey).fftP->dojob(data, fftdir);
    }
  }
...
template <typename Ty>
void FFT_vecs_corr(qlat::vector_gpu<Ty>& src,
                   std::vector<qlat::FieldM<Ty, 1>>& FFT_data, Int off = 0)
{
  const Geometry& geo = FFT_data[0].geo();
  const Long volume = geo.local_volume();
  const Long Sdata = src.size();
  const Int nvecs = Sdata / volume;
  //
  fft_fieldM<Ty>(src.data(), nvecs, 1, geo, true);
  for (Int iv = 0; iv < nvecs; iv++) {
    qlat::FieldM<Ty, 1>& buf = FFT_data[off * nvecs + iv];
    Qassert(buf.initialized);
    Ty* s0 = &src[iv * volume];
                ...
                bool fft4d = false)
{
  ...
  for (unsigned int si = 0; si < src.size(); si++) {
    Qassert(src[si].initialized and src[si].multiplicity == civ and
