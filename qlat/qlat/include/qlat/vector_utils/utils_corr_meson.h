...
void meson_vectorE(std::vector<Propagator4dT<Td>*>& pV1,
                   std::vector<Propagator4dT<Td>*>& pV2, ga_M& ga1, ga_M& ga2,
                   qlat::ComplexT<Td>* res, qlat::fft_desc_basic& fd,
                   Int invmode = 1)
{
  TIMER("Meson_vectorE");
        ...
        for (Int c1 = 0; c1 < 3; c1++)
          for (Int d2 = 0; d2 < 4; d2++) {
            ...
            for (Int c2 = 0; c2 < 3; c2++) {
              if (invmode == 1) {
                pres += g_tem * p1(ga1.ind[d1] * 3 + c1, d2 * 3 + c2) *
                        qlat::qconj(p2(d1 * 3 + c1, ga2.ind[d2] * 3 + c2));
              }
              if (invmode == 0) {
                pres += g_tem * p1(ga1.ind[d1] * 3 + c1, d2 * 3 + c2) *
                        (p2(d1 * 3 + c1, ga2.ind[d2] * 3 + c2));
              }
            }
          }
...
template <typename Td>
void meson_vectorE(Propagator4dT<Td>& pV1, Propagator4dT<Td>& pV2, ga_M& ga1,
                   ga_M& ga2, qlat::ComplexT<Td>* res, Int clear = 1,
                   Int invmode = 1)
{
...
void meson_vectorE(std::vector<qpropT>& prop1, std::vector<qpropT>& prop2,
                   ga_M& ga1, ga_M& ga2, qlat::vector<Ty>& res, Int clear = 1,
                   Int invmode = 1, const Ty factor = Ty(1.0, 0.0))
{
  TIMER("Meson_vectorE");
...
//
            ...
            if (invmode == 1) {
              qacc_forNB(i, Long(Nxyz), {
                tr0[i] += factor * (tp1[i] * qlat::qconj(tp2[i]) * g_tem);
              });
            }
            if (invmode == 0) {
              qacc_forNB(i, Long(Nxyz),
                         { tr0[i] += factor * (tp1[i] * (tp2[i]) * g_tem); });
            }
#else
            EAy vp1(tp1, Nxyz);
            EAy vp2(tp2, Nxyz);
...
__global__ void meson_vectorEV_global(Ty** p1, Ty** p2, Ty** resP, int8_t** gPP,
                                      uint8_t** oPP, const Int* ivP,
                                      const Int nmass, const Int NTt,
    ...
    for (Int iv = 0; iv < Ngv; iv++) {
      ...
      for (Int ji = 0; ji < jobN; ji++) {
        ...
        while (off < dv) {
          ...
          if (bfacC == 1) {
            if (invmode == 0) {
              buf[aa * bfac + ba] += (t1[0] * (t2[0]) * gtem);
            }
            if (invmode == 1) {
              buf[aa * bfac + ba] += (t1[0] * qlat::qconj(t2[0]) * gtem);
            }
          } else {
            Ty* b0 = &buf[aa * bfac + ba];
            for (Int bz = 0; bz < bfacC; bz++) {
              if (invmode == 0) {
                b0[bz] += (t1[bz] * (t2[bz]) * gtem);
              }
              if (invmode == 1) {
                b0[bz] += (t1[bz] * qlat::qconj(t2[bz]) * gtem);
              }
            }
          }
          off += Nth * bfacC;
...
void meson_vectorEV_kernel(Ty** p1, Ty** p2, Ty** resP, int8_t** gPP,
                           uint8_t** oPP, const Int* ivP, const Int nmass,
                           const Int NTt, const Long Nxyz, const Int Ngv)
{
  Long Ntotal = nmass * NTt * Nxyz;
  if (Ntotal % bfac != 0) {
    abort_r("Please correct your bfac! \n");
  }
  ...
  meson_vectorEV_global<Ty, invmode, bfac, Blocks><<<dimGrid, dimBlock>>>(
      p1, p2, resP, gPP, oPP, ivP, nmass, NTt, Nxyz, Ngv);
  qacc_barrier(dummy);
#else
  if ((nmass * NTt) % bfac != 0) {
    abort_r("Please correct your bfac! \n");
  ...
  qacc_for(gi, Nbfac, {
    ...
    for (Int iv = 0; iv < Ngv; iv++) {
      ...
      for (Int off = 0; off < ivP[iv]; off++) {
        ...
        for (Int bi = 0; bi < bfac; bi++) {
          if (invmode == 1) {
            buf[bi] += (t1[bi] * qlat::qconj(t2[bi]) * gtem);
          }
          if (invmode == 0) {
            buf[bi] += (t1[bi] * (t2[bi]) * gtem);
          }
        }
      }
      //
...
//
#if USEKERNEL == 1
  std::vector<std::vector<int8_t>> giEL;
  giEL.resize(Ngv);
  std::vector<std::vector<uint8_t>> oiL;
  oiL.resize(Ngv);
  //
  ////reformulate index
  for (Int iv = 0; iv < Ngv; iv++) {
  ...
  //
  std::vector<qlat::vector_gpu<int8_t>> giEG;
  giEG.resize(Ngv);
  for (Int iv = 0; iv < Ngv; iv++) {
  ...
  int8_t** gPP = gP.data();
  //
  std::vector<qlat::vector_gpu<uint8_t>> oiG;
  oiG.resize(Ngv);
  for (Int iv = 0; iv < Ngv; iv++) {
...
#endif
  if (invmode == 0)
    meson_vectorEV_kernel<Ty, 0, BFACG>(p1, p2, resP, gPP, oPP, ivP, nmass, NTt,
                                        Nxyz, Ngv);
  if (invmode == 1)
    meson_vectorEV_kernel<Ty, 1, BFACG>(p1, p2, resP, gPP, oPP, ivP, nmass, NTt,
                                        Nxyz, Ngv);
  // if(mode==2)meson_vectorEV_kernel<Ty,1,0, BFACG>(p1, p2, resP, gPP, oPP,
  // ivP, nmass, NTt, Nxyz, Ngv); if(mode==3)meson_vectorEV_kernel<Ty,1,1,
  // BFACG>(p1, p2, resP, gPP, oPP, ivP, nmass, NTt, Nxyz, Ngv);
  qacc_barrier(dummy);
#endif
//
#if USEKERNEL == 0
...
#pragma omp parallel for
            for (Int ji = 0; ji < nmass * NTt; ji++) {
              ...
              //
              const Int off1 =
                  massi * 12 * 12 + (d2 * 3 + c2) * 12 + gI1[d1] * 3 + c1;
              const Int off2 =
                  massi * 12 * 12 + (gI2[d2] * 3 + c2) * 12 + d1 * 3 + c1;
              //
...
//
              ...
              if (invmode == 1) {
                qacc_forNB(i, Long(Nxyz), {
                  tr0[i] += (tp1[i] * qlat::qconj(tp2[i]) * g_tem);
                });
              }
              if (invmode == 0) {
                qacc_forNB(i, Long(Nxyz),
                           { tr0[i] += (tp1[i] * (tp2[i]) * g_tem); });
              }
#else
              EAy vp1(tp1, Nxyz);
              EAy vp2(tp2, Nxyz);
...
// void meson_vectorEV(EigenMTa &prop1, EigenMTa &prop2, EigenVTa &res,
// std::vector<ga_M > &ga1V, std::vector<ga_M > &ga2V,
//         qlat::fft_desc_basic &fd, Int clear=1, Int invmode=1){
//   check_prop_size(prop1);check_prop_size(prop2);
                    ...
                    qlat::fft_desc_basic& fd, Int clear = 1, Int invmode = 1)
{
  ...
  Ty** p1 = prop1P.data();
  Ty** p2 = prop2P.data();
  qlat::vector<Ty*> resvP;
  resvP.resize(Ngv * nmass);
...
template <typename Ty>
void meson_vectorEV(std::vector<qlat::FieldG<Ty>>& prop1,
                    std::vector<qlat::FieldG<Ty>>& prop2,
                    std::vector<qlat::FieldG<Ty>>& res, std::vector<ga_M>& ga1V,
                    std::vector<ga_M>& ga2V, Int clear = 1, Int invmode = 1)
{
  ...
  for (Int mi = 0; mi < nmass; mi++) {
    ...
    Qassert(prop1[mi].mem_order == QLAT_OUTTER and
            prop2[mi].mem_order == QLAT_OUTTER);
  }
  //
...
void meson_corrE(std::vector<Propagator4dT<Td>>& pV1,
                 std::vector<Propagator4dT<Td>>& pV2, ga_M& ga1, ga_M& ga2,
                 qlat::vector<Ta>& res, qlat::fft_desc_basic& fd, Int clear = 1,
                 const Coordinate& mom = Coordinate(), const Int invmode = 1,
                 const Int tini = 0)
{
  /// int NTt  = fd.Nv[3];
  /// LInt Nxyz = fd.Nv[0]*fd.Nv[1]*fd.Nv[2];
...
template <typename Td, typename Ta>
void meson_corrE(Propagator4dT<Td>& p1, Propagator4dT<Td>& p2, ga_M& ga1,
                 ga_M& ga2, qlat::vector<Ta>& res, Int clear = 1,
                 const Coordinate& mom = Coordinate(), Int invmode = 1,
                 const Int tini = 0)
{
...
template <typename Td>
void meson_corrE(Propagator4dT<Td>& p1, Propagator4dT<Td>& p2, const Int ga,
                 const Int gb, const std::string& filename,
                 const Coordinate& mom = Coordinate(), Int invmode = 1,
                 const Int tini = 0,
                 const std::string& info = std::string("NONE"),
                 const Int shift_end = 1)
{
...
template <typename Ty>
void meson_corrE(std::vector<qpropT>& prop1, std::vector<qpropT>& prop2,
                 ga_M& ga1, ga_M& ga2, qlat::vector<Ty>& res, Int clear = 1,
                 const Coordinate& mom = Coordinate(), Int invmode = 1)
{
  qlat::vector<Ty> resE;
...
template <typename Td>
void print_pion(std::vector<Propagator4dT<Td>>& prop1,
                std::vector<Propagator4dT<Td>>& prop2,
                const std::string& tag = std::string(""), RealD factor = 1.0)
{
  ga_matrices_cps ga_cps;
  const qlat::Geometry& geo = prop1[0].geo();
  fft_desc_basic& fd = get_fft_desc_basic_plan(geo);
  ...
  for (Int iv = 0; iv < nv; iv++)
    for (Int t = 0; t < fd.nt; t++) {
      qlat::ComplexT<Td> v =
          resC[iv * fd.nt + t] * qlat::ComplexT<Td>(factor, 0.0);
...
template <typename Ty>
void pion_corr_simple(qlat::FieldG<Ty>& prop1, qlat::FieldG<Ty>& prop2,
                      vector<Ty>& corr, const Coordinate& mom = Coordinate(),
                      const Ty src_phase = Ty(1.0, 0.0), const Int tini = 0)
{
  ga_matrices_cps ga_cps;
  std::vector<ga_M> gI;
  gI.resize(1);
...
template <typename Td>
void print_pion(Propagator4dT<Td>& p1, Propagator4dT<Td>& p2,
                const std::string& tag = std::string(""), RealD factor = 1.0)
{
