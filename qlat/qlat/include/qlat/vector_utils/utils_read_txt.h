...
inline Int stringtonum(std::string& tem_string)
{
  ...
  //   for(Int i=0;i<tem_length;i++){t_Total =
  //   t_Total+(tem_string.c_str()[i]-'0')*std::pow(10,tem_length-i-1);};
  // }
  // return t_Total;
  double use = 0.0;
  if (tem_string != "_NONE_") {
...
inline Coordinate string_to_Coordinate(
    const std::string& paraI = std::string("NONE"))
{
  Coordinate sp;
  for (Int i = 0; i < 4; i++) {
    sp[i] = 0;
...
inline size_t get_file_size_MPI(const std::string& filename,
                                bool silence = false)
{
  // char tmp[1000];
  std::string tmp;
...
inline size_t get_write_factor(const size_t size)
{
  ...
  if (val != "") {
    large = 1024 * 1024 * stringtonum(val);
  }
  //
  size_t factor = 1;
  if (size < large) {
...
inline size_t file_operation(void* buf, const size_t size, const size_t count,
                             FILE* file, const bool read)
{
  ...
  //
  if (read == true) {
    check = fread(buf, currN, count * factor, file);
  }
  if (read == false) {
    check = fwrite(buf, currN, count * factor, file);
  }
  //
  if (check > 0) {
    return 1;
...
void write_data(Ty* dat, FILE* file, size_t size, bool read = false,
                bool single_file = false)
{
  ...
  if (qlat::get_id_node() == 0) {
    ...
    // if(read==true){size_t sizeF = get_file_size_o(filename);size =
    // sizeF/bsize;dat.resize(size);}
    //
    ////Set buf with size
    // char* buf=NULL;
    ...
    if (read == false) {
      if (single_file == false)
        cpy_data_thread((double*)(&buf[0]), &dat[0], size, 0);
      if (single_file == true)
        cpy_data_thread((RealF*)(&buf[0]), &dat[0], size, 0);
      /////memcpy(&buf[0],&dat[0],size*sizeof(RealD));
      if (Rendian == false)
        if (!is_big_endian_gwu()) switchendian((char*)&buf[0], size, bsize);
      if (Rendian == true)
        if (is_big_endian_gwu()) switchendian((char*)&buf[0], size, bsize);
    }
    //
    ...
    if (sem != 1) {
      printf("Reading/Writing error %zu %zu \n", sem, size_t(1));
    }
    //
    /////Switch endian of the file write
    if (read == true) {
      ...
      if (single_file == false)
        cpy_data_thread(&dat[0], (double*)&buf[0], size, 0);
      if (single_file == true)
        cpy_data_thread(&dat[0], (RealF*)&buf[0], size, 0);
    }
    //
    ////delete []buf;
    ////free(buf);
...
template <typename Ty>
void write_data(std::vector<Ty> dat, const char* filename, bool read = false,
                bool single_file = false)
{
  if (qlat::get_id_node() == 0) {
    Int bsize = sizeof(RealD);
    ...
    if (read == false) size = dat.size();
    if (read == true) {
      size_t sizeF = get_file_size_o(filename);
      size = sizeF / bsize;
      dat.resize(size);
    }
    FILE* file = NULL;
...
template <typename Ty>
void read_data(std::vector<Ty> dat, const char* filename,
               bool single_file = false)
{
  write_data(dat, filename, true, single_file);
}

inline size_t read_input(const char* filename,
                         std::vector<std::vector<std::string>>& read_f)
{
  read_f.resize(0);
  if (get_file_size_o(filename) == 0) {
    qmessage("input file size zero %s !\n", filename);
    return 0;
  }
  ...
  for (Int i = 0; i < 5000; i++) {
    tem[LINE_LIMIT] = 0;
    if (fgets(tem, LINE_LIMIT + 1, filer) == NULL) {
      binary = true;
      break;
    };
    /// for(Int j=0;j<1000;j++){if(tem[i] < 0){binary =
    /// true;break;}}if(binary){break};
    ...
    // if(tem[0]    <  0){binary = true;break;}////need to check whether it
    // works or not
    if (std::string(tem).size() >= LINE_LIMIT) {
      binary = true;
      break;
    }
  ...
  if (binary) {
    printf("Binary file or file line too Long! \n");
    read_f.resize(0);
    off_file = 0;
  }
  fclose(filer);
...
inline void bcast_vstring(std::vector<std::string>& conf_l,
                          const Int Host_rank = 0)
{
  Int rank = get_node_rank_funs0();
  ////Bcast strings
  size_t sizen = 0;
  if (rank == Host_rank) sizen = conf_l.size();
...
struct inputpara {
  ...
  ~inputpara()
  {
    ...
    //  for(unsigned int
    //  ic=0;ic<read_f[is].size();ic++){read_f[is][ic].resize(0);}
    //  read_f[is].resize(0);
    // }
    read_f.resize(0);
  }
  ...
  inline void load_para(const char* filename, const bool printlog_set = true)
  {
    ...
    if (find_para(std::string("twist_angle"), twist_angle) == 0) twist_angle = std::string("NONE");
    if (find_para(std::string("twist_with_eig"), twist_with_eig) == 0) twist_with_eig = 0;
    ...
    if (find_para(std::string("fermion_type"), fermion_type) == 1)
      fermion_type = 1;
    if (find_para(std::string("gridtem"), gridtem) == 0) gridtem = 1;
    if (find_para(std::string("sparsefactor"), sparsefactor) == 0)
      sparsefactor = 16;
    ...
    if (find_para(std::string("mode_FFT_MPI"), mode_FFT_MPI) == 0)
      mode_FFT_MPI = -1;
    ...
    if (find_para(std::string("Link_name"), Link_name) == 0)
      Link_name = std::string("NONE");
    if (find_para(std::string("Ename"), Ename) == 0)
      Ename = std::string("NONE");
    if (find_para(std::string("Ename_Sm"), Ename_Sm) == 0)
      Ename_Sm = std::string("NONE");
    if (find_para(std::string("Sname"), Sname) == 0)
      Sname = std::string("NONE");
    if (find_para(std::string("Pname"), Pname) == 0)
      Pname = std::string("NONE");
    if (find_para(std::string("do_all_low"), do_all_low) == 0) do_all_low = 1;
    if (find_para(std::string("output"), output) == 0)
      output = std::string("NONE");
    if (find_para(std::string("output_vec"), output_vec) == 0)
      output_vec = std::string("NONE");
    ...
    if (find_para(std::string("eig_poly_deg"), eig_poly_deg) == 0)
      eig_poly_deg = 0;
    if (find_para(std::string("eig_amin"), eig_amin) == 0) eig_amin = 0;
    if (find_para(std::string("eig_amax"), eig_amax) == 0) eig_amax = 0;
    if (find_para(std::string("eig_check_interval"), eig_check_interval) == 0)
      eig_check_interval = 10;
    if (find_para(std::string("eig_max_restarts"), eig_max_restarts) == 0)
      eig_max_restarts = 100000;
    if (find_para(std::string("eig_batched_rotate"), eig_batched_rotate) == 0)
      eig_batched_rotate = 0;
    if (find_para(std::string("eig_qr_tol"), eig_qr_tol) == 0) eig_qr_tol = 0.1;
    //
    if (find_para(std::string("Propname"), Propname) == 0)
      Propname = std::string("NONE");
    if (find_para(std::string("Srcname"), Srcname) == 0)
      Srcname = std::string("NONE");
    //
    if (find_para(std::string("layout"), layout) == 0)
      layout = std::string("NONE");
    if (find_para(std::string("paraI"), paraI) == 0)
      paraI = std::string("NONE");
    if (find_para(std::string("paraIA"), paraIA) == 0)
      paraIA = std::string("NONE");
    if (find_para(std::string("paraIB"), paraIB) == 0)
      paraIB = std::string("NONE");
    if (find_para(std::string("paraIC"), paraIC) == 0)
      paraIC = std::string("NONE");
    //
    if (find_para(std::string("paraI_2pt"), paraI_2pt) == 0)
      paraI_2pt = std::string("NONE");
    if (find_para(std::string("paraI_3pt"), paraI_3pt) == 0)
      paraI_3pt = std::string("NONE");
    if (find_para(std::string("sink_list"), sink_list) == 0)
      sink_list = std::string("NONE");
    //
    if (find_para(std::string("para_stag"), para_stag) == 0)
      para_stag = std::string("NONE");
    if (find_para(std::string("fermion_mass"), fermion_mass) == 0)
      fermion_mass = 0.11;
    if (find_para(std::string("niter"), niter) == 0) niter = 100000;
    if (find_para(std::string("niter_sloppy"), niter_sloppy) == 0)
      niter_sloppy = 100000;
    if (find_para(std::string("cg_err"), cg_err) == 0) cg_err = 1e-8;
    if (find_para(std::string("solver_type"), solver_type) == 0)
      solver_type = 0;
    if (find_para(std::string("inv_deflate"), inv_deflate) == 0)
      inv_deflate = 0;
    if (find_para(std::string("prec_type"), prec_type) == 0) prec_type = 0;
    //
    if (find_para(std::string("job_tag"), job_tag) == 0)
      job_tag = std::string("NONE");
    if (find_para(std::string("src_smear_para"), src_smear_para) == 0)
      src_smear_para = std::string("NONE");
    if (find_para(std::string("sink_smear_para"), sink_smear_para) == 0)
      sink_smear_para = std::string("NONE");
    if (find_para(std::string("save_type"), save_type) == 0)
      save_type = std::string("NONE");
    if (find_para(std::string("total_size"), total_size) == 0)
      total_size = std::string("NONE");
    //
    ...
    if (find_para(std::string("SRC_PROP_WITH_LOW"), SRC_PROP_WITH_LOW) == 0)
      SRC_PROP_WITH_LOW = 0;
    //
    if (find_para(std::string("key_T"), key_T) == 0)
      key_T = std::string("NONE");
    if (find_para(std::string("dim_name"), dim_name) == 0)
      dim_name = std::string("NONE");
    if (find_para(std::string("corr_name"), corr_name) == 0)
      corr_name = std::string("NONE");
    if (find_para(std::string("INFO_LIST"), INFO_LIST) == 0)
      INFO_LIST = std::string("NONE");
    if (find_para(std::string("FILE_ENDIAN"), FILE_ENDIAN) == 0)
      FILE_ENDIAN = std::string("NONE");
    if (find_para(std::string("VECS_TYPE"), VECS_TYPE) == 0)
      VECS_TYPE = std::string("NONE");
    //
    Long maxline = 10000;
    if (Long(read_f.size()) < maxline) {
      maxline = read_f.size();
    }
    for (Long li = 0; li < maxline; li++) {
      std::string tem = std::string("NONE");
      std::string mname = ssprintf("INFOA%02d", int(li));
      if (find_para(mname, tem) != 0) {
        INFOA.push_back(tem);
      }
    }
    //
    ...
    for (Int mi = 0; mi < nmass; mi++) {
      ...
      if (find_para(mname, tem) == 0) {
        ...
        masses.push_back(stringtodouble(tem));
      }
    }
    //
    ...
    for (Int si = 0; si < nsource; si++) {
      ...
      if (find_para(std::string(sname), tem) == 0) {
        srcN.push_back(std::string("NONE"));
      } else {
        srcN.push_back(tem);
      }
      if (find_para(std::string(pname), tem) == 0) {
        propN.push_back(std::string("NONE"));
      } else {
        propN.push_back(tem);
      }
      if (find_para(std::string(Gname), tem) == 0) {
        smearN.push_back(std::string("NONE"));
      } else {
        smearN.push_back(tem);
      }
    }
    //
    if (find_para(std::string("OBJECT"), tem) == 0) {
      OBJECT = std::string("NONE");
    } else {
      std::vector<std::string> temL = stringtolist(tem);
      OBJECT = temL[0];
    }
    //
    if (find_para(std::string("debuga"), debuga) == 0) debuga = 0;
    if (find_para(std::string("save_prop"), save_prop) == 0) save_prop = 0;
    if (find_para(std::string("checksum"), tem) == 0) {
      checksum = 0;
    } else {
      std::sscanf(tem.c_str(), "%X", &checksum);
    }
    if (printlog)
  ...
  inline void load_para(Int argc, char* argv[])
  {
    ...
    for (Int i = 1; i < argc - 1; ++i) {
      if (std::string(argv[i]) == std::string("--icfg")) {
        std::string tem = std::string(argv[i + 1]);
        icfg = stringtonum(tem);
        if (get_node_rank_funs0() == 0)
          printf("==Current  %20s %10d \n", "icfg", icfg);
      }
      if (std::string(argv[i]) == std::string("--lat")) {
        std::string tem = std::string(argv[i + 1]);
        lat = tem;
        if (get_node_rank_funs0() == 0)
          printf("==Current  %20s %20s \n", "lat", lat.c_str());
      }
...
inline void readuce_input_Coordinate_info(std::string& a0,
                                          std::vector<std::string>& a1,
                                          const Int max_pos = 8)
{
  if (a0.length() < int(LINE_LIMIT / 2)) {
    return;
  }
...
inline size_t vec_head_write(inputpara& in, const char* filename, Int type = -1,
                             bool clear = true)
{
  ...
  if (qlat::get_id_node() == 0) {
    ...
    if (clear == false) {
      if (get_file_size_o(filename) == 0) {
        ...
        filew = fopen(filename, "r+");
      }
    }
    //
    fseek(filew, 0, SEEK_SET);
    Int version = 0;
    /// if(type == -1)fprintf(filew, "OBJECT BEGIN_Prop_HEAD VER %d, sink 12,
    /// src 12, zyxt, R/I \n" , version); if(type ==  0)fprintf(filew, "OBJECT
    /// BEGIN_Noise_HEAD VER %d \n", version); if(type ==  1)fprintf(filew,
    /// "OBJECT BEGIN_Eigen_HEAD VER %d \n", version);
    //
...
/// inline size_t prop_head_write(inputpara &in, const char* filename, bool
/// clear=true){
///   return vec_head_write(in, filename, -1, clear);
/// }
/// inline size_t noise_head_write(inputpara &in, const char* filename, bool
/// clear=true){
///   return vec_head_write(in, filename,  0, clear);
/// }
inline size_t vecs_head_write(inputpara& in, const char* filename,
                              bool clear = true)
{
  return vec_head_write(in, filename, 0, clear);
}
inline size_t corr_head_write(inputpara& in, const char* filename,
                              bool clear = true)
{
  return vec_head_write(in, filename, 1, clear);
}
...
{
  if (save_type.c_str() == std::string("double")) {
    return 0;
  }
  if (save_type.c_str() == std::string("Double")) {
    return 0;
  }
  if (save_type.c_str() == std::string("float")) {
    return 1;
  }
  if (save_type.c_str() == std::string("Float")) {
    return 1;
  }
  if (save_type.c_str() == std::string("single")) {
    return 1;
  }
  if (save_type.c_str() == std::string("Single")) {
    return 1;
...
inline std::string print_size(size_t size, Int limit = 0)
{
  ...
  std::string tem_size;
  if (limit == 0) {
    tem_size = ssprintf("%zu", size_t(size));
  }
  if (limit == 1) {
    tem_size = ssprintf("%-20zu", size_t(size));
  }
  return tem_size;
...
struct corr_dat {
  ...
  inline const Ty& operator[](const Long i) const
  {
    Qassert(i < total);
    return dat[i];
  }
  inline Ty& operator[](const Long i)
  {
    Qassert(i < total);
    return dat[i];
  }
  ...
  ////write mode
  corr_dat(const std::string& key, const std::string& dimN = "NONE",
           const std::string& corr = "NONE")
  {
  ...
  inline void create_dat(const std::string& key, const std::string& dimN,
                         const std::string& corr = "NONE")
  {
    ...
    initialize();
    //
    if (sizeof(Ty) != sizeof(float) and sizeof(Ty) != sizeof(RealD)) {
      Qassert(false);
    };
    ...
    for (LInt i = 0; i < tem.size(); i++) {
      ...
      if (i != 0) {
        Qassert(key_T[i] != 0);
      }  ////zero dim can be zero
      c_a_t[i] = 0;
      total = total * key_T[i];
    }
    Qassert(total >= 0 and dim >= 0);
    if (dimN != std::string("NONE")) {
      dim_name = stringtolist(dimN);
    } else {
      dim_name.resize(dim);
      for (Int d = 0; d < dim; d++) {
        dim_name[d] = std::string(" ");
      }
    }
  ...
  inline void shift_end() { c_a_t = get_site(total); }
  ...
  inline Int read_dat(const char* filename, Int check_load = 0)
  {
    ...
    if (in.OBJECT != std::string("BEGIN_Corr_HEAD")) {
      qmessage("File %s head wrong!\n", filename);
  ...
  inline std::string get_key_T()
  {
    ...
    // for(Int d=0;d<dim;d++){key += (std::string("  ") +
    // std::to_string(key_T[d]));}
  ...
  inline void write_head(const char* filename)
  {
    ...
    //
    if (sizeof(Ty) == sizeof(RealD)) {
      in_buf.save_type = std::string("Double");
      write_type = 0;
    }
    if (sizeof(Ty) == sizeof(float)) {
      in_buf.save_type = std::string("Single");
      write_type = 1;
    }
    write_bsize = sizeof(RealD);
    if (write_type == 1) {
      write_bsize = sizeof(float);
    }
    //
    in_buf.checksum = 0;
    update_info();
  ...
  inline crc32_t write_part(size_t off, size_t Psize)
  {
    ...
    if (qlat::get_id_node() == node_control) {
      ...
      //
      if (write_type == 0)
        cpy_data_thread((double*)&buf[0], &dat[off], Psize, 0);
      if (write_type == 1)
        cpy_data_thread((RealF*)&buf[0], &dat[off], Psize, 0);
      //
      if (write_type == 0)
        write_data((double*)&buf[0], file_open, Psize, false, false);
      if (write_type == 1)
        write_data((RealF*)&buf[0], file_open, Psize, false, true);
      //
  ...
  inline void write_dat(const char* filename)
  {
    ...
    if (head_off != 0) {
      ...
      for (Long si = 0; si < Ns; si++) {
        ...
        crc32_tem =
            crc32_combine(crc32_tem, 0, end_of_file - pos_file_cur - cur);
    ...
    if (head_off == 0) {
      ...
      for (size_t fi = 0; fi < factor; fi++) {
        ...
        crc32_tem = crc32_combine(
            crc32_tem, 0, end_of_file - pos_file_cur - partF * write_bsize);
        crc32_total ^= crc32_tem;
      }
    }
    //
  ...
  void write_corr(Ta* src, const Long size, Int mode_copy = 0)
  {
    ...
    // if( is_double){cpy_data_thread(wdat, (double*) src, double_size,
    // mode_copy);} if(!is_double){cpy_data_thread(wdat, (float* ) src,
    // double_size, mode_copy);}
    //
    if (!small_size) {
  ...
  inline void print_info()
  {
    if (qlat::get_id_node() == node_control) {
      printf("===Corr %s, dim %d, mem size %.3e MB \n", corr_name.c_str(), dim,
             total * sizeof(RealD) * 1.0 / (1024.0 * 1024.0));
      for (Int d = 0; d < dim; d++) {
...
inline void corr_dat_create(const std::string& filename,
                            const std::string& key_T, const std::string& dimN,
...
inline void corr_dat_info(const std::string& filename,
                          const std::string& info = std::string("NONE"))
{
  corr_dat<double> corr(std::string(""));
