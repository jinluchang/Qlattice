...
struct eigen_ov {
  ...
  eigen_ov(const Geometry& geo_, Int n_vec_or, Long bsize0 = -1,
           double extra_mem_factor_set = 0.82);
  ...
  template <typename Ty>
  void copy_FieldM_to_Mvec(qlat::FieldM<Ty, 12>& src, Int ncur, Int sm = 0,
                           Int dir = 1);
  template <typename Ty>
  ...
  void load_eigen_Mvec(const std::string& ename, Int sm = 0, Int nini = 0,
                       Int checknorm = 1);

  template <typename Tg>
  void load_eigen_Mvec_smear(
      const std::string& ename, const GaugeFieldT<Tg>& gf,
      std::vector<qlat::GaugeFieldT<Ftype>>& gfL,
      std::vector<qlat::vector_gpu<qlat::ComplexT<Ftype>>>& propS, Int nini = 0,
      Int checknorm = 0, const double src_width = 0.0, const Int src_step = 0,
  ...
  template <typename Tg>
  void smear_eigen(const std::string& Ename_Sm, const GaugeFieldT<Tg>& gf,
  ...
  void load_eivals(const std::string& enamev, double rho_or,
                   double Eerr = EIGENERROR, Int nini = 0);

  void load_eigen(const std::string& ov_evecname, Int checknorm = 1,
                  double kappa = 0.2, double eigenerror = EIGENERROR,
                  Int nini = 0);
  ...
  void print_info()
  {
    ...
    qmessage(
        "Lattice sites on node %10ld bfac %10ld bsize %10ld, tsize %5d !\n",
    ...
    qmessage(
        "===prop %.3e GB, v %d  %.3e GB, buf %d  %.3e GB, bfac_group %d, E "
        "%.3e GB, smear_buf %1d. \n",
        mem_prop, ncutgpu, memV * ncutgpu, ncutbuf, memV * ncutbuf,
        int(bfac_group), memV * n_vec, int(enable_smearE));
  }

  /////void checknormM(Ftype err=1e-3);
  void initialize_mass(const std::vector<double>& mass, Int Nv = 12,
                       Int one_minus_halfD_or = 1, Int nprop = 1);
  void initialize_mass();
  void alpha_clear();

  /// void seq_L(vector **prop3,double mass_d,double rho,int_vector &map,bool
  /// bcast,double_complex* ker_low); void seq_L(vector **prop3,double
  /// mass_d,double rho,int_vector &map,double_complex* ker_low); void
  /// setup_L(vector **prop2);
...
void eigen_ov::setup_bfac(Long bsize0)
{
  ...
  if (bfac % 6 != 0) {
    qmessage(
        "Cannot understand sites on bfac %5ld, bsize %10ld, Total %10ld !\n",
        bfac, b_size, noden * 6);
    abort_r("");
  }
  if ((noden * 6) % bfac != 0) {
    qmessage("Cannot understand sites on node*6/Nt %10ld bfac %10ld!\n",
             noden * 6 / Nt, bfac);
    abort_r("");
  }
  if (bfac % (Nt * 6) != 0) {
    qmessage("Cannot understand sites on node %10ld bfac %10ld, tsize %5d!\n",
             noden, bfac, Nt);
    abort_r("");
  }
}

eigen_ov::eigen_ov(const Geometry& geo_, Int n_vec_or, Long bsize0,
                   double extra_mem_factor_set)
{
  geo = geo_;
  fft_desc_basic& fd = get_fft_desc_basic_plan(geo);
  /// if(fd.order_ch != 0){abort_r("Currently not supported for change fd
  /// order.\n ");}
  //
  rho = 1.5;
...
/////redefine variables on GPU
  ...
  if (totalD < mem_prop + 6 * 12 * memV) {
    qmessage(
        "===GPU Memory too small, Total %.3e, prop %5d %.3e, 6 prop %.3e, "
        "increase nodes! \n",
  ...
  if (vres > n_vec) {
    ...
    if (vres > vini) {
      ...
      ncutbuf = 0;
      ncutgpu = ((vres) / 16) * 16;
      if (ncutgpu < 16) ncutgpu = 16;
    }
  }
  //
  if (ncutgpu == 0 or ncutbuf == 0) {
    bfac_group = 2 * bfac;
  } else {
    bfac_group = 32;
    for (Int bini = bfac_group; bini >= 1; bini--) {
      size_t tem = get_threads(bini, bfac, 0);
      if (tem != 0 and bfac % tem == 0) {
        bfac_group = tem;
        break;
      }
      bfac_group = 1;
...
void eigen_ov::allocate_GPU_mem(Int nprop)
{
  ...
  //
  if (Eigenbuf.size() == Lae and Eigendyn.size() == Lax) {
    return;
  }
  //
  {
    ...
    {
      if (ncutbuf != 0) {
        allocate_buf(Eigenbuf, Lae, Lbe);
      }
      if (ncutgpu != 0) {
        allocate_buf(Eigendyn, Lax, Lbx);
      }
    }
    //
    {
      if (ncutgpu != 0 and enable_smearE == true) {
        allocate_buf(Eigendyn_Sm, Lax, Lbx);
      }
      if (ncutbuf != 0 and enable_smearE == true) {
        allocate_buf(Eigenbuf_Sm, Lae, Lbe);
      }
    }
    gpu_mem_set = true;
...
{
  for (LInt i = 0; i < Eigenbuf.size(); i++) {
    Eigenbuf[i].resize(0);
  }
  Eigenbuf.resize(0);
  for (LInt i = 0; i < Eigendyn.size(); i++) {
    Eigendyn[i].resize(0);
  }
  Eigendyn.resize(0);
  for (LInt i = 0; i < Eigenbuf_Sm.size(); i++) {
    Eigenbuf_Sm.resize(0);
  }
  Eigenbuf_Sm.resize(0);
  for (LInt i = 0; i < Eigendyn_Sm.size(); i++) {
    Eigendyn_Sm.resize(0);
  }
  Eigendyn_Sm.resize(0);
  //
...
void eigen_ov::load_eivals(const std::string& enamev, double rho_or,
                           double Eerr, Int nini)
{
  rho = rho_or;
  //
  std::vector<double> v, e;
  load_txt_eigenvalues(v, e, enamev.c_str());
  if ((nini + n_vec) * 2 > int(v.size()))
    abort_r("Eigen value size too small! ");
  ...
  for (Int j = 0; j < n_vec; ++j) {
    ...
    if (std::sqrt(qnorm(eval_self[j])) < Eerr)
      num_zero += 1;  // this will square the values
    // if(abs(eval_self[j]) < Eerr) num_zero += 1;
    // qmessage("n %3d, abs %.8e, norm %.8e, zero %3d \n", j, abs(eval_self[j]),
    // qnorm(eval_self[j]), num_zero);
  }
}
                                   ...
                                   bool data_GPU)
{
  ...
  if (ncur >= n_vec) {
    abort_r("Cannot copy to position larger than n_vec ! \n");
  }
  /////int nread = nb - ba;
  Complexq* s1 = NULL;
  Ty* s0 = NULL;
  ...
  Long sizeF = noden;
  if (dir == 1) {
    mv_civ.dojob(s0, s0, 1, 12, sizeF, 1, 1, data_GPU);
  }
  //
  ...
  for (LInt xini = 0; xini < total / b_size; xini++) {
    ...
    if (dir == 0) {
      cpy_data_thread(&s0[xi], s1, b_size, GPU_cpy, QFALSE);
    }
  }
  qacc_barrier(dummy);
  //
  if (dir == 0) {
    mv_civ.dojob(s0, s0, 1, 12, sizeF, 0, 1, data_GPU);
  }
  s0 = NULL;
  s1 = NULL;
}
...
template <typename Ty>
void eigen_ov::copy_FieldM_to_Mvec(qlat::FieldM<Ty, 12>& src, Int ncur, Int sm,
                                   Int dir)
{
  TIMERA("COPY Eigen Vectors Mvec");
  if (ncur >= n_vec) {
...
{
  if (sm == 1 and enable_smearE == false) {
    abort_r("Smeared Eigen system not loaded or computed! \n");
  }
  if (ni >= n_vec) {
    abort_r("Request ni too large! \n");
  }
  //
  Complexq* buf = NULL;
  Complexq* s0 = NULL;
...
/////On CPU use Mvec and Mvec_Sm
  ...
  if (bfac_group <= 0) {
    abort_r("Setup bfac_group first !\n");
  }
...
void eigen_ov::smear_eigen(const std::string& Ename_Sm,
                           const GaugeFieldT<Tg>& gf, const double width,
                           const Int step, const CoordinateD& mom,
                           const bool smear_in_time_dir)
{
  TIMER("smear eigen system");
  ////load if exist
  if (Ename_Sm != std::string("NONE") and
      get_file_size_MPI(Ename_Sm.c_str(), true) != 0) {
  ...
  qlat::vector_gpu<Complexq> buf;
  buf.resize(each * Ncopy);
  //
  move_index mv_idx;
  ...
  for (LInt ji = 0; ji < job.size() / 2; ji++) {
    ...
    for (Int iv = 0; iv < job[ji * 2 + 1]; iv++) {
      copy_to_FieldM(&buf[iv * Ncopy], job[ji * 2 + 0] + iv, 0, true);
    }
    flag = 0;
    mv_idx.dojob(buf.data(), buf.data(), 1, each, geo.local_volume() * 12, flag,
                 1, true);
    //
    smear_propagator_gwu_convension_inner<Complexq, 4, each, Tg>(
        buf.data(), gf, width, step, mom, smear_in_time_dir);
    //
    flag = 1;
    mv_idx.dojob(buf.data(), buf.data(), 1, each, geo.local_volume() * 12, flag,
                 1, true);
    ////copy from buf
    for (Int iv = 0; iv < job[ji * 2 + 1]; iv++) {
      copy_FieldM_to_Mvec(&buf[iv * Ncopy], job[ji * 2 + 0] + iv, 1, 1, true);
    }
  }
  //
  ////erase smear
...
void eigen_ov::save_eigen_Mvec(const std::string& ename, Int sm, Int save_type)
{
  ...
  FILE* file_write = open_eigensystem_file(ename.c_str(), nini, ntotal, read,
                                           io_use, in_write_eigen, save_type);
  //
  Int each = io_use.ionum;
  std::vector<qlat::FieldM<Complexq, 12>> buf;
  buf.resize(each);
  for (Int iv = 0; iv < each; iv++) {
    buf[iv].init(io_use.geo());
  }
  //
  std::vector<Long> job = job_create(n_vec, each);
  for (LInt ji = 0; ji < job.size() / 2; ji++) {
    for (Int iv = 0; iv < job[ji * 2 + 1]; iv++) {
      copy_to_FieldM(buf[iv], job[ji * 2 + 0] + iv, sm);
    }
    /////write to file
    load_eigensystem_vecs(file_write, buf, io_use, in_write_eigen, 0,
                          job[ji * 2 + 1]);
  }
  //
...
void eigen_ov::print_norm_Mvec()
{
  ...
  for (Int sm = 0; sm < Nsm; sm++)
    for (Int ni = 0; ni < n_vec; ni++) {
      ...
      qmessage("Eigen vector %8d, sm %1d, norm 1.0 + %+.8e flag .", int(ni), sm,
               normf - 1.0);
    }
}
...
template <typename Tg>
void eigen_ov::load_eigen_Mvec_smear(
    const std::string& ename, const GaugeFieldT<Tg>& gf,
    std::vector<qlat::GaugeFieldT<Ftype>>& gfL,
    std::vector<qlat::vector_gpu<qlat::ComplexT<Ftype>>>& propS, Int nini,
    Int checknorm, const double src_width, const Int src_step,
    const double sink_width, const Int sink_step, const CoordinateD& src_mom,
    const CoordinateD& sink_mom, const bool src_smear_in_time_dir,
    const bool sink_smear_in_time_dir)
{
  ...
  FILE* file_read = open_eigensystem_file(name_read.c_str(), nini, ntotal, true,
                                          io_use, in_read_eigen, 2);
  //
  std::vector<double> widthL = {sink_width, src_width};
  std::vector<Int> stepL = {sink_step, src_step};
  std::vector<CoordinateD> momL;
  momL.resize(2);
  momL[0] = sink_mom;
  momL[1] = src_mom;
  std::vector<bool> smear_in_time_dirL = {sink_smear_in_time_dir,
                                          src_smear_in_time_dir};
  //
  {
    TIMER("load eigen from DISC");
    std::vector<qlat::FieldM<Complexq, 12>> buf;
    buf.resize(each_io);
    for (Int iv = 0; iv < each_io; iv++) {
      buf[iv].init(io_use.geo());
    }
    ...
    for (LInt ji = 0; ji < job.size() / 2; ji++) {
      ...
      /////load from file
      load_eigensystem_vecs(file_read, buf, io_use, in_read_eigen, 0,
                            job[ji * 2 + 1]);
  ...
  //
  {
    ...
    std::vector<qlat::vector_gpu<Complexq>> bufL;
    bufL.resize(2);
    ...
    for (LInt ji = 0; ji < job.size() / 2; ji++) {
      ...
      for (Int sm = 0; sm < 2; sm++) {
        if (Long(bufL[sm].size()) != Long(each_sm * Ncopy)) {
          bufL[sm].resize(each_sm * Ncopy);
        }
        ////copy to buf
        for (Int iv = 0; iv < job[ji * 2 + 1]; iv++) {
          copy_to_FieldM(&bufL[sm][iv * Ncopy], job[ji * 2 + 0] + iv, 0, true);
        }
        if (stepL[sm] == 0) {
          continue;
        }
        ...
        flag = 0;
        mv_idx.dojob(bufP, bufP, 1, each_sm, geo.local_volume() * 12, flag, 1,
                     true);
        //
        if (gfL.size() == 0) {
          smear_propagator_gwu_convension_inner<Complexq, 4, each_sm, Tg>(
              bufP, gf, widthL[sm], stepL[sm], momL[sm],
              smear_in_time_dirL[sm]);
        } else {
          Qassert(smear_in_time_dirL[sm] ==
                  0);  // not transferred to 2link smearings and may not work
          smear_propagator_gwu_convension_2shift_modi<Complexq, Ftype, 4,
                                                      each_sm>(
              bufP, geo, gfL, widthL[sm], stepL[sm], propS, -1, 0, momL[sm]);
        }
        flag = 1;
        mv_idx.dojob(bufP, bufP, 1, each_sm, geo.local_volume() * 12, flag, 1,
                     true);
      ...
      for (Int sm = 0; sm < 2; sm++) {
        for (Int iv = 0; iv < job[ji * 2 + 1]; iv++) {
          copy_FieldM_to_Mvec(&bufL[sm][iv * Ncopy], job[ji * 2 + 0] + iv, sm,
                              1, true);
        }
      }
    }
...
void eigen_ov::load_eigen_Mvec(const std::string& ename, Int sm, Int nini,
                               Int checknorm)
{
  std::string name_read = get_eigen_name_string(ename);
  //
...
void eigen_ov::load_eigen(const std::string& ov_evecname, Int checknorm,
                          double kappa, double eigenerror, Int nini)
{
...
void eigen_ov::random_eigen(Int sm, Int seed)
{
  ...
  eval_self.resize(n_vec);
  random_EigenM(eval_self, 0, seed + 10);
  //
  Long La = 2 * bfac / BFAC_GROUP_CPU;
  Long Lb = BFAC_GROUP_CPU * n_vec * Long(b_size);
  print_mem_info("Before Eigen Memory Allocate");
  if (sm == 0) {
    resize_EigenM(Mvec, La, Lb);
    for (Long iv = 0; iv < La; iv++)
      random_Ty(Mvec[iv].data(), Lb, 0, seed + 20 + iv);
  }
  if (sm == 1) {
    resize_EigenM(Mvec_Sm, La, Lb);
    for (Long iv = 0; iv < La; iv++)
      random_Ty(Mvec_Sm[iv].data(), Lb, 0, seed + 30 + iv);
    enable_smearE = true;
  }
  //
...
void eigen_ov::initialize_mass(const std::vector<double>& mass, Int Ns,
                               Int one_minus_halfD_or, Int nprop)
{
  Int need_update_size = 0;
  //
  if (mass.size() != massL.size()) {
    need_update_size = 1;
  } else {
    if (mass.size() == 0) {
      need_update_size = 1;
    ...
    if (one_minus_halfD_or != one_minus_halfD) {
      need_update_size = 1;
    }
    if (nprop != nprop_buf) {
      need_update_size = 1;
...
// less the memory needed, allocate only large props needed
    ...
    if (gpu_mem_set == false or nprop > nV_prop + 12) {
      qmessage("===mem %1d, n %5d, %5d, pos %5d %5d \n", int(gpu_mem_set),
               int(nprop), int(nV_prop), int(npos_Eigenbuf),
               int(npos_Eigendyn));
    ...
    //
    LInt nlarge = ncutgpu;
    if (ncutbuf > ncutgpu) nlarge = ncutbuf;
    //
    ...
    //
    if (eval_tem.size() != Long(mN * n_vec)) {
      eval_tem.resize(mN * n_vec);
    }
#pragma omp parallel for
    for (Int mki = 0; mki < mN * n_vec; mki++) {
      Int mi = mki / n_vec;
      Int kn = mki % n_vec;
      eval_tem[mi * n_vec + kn] =
          inv_self(eval_self[kn], mass[mi], rho, one_minus_halfD);
    }
    //
    if (Long(eval_list.size()) != Long(mN * n_vec)) {
      eval_list.resize(mN * n_vec);
    }
    cpy_data_thread(eval_list.data(),
                    (Complexq*)qlat::get_data(eval_tem).data(),
                    eval_list.size(), 1, QFALSE);
    //
...
/*
  conj_prop :
    ...
    true from sink to src prop ? with gamma5's without sink gamma5 since Ns is
  not know tsrcL : construct only from src time t_src, other time-slice will be
  ignored result format : (2, bfac, mN, Ns, b_size) Nb_eigen_prod = bfac / ( 6 *
  Nt);
    ### Qassert(bfac % ( 6 * Nt) == 0);
    bfac --> [6, Nt, Nb_eigen_prod]
*/
void prop_L_device(eigen_ov& ei, Complexq* src, Complexq* props, Int Ns,
                   const std::vector<double>& mass, Int mode_sm = 0,
                   const std::vector<Int>& tsrcL = std::vector<Int>(),
                   const bool conj_prop = false, Int one_minus_halfD_or = 1)
{
  TIMER_FLOPS("==prop_L");
  //
  const Int mN = mass.size();
  ...
  Int Fcount1 = 6 + 2;
  long long Tfloat =
      ei.n_vec * Ns * mN * vGb * Fcount1 + ei.n_vec * Ns * vGb * Fcount0;
  timer.flops += Tfloat;
  ...
  std::vector<bool> t_multi;
  t_multi.resize(fd.nt);
  ...
  if (conj_prop) {
    ...
    qacc_for(isp, Nd, { r[isp] = qconj(r[isp]); })
  }
  //
  ...
  for (Int ng = 0; ng < Ng + 1; ng++) {
    ...
    //
    {
      ...
      if (count_b > 0) {
        ...
        */
        matrix_prodP(EpL.data(), spL.data(), rpL.data(), m, n, w, count_b,
                     !conj_prop);
      }
      qacc_barrier(dummy);
      //
      ...
      // vector size groups for(LInt jobi=0;jobi < jobA.size()/2; jobi++)
      //{
      ...
      qacc_for(coff, Long(2 * ncur * Ns), {
        ...
        if (!conj_prop) {
          for (Long bi = 0; bi < bfac; bi++) {
            alpha[coff] +=
                alpha_bfac[chi * bfac * ncur * Ns + bi * ncur * Ns + xi];
          }
        } else {
    ...
    {
      TIMERA("Global sum");
      sum_all_size((Ftype*)(ei.alpha.data()), (Ftype*)(ei.alpha_buf.data()),
                   2 * (2 * ncur * Ns), 1);
    }
    //
    ...
    //
    {
      ...
      matrix_prodP(EpL.data(), spL.data(), rpL.data(), m, n, w, 2 * bfac, false,
                   true);
      // for(Long coff=0;coff<2*bfac;coff++)
      //{
      //   Long chi = coff/bfac;
...
*/
template <typename Ty>
void ov_prop_L(std::vector<FieldG<Ty>>& res, std::vector<FieldG<Ty>>& src,
               eigen_ov& ei, vector_gpu<Complexq>& buf_eig,
               const std::vector<double>& massL, Int mode_sm = 0,
               const Int c_add = 0, const Int Ngroup_ = -1,
               const std::vector<Int>& tsrcL = std::vector<Int>(),
               const bool conj_prop = false, Int one_minus_halfD_or = 1,
               const std::vector<Int> sec_info = std::vector<Int>())
{
  ...
  for (LInt si = 0; si < src.size(); si++) {
    Qassert(src[si].initialized and src[si].mem_order == QLAT_OUTTER and
            src[si].multiplicity == Ndc);
  }
  if (res.size() != src.size() * Nmass) {
    res.resize(src.size() * Nmass);
  }
  for (LInt si = 0; si < res.size(); si++) {
    ...
    Qassert(res[si].initialized and res[si].mem_order == QLAT_OUTTER and
            res[si].multiplicity == Ndc);
    // clear
    // if(c_add == 0){
    //   set_zero(res[si]);
    // }
  ...
  //
  std::vector<std::vector<Int>> tbufL;
  tbufL.resize(tsrcL.size());
  if (Nbuf_T == 2) {
  ...
  Complexq* buf_res =
      &buf_eig[total * Ngroup * 12 *
               (1 + 0)];  // size Ngroup * 12 * (0 + Nmass) * total
  Complexq* buf_res1 = NULL;
  if (Nbuf_T == 2) {
    buf_res1 = &buf_eig[total * Ngroup * 12 * (1 + Nmass)];
  }  // size Ngroup * 12 * (0 + Nmass) * total
  ...
  for (LInt jobi = 0; jobi < jobA.size() / 2; jobi++) {
    ...
    if (Long(srcP.size()) != bcut) {
      srcP.resize(0);
      srcP.resize(bcut);
    }
    if (Long(resP.size()) != Nmass * bcut) {
      resP.resize(0);
      resP.resize(Nmass * bcut);
    }
    ...
    //
    copy_bsize_prop_to_FieldP<Complexq, FieldG<Ty>, 0>(srcP, buf_src, Nk,
                                                       b_size, fd, 1, false, 1);
    // buf_res.set_zero();
    ...
    */
    // qmessage("Nt %5d \n", int(tbufL[0].size()));
    prop_L_device(ei, buf_src, buf_res, Nk, massL, mode_sm, tbufL[0], conj_prop,
                  one_minus_halfD_or);
    ...
    if (Nbuf_T == 2) {
      for (unsigned int ti = 1; ti < tbufL.size(); ti++) {
        ...
        prop_L_device(ei, buf_src, buf_res1, Nk, massL, mode_sm, tbufL[ti],
                      conj_prop, one_minus_halfD_or);
        ...
        */
        ...
        qacc_for(isp, Ndata, {
          ...
          //  map src number, first shit and each src have two sectors, then
          //  treat final sections
    ...
    if (c_add == 0)
      copy_bsize_prop_to_FieldP<Complexq, FieldG<Ty>, 0>(
          resP, buf_res, Nk * Nmass, b_size, fd, 1, false, 0);
    if (c_add == 1)
      copy_bsize_prop_to_FieldP<Complexq, FieldG<Ty>, 1>(
          resP, buf_res, Nk * Nmass, b_size, fd, 1, false, 0);
    if (c_add == -1)
      copy_bsize_prop_to_FieldP<Complexq, FieldG<Ty>, -1>(
          resP, buf_res, Nk * Nmass, b_size, fd, 1, false, 0);
