...
struct ScalarAction {
  ...
  qacc RealD action_point(const Field<RealD>& sf, const Int multiplicity,
                          Coordinate xl)
  {
    ...
    return dphi_sq / 2.0 + m_sq * phi_sq / 2.0 + lmbd * phi_sq * phi_sq / 24.0 +
           alpha * phi_0;
  }
  ...
  qacc RealD hmc_mass_p(const Coordinate& L, const Coordinate& pg)
  {
    ...
    return 4 / (PI * PI) *
           (m_sq + 8 -
            2 * (std::cos(2 * PI * pg[0] / L[0]) +
                 std::cos(2 * PI * pg[1] / L[1]) +
                 std::cos(2 * PI * pg[2] / L[2]) +
                 std::cos(2 * PI * pg[3] / L[3])));
  }

  inline void hmc_estimate_mass(Field<RealD>& masses,
    ...
    qthread_for(index, geo.local_volume(), {
      ...
      for (Int m = 0; m < M; ++m) {
        ...
        if (gindex == 0 && m == 0) {
          masses_v[m] =
              4 / (PI * PI) *
              std::pow((frc.real() * frc.real() + frc.imag() * frc.imag()) /
                           ((fld.real() - phi0) * (fld.real() - phi0) +
                            fld.imag() * fld.imag()),
                       0.5);
        } else {
          masses_v[m] =
              4 / (PI * PI) *
              std::pow((frc.real() * frc.real() + frc.imag() * frc.imag()) /
                           (fld.real() * fld.real() + fld.imag() * fld.imag()),
                       0.5);
        }
      }
    });
  ...
  inline RealD hmc_m_hamilton_node(const Field<ComplexD>& sm_complex,
                                   const Field<RealD>& masses)
  {
    ...
    qthread_for(index, geo_r.local_volume(), {
      ...
      for (Int m = 0; m < multiplicity; ++m) {
        ...
        s += (c.real() * c.real() + c.imag() * c.imag()) / 2 /
             masses.get_elem(xl,
                             m);  // /hmc_mass_p(L,geo.coordinate_g_from_l(xl));
      }
      fd.get_elem(index) = s;
    });
  ...
  inline void hmc_set_force_no_comm(Field<RealD>& sm_force,
                                    const Field<RealD>& sf)
  {
  ...
  inline void hmc_field_evolve(Field<ComplexD>& sf_complex,
  ...
  inline void axial_current_node_no_comm(Field<RealD>& axial_current,
                                         const Field<RealD>& sf)
  {
  ...
  inline void axial_current_node(Field<RealD>& axial_current,
                                 const Field<RealD>& sf)
  {
  ...
  inline void hmc_set_rand_momentum(Field<ComplexD>& sm_complex,
                                    const Field<RealD>& masses,
                                    const RngState& rs)
  {
  ...
  inline void hmc_predict_field(Field<ComplexD>& field_ft,
                                const Field<ComplexD>& momentum_ft,
                                const Field<RealD>& masses,
                                const RealD vev_sigma)
  {
  ...
  inline void get_polar_field(Field<RealD>& polar_fields,
                              const Field<RealD>& sf)
  {
