...
struct QMAction {
  ...
  qacc QMAction(const RealD alpha_, const RealD beta_, const RealD V_FV_min_,
                ...
                const RealD barrier_strength_, const RealD L_, const RealD M_,
                const RealD epsilon_, const Long t_FV_out_,
  ...
  inline RealD dV(const Vector<RealD>& x, const Long t, const int idx)
  {
    switch (vtype) {
      case GET_L:
        return dV_t_L(x, t, idx);
      case GET_M:
        return dV_t_M(x, t, idx);
      default:
        return dV_t_dtTV(x, t, idx);
    }
  }
  ...
  inline RealD V_t_L(const Vector<RealD>& x, const Long t)
  {
    ...
    // Start with H_proj
    if (t == 0) return V_L(x, V_full(x) + V_proj(x));
  ...
  inline RealD dV_t_L(const Vector<RealD>& x, const Long t, const int idx)
  {
    ...
    // Start with H_proj
    if (t == 0)
      return dV_L(x, V_full(x) + V_proj(x), dV_full(x, idx) + dV_proj(x, idx),
                  idx);
    // Until t_FV_out has past, use H_FV_out
    else if (t <= t_FV_out)
  ...
  inline RealD V_t_M(const Vector<RealD>& x, const Long t)
  {
    // Returns the potential evaluated at point x
    // Start with H_proj
    if (t == 0) return V_M(x, V_full(x) + V_proj(x));
    // Until t_FV_out has past, use H_FV_out
    else if (t <= t_FV_out)
      return V_FV_out(x);
    // Until t_FV_mid has past, use H_FV_mid
  ...
  inline RealD dV_t_M(const Vector<RealD>& x, const Long t, const int idx)
  {
    ...
    // Start with H_proj
    if (t == 0)
      return dV_M(x, V_full(x) + V_proj(x), dV_full(x, idx) + dV_proj(x, idx),
                  idx);
    // Until t_FV_out has past, use H_FV_out
    else if (t <= t_FV_out)
      return dV_FV_out(x, idx);
  ...
  inline RealD V_t_dtTV(const Vector<RealD>& x, const Long t)
  {
    // Returns the potential evaluated at point x
    // Start with H_proj
    if (t == 0) return V_full(x) + V_proj(x);
    // Until t_FV_out has past, use H_FV_out
    else if (t <= t_FV_out)
      return V_FV_out(x);
    // Until t_FV_mid has past, use H_FV_mid
    ...
    // Right after t_FV_out has past, use H_proj or V_max
      ...
      if (measure_offset_L)
        return V_max(V_full(x) + V_proj(x), V_FV_out(x), 1.0);
      if (measure_offset_M)
        return V_max(V_FV_out(x), V_full(x) + V_proj(x), 1.0);
      else
    ...
    // One timeslice after t_FV_out has past, use H_TV or V_max
      ...
      if (measure_offset_M)
        return V_max(V_full(x) + V_proj(x), V_TV(x), 1.0);
      else
        return V_TV(x);
    }
    // For the rest of the time, use H_TV
  ...
  inline RealD dV_t_dtTV(const Vector<RealD>& x, const Long t, const int idx)
  {
    // Returns the potential evaluated at point x
    // Start with H_proj
    if (t == 0) return dV_full(x, idx) + dV_proj(x, idx);
    // Until t_FV_out has past, use H_FV_out
    else if (t <= t_FV_out)
      return dV_FV_out(x, idx);
    // Until t_FV_mid has past, use H_FV_mid
    ...
    // Right after t_FV_out has past, use H_proj or V_max
      ...
      if (measure_offset_L)
        return dV_max(V_full(x) + V_proj(x), dV_full(x, idx) + dV_proj(x, idx),
                      V_FV_out(x), dV_FV_out(x, idx), 1.0);
      if (measure_offset_M)
        return dV_max(V_FV_out(x), dV_FV_out(x, idx), V_full(x) + V_proj(x),
                      dV_full(x, idx) + dV_proj(x, idx), 1.0);
      else
    ...
    // One timeslice after t_FV_out has past, use H_TV or V_max
      ...
      if (measure_offset_L)
        return dV_max(V_TV(x), dV_TV(x, idx), V_full(x) + V_proj(x),
                      dV_full(x, idx) + dV_proj(x, idx), 1.0);
      if (measure_offset_M)
        return dV_max(V_full(x) + V_proj(x), dV_full(x, idx) + dV_proj(x, idx),
                      V_TV(x), dV_TV(x, idx), 1.0);
      else
        return dV_TV(x, idx);
    }
    // For the rest of the time, use H_TV
  ...
  inline RealD dV_phi4(const RealD x, const RealD y, const int idx)
  {
    ...
    // else return beta*y;
    if (idx == 0)
      return beta * (2.0 * x - alpha * y * y);
    else
      return beta * 2.0 * (1 - alpha * x) * y;
  }
  ...
  inline RealD dV_full_op_fixed(const Vector<RealD>& x, RealD op, const int idx)
  {
    ...
    if (x[0] < 0) {
      if (idx == 0)
        return dV_full_xy(x[0], x[1] / norm, idx);
      else
        return (1 / norm -
                0.5 * x[idx] * d_order_param(x, idx) / norm / order_param(x)) *
               dV_full_xy(x[0], x[1] / norm, idx);
    } else
      return (1 / norm -
              0.5 * x[idx] * d_order_param(x, idx) / norm / order_param(x)) *
  ...
  inline RealD V_FV_out(const Vector<RealD>& x)
  {
    if (order_param(x) > center_bar + FV_offset)
      return V_full_op_fixed(x, center_bar + FV_offset) +
             barrier_strength * (order_param(x) - center_bar - FV_offset) *
                 (order_param(x) - center_bar - FV_offset);
    else
      return V_full(x);
  }
  ...
  {
    if (order_param(x) > center_bar + FV_offset) {
      return dV_full_op_fixed(x, center_bar + FV_offset, idx) +
             d_order_param(x, idx) * 2.0 * barrier_strength *
                 (order_param(x) - center_bar - FV_offset);
    } else
      return dV_full(x, idx);
  }

  inline RealD V_FV_mid(const Vector<RealD>& x)
  {
    if (order_param(x) > center_bar)
      return V_full_op_fixed(x, center_bar) +
             barrier_strength * (order_param(x) - center_bar) *
                 (order_param(x) - center_bar);
    else
      return V_full(x);
  }
  ...
  inline RealD V_TV(const Vector<RealD>& x)
  {
    if (order_param(x) < center_bar + TV_offset)
      return V_full(x) + barrier_strength *
                             (order_param(x) - center_bar - TV_offset) *
                             (order_param(x) - center_bar - TV_offset);
    else
      return V_full(x);
  }
  ...
  {
    if (order_param(x) < center_bar + TV_offset) {
      return dV_full(x, idx) + d_order_param(x, idx) * 2.0 * barrier_strength *
                                   (order_param(x) - center_bar - TV_offset);
    } else
      return dV_full(x, idx);
  }
  ...
  {
    RealD rtn =
        -log((1 - exp(-(V_FV_out(x) - V_full(x) + epsilon) * dt)) / dt) / dt;
    // When x is low enough that epsilon is relevant, remove V_full (which
    // will be added later) to avoid ergodicity issues
    if (order_param(x) < center_bar + FV_offset) {
      rtn += barrier_strength *
             std::pow(center_bar + FV_offset - order_param(x), 0.5);
    }
    return rtn;
  }
  ...
  inline RealD dV_proj(const Vector<RealD>& x, const int idx)
  {
    ...
    RealD rtn =
        -((dV_FV_out(x, idx) - dV_full(x, idx)) * exp(-(Vbar + epsilon) * dt)) /
        (1 - exp(-(Vbar + epsilon) * dt));
    if (order_param(x) < center_bar + FV_offset) {
      rtn += -d_order_param(x, idx) * 0.5 * barrier_strength /
             std::pow(center_bar + FV_offset - order_param(x), 0.5);
    }
    return rtn;
  }
  ...
  inline RealD V_max(const RealD V_D, const RealD V_N, const RealD P)
  {
    if (V_N < V_D)
      ...
      return (1 - P) * V_D + P * V_N;
  }

  inline RealD dV_max(const RealD V_D, const RealD dV_D, const RealD V_N,
                      const RealD dV_N, const RealD P)
  {
    if (V_N < V_D)
      return dV_D;
    else
      return (1 - P) * dV_D + P * dV_N;
  }
  ...
  inline RealD dV_FV_floored(const Vector<RealD>& x, const RealD P,
                             const int idx)
  {
    RealD floor = V_FV_min + P * (V_bar_max() - V_FV_min);
    if (V_FV_mid(x) < floor)
  ...
  inline RealD V_L(const Vector<RealD>& x, const RealD V_N)
  {
    if (L < 0.5)
      ...
      return V_max(V_FV_floored(x, 1), V_N, 2 * (L - 0.5));
  }

  inline RealD dV_L(const Vector<RealD>& x, const RealD V_N, const RealD dV_N,
                    const int idx)
  {
    if (L < 0.5)
      return dV_FV_floored(x, 2 * L, idx);
    else
      return dV_max(V_FV_floored(x, 1), dV_FV_floored(x, 1, idx), V_N, dV_N,
                    2 * (L - 0.5));
  }
  ...
  inline RealD dV_M(const Vector<RealD>& x, const RealD V_N, const RealD dV_N,
                    const int idx)
  {
    return dV_max(V_N, dV_N, V_FV_floored(x, 1), dV_FV_floored(x, 1, idx), M);
  }

  inline RealD action_point(QMAction& qma, Field<RealD>& f,
                            const Int multiplicity, const Geometry& geo,
                            Coordinate xl)
  {
    ...
    return (beta / 2.0 / qma.dt / qma.dt) * dpsi_sq +
           qma.V_zeroed(psi, geo.coordinate_g_from_l(xl)[3]);
  }
