...
def open_fields(path, mode, new_size_node=None):
    ...
    if path[-14:] == "/geon-info.txt":
        path = path[:-14]
    if mode == "r":
        return q.ShuffledFieldsReader(path, new_size_node)
    elif mode == "w":
        assert new_size_node is not None
        return q.ShuffledFieldsWriter(path, new_size_node)
    elif mode == "a":
        if new_size_node is None:
            new_size_node = q.Coordinate()
        return q.ShuffledFieldsWriter(path, new_size_node, True)
    else:
...
def check_fields(path, is_check_all=True, new_size_node=None):
    ...
    if path[-14:] == "/geon-info.txt":
        path = path[:-14]
    is_only_check = True
    return q.properly_truncate_fields(path, is_check_all, is_only_check, new_size_node)
