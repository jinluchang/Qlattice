...
// compare in bytes may have issues when geo is have undefined values due to
// alignment
bool Compare_geo(const Geometry& g0, const Geometry& g1,
                 const bool compare_bytes)
{
  if (compare_bytes) {
    ...
    Int equal = 1;
    if (g0.initialized != g1.initialized) {
      return 0;
    }
    if (g0.eo != g1.eo) {
      return 0;
    }
    if (g0.is_only_local != g1.is_only_local) {
      return 0;
    }
    //
    if (g0.geon != g1.geon) {
    ...
    if (g0.node_site != g1.node_site) {
      return 0;
    }
    if (g0.node_site_expanded != g1.node_site_expanded) {
      return 0;
    }
    //
    if (g0.expansion_left != g1.expansion_left) {
      return 0;
    }
    if (g0.expansion_right != g1.expansion_right) {
      return 0;
    }
    //
    if (g0.total_site() != g1.total_site()) {
      return 0;
    }
    //
    return equal;
                      ...
                      const bool compare_bytes)
{
  ...
  if (compare_bytes) {
    ...
    if (g1.initialized < g0.initialized) {
      return false;
    }
    //
    if (g0.total_site() < g1.total_site()) {
    ...
    if (g1.total_site() < g0.total_site()) {
      return false;
    }
    //
    if (g0.is_only_local < g1.is_only_local) {
    ...
    if (g1.is_only_local < g0.is_only_local) {
      return false;
    }
    //
    if (g0.eo < g1.eo) {
    ...
    //
    if (g0.geon.size_node < g1.geon.size_node) {
      return true;
    }
    if (g1.geon.size_node < g0.geon.size_node) {
      return false;
    }
    //
    if (g0.geon.coor_node < g1.geon.coor_node) {
      return true;
    }
    if (g1.geon.coor_node < g0.geon.coor_node) {
      return false;
    }
    //
    if (g0.node_site < g1.node_site) {
    ...
    if (g1.node_site_expanded < g0.node_site_expanded) {
      return false;
    }
    //
    if (g0.expansion_left < g1.expansion_left) {
    ...
    if (g1.expansion_left < g0.expansion_left) {
      return false;
    }
    //
    if (g0.expansion_right < g1.expansion_right) {
    ...
    if (g1.expansion_right < g0.expansion_right) {
      return false;
    }
    //
    return false;
...
Geometry& get_geo_cache_resize(const Geometry& geo, const Coordinate& gl,
                               const Coordinate& gr, box<Geometry>& geo_BOX)
{
...
void geo_to_nv(const Geometry& geo, std::vector<Int>& nv, std::vector<Int>& Nv,
               std::vector<Int>& mv)
{
...
void geo_to_nv(const Geometry& geo, vector<Int>& nv, vector<Int>& Nv,
               vector<Int>& mv)
{
...
{
  static Cache<Gbox_key, box<Geometry>> cache("Gbox_Key",
                                              QLAT_MAX_GEO_CACHE + 3);
  return cache;
}
...
Geometry& get_geo_cache(const Geometry& geo, const Coordinate& gl,
                        const Coordinate& gr)
{
  Geometry geo_ex = geo_resize(geo, gl, gr);
  // if(get_id_node() == 0){
...
void clear_geo_cache() { get_Gbox_key_cache().clear(); }
