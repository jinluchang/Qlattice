...
time make \
    ...
    CXX="nvcc -x cu" \
    CXXLD="nvcc -link" \
    FLAGS="-w --shared -std=c++14 -O3 -ccbin mpicxx -gencode arch=compute_70,code=sm_70 -Xcompiler -fPIC -Xcompiler -fopenmp -Xcompiler -fno-strict-aliasing --expt-extended-lambda --expt-relaxed-constexpr"
