...
def get_cexpr_meson_bk_bpi_corr():
    fn_base = "cache/auto_contract_cexpr/get_cexpr_meson_bk_bpi_corr"
    #
    def calc_cexpr():
...
def get_cexpr_meson_jt_zv():
    fn_base = "cache/auto_contract_cexpr/get_cexpr_meson_jt_zv"
    #
    def calc_cexpr():
        ...
        diagram_type_dict[
            ((("t_1p", "t_2p"), 1), (("t_2p", "x_1"), 1), (("x_1", "t_1p"), 1))
        ] = "Type2"
        diagram_type_dict[
            ((("t_1p", "t_2p"), 1), (("t_2p", "t_1p"), 1), (("x_1", "x_1"), 1))
        ] = None
        exprs = [
            mk_sym(1)
            / 2
...
def get_cexpr_meson_jj_mm():
    ...
    #
    def calc_cexpr():
        jj_op = sum([mk_j_mu("x_1", mu) * mk_j_mu("x_2", mu) for mu in range(4)])
        exprs = [
            jj_op * mk_pi_0("t_1", True) * mk_pi_0("t_2") + "pi0 j_mu j_mu pi0",
            mk_sym(1)
            / 2
            * jj_op
            * (
                mk_pi_p("t_1", True) * mk_pi_p("t_2")
                + mk_pi_m("t_1", True) * mk_pi_m("t_2")
            )
            + "pi+ j_mu j_mu pi+",
            mk_sym(1)
            / 2
            ...
            * jj_op
            * (
                mk_k_p("t_1", True) * mk_k_p("t_2")
                + mk_k_m("t_1", True) * mk_k_m("t_2")
            )
            + "K+ j_mu j_mu K+",
            jj_op + "j_mu j_mu",
        ]
...
def get_cexpr_meson_jj_xx():
    fn_base = "cache/auto_contract_cexpr/get_cexpr_meson_jj_xx"
    #
    def calc_cexpr():
        jj_op = sum(
            [
...
def get_cexpr_meson_jj_mm_types():
    fn_base = "cache/auto_contract_cexpr/get_cexpr_meson_jj_mm_types"
    #
    def calc_cexpr():
        diagram_type_dict = dict()
        diagram_type_dict[
