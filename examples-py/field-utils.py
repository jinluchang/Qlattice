...
json_results.append((f"qnorm", qnorm, 1e-12,))
...
json_results.append((f"gf sig1", sig1, 1e-12,))
...
json_results.append((f"gf sig12", sig12, 1e-12,))
...
json_results.append((f"gf sig2", sig2, 1e-12,))
...
json_results.append((f"gf sig22", sig22, 1e-12,))
...
json_results.append((f"t_dir=3 (default) qnorm(gf_sum_tslice)", qnorm, 1e-12,))
...
json_results.append((f"diff norm", gf1.qnorm(), 1e-12,))
...
json_results.append((f"sum_tslice1.qnorm()", sum_tslice1.qnorm(), 1e-12,))
json_results.append((f"sum_tslice2.qnorm()", sum_tslice2.qnorm(), 1e-12,))
json_results.append((f"sum_tslice.qnorm()", sum_tslice.qnorm(), 1e-12,))
...
json_results.append((f"sf sig", sig, 1e-7,))

sf1 = sf.shift()
sig = q.glb_sum(q.get_data_sig(sf1[:], q.RngState(f"seed-sf1-sig-{q.get_id_node()}")))
json_results.append((f"sf1 sig", sig, 1e-7,))
...
json_results.append((f"sf2 sig", sig, 1e-7,))
...
json_results.append((f"sf3 sig", sig, 1e-7,))
...
json_results.append((f"sf4 sig", sig, 1e-7,))
