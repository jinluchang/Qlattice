...
q.json_results_append("gf", q.get_data_sig_arr(gf, q.RngState(), 3), 1e-12)
...
q.json_results_append("gfm", q.get_data_sig_arr(gfm, q.RngState(), 3), 1e-12)

gf1 = fft_b * gfm

gf1.show_info()

q.json_results_append("gf1", q.get_data_sig_arr(gf1, q.RngState(), 3), 1e-12)

f_factor = q.mk_phase_field(
    gf.geo,
    [
        1,
        0,
...
q.json_results_append("qnorm", qnorm, 1e-12)
...
q.json_results_append("t_dir=3 (default) qnorm(gf_sum_tslice)", qnorm, 1e-12)
...
q.json_results_append("f sig", sig, 1e-7)

f1 = f.shift()
sig = q.glb_sum(q.get_data_sig(f1[:], q.RngState(f"seed-f1-sig-{q.get_id_node()}")))
q.json_results_append("f1 sig", sig, 1e-7)
...
q.json_results_append("f2 sig", sig, 1e-7)
...
q.json_results_append("sf sig", sig, 1e-7)

sf1 = sf.shift()
sig = q.glb_sum(q.get_data_sig(sf1[:], q.RngState(f"seed-sf1-sig-{q.get_id_node()}")))
q.json_results_append("sf1 sig", sig, 1e-7)
...
q.json_results_append("sf2 sig", sig, 1e-7)
...
q.json_results_append("sf3 sig", sig, 1e-7)
