...
def mk_fgf_gf_evolve(job_tag, geo, fgf, fgf_g):
    ...
    def gf_evolve(gf, af, gm_v, dt, *, tag=None, is_initial_gauge_fixed=False):
        ...
        #
        if is_no_af:
            assert tag == "project_gauge_transform"
            assert af is None
        #
        gf_init = gf
        af_init = af
        ...
        def evolve_project_gauge_transform(dt_step):
            ...
            gm_v_left += gm_v
            q.gf_evolve(gf, gm_v_left, dt_step)
            q.gf_evolve_dual(gf, gm_v_right, dt_step)
            gt_inv = fgf(gf).inv()
            gf = gt_inv * gf
            gt_inv -= gt_unit
...
set_param(job_tag, "hmc", "max_traj_always_accept")(0)
set_param(job_tag, "hmc", "max_traj_reverse_test")(0)
set_param(job_tag, "hmc", "md_time")(0.5)
set_param(job_tag, "hmc", "n_step")(16)
set_param(job_tag, "hmc", "beta")(2.80)
set_param(job_tag, "hmc", "c1")(-0.331)
set_param(job_tag, "hmc", "diff_eps")(1e-5)
set_param(job_tag, "hmc", "gf_integrator_tag")("force_gradient")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Flexible gauge fixed HMC.")
    parser.add_argument(
        "--job_tag_list",
        type=str,
        default="",
