...
jj_op = (
    ...
    + "(xx_1-xx_2)_i * (xx_1-xx_2)_j * j_i(xx_1) * j_j(xx_2)"
)
...
exprs = [
    ...
    qac.mk_m("u", "xx")
    * qac.mk_proton("x_2", "u", is_dagger=True)
    * qac.mk_proton("x_1", "u"),
    qac.mk_m("d", "xx")
    * qac.mk_proton("x_2", "u", is_dagger=True)
    * qac.mk_proton("x_1", "u"),
...
for is_cython in [
    ...
    for name in qac.get_expr_names(ccexpr):
        q.json_results_append(name)
    base_positions_dict = dict()
    check, check_ama = qac.benchmark_eval_cexpr(
        ccexpr, base_positions_dict=base_positions_dict
    )
