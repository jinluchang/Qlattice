...
if mp_pool_n_proc > 0:
    ...
    mp_pool = multiprocessing.get_context("fork").Pool(
        mp_pool_n_proc, initializer=q.q_fit_corr.mp_initializer
    )
else:
    mp_pool = None
...
param_arr_setup = q.q_fit_corr_2.sort_param_arr_free_eig(
    param_arr_setup, n_ops, free_eig_idx_arr=np.arange(n_eigs)
)
...
q.json_results_append("param_arr_setup sig1", sig1, 1e-7)

sig2 = q.get_data_sig(jk_corr_data, q.RngState())

q.json_results_append("jk_corr_data sig2", sig2, 1e-7)

sig3 = q.get_data_sig(corr_data_sigma, q.RngState())

q.json_results_append("corr_data_sigma sig3", sig3, 1e-7)
...
q.json_results_append("jk_chisq sig4", sig4, 1e-4)

sig5 = q.get_data_sig(abs(res["jk_param_arr"]), q.RngState())

q.json_results_append("jk_param_arr sig5", sig5, 1e-4)

sig6 = q.get_data_sig(abs(res["jk_param_grad_arr"]), q.RngState())

q.json_results_append("jk_param_grad_arr sig6", sig6, 1e-4)

sig7 = q.get_data_sig(abs(res["jk_param_for_scaled_corr_arr"]), q.RngState())

q.json_results_append("jk_param_for_scaled_corr_arr sig7", sig7, 1e-4)
...
q.json_results_append("jk_chisq sig8", sig8, 1e-2)
