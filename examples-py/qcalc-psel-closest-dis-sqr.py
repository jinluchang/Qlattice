...
def run_psel_closest_dis_sqr(fn_list):
    ...
    for fn in fn_list_init:
        if not fn.endswith(".lati"):
            q.displayln_info(
                -1, f"WARNING: '{fn}' does not endswith '.lati'. Skip this file."
            )
            continue
    ...
    q.displayln_info(
        "INFO: idx closest_dis_sqr_list[idx] len(psel_list[idx]) psel_list[idx].total_site fn_list[idx]"
    )
    ...
    if is_test():
        num_test = 10
        assert closest_dis_sqr_list[:num_test] == q.find_closest_dis_sqr_for_psel_list(
            psel_list[:num_test], is_parallel=False
        )
        assert closest_dis_sqr_list[:num_test] == q.find_closest_dis_sqr_for_psel_list(
            psel_list[:num_test], is_parallel=True
        )
    return ret
