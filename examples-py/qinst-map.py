...
def run_inst_map(fn_out, fn_gf=None):
    ...
    if q.does_file_exist_qar_sync_node(fn_out):
        q.displayln_info(
            -1,
            f"{fname}: WARNING: '{fn_out}' for '{fn_gf}' already exist. Simply display its contents.",
        )
        q.json_results_append(f"{fname}: Start show inst_map_obj fn='{fn_out}'")
        inst_map_obj = q.load_pickle_obj(fn_out)
        q.displayln_info_inst_map_obj(inst_map_obj)
    ...
    q.json_results_append(
        f"{fname}: Start compute inst_map_obj fn='{fn_out}' for '{fn_gf}'"
    )
    gf = q.GaugeField()
    gf.load(fn_gf)
    ...
    q.json_results_append(
        f"{fname}: End compute inst_map_obj fn='{fn_out}' for '{fn_gf}'"
    )
    q.json_results_append(f"{fname}: Start show inst_map_obj fn='{fn_out}'")
    inst_map_obj = q.load_pickle_obj(fn_out)
    q.displayln_info_inst_map_obj(inst_map_obj)
