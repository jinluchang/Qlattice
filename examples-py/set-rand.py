...
json_results_append(f"total_site = {total_site}")
json_results_append(f"geo = {geo}")

prop = q.Prop(geo)
prop.set_rand(rs_prop, 1.0, 0.0)
json_results_append(f"prop.crc32() = {prop.crc32()} ; prop.qnorm()", prop.qnorm())
...
sc_prop @= prop
json_results_append(f"sc_prop.qnorm()", sc_prop.qnorm())

sc_prop1 = q.SelProp(fselc)
sc_prop1.set_rand(rs_prop, 1.0, 0.0)
sc_prop1 -= sc_prop
json_results_append(f"sc_prop1.qnorm()", sc_prop1.qnorm())

s_prop = q.SelProp(fsel)
s_prop @= prop
json_results_append(f"s_prop.qnorm()", s_prop.qnorm())

s_prop1 = q.SelProp(fsel)
s_prop1.set_rand(rs_prop, 1.0, 0.0)
s_prop1 -= s_prop
json_results_append(f"s_prop1.qnorm()", s_prop1.qnorm())

s_prop2 = q.SelProp(fsel)
s_prop2 @= sc_prop
s_prop2 -= s_prop
json_results_append(f"s_prop2.qnorm()", s_prop2.qnorm())

sp_prop = q.PselProp(psel)
sp_prop @= prop
json_results_append(f"sp_prop.qnorm()", sp_prop.qnorm())

sp_prop1 = q.PselProp(psel)
sp_prop1 @= sc_prop
sp_prop1 -= sp_prop
json_results_append(f"sp_prop1.qnorm()", sp_prop1.qnorm())

prop_norm = q.sqrt_field(q.qnorm_field(prop))
json_results_append(f"prop_norm.qnorm()", prop_norm.qnorm())

s_prop_norm = q.sqrt_field(q.qnorm_field(s_prop))
json_results_append(f"s_prop_norm.qnorm()", s_prop_norm.qnorm())

s_prop_norm1 = q.SelectedField(q.ElemTypeRealD, fsel)
s_prop_norm1 @= prop_norm
s_prop_norm1 -= s_prop_norm
json_results_append(f"s_prop_norm1.qnorm()", s_prop_norm1.qnorm())

sp_prop_norm = q.sqrt_field(q.qnorm_field(sp_prop))
json_results_append(f"sp_prop_norm.qnorm()", sp_prop_norm.qnorm())

sp_prop_norm1 = q.SelectedPoints(q.ElemTypeRealD, psel)
sp_prop_norm1 @= prop_norm
sp_prop_norm1 -= sp_prop_norm
json_results_append(f"sp_prop_norm1.qnorm()", sp_prop_norm1.qnorm())
