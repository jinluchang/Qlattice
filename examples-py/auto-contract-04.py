...
diagram_type_dict[()] = "1"
diagram_type_dict[((("t_1", "x_1"), 1), (("x_1", "t_1"), 1), (("x_2", "x_2"), 1))] = (
    "TypeD"
)
diagram_type_dict[((("t_1", "x_1"), 1), (("x_1", "x_2"), 1), (("x_2", "t_1"), 1))] = (
    "TypeC"
)
diagram_type_dict[((("t_2", "x_1"), 1), (("x_1", "t_2"), 1), (("x_2", "x_2"), 1))] = (
    "TypeD"
)
diagram_type_dict[((("t_2", "x_1"), 1), (("x_1", "x_2"), 1), (("x_2", "t_2"), 1))] = (
    "TypeC"
)
...
cexpr = qac.contract_simplify_compile(
    *exprs, is_isospin_symmetric_limit=True, diagram_type_dict=diagram_type_dict
)

q.json_results_append(qac.display_cexpr(cexpr)[:256])

for v in cexpr.list():
    q.json_results_append(str(v)[:256])

q.json_results_append("diagram_type_dict = qac.get_diagram_type_dict(cexpr)")
for k, v in qac.get_diagram_type_dict(cexpr).items():
    q.json_results_append(f"diagram_type_dict[{k!r}] = {v!r}")

q.json_results_append("qac.get_expr_names(cexpr)")
...
q.json_results_append(qac.display_cexpr(cexpr_opt)[:256])

for v in cexpr_opt.list():
    q.json_results_append(str(v)[:256])

q.json_results_append("diagram_type_dict = qac.get_diagram_type_dict(cexpr_opt)")
for k, v in qac.get_diagram_type_dict(cexpr_opt).items():
    q.json_results_append(f"diagram_type_dict[{k!r}] = {v!r}")

q.json_results_append("qac.get_expr_names(cexpr_opt)")
...
def get_cexpr_test(is_cython=False):
    ...
    def calc_cexpr():
        ...
        return cexpr
    #
    return qac.cache_compiled_cexpr(
        calc_cexpr, fn_base, is_cython=is_cython, base_positions_dict=None
    )
...
for is_cython in [
    ...
    q.json_results_append(qac.cexpr_code_gen_py(cexpr_opt, is_cython=is_cython)[:256])
    ccexpr = get_cexpr_test(is_cython=is_cython)
    q.json_results_append("diagram_type_dict = qac.get_diagram_type_dict(ccexpr)")
    for k, v in qac.get_diagram_type_dict(ccexpr).items():
        q.json_results_append(f"diagram_type_dict[{k!r}] = {v!r}")
    q.json_results_append("qac.get_expr_names(ccexpr)")
    for name in qac.get_expr_names(ccexpr):
        q.json_results_append(name)
    check, check_ama = qac.benchmark_eval_cexpr(ccexpr)
    q.json_results_append(
        f"get_cexpr_test benchmark_eval_cexpr check get_data_sig is_cython={is_cython}",
        q.get_data_sig(np.array(check, dtype=np.complex128), q.RngState()),
    )
    q.json_results_append(
        f"get_cexpr_test benchmark_eval_cexpr check_ama get_data_sig is_cython={is_cython}",
