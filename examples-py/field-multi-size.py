...
for total_site in total_site_list:
    for seed in range(2):
        ...
        q.json_results_append(
            f"smear {total_site} {seed} prop", q.get_data_sig_arr(prop, rs, 2), 1e-12
        )
        q.json_results_append(
            f"smear {total_site} {seed} prop0", q.get_data_sig_arr(prop0, rs, 2), 1e-12
        )
        q.json_results_append(
            f"smear {total_site} {seed} prop1", q.get_data_sig_arr(prop1, rs, 2), 1e-12
        )
...
for is_forward in [
    ...
    for is_only_spatial in [
        False,
        True,
    ]:
        fft0 = q.mk_fft(
            is_forward=is_forward, is_only_spatial=is_only_spatial, mode_fft=0
        )
        fft1 = q.mk_fft(
            is_forward=is_forward, is_only_spatial=is_only_spatial, mode_fft=1
        )
        for total_site in total_site_list:
            for multiplicity in multiplicity_list:
                for seed in range(1):
                    ...
                    f1 = fft1 * f
                    q.json_results_append(
                        f"fft {total_site} {multiplicity} {seed} f",
                        q.get_data_sig(f, rs),
                        1e-12,
                    )
                    q.json_results_append(
                        f"fft {total_site} {multiplicity} {seed} f0",
                        q.get_data_sig(f0, rs),
                        1e-12,
                    )
                    q.json_results_append(
                        f"fft {total_site} {multiplicity} {seed} f1",
                        q.get_data_sig(f1, rs),
                        1e-12,
                    )
