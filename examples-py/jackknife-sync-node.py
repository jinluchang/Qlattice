...
ok = bool(np.array_equal(jk_default_seq, jk_default_sync))
...
# ---- jk_type = "super" with is_sync_node must raise ----
super_raised = False
try:
    q.g_mk_jk(data_arr_1, jk_idx_list_1, jk_type="super", is_sync_node=True)
except Exception:
    super_raised = True
q.json_results_append(f"super + is_sync_node raises = {super_raised}")
assert super_raised
