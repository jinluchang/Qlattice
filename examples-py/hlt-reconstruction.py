...
q.json_results_append("g_t_arr_o", q.get_data_sig(g_t_arr_o, q.RngState()), check_eps)
...
q.json_results_append("aa", aa, check_eps)
q.json_results_append("aa0", aa0, check_eps)
q.json_results_append("aa / aa0", aa / aa0, check_eps)
...
q.json_results_append(
    "delta_target", q.get_data_sig(delta_target, q.RngState()), check_eps
)

delta = delta_from_g(g_t_arr_o, t_arr, e_arr)
delta = np.array(delta)

q.json_results_append("delta", q.get_data_sig(delta, q.RngState()), check_eps)
...
q.json_results_append("aa ; via sum", aa, check_eps)
q.json_results_append("aa0 ; via sum", aa0, check_eps)
q.json_results_append("aa / aa0 ; via sum", aa / aa0, check_eps)

aa = aa_from_g(g_t_arr_o_via_sum, params).item()
aa0 = aa_from_g(np.zeros_like(g_t_arr_o_via_sum), params).item()
q.displayln_info(f"A={aa} ; A_0={aa0} ; d={np.sqrt(aa / aa0)} via sum")

q.json_results_append("aa ; g via sum", aa, check_eps)
q.json_results_append("aa0 ; g via sum", aa0, check_eps)
q.json_results_append("aa / aa0 ; g via sum", aa / aa0, check_eps)

delta_via_sum = delta_from_g(g_t_arr_o_via_sum, t_arr, e_arr)
delta_via_sum = np.array(delta_via_sum)

q.json_results_append(
    "delta via sum", q.get_data_sig(delta_via_sum, q.RngState()), check_eps
)
