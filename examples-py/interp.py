...
q.json_results_append("i_arr", i_arr)
q.json_results_append("approx_x_arr", approx_x_arr)

interp_data_arr = q.interp(data_arr, i_arr)

q.json_results_append("interp_data_arr", interp_data_arr.ravel())
...
q.json_results_append("i2_arr", i2_arr)
assert np.all(
    np.array(
        [
            q.get_threshold_idx(data_arr[i], threshold_arr[i])
...
assert np.allclose(
    np.array(
        [q.interp(data_arr[i], i2_arr[i]) for i in range(len(i2_arr))], dtype=np.float64
    ),
    threshold_arr,
    rtol=1e-08,
    atol=1e-08,
...
q.json_results_append("x_arr", x_arr)
