...
def benchmark_prop_spatial_smear(total_site, seed):
    ...
    for idx in range(3):
        ...
        if is_show_sig:
            ...
            q.json_results_append("ss_prop reference sig", ss_prop_sig, 1e-10)
        with q.TimerFork(
            show_display=True, display_tag="TimerFork(prop_spatial_smear)"
        ):
            ss_prop = q.prop_spatial_smear(prop, gf, coef, step, mom, chunk_size=12)
        if is_show_sig:
            ss_prop_sig = q.get_data_sig_arr(ss_prop, rs, 3)
            q.json_results_append("ss_prop spatial sig", ss_prop_sig, 1e-10)
