...
def run_hmc_evolve(
    ...
    *,
    ...
    energy = (
        gm_hamilton_node_fa(gm, mf)
        + gm_hamilton_node_fa(gm_dual, mf_dual)
    ...
    *,
    ...
    if is_project_gauge_transform:
        ...
        if change_qnorm > 1e-8:
            q.displayln_info(
                f"{fname}: project_gauge_transform: change_qnorm={change_qnorm}"
            )
    ...
    if is_reverse_test:
        ...
        q.displayln_info(
            f"{fname}: reversed gm_dual_diff: {gm_dual_diff_norm} / {gm_dual_norm}"
        )
...
def run_hmc_mass_mom_refresh(
    ...
    is_project_gauge_transform = get_param(
        job_tag, "hmc", "fa", "is_project_gauge_transform"
    )
    complete_refresh_interval = get_param(
        job_tag, "hmc", "fa", "complete_refresh_interval", default=1
    )
    ...
    if is_force_refresh:
        if not is_refresh:
            q.displayln_info(
                f"{fname}: Force complete refresh of mass and momentum. {job_tag} {traj}"
            )
        is_refresh = True
    if is_refresh:
...
def run_hmc(job_tag):
    ...
    assert (save_traj_interval % complete_refresh_interval == 0) or (
        complete_refresh_interval % save_traj_interval == 0
    )
    ...
    for traj in range(traj, max_traj):
        ...
        if traj % complete_refresh_interval == 0:
            mom_auto_corr.save_mac(
                get_save_path(f"{job_tag}/hmc-mom-auto-corr/traj-{traj}.pickle")
            )
...
set_param(job_tag, "hmc", "fa", "interval_list")(
    [
        1,
        2,
    ]
...
set_param(job_tag, "hmc", "fa", "mass_list")(
    [
        1.0,
        1.25,
        1.5,
