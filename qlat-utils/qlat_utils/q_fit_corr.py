...
def sort_param_arr_free_energy(param_arr, n_ops, free_energy_idx_arr):
    ...
    free_e_arr, free_c_arr = zip(
        *sorted(zip(free_e_arr, free_c_arr), key=lambda p: p[0])
    )
    e_arr[free_energy_idx_arr] = free_e_arr
    c_arr[free_energy_idx_arr] = free_c_arr
    new_param_arr = np.concatenate(
        [
            e_arr,
            c_arr.ravel(),
        ],
...
def apply_energy_minimum(param_arr, energy_minimum_arr=None, free_energy_idx_arr=None):
    ...
    new_param_arr[..., free_energy_idx_arr] = energy_minimum_arr + np.abs(
    ...
    def fcn_f(param_arr):
        ...
        if energy_minimum_arr is not None:
            es = es.at[free_energy_idx_arr].set(
                energy_minimum_arr
                + jnp.abs(es[free_energy_idx_arr] - energy_minimum_arr)
            )
        # corr[op1_idx, op2_idx, t_idx]
        corr = (
            cs[:, :, None, None]
...
def adaptive_minimize(
    ...
    while True:
        ...
        param_arr, i_step = minimize(
            fcn, n_step=n_step, step_size=step_size, param_arr=param_arr
        )
        total_steps += i_step
...
def mk_mp_pool(n_proc=None):
    ...
    if n_proc == 0:
        ...
        #
        mp_pool = multiprocessing.get_context("spawn").Pool(
            mp_pool_n_proc, initializer=mp_initializer
        )
    mp_map = mp_pool.imap
        ...
        *,
        ...
        for i in range(n_step_mini_jk):
            ...
            if is_sorting_energy_state:
                param_arr = sort_param_arr_free_energy(
                    param_arr, n_ops, free_energy_idx_arr
                )
...
def fit_energy_amplitude(
    ...
    *,
    ...
    if rng_seed_list is None:
        rng_seed_list = [
            "fit_energy_amplitude-seed-param",
        ]
    ...
    c_arr = (
        ...
        * np.exp(
            -e_arr[:, None]
            * (t_start_fit - (t_start_fcn_arr - t_start_param_arr)[:, None])
            / 2
        )
    )
    #
    param_arr_initial = np.concatenate(
        [
            e_arr,
            c_arr.ravel(),
        ],
    ...
    for idx, v in enumerate(
        mp_map(
            jk_mini_task_in_fit_energy_amplitude,
            [
                mk_kwargs(0, True, rng_seed, verbose_level if idx == 0 else -1)
    ...
    jk_c_arr = (
        ...
        / np.exp(
            -jk_e_arr[:, :, None]
            * (t_start_fit - (t_start_fcn_arr - t_start_param_arr)[None, :, None])
            / 2
        )
    )
...
def hmc_traj(fcn, hmc_params):
    ...
    mom_arr = (
        hmc_params.rng.g_rand_arr(hmc_params.n_params)
        * np.sqrt(hmc_mass_arr)
        * np.sqrt(hmc_params.temperature)
    )
    ...
    if hmc_params.hmc_mass_adaptive_rate != 0:
        ...
        hmc_mass_arr = (
            1 - hmc_params.hmc_mass_adaptive_rate
