...
qacc CoordinateD operator*(const RealD a, const CoordinateD& c)
{
  qassert(false == qisnan(c));
...
qacc CoordinateD operator*(const CoordinateD& c, const RealD a)
{
  return a * c;
}
...
inline RealD coordinate_len(const CoordinateD& c)
{
  const RealD ans = std::sqrt(sqr(c[0]) + sqr(c[1]) + sqr(c[2]) + sqr(c[3]));
  RealD cmax = 0.0;
  ...
  if (0.0 == ans) {
    if (0.0 == cmax) {
      ...
      const RealD ans = cmax * coordinate_len(c / cmax);
...
qacc RealD dot_product(const CoordinateD& c1, const CoordinateD& c2)
{
  qassert(false == qisnan(c1));
  qassert(false == qisnan(c2));
