                                        ...
                                        const int type4)
{
  ...
  std::vector<std::string> fn_backward_list(num_type);
  for (int i = 0; i < num_type; ++i) {
    ...
    fn_forward_list[i] = path + ssprintf("/forward-%d-%d-%d-%d.field", type1,
                                         type2, type3, type4);
    fn_backward_list[i] = path + ssprintf("/backward-%d-%d-%d-%d.field", type1,
                                          type2, type3, type4);
  }
  ...
  for (int i = 0; i < num_type; ++i) {
    ...
    if (not is_d_field(fn_backward_list[i])) {
      is_complete = false;
      break;
    }
  }
  ...
  std::vector<FieldM<Complex, 8 * 8> > meson_vv_meson_forward_list(num_type);
  std::vector<FieldM<Complex, 8 * 8> > meson_vv_meson_backward_list(num_type);
  for (int i = 0; i < num_type; ++i) {
    meson_vv_meson_forward_list[i].init(geo);
    meson_vv_meson_backward_list[i].init(geo);
    set_zero(meson_vv_meson_forward_list[i]);
    set_zero(meson_vv_meson_backward_list[i]);
  }
  long iter = 0;
  for (long n = 0; n < n_points; ++n) {
    ...
    for (int i = 0; i < num_type; ++i) {
      ...
      contract_meson_vv_meson_acc(meson_vv_meson_forward_list[i],
                                  meson_vv_meson_backward_list[i], wsp1, wsp2,
                                  wsp3, prop4_x_y, xg_y, xg_y_psel_idx, tsep,
  ...
  for (int i = 0; i < num_type; ++i) {
    ...
    meson_vv_meson_forward_list[i] *= coef;
    meson_vv_meson_backward_list[i] *= coef;
  }
  for (int i = 0; i < num_type; ++i) {
    write_field_float_from_double(meson_vv_meson_forward_list[i],
                                  fn_forward_list[i]);
    write_field_float_from_double(meson_vv_meson_backward_list[i],
                                  fn_backward_list[i]);
  }
}
