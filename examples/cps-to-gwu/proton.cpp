...
int main(int argc, char* argv[])
{
  ...
  sprintf(prop_nP,"/global/homes/g/genwang/cscratch/24DH/confs/f.000330.tem.No02.prop");
  sprintf(prop_sP,"/global/homes/g/genwang/cscratch/24DH/confs/f.000330.tem.No02.src");
  ...
  sprintf(filename,"data/dwf_gwu/%s/rbc.%s.%06d.s.N%06d.prop",lat,lat,traj,countsave);
  ...
  std::vector<qlat::FermionField4dT<qlat::Complex> > prop_G;
  prop_G.resize(12);for(int iv=0;iv<12;iv++){prop_G[iv].init(geo);qlat::set_zero(prop_G[iv]);}
  ...
  for (long index = 0; index < noi.geo().local_volume(); ++index) {
    ...
    if(sum > 1e-6){
      ...
      //diffp += diff;
      //if(diff > 0.3*1e-2 and countp < 24)
      //{
      ...
      for(int d0=0;d0<12;d0++){
        ...
        double* sG = (double*)&( prop_G[d0].get_elem(index));
        for(int d1=0;d1<24;d1++)
        {
          ...
          if(fabs(p0) > 1e-28){diff = std::fabs((p0-p1)/p0);}
          diffp += diff;
          if(diff > 0.5*1e-2 and countp < 24)
          {
  ...
  for(unsigned int ti=0;ti<write.size();ti++){write[ti]=0.0;}
  for(unsigned int ti=0;ti<writ0.size();ti++){writ0[ti]=0.0;}
  get_corr_pion(prop_qlat,xg1,write);
  //get_corr_pion(prop_inv,xg,writ0);
  get_corr_pion(prop_G,xg,writ0);
  for(int ti=0;ti<nt;ti++)
  {
    double v0 =  write[ti*2+0];
    double v1 =  writ0[ti*2+0];
    print0("pion t %3d, %.9e %.9e %.9e \n",ti, write[ti*2+0], writ0[ti*2+0], (v0-v1)/v0);
  }
