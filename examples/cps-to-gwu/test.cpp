...
int main(int argc, char* argv[])
{
  ...
  qlat::vector<qlat::FermionField4dT<qlat::ComplexF>  > test;
  test.resize(0);test.resize(10);
  for(int i=0;i<10;i++){test[i].init(geo);}

  std::vector<qlat::FermionField4dT<qlat::ComplexF>  > eigen;
  eigen.resize(0);eigen.resize(30);
  for(int i=0;i<30;i++){eigen[i].init(geo);}
