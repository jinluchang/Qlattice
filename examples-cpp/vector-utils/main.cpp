...
void simple_tests()
{
  ...
  {
    vector_gpu<char> buf;
    buf.resize(500, false);
    buf[0] = 0;
    displayln_info(ssprintf("CHECK: vector gpu: OK"));
  }
  //
  {
    VectorGPUKey gkey(100 * sizeof(qlat::ComplexD), std::string("test_buf"),
                      false);
  ...
  //
  {
    ...
    //
    qlat::vector_gpu<qlat::ComplexD> gauge;
    gauge.resize(Nsize);
    random_Ty(gauge.data(), gauge.size(), 1, int(qlat::u_rand_gen(rs) * 100));
    //
    qlat::vector_gpu<qlat::ComplexD> gfT;
    gfT.resize(NVmpi * Nsize);
    qlat::vector_gpu<qlat::ComplexD> gfT_buf;
    gfT_buf.resize(NVmpi * Nsize);
    for (Long vi = 0; vi < NVmpi; vi++) {
      cpy_data_thread(&(gfT.data()[vi * Nsize]), gauge.data(), Nsize, 1);
    }
    //
    ...
    qlat::vector_gpu<qlat::ComplexD> diff;
    diff.resize(Nsize);
    ComplexD* p1 = gfT.p;
    ComplexD* p2 = gauge.p;
    ComplexD* p3 = diff.p;
    qacc_for(isp, Nsize, { p3[isp] = qnorm(p1[isp] - p2[isp]); });
    displayln_info(
