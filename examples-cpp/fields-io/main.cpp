...
inline void test_read_sbs(const std::string& path, const std::string& fn,
                 ...
                 const Coordinate& new_size_node)
{
  ...
  //
  {
    ...
    for (Long i = 0; i < (Long)fns.size(); ++i) {
      displayln_info(fname + ssprintf(": %5d : '%s' from '%s'.", i,
                                      fns[i].c_str(), path.c_str()));
    }
  }
  //
  {
  ...
  //
  {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      displayln_info(fname + ssprintf(": compute crc32=%08X.", crc_4));
      qassert(crc_4 == crc_2);
      //
      ...
      displayln_info(fname + ssprintf(": compute crc32=%08X.", crc_5));
      qassert(crc_5 == crc_3);
      //
      rf.init();
      read(sfr, "f.sfield", rf);
      const crc32_t crc_6 = field_crc32(rf);
      displayln_info(fname + ssprintf(": compute crc32=%08X.", crc_6));
      qassert(crc_6 == crc_3);
      //
      rf.init();
      read(sfr, "sf.sfield", rf);
      const crc32_t crc_7 = field_crc32(rf);
      displayln_info(fname + ssprintf(": compute crc32=%08X.", crc_7));
      qassert(crc_7 == crc_3);
      //
      rf.init();
