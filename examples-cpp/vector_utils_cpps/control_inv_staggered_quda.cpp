...
int main(int argc, char* argv[])
{
  ...
  const Long Dim = 3;
  const int Nsrc = Dim;
  std::vector<colorFD> qlat_cf;
  qlat_cf.resize(Nsrc);
  for (int i = 0; i < qlat_cf.size(); i++) {
    qlat_cf[i].init(geo);
  }
  //
  //{
...
#pragma omp parallel for
      for (Long qlat_idx_4d = 0; qlat_idx_4d < V; qlat_idx_4d++) {
        ...
        for (int dc = 0; dc < Dim; dc++) {
          if (xg[0] == sp[0] and xg[1] == sp[1] and xg[2] == sp[2] and
              xg[3] == sp[3] and dc == i) {
            res[quda_idx * Dim + dc] = qlat::ComplexD(1.0, 0.0);
          } else {
            res[quda_idx * Dim + dc] = qlat::ComplexD(0.0, 0.0);
          }
        }
      }
      //////set point src at zero
      //
      qinv.do_inv(qinv.cres->V(), qinv.csrc->V(), in.fermion_mass, in.cg_err,
                  in.niter);
      //
      quda_cf_to_qlat_cf(qlat_cf[i], (qlat::ComplexD*)qinv.cres->V());
    }
  }
  //
  EigenV corr;
