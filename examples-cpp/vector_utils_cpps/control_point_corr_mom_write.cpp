    ...
    for (int isource = 0; isource < in.nsource; isource++)
      if (in.propN[isource] != std::string("NONE")) {
        ...
        // check file done
        {
          ...
          if (get_file_size_MPI(name_fft.c_str()) >=
                  size_t(32 * mvol * 2 * 8) and
              check_qlat_head_exist(name_fft.c_str())) {
            fft_done = 1;
          }
          //
          name_corr = qlat::ssprintf("%s.zero.corr", nameQ.c_str());
          if (get_file_size_MPI(name_corr.c_str()) >=
              size_t(32 * long(mdat.nv[3]) * 2 * 8)) {
            corr_done = 1;
          }
          //
          bool DONE = true;
          if (do_fft == 1 and fft_done != 1) {
            DONE = false;
          }
          if (do_corr == 1 and corr_done != 1) {
            DONE = false;
          }
          //
          if (DONE == true) {
        ...
        name_info.replace(name_info.end() - 5 - extra_info_flag,
                          name_info.end(), qlat::ssprintf("GInfo"));
        // qmessage("%s \n", name_info.c_str());
        ...
        for (unsigned int fi = 0; fi < mdat.fn_list.size(); fi++) {
          if (do_fft == 1) {
            if (average.dim == 0) {
              ...
              // fft_local_to_global(FFT_data_global, FFT_data, mdat,
              // mom_shiftp); nvec = FFT_data_global.size()/mvol;
              /// sprintf(key_T, "%d  %d %d  %d %d %d %d",
              /// int(mdat.fn_list.size()), nvec, in.nt, mc, mc, mc, 2);
              sprintf(key_T, "%d  %d %d  %d %d %d %d", int(1), nvec, in.nt, mc,
                      mc, mc, 2);
          ...
          if (do_corr == 1) {
            ...
            corr_zero.INFOA.push_back(std::string("mom shift ") +
                                      Coordinate_to_string(mom_shiftp));
          }
        }
        //
        // char namew[500];sprintf(namew, "%s.%s", nameQ.c_str(),
        // flag_add.c_str());
        if (do_fft == 1) {
