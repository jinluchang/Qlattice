...
void resize_EigenM(std::vector<qlat::vector<qlat::ComplexD>>& a, size_t n0,
                   size_t n1)
{
...
int main(int argc, char* argv[])
{
  ...
  prop0.resize(nmass);
  for (unsigned int i = 0; i < prop0.size(); i++) prop0[i].init(geo);
  ...
  //
  std::vector<Propagator4d> propD;
  propD.resize(nmass);
  for (unsigned int i = 0; i < propD.size(); i++) propD[i].init(geo);
  for (int im = 0; im < nmass; im++) {
    propD[im] = prop0[im];
  }
  //
