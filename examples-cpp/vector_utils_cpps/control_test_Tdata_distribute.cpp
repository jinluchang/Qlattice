...
int main(int argc, char* argv[])
{
  using namespace qlat;
  {
    ...
    if (Li.size() >= 4) {
      if (stringtonum(Li[0]) == 1) {
        test_simple = true;
      }
      if (stringtonum(Li[1]) == 1) {
        test_gather3D = true;
      }
      if (stringtonum(Li[2]) == 1) {
        test_gather4D = true;
      }
      if (stringtonum(Li[3]) == 1) {
        test_original = true;
      }
    } else {
      qmessage("======Need paraI to set test sets. \n");
    }
    //
    if (test_simple) {
      Vec_redistribute vec_large(fd, 1);
    ...
    if (test_gather4D) {
      ...
      //
      qlat::vector_gpu<qlat::ComplexD> sendT;
      sendT.resize(sendbuf.size());
      for (int i = 0; i < 100; i++) {
        TIMER("=====Rotate 4D");
        rot.reorder(false, (double*)sendT.data());
      }
    }
    //
    if (test_gather3D) {
    ...
    if (test_original) {
      ...
      int biva = 2 * (fd.vol * fd.Nt) / (fd.Nvol);
      int civ = 6;
      ...
      for (Long bi = 0; bi < biva; bi++)
        for (unsigned long long vi = 0; vi < fd.Nvol; vi++)
          for (Long ci = 0; ci < civ; ci++) {
            ...
            // int zi = fd.Pos0[fd.rank][2] +
            // (vi%(fd.Nz*fd.Ny*fd.Nx))/(fd.Ny*fd.Nx); int yi =
            // fd.Pos0[fd.rank][1] + (vi%(fd.Ny*fd.Nx))/fd.Nx; int xi =
            // fd.Pos0[fd.rank][0] + (vi%fd.Nx);
            const Coordinate& p = fd.coordinate_g_from_index(vi, fd.rank);
            databuf[off] = ((p[3] * 900 + p[2]) * 900 + p[1]) * 900 + p[0] +
                           std::cos(ci) + qlat::ComplexD(0.0, std::cos(bi));
          }
      //
      /// for(int i=0;i<biva*12*12/civ;i++){memcpy(&sendbuf[i*Nt*N0*N1*N2*civ +
      /// 0],&srcE_civ[i](0),sizeof(Ftype)*2*Nt*N0*N1*N2*civ);}
      memcpy(&sendbuf[0], &databuf[0], 2 * sizeof(double) * sendbuf.size());
      //
