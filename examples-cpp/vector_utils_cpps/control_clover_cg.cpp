...
int main(int argc, char* argv[])
{
  ...
  qlat::vector<qlat::Complex> quda_gf;
  quda_gf.resize(V * 4 * 3 * 3);
  //
  ...
  gauge_param.reconstruct =
      QUDA_RECONSTRUCT_NO;  ////gauge_param.reconstruct = link_recon;
  ...
  //  Set dslash type
  // if (dslash_type == QUDA_CLOVER_WILSON_DSLASH || dslash_type ==
  ...
  //
  inv_param.tol_hq =
      err;  // specify a tolerance for the residual for heavy quark residual
  //
  ...
  if (setup_clover == 1) {
    ...
    ////size_t host_clover_data_type_size = (cpu_prec == QUDA_DOUBLE_PRECISION)
    ///? sizeof(double) : sizeof(float);
    int host_clover_data_type_size = sizeof(qlat::Complex) / 2;
    ////size_t host_spinor_data_type_size = (cpu_prec == QUDA_DOUBLE_PRECISION)
    ///? sizeof(double) : sizeof(float);
    int host_spinor_data_type_size = sizeof(qlat::Complex) / 2;
    qlat::vector<char> quda_clover;
    quda_clover.resize(V * clover_site_size * host_clover_data_type_size);
    qlat::vector<char> quda_clover_inv;
    ...
    // double norm = 0.00; // clover components are random numbers in the range
    // (-norm, norm) double diag = 1.0;  // constant added to the diagonal if
    // (!compute_clover) constructQudaCloverField((void*) quda_clover.data(),
    // norm, diag, inv_param.clover_cpu_prec, V);
  ...
  if (do_inv == 1) {
    ...
    for (int bi = 0; bi < bfac; bi++) {
      ...
      for (int i = 0; i < Nsrc; i++) {
        ...
        //  if(xg[0] == sp[0] and xg[1] == sp[1] and xg[2] == sp[2] and xg[3] ==
        //  sp[3] and dc == i){
      ...
      for (int i = 0; i < Nsrc; i++) {
        ...
        // quda_ff_to_Ffield4d(qlat_ff[i], (qlat::Complex*)
        // quda_out[i]->data());
      }
      //
      qmessage(
          "Done g%03d: %8d iter / %.6f secs = %.3f Gflops, Cost %.3f Gflops \n",
          bi, inv_param.iter, inv_param.secs, inv_param.gflops / inv_param.secs,
          inv_param.gflops);
    }
    for (auto p : quda_in) {
      delete p;
