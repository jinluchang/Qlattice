...
int main(int argc, char* argv[])
{
  ...
  if (mom_smear_ != std::string("NONE")) {
    ...
    Qassert(Li.size() == 4);
    for (int i = 0; i < 4; i++) {
      mom_smear[i] = stringtodouble(Li[i]);
    }
  }
  //
  int icfg = in.icfg;
  ...
  //
  {
    ...
    if (src_step != 0 or sink_step != 0) {
      ...
      if (in.anti_peri == 1) {
        ...
        Qassert(average_all_2pt ==
                0);  // anti-periodic signs not enabled for average yet
        sec_src.antiP = false;
      }
      /////set_left_expanded_gauge_field(gfD, gf);
    }
    /////========load links
    int nmass_group = in.nmass;
    Qassert(nmass_group > 0);
    ...
    /////ei.load_eigen(std::string(ename));
    {
      ...
      if (src_step == sink_step) {
        ei.load_eigen_Mvec_smear(ename, gf, gfL, propS, nini, checknorm,
                                 src_width, src_step, 0.0, 0, mom_smear,
                                 mom_smear);
      }
      //
      if (src_step != sink_step) {
    ...
    qnoi noi;
    noi.init(geo);
    Propagator4dT<Ftype> tmp;
    tmp.init(geo);
    //
    for (LInt jobi = 0; jobi < mass_jobA.size() / 2; jobi++) {
      Long bji = mass_jobA[jobi * 2 + 0];
      Long bcut = mass_jobA[jobi * 2 + 1];
      //
      ...
      if (nmass_group != in.nmass) {
        ...
        if (out_vec_sinkG != std::string("NONE")) {
          out_vec_sinkG =
              ssprintf("%s.mgroup%02d", out_vec_sinkG.c_str(), jobi);
        }
        //
        ...
        if (out_vec_sparseG != std::string("NONE")) {
          out_vec_sparseG =
              ssprintf("%s.mgroup%02d", out_vec_sparseG.c_str(), jobi);
        }
      }
      //
      ...
      if (sink_step != 0) {
        sprintf(key_T, "%d   %d    %d   %d  %d  %d %d", in.nsource, 2, 3, 32,
                int(massL.size()), in.nt, 2);
      } else {
        sprintf(key_T, "%d   %d    %d   %d  %d  %d %d", in.nsource, 1, 3, 32,
                int(massL.size()), in.nt, 2);
      }
      ...
      for (int si = 0; si < in.nsource; si++) {
        ...
        // point sink
        if (do_point_sink == 1) {
          point_corr(noi, FpropV, massL, ei, fd, res, srcI, mdat, sec_src, 1,
                     Zn_src);
        }
        //
        if (do_smear_sink == 1) {
          ...
          for (int im = 0; im < bcut; im++) {
            if (smear_2link == 0) {
              smear_propagator_gwu_convension(FpropV[im], gf, sink_width,
                                              sink_step, mom_smear);
            } else {
              smear_propagator_gwu_convension_2shift(
                  FpropV[im], gfL, sink_width, sink_step, propS, -1, mom_smear);
            }
