...
inline Complexq inv_self(const Complexq& lam, double m, double rho,
                         int one_minus_halfD = 1)
{
  Complexq tem = (one_minus_halfD > 0)
                     ? (1 - lam / 2) / (rho * lam + m * (1 - lam / 2))
                     : 1.0 / (rho * lam + m * (1 - lam / 2));
  return tem;
}
...
int main(int argc, char* argv[])
{
  ...
  if (mode == 0) {
    ...
    //
    qlat::vector<Complexq> res0;
    res0.resize(Nt);
    set_zero(res0);
    qlat::vector<Complexq> res1;
    res1.resize(Nt);
    set_zero(res1);
    qlat::vector<Complexq> res2;
    res2.resize(Nt);
    set_zero(res2);
    //
    ////for(size_t
    ...
    //
    {
      ...
      {
        {
          TIMER("Reduce GPU 4");
          set_zero(res2);
          // reduce_gpu2d_6_cach((thrust::complex<float>*)&fd.get_elem(0),(thrust::complex<float>*)&res2[0]
          // , Nsum, Nt);
          // reduce_gpu2d_6_cach((float*)&fd.get_elem(0),(float*)&res2[0] ,
          // Nsum, Nt, 2);
          // reduce_gpu2d_6_cach((double*)&fd.get_elem(0),(double*)&res2[0] ,
          // Nsum, Nt, 1);
          reduce_gpu2d_6_cach((double*)&fd.get_elem(0), (double*)&res2[0], Nsum,
                              Nt, 2);
          ////////Cuda cach reduce
        }
      }
      ...
      double sum2 = 0.0;
      for (int it = 0; it < Nt; it++) {
        diff += (res0[it].real() - res1[it].real()) *
                (res0[it].real() - res1[it].real());
        diff += (res0[it].imag() - res1[it].imag()) *
                (res0[it].imag() - res1[it].imag());
        //
        diff2 += (res0[it].real() - res2[it].real()) *
                 (res0[it].real() - res2[it].real());
        diff2 += (res0[it].imag() - res2[it].imag()) *
                 (res0[it].imag() - res2[it].imag());
        //
        sum0 += (res0[it].real()) * (res0[it].real()) +
                (res0[it].imag()) * (res0[it].imag());
        sum1 += (res1[it].real()) * (res1[it].real()) +
                (res1[it].imag()) * (res1[it].imag());
        sum2 += (res2[it].real()) * (res2[it].real()) +
                (res2[it].imag()) * (res1[it].imag());
      }
    ...
    //
    //  for(int iv=0;iv<n_vec;iv++){
    //    //{TIMER("Reduce Global  V1");reduce_gpu(a,Vres1,geo.local_volume(),
    //    1, 1,16, true);}
    //    //Vres1 = 0;
    ...
    //  ////Vres0 = 0;
    //  ////{TIMER("Reduce GPU V 9");reduce_gpu(a,Vres0,geo.local_volume(),32,
    //  32,32);}
    //  ////Vres0 = 0;
  ...
  //   for(size_t
  //   isp=0;isp<size_t(geo.local_volume());isp++){fint.get_elems(isp)[0]=1.0*std::cos(isp);}
  //
  //  ///touchv(fint);
  ...
  //
  //    res3 = 0;
  //    {TIMER("Reduce Global 1");reduce_gpu(a,res3,geo.local_volume(), 1, 1,16,
  //    true);}
  //
  //    //res2 = 0;
  ...
  //
  //  double length =
  //  n_vec*sizeof(double)*(geo.local_volume()/(1024*1024*1024.0));
