...
int main(int argc, char* argv[])
{
  ...
  for (int nx = int(nxi / 4); nx <= nxi; nx = nx + int(nxi / 4)) {
    ...
    if (check_fft_with_qlat) {
      ...
      qlat::vector_gpu<TyF> check_sum;
      check_sum.resize(Nvec * Nvol);
      //
      std::vector<qlat::FieldM<TyF, Nvec>> src;
      src.resize(in.nvec);
      ...
      for (int iv = 0; iv < in.nvec; iv++) {
        ...
        qacc_for(isp, geo.local_volume(), {
          for (int di = 0; di < Nvec; di++) {
            P0[isp * Nvec + di] = TyF(std::cos((ini + isp + di) * 0.5),
                                      (5.0 / (isp + 1)) * (ini + di) * 0.1);
          }
        });
      }
      //
      ...
      // dimN.resize(4);dimN[0] = nv[3];dimN[1] = nv[2];dimN[2] = nv[1];dimN[3]
      // = nv[0]; if( ft4D){ fft4D.set_mem<TyF >(src.size(), Nvec, dimN,
      // mode_FFT_MPI,   Nvec); fft4D.print_info();}
      //
      qlat::FieldM<TyD, Nvec> srcF;
      srcF.init(geo);
      qlat::FieldM<TyF, Nvec> srcT;
      srcT.init(geo);
      srcT = src[0];
      //
      ...
      if (ft4D) {
        if (checkdiff) {
          TIMER("qlat fft");
          qlat::fft_complex_field(srcF, false);
        }
        {
          TIMER("new fft ");
          fft_fieldM(src, false, true);
        }
      }
      //
      if (!ft4D) {
        if (checkdiff) {
          ...
          // if(int iv=0;iv<in.nvec;iv++){qlat::fft_complex_field_spatial(srcF,
          // false);}
        }
        {
          TIMER("new fft ");
      ...
      if (checkdiff) {
        ...
        for (Long di = 0; di < Long(dat0.size()); di++) {
          ...
          if (qlat::qnorm(dat0[di] - dat1[di]) > 1e-5 or
              qlat::qnorm(dat0[di] - dat2[di]) > 1e-5)
            qmessage(
                "diff di %5ld, expect %.3e %.3e, new %.3e %.3e, qlat %.3e %.3e "
                "\n",
                di, dat0[di].real(), dat0[di].imag(), dat1[di].real(),
                dat1[di].imag(), dat2[di].real(), dat2[di].imag());
        }
