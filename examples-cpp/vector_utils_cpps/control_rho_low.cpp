...
//
  ...
  // int threadio = true;
  {
    ...
    FILE* file_read = open_eigensystem_file(ename, vini, n_vec + vini, true,
                                            io_use, in_read_eigen);
    load_eigensystem_vecs(file_read, eigen, io_use, in_read_eigen, vini,
                          n_vec + vini);
  ...
  // if(ordermem == 1)
  {
    ...
    int Nt = geo.node_site[3];
    qlat::FermionField4dT<Ftype> tmp;
    tmp.init(geo);
...
#pragma omp parallel for
        for (LInt isp = 0; isp < Nsum; isp++)
          for (int it = 0; it < Nt; it++) {
            for (int d = 0; d < 4; d++)
              for (int c = 0; c < 3; c++) {
                src[it * 12 * Nsum + (d * 3 + c) * Nsum + isp] =
                    buf[(it * Nsum + isp) * 12 + d * 3 + c];
              }
          }
      }
    }
  ...
  for (int mi = 0; mi < Nmass; mi++)
    for (int iv = 0; iv < n_vec; iv++) {
      values[iv * Nmass + mi] =
          inv_self(values_sys[iv], mass[mi], rho, one_minus_halfD);
    }
  qmessage("===num zero %d \n", n_zero);
  /////====Set up values
  fflush_MPI();
  //
  {
    TIMER("Kernal functions");
    get_low_rho(eigen, values, n_zero, Mres, geo, GPUFM);
  }
  //
  fflush_MPI();
  {
