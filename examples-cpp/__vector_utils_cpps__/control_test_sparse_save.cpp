...
int main(int argc, char* argv[])
{
  ...
  for (int iv = 0; iv < 10; iv++) {
    for (int j = 0; j < 4; j++) {
      tem[j] = int(qlat::u_rand_gen(rG) * total_site[j]);
    }
    pconf.push_back(tem);
  }
  //
  ...
  ////===save point selection info
  //
  qlat::FieldM<ComplexD, 12 * 12> propM;
  propM.init(geo);
  ...
  for (Long isp = 0; isp < geo.local_volume(); isp++) {
    ...
    for (int j = 0; j < 12 * 12; j++) {
      data[isp * 12 * 12 + j] = ComplexD(ini, ini / 2.0);
    }
  }
  //
  qlat::SelectedField<ComplexD> sf;
  sf.init(fsel, 12 * 12);
  ...
  //
  {
    ...
    //
    {
      ...
      for (Long i = 0; i < (Long)fns.size(); ++i) {
        displayln_info(ssprintf(": %5d : '%s' from '%s'.", i, fns[i].c_str(),
                                path.c_str()));
      }
    }
  }
