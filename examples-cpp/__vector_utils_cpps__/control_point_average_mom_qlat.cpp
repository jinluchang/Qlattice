...
inline void write_corr_zero(corr_dat<Ftype>& res,
                            qlat::vector_gpu<Complexq>& FFT_data,
                            const long off, std::vector<bool>& Bsite,
                            std::vector<long>& Zsite)
{
  ...
  qlat::vector<Complexq> buf_gpu;
  buf_gpu.resize(nvec * nt);
  qlat::set_zero(buf_gpu);
  Complexq* Pbuf = (Complexq*)(qlat::get_data(buf_gpu).data());
  Complexq* PFFT = (Complexq*)(qlat::get_data(FFT_data).data());
  qlat::vector<long> PZsite;
  PZsite.resize(Zsite.size());
  for (Long t = 0; t < PZsite.size(); t++) {
    PZsite[t] = Zsite[t];
  }
  //
  ...
  for (int t = 0; t < nt; t++) {
    if (Bsite[t]) {
      qacc_for(iv, nvec, { Pbuf[iv * nt + t] += PFFT[PZsite[t] * nvec + iv]; });
    }
  }
  //
  for (int t = 0; t < nt; t++) {
    if (Bsite[t]) {
      qthread_for(iv, nvec, { Pres[iv * nt + t] += Pbuf[iv * nt + t]; });
    }
  }
}
...
inline std::vector<Coordinate> read_positions(corr_dat<Ftype>& mom_info)
{
  ...
  for (int i = 0; i < Ninfo; i++) {
    if (mom_info.INFOA[i].find("Positions") != std::string::npos) {
      int ini =
          mom_info.INFOA[i].find("Positions") + std::string("Positions").size();
      std::string str5 =
          mom_info.INFOA[i].substr(ini);  // get from "live" to the end
      // qmessage("test %s \n", str5.c_str());
  ...
  //
  {
    ...
    std::vector<Coordinate> mom_shiftL;
    {
      ...
      for (int momi = 0; momi < Nmom; momi++) {
        for (int i = 0; i < 4; i++) {
          mom_shiftL[momi][i] = stringtonum(Li[momi * 4 + i]);
        }
      }
    }
    //
    ...
    for (int mg = 0; mg < mass_group; mg++) {
      ...
      char name[450], namecr[500], namesm[500], nameQ[550], name_mom[550],
          namei[500];
      //
      std::string outputG =
          ssprintf("%s%s", in.output.c_str(), flag_add.c_str());
      std::string output_vecG =
          ssprintf("%s%s", in.output_vec.c_str(), flag_add.c_str());
      std::string output_vecGs =
          ssprintf("%s%s", output_vecs.c_str(), flag_add.c_str());
      //
      if (mass_group > 1) {
      ...
      for (unsigned int i = 0; i < pt_infoS.size(); i++) {
        pt_info.push_back(stringtonum(pt_infoS[i]));
      }
      for (unsigned int i = 0; i < gr_infoS.size(); i++) {
        gr_info.push_back(stringtonum(gr_infoS[i]));
      }
      //
      Qassert(pt_info.size() == 2);
      Qassert(gr_info.size() == 3);
      ...
      pL[3] = nt;
      double factor_pt = 0.0;
      int gr_start = 1;
      if (pt_info[1] == pt_info[0]) {
        factor_pt = 0.0;
        gr_start = 0;
      } else {
      ...
      //
      //  sprintf(key_T, "%d   %d    %d   %d  %d  %d %d", nsource, 2, 3, 32,
      //  int(massL.size()), in.nt, 2); sprintf(dimN , "src  sm  LHF operator
      //  masses nt complex");
      //
      ...
      for (int sm = 0; sm < 2; sm++) {
        ...
        for (int si = gr_info[0]; si < gr_info[1]; si++) {
          ...
          if (sm == 0) {
            sprintf(namesm, "%s.pt", name);
          }
          if (sm == 1) {
            sprintf(namesm, "%s.sm", name);
          }
          ...
          if (average.dim == 0) {
            ...
            // std::vector<Coordinate > src_pos =
            // string_to_Coordinates(mom_info.INFO_LIST);
            //
            std::string ktem = "NULL";
            std::string dtem = "NULL";
            Qassert(split_pt_gr == 0 or split_pt_gr == 1) if (split_pt_gr == 0)
            {
              ktem =
                  std::string("1 ") + std::string("32 ") + mom_info.get_key_T();
              dtem = std::string("LH ") + std::string("operator ") +
                     mom_info.get_dim_name();
            }
            if (split_pt_gr == 1) {
              ktem =
                  std::string("2 ") + std::string("32 ") + mom_info.get_key_T();
              dtem = std::string("LH ") + std::string("operator ") +
                     mom_info.get_dim_name();
            }
            ...
            nt = average.key_T[3];
            for (int i = 0; i < 3; i++) {
              pL[i] = average.key_T[i + 4];
            }
            pL[3] = nt;
            //
            char tem[500];
            sprintf(tem, "point %s, grid %s ", in.paraIA.c_str(),
                    in.paraIB.c_str());
            average.INFO_LIST =
                std::string("Average data, ") + std::string(tem);
            average.INFOA = mom_info.INFOA;
            average.INFOA.push_back(std::string("mom shift ") +
                                    Coordinate_to_string(mom_shiftp));
          }
          //
          ...
          if (data_sm[sm])
            for (int gi = 0; gi < gr_info[2]; gi++) {
              fflush_MPI();
              sprintf(namei, "%09d", gi);
              if (!mdat.check_fn_momcut(std::string(nameQ),
                                        std::string(namei))) {
      ...
      int Nsm = 2;
      {
        ...
        sprintf(key_T, "%d   %d    %d   %d  %d  %d %d", nsource, Nsm, 3, 32,
                int(massL.size()), in.nt, 2);
      ...
      for (int sm = 0; sm < 2; sm++) {
        ...
        for (int si = gr_info[0]; si < gr_info[1]; si++) {
          ...
          for (int gi = 0; gi < gr_info[2]; gi++) {
            fflush_MPI();
            sprintf(namei, "%09d", gi);
            if (mdat.read_momcut(FFT_data, std::string(nameQ),
                                 std::string(namei)) == 0) {
              Qassert(false);
            }
            if (FFT_data_average_gr.size() == 0) {
              FFT_data_average_gr.resize(FFT_data.size());
              FFT_data_average_gr.set_zero();
            }
            //
            ...
            //
            if (gi != 0) {
              long corr_zero_off = (((long(nsi) + npoint) * Nsm + sm) * 3 + 2) *
                                   32 * nmass * in.nt * 2;
              write_corr_zero(corr_zero, FFT_data, corr_zero_off, Bsite, Zsite);
            }
            if (gi == 0) {
              long corr_zero_off = (((long(nsi) + npoint) * Nsm + sm) * 3 + 1) *
                                   32 * nmass * in.nt * 2;
              write_corr_zero(corr_zero, FFT_data, corr_zero_off, Bsite, Zsite);
            }
            //
            ////omit high mode for grid source
            if (gi >= gr_start) {
              cpy_data_thread(FFT_data_average_gr.data(), FFT_data.data(),
                              FFT_data.size(), 1, QTRUE, factor_gr);
            }
          }
          nsi += 1;
        }
        ...
        for (int si = pt_info[0]; si < pt_info[1]; si++) {
          ...
          for (int gi = 0; gi < 2; gi++) {
            ...
            //
            if (FFT_data_average_pt.size() == 0) {
              FFT_data_average_pt.resize(FFT_data.size());
              FFT_data_average_pt.set_zero();
            }
            if (corr_with_phase_shift == 0) {
              mdat.apply_src_phases(FFT_data, src_pos[gi]);
            ...
            //
            if (gi != 0) {
              long corr_zero_off = (((long(nsi) + 0) * Nsm + sm) * 3 + 2) * 32 *
                                   nmass * in.nt * 2;
              write_corr_zero(corr_zero, FFT_data, corr_zero_off, Bsite, Zsite);
            }
            if (gi == 0) {
              long corr_zero_off = (((long(nsi) + 0) * Nsm + sm) * 3 + 1) * 32 *
                                   nmass * in.nt * 2;
              write_corr_zero(corr_zero, FFT_data, corr_zero_off, Bsite, Zsite);
            }
            //
            ////FFT_data[];
            if (gi == 0) {
              cpy_data_thread(FFT_data_average_pt.data(), FFT_data.data(),
                              FFT_data.size(), 1, QTRUE, factor_pt);
            }
          }
          nsi += 1;
        }
        //
        {
          ...
          if (split_pt_gr == 0) {
            ...
            fft_local_to_global(FFT_data_global, FFT_data_average_gr, mdat,
                                mom_shiftp);
            average.write_corr(FFT_data_global.data(), FFT_data_global.size(),
                               3);
          }
          //
          if (split_pt_gr == 1) {
            ...
            average.write_corr(FFT_data_global.data(), FFT_data_global.size(),
                               3);
            fft_local_to_global(FFT_data_global, FFT_data_average_pt, mdat,
                                mom_shiftp);
            average.write_corr(FFT_data_global.data(), FFT_data_global.size(),
                               3);
          }
