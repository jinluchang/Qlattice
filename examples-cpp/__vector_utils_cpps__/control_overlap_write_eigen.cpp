...
int main(int argc, char* argv[])
{
  ...
  //
  {
    ...
    int nvec = in0.nvec;
    if (nvec <= nini) {
      abort_r("No vectors for reading!!! \n");
    }
    int nread = nvec - nini;
    //
    ...
    if (in0.debuga == 0)
      sprintf(sDescription, "Overlap operator: half spectrum");
    if (in0.debuga == 1) sprintf(sDescription, "H_wilson");
    load_txt_eigenvalues(values, errors, enamev.c_str());
    v0.resize(nread * 2);
    e0.resize(nread);
    for (int iv = 0; iv < nread * 2; iv++) {
      v0[iv] = values[nini * 2 + iv];
    }
    for (int iv = 0; iv < nread; iv++) {
      e0[iv] = errors[nini + iv];
    }
    //
    ...
    inputpara in_tem;
    const int file_type =
        check_qlat_eigen_file_type(ename.c_str(), io_read, nvec, in_tem);
    //
    ...
    if (step != 0) {
      ...
      // LInt cpy_size =
      // (LInt)(qlat::get_data(gfD).data_size()/(sizeof(qlat::ComplexD)));
      // cpy_data_thread(tem1, tem0, cpy_size, 0);
    }
    //
    FILE* file_read = open_eigensystem_file(ename.c_str(), nini, nvec, true,
                                            io_read, in_read_eigen, file_type);
    FILE* file_write =
        open_eigensystem_file(Sname.c_str(), nini, nvec, false, io_write,
                              in_write_eigen, stringtonum(in0.save_type));
    //
    std::vector<Long> job = job_create(nread, each);
    for (LInt ji = 0; ji < job.size() / 2; ji++) {
      /////int n0 = nini + job[ji*2 + 0]; int n1 = n0 + job[ji*2 + 1];
      if (!F_single) {
        load_eigensystem_vecs(file_read, eigenD, io_read, in_read_eigen, 0,
                              job[ji * 2 + 1]);
      }
      if (F_single) {
        load_eigensystem_vecs(file_read, eigenF, io_read, in_read_eigen, 0,
                              job[ji * 2 + 1]);
      }
      //
      ...
      if (step != 0) {
        for (int iv = 0; iv < each; iv++) {
          if (!F_single) {
            smear_propagator_gwu_convension(eigenD[iv], gf, width, step);
          }
          if (F_single) {
            smear_propagator_gwu_convension(eigenF[iv], gf, width, step);
          }
        }
      }
      /////===smearing of eigen
      //
      if (!F_single) {
        load_eigensystem_vecs(file_write, eigenD, io_write, in_write_eigen, 0,
                              job[ji * 2 + 1]);
      }
      if (F_single) {
        load_eigensystem_vecs(file_write, eigenF, io_write, in_write_eigen, 0,
                              job[ji * 2 + 1]);
      }
    }
    //
