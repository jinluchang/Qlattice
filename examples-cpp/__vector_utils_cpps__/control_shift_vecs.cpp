...
int main(int argc, char* argv[])
{
  ...
  ////int ns   = in.bfac;
  //
  std::vector<qlat::FieldM<TyD, inner>> src;
  src.resize(Nvec);
  std::vector<qlat::FieldM<TyD, inner>> res;
  res.resize(Nvec);
  std::vector<qlat::FieldM<TyF, inner>> srcF;
  srcF.resize(Nvec);
  std::vector<qlat::FieldM<TyF, inner>> resF;
  resF.resize(Nvec);
  //
  ...
  for (int iv = 0; iv < Nvec; iv++) {
    ...
    for (Long isp = 0; isp < geo.local_volume(); isp++) {
      ...
      for (int di = 0; di < inner; di++) {
        ////P1[isp*Nvec + di ] = TyD(std::cos((iv + ini+isp+di)*0.5) ,
        ///(5.0/(isp+1))*(iv + ini+di)*0.1);
        TyD ref0 =
            TyD(offV0 + std::cos((di) * 0.5), (5.0 / (1.0)) * (iv + di) * 0.1);
  ...
  for (int i = 0; i < 4; i++) {
    iDir[i] = stringtonum(temL[i]);
  }
  //
  /// svec.set_MPI_size(2*civ, biva);
  //
  // std::vector<qlat::vector<TyF > > srcE;std::vector<qlat::vector<TyF > >
  // resE; srcE.resize(biva);resE.resize(biva);
  //
  ...
  double diff0 = 0.0;
  double diff1 = 0.0;
  qlat::vector<int> iDir_acc;
  iDir_acc.resize(4);
  for (int i = 0; i < 4; i++) {
    iDir_acc[i] = iDir[i] * in.nvec;
  }
  qlat::vector<int> nv_acc;
  nv_acc.resize(4);
  for (int i = 0; i < 4; i++) {
    nv_acc[i] = fd.nv[i];
  }
  for (LInt iv = 0; iv < src.size(); iv++) {
    ...
    for (Long isp = 0; isp < geo.local_volume(); isp++) {
      ...
      double offV0 = ((p[3] * 900 + p[2]) * 900 + p[1]) * 900 + p[0];
      for (int i = 0; i < 4; i++) {
        p[i] = (p[i] - iDir_acc[i] + nv_acc[i]) % nv_acc[i];
      }
      // for(int i=0;i<4;i++){p[i] = (p[i] + iDir_acc[i] +
      // nv_acc[i])%nv_acc[i];}
      ...
      for (int di = 0; di < inner; di++) {
        ...
        TyD ref0 =
            TyD(offV0 + std::cos((di) * 0.5), (5.0 / (1.0)) * (iv + di) * 0.1);
        TyD ref1 =
            TyD(offV1 + std::cos((di) * 0.5), (5.0 / (1.0)) * (iv + di) * 0.1);
        //
