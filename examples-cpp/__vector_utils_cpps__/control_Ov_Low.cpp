...
int main(int argc, char* argv[])
{
  ...
  sysinfo(&s_info);
  qmessage(
      "Eign system vector size %.3e GB, Total %.3e GB; CPU free %.3e GB, total "
      "%.3e GB; GPU free %.3e GB, total %.3e GB. \n",
      length, n_vec * length, s_info.freeram * pow(0.5, 30),
      s_info.totalram * pow(0.5, 30), freeD, totalD);
  //
  int nmass = in.nmass;
  ...
  propS.resize(1);
  for (unsigned int i = 0; i < propS.size(); i++) propS[i].init(geo);
  //
  sprintf(names, in.Sname.c_str(), icfg);
  fflush_MPI();
  //
  std::vector<qlat::FieldM<Complexq, 1>> noi;
  noi.resize(1);
  noi[0].init(geo);
  ...
  //
  std::vector<qlat::FieldM<qlat::ComplexD, 12 * 12>> FpropV;
  FpropV.resize(nmass);
  //
  ...
  // qmessage("===res norm ");ptmp.print_norm2();
  //
  std::vector<Propagator4d> propS_new;
  propS_new.resize(nmass);
  ...
  if (in.debuga != 0) {
    ...
    for (int i = 0; i < in.debuga; i++) {
      ...
      // #ifdef QLAT_USE_ACC
      // qlat_GPU_Memcpy(  ei.stmp, &src[0]  ,
      // ei.stmp_size*sizeof(Complexq),qlat_GPU_MemcpyDeviceToDevice); #else
