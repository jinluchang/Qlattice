...
int main(int argc, char* argv[])
{
  ...
  //
  qlat::vector_gpu<ComplexM> a;
  a.resize(L * m * w);
  qlat::vector_gpu<ComplexM> b;
  b.resize(L * w * n);
  qlat::vector_gpu<ComplexM> c0;
  c0.resize(L * m * n);
  qlat::vector_gpu<ComplexM> c1;
  c1.resize(L * m * n);
  //
  ...
  // for(int i=0;i< in.debuga;i++){matrix_prod_gpu(a.data(), b.data(),
  // c1.data(), m,n,w,L, Conj, true,false, modeGPU);fflush_MPI();}
  for (int i = 0; i < in.debuga; i++) {
    matrix_prod(a.data(), b.data(), c1.data(), m, n, w, L, Conj, false,
                modeGPU);
    fflush_MPI();
  }
  ...
  if (test_pointers >= 1) {
    c1.set_zero();
    qlat::vector<ComplexM*> aP;
    aP.resize(L);
    ComplexM* A = a.data();
    qlat::vector<ComplexM*> bP;
    bP.resize(L);
    ComplexM* B = b.data();
    qlat::vector<ComplexM*> cP;
    cP.resize(L);
    ComplexM* C = c1.data();
    ...
    for (int i = 0; i < in.debuga; i++) {
      matrix_prodP(aP.data(), bP.data(), cP.data(), m, n, w, L, Conj, false,
                   modeGPU);
      fflush_MPI();
    }
    ComplexM ra = c1.norm2();
    qmessage("===result %.12e %.12e, %.12e %.12e \n", ra.real(), ra.imag(),
             (ra - res).real(), (ra - res).imag());
  }
  if (test_pointers >= 2) {
    c1.set_zero();
    qlat::vector<ComplexM*> aP;
