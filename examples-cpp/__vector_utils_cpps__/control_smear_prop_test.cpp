...
int main(int argc, char* argv[])
{
  ...
  if (in.paraI != "None") {
    ...
    for (int si = 0; si < Li.size() / 3; si++) {
      ...
      //
      EigenV propT_tmp;
      propT_tmp.resize(repeat * NVmpi * Nvol * groupP * 12);
      EigenV gfET_tmp;
      gfET_tmp.resize(NVmpi * 6 * Nvol * 9);
      //
      ////EigenV propT;EigenV propT_buf;
      Complexq* propT = NULL;
      ...
      // gpuErrchk(qlat_GPU_Malloc(&propT_buf,
      // propT_tmp.size()*sizeof(Complexq))); gpuErrchk(qlat_GPU_Malloc(&gfET,
      // gfET_tmp.size()*sizeof(Complexq)));
      //
      qlat_GPU_Memcpy(&propT[0], &propT_tmp[0],
                      propT_tmp.size() * sizeof(Complexq),
                      qlat_GPU_MemcpyDeviceToDevice);
      qlat_GPU_Memcpy(&gfET[0], &gfET_tmp[0],
                      gfET_tmp.size() * sizeof(Complexq),
                      qlat_GPU_MemcpyDeviceToDevice);
      //
      ...
      //
      {
        ...
        {
          TIMER("Vec prop");
          vec_large.reorder(&propT[0], &propT_buf[0], repeat, groupP * 12, 0);
        }
        for (int i = 0; i < repeat; i++) {
          Long off = i * NVmpi * Nvol * groupP * 12;
          int bfac = groupP;
          int d0 = 4;
          smear_propagator4(&propT[off], &gfET[0], width, step, &propT_buf[0],
                            smf, bfac, d0);
        }
        {
          TIMER("Vec prop");
          vec_large.reorder(&propT[0], &propT_buf[0], repeat, groupP * 12, 100);
        }
        timer.flops += Tfloat;
      }
      //
