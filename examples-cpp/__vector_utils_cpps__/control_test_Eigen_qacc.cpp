...
__global__ void eigen_test(Cy* va, Cy* vb, Cy* vc, const Long m)
{
  ...
  if (index < m) {
    ...
    // Eigen::Matrix<Cy, 3   , 3, Et>&   lE = *((Eigen::Matrix<Cy, 3   , 3,
    // Et>*) &va[index*9 + 0]);
    Eigen::Map<const Eigen::Matrix<Cy, 3, 3, Et>> lE(&va[index * 9 + 0]);
    //
    Cy* bP = &vb[index * 3 * D];
    // Eigen::Matrix<Cy, D, 3, Et>&     bE = *((Eigen::Matrix<Cy, D, 3, Et>*)
    // bP);
    Eigen::Map<const Eigen::Matrix<Cy, D, 3, Et>> bE(bP);
    Cy* cP = &vc[index * 3 * D];
    // Eigen::Matrix<Cy, D, 3, Et>&     cE = *((Eigen::Matrix<Cy, D, 3, Et>*)
    // cP);
...
int main(int argc, char* argv[])
{
  ...
  //
  Cy* va = NULL;
  qlat_GPU_Malloc(&va, MAX * 3 * 3 * sizeof(Cy));
  Cy* vb = NULL;
  qlat_GPU_Malloc(&vb, MAX * 3 * D * sizeof(Cy));
  Cy* vc = NULL;
  qlat_GPU_Malloc(&vc, MAX * 3 * D * sizeof(Cy));
  Cy* vd = NULL;
  qlat_GPU_Malloc(&vd, MAX * 3 * D * sizeof(Cy));
  Cy* ve = NULL;
  qlat_GPU_Malloc(&ve, MAX * 3 * D * sizeof(Cy));
  //
  Cy* vaH = NULL;
  vaH = (Cy*)malloc(MAX * 3 * 3 * sizeof(Cy));
  Cy* vbH = NULL;
  vbH = (Cy*)malloc(MAX * 3 * D * sizeof(Cy));
  Cy* vcH = NULL;
  vcH = (Cy*)malloc(MAX * 3 * D * sizeof(Cy));
  Cy* vdH = NULL;
  vdH = (Cy*)malloc(MAX * 3 * D * sizeof(Cy));
  Cy* veH = NULL;
  veH = (Cy*)malloc(MAX * 3 * D * sizeof(Cy));
  //
  ...
  qacc_for(isp, MAX,
           {
             for (int ic = 0; ic < 9; ic++) {
               va[isp * 9 + ic] = Cy(std::cos((ini + isp + ic) * 0.5),
                                     (5.0 / (isp + ic + 1)) * ini * 0.1);
             }
             for (int ic = 0; ic < 3 * D; ic++) {
               vb[isp * 3 * D + ic] = Cy(std::cos((ini + isp + ic) * 0.8),
                                         (8.0 / (isp + ic + 1)) * ini * 0.2);
               vc[isp * 3 * D + ic] = vb[isp * 3 * D + ic];
               ve[isp * 3 * D + ic] = vb[isp * 3 * D + ic];
             }
           }) qlat_GPU_Memcpy(vaH, va, MAX * 3 * 3 * sizeof(Cy),
                              qlat_GPU_MemcpyDeviceToHost);
  qlat_GPU_Memcpy(vbH, vb, MAX * 3 * D * sizeof(Cy),
                  qlat_GPU_MemcpyDeviceToHost);
  qlat_GPU_Memcpy(vcH, vc, MAX * 3 * D * sizeof(Cy),
                  qlat_GPU_MemcpyDeviceToHost);
  ...
  //  Cy* bP = &vbH[index*3*D];
  //  Eigen::Matrix<Cy, D, 3, Et>&     bE = *((Eigen::Matrix<Cy, D, 3, Et>*)
  //  bP); Cy* eP = &veH[index*3*D]; Eigen::Matrix<Cy, D, 3, Et>&     eE =
  //  *((Eigen::Matrix<Cy, D, 3, Et>*) eP); bE *= lE;
  //  //eE = bE * lE;
  //  //bE = eE;
  //  //bE *= lE;
  //}
  ...
  //  Eigen::Matrix<Cy, D, 3, Et>&     bE = *((Eigen::Matrix<Cy, D, 3, Et>*)
  //  bP); Cy* eP = &veH[index*3*D]; Eigen::Matrix<Cy, D, 3, Et>&     eE =
  //  *((Eigen::Matrix<Cy, D, 3, Et>*) eP); eE = bE * lE; bE = eE;
  //  //bE *= lE;
  //});
  ...
  qacc_for(index, MAX, {
    // Cy buf[9];
    // for(int ci=0;ci<9;ci++){buf[ci] = va[index*9 +  ci];}
    Eigen::Matrix<Cy, 3, 3, Et>& lE =
        *((Eigen::Matrix<Cy, 3, 3, Et>*)&va[index * 9 + 0]);
    //
    Cy* bP = &vb[index * 3 * D];
  ...
  for (Long index = 0; index < MAX; index++) {
    ...
    // for(int ci=0;ci<9;ci++){buf[ci] = vaH[index*9 +  ci];}
    Eigen::Matrix<Cy, 3, 3, Et>& lE =
        *((Eigen::Matrix<Cy, 3, 3, Et>*)&vaH[index * 9 + 0]);
    //
    Cy* bP = &vcH[index * 3 * D];
    Eigen::Matrix<Cy, D, 3, Et>& bE = *((Eigen::Matrix<Cy, D, 3, Et>*)bP);
    bE *= lE;
  ...
  qlat_GPU_Memcpy(vbH, vb, MAX * 3 * D * sizeof(Cy),
                  qlat_GPU_MemcpyDeviceToHost);
  qlat_GPU_Memcpy(vdH, vd, MAX * 3 * D * sizeof(Cy),
                  qlat_GPU_MemcpyDeviceToHost);
  qlat_GPU_Memcpy(veH, ve, MAX * 3 * D * sizeof(Cy),
                  qlat_GPU_MemcpyDeviceToHost);
  //
