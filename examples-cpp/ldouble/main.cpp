...
void simple_tests()
{
  ...
  {
    double a = (1 - 2 * int(qlat::u_rand_gen(rs) * 2)) * qlat::u_rand_gen(rs) *
               (std::pow(10, int(qlat::u_rand_gen(rs) * 10)));
    double b = (1 - 2 * int(qlat::u_rand_gen(rs) * 2)) * qlat::u_rand_gen(rs) *
               (std::pow(10, int(qlat::u_rand_gen(rs) * 10)));
    //
    CFloat a128 = a;
    CFloat b128 = b;
    ...
    for (int casei = 0; casei < Ncase; casei++) {
      ...
      if (casei == 4) {
        ss += "qrt ";
        z128 = sqrtq(fabsq(a128));
      }
#else
      if (casei == 4) {
        ss += "qrt ";
        z128 = std::sqrt(std::fabs(a128));
      }
#endif
      if (casei == 5) {
        ss += "sin ";
        z128 = qsin(double(a128 + b128));
      }
      if (casei == 6) {
        ss += "cos ";
        z128 = qcos(double(a128 + b128));
      }
      if (casei == 7) {
        ss += "acos";
        z128 = qacos(qsin(double(a128 + b128)));
      }
  ...
  //
  {
    double a0 = (1 - 2 * int(qlat::u_rand_gen(rs) * 2)) * qlat::u_rand_gen(rs) *
                (std::pow(10, int(qlat::u_rand_gen(rs) * 10)));
    double a1 = (1 - 2 * int(qlat::u_rand_gen(rs) * 2)) * qlat::u_rand_gen(rs) *
                (std::pow(10, int(qlat::u_rand_gen(rs) * 10)));
    double b0 = (1 - 2 * int(qlat::u_rand_gen(rs) * 2)) * qlat::u_rand_gen(rs) *
                (std::pow(10, int(qlat::u_rand_gen(rs) * 10)));
    double b1 = (1 - 2 * int(qlat::u_rand_gen(rs) * 2)) * qlat::u_rand_gen(rs) *
                (std::pow(10, int(qlat::u_rand_gen(rs) * 10)));
    //
    ...
    //
    displayln_info(
        ssprintf("CHECK: a   %+.8e %+.8e, b %+.8e %+.8e \n", a0, a1, b0, b1));
    const int Ncase = 5;
    qlat::vector<double> ld;
    ld.resize(8);
    //
    for (int casei = 0; casei < Ncase; casei++) {
      ...
      if (casei == 4) {
        ss += "nor ";
        zC = aC.real() * aC.real() + aC.imag() * aC.imag();
      }
      CFloat zt;
      zt = zC.real();
      ...
      // ss += ssprintf("%+.5e %+.5e, %+.5e %+.5e; "    , l[0], l[1], l[2],
      // l[3]); ss += ssprintf("float128 %+.5e %+.5e, %+.5e %+.5e "    , l[4],
      // l[5], l[6], l[7]);
...
int main(int argc, char* argv[])
{
  ...
  get_global_rng_state() =
      RngState(get_global_rng_state(), "qlat-basic-structure");
  simple_tests();
